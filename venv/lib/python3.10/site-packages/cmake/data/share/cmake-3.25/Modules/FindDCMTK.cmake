...
#
# Written for VXL by Amitha Perera. Upgraded for GDCM by Mathieu Malaterre.
# Modified for EasyViz by Thomas Sondergaard.
#

set(_dcmtk_dir_description "The directory of DCMTK build or install tree.")

# Ensure that DCMTK_DIR is set to a reasonable default value so that DCMTK
# libraries can be found on a standard Unix distribution. It also overwrite the
# value of DCMTK_DIR after this one has been set by a successful discovery of
# DCMTK by the unpatched FindDCMTK.cmake module distributed with CMake (as of
# 0167cea)
if(NOT DCMTK_DIR OR DCMTK_DIR STREQUAL "/usr/include/dcmtk")
  set(DCMTK_DIR
      "/usr"
      CACHE PATH ${_dcmtk_dir_description} FORCE)
endif()
...
# Restore the value reset by the previous call to 'find_package(DCMTK QUIET
# NO_MODULE)'
set(DCMTK_DIR
    ${_SAVED_DCMTK_DIR}
    CACHE PATH ${_dcmtk_dir_description} FORCE)
...
#
# Step2: Attempt to find a version of DCMTK that does NOT provide a
# DCMTKConfig.cmake file.
#

# prefer DCMTK_DIR over default system paths like /usr/lib
if(DCMTK_DIR)
  set(CMAKE_PREFIX_PATH ${DCMTK_DIR}/lib ${CMAKE_PREFIX_PATH}) # this is given
                                                               # to FIND_LIBRARY
                                                               # or FIND_PATH
endif()
...
foreach(
  ...
  find_library(
    DCMTK_${lib}_LIBRARY_RELEASE ${lib}
    PATHS ${DCMTK_DIR}/${lib}/libsrc ${DCMTK_DIR}/${lib}/libsrc/Release
          ${DCMTK_DIR}/${lib}/Release ${DCMTK_DIR}/lib ${DCMTK_DIR}/lib/Release
          ${DCMTK_DIR}/dcmjpeg/lib${lib}/Release
    NO_DEFAULT_PATH)

  # Find Debug libraries
  find_library(
    DCMTK_${lib}_LIBRARY_DEBUG ${lib}${DCMTK_CMAKE_DEBUG_POSTFIX}
    PATHS ${DCMTK_DIR}/${lib}/libsrc ${DCMTK_DIR}/${lib}/libsrc/Debug
          ${DCMTK_DIR}/${lib}/Debug ${DCMTK_DIR}/lib ${DCMTK_DIR}/lib/Debug
...
if(DCMTK_oflog_LIBRARY_RELEASE OR DCMTK_oflog_LIBRARY_DEBUG)
  ...
  # will attempt to find the library in all cases. Ideally, pthread library
  # should be discovered only if DCMTK_WITH_THREADS is enabled.
...
#
# SPECIFIC CASE FOR DCMTK BUILD DIR as DCMTK_DIR (as opposed to a DCMTK install
# dir) Have to find the source directory.
if(EXISTS ${DCMTK_DIR}/CMakeCache.txt)
  load_cache(${DCMTK_DIR} READ_WITH_PREFIX "EXT" DCMTK_SOURCE_DIR)
  if(NOT EXISTS ${EXTDCMTK_SOURCE_DIR})
    message(FATAL_ERROR "DCMTK build directory references
...
foreach(
  ...
  if(EXTDCMTK_SOURCE_DIR)
    set(SOURCE_DIR_PATH ${EXTDCMTK_SOURCE_DIR}/${dir}/include/dcmtk/${dir})
  endif()
  find_path(
    DCMTK_${dir}_INCLUDE_DIR ${DCMTK_${dir}_TEST_HEADER}
  ...
  if(DCMTK_${dir}_INCLUDE_DIR)
    # add the 'include' path so eg include "dcmtk/dcmimgle/dcmimage.h" works
...
if(DCMTK_ofstd_INCLUDE_DIR)
  get_filename_component(DCMTK_dcmtk_INCLUDE_DIR ${DCMTK_ofstd_INCLUDE_DIR}
                         PATH CACHE)
...
# Workaround bug in packaging of DCMTK 3.6.0 on Debian. See
# http://bugs.debian.org/cgi-bin/bugreport.cgi?bug=637687
if(DCMTK_FOUND
   AND UNIX
   AND NOT APPLE)
  ...
  check_include_files("dcmtk/config/osconfig.h;dcmtk/ofstd/ofstream.h"
