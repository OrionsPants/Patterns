...
set(_MPI_CXX_GENERIC_COMPILER_NAMES
    mpicxx
    mpiCC
    mpcxx
    mpCC
    ...
    mpic++_r
    mpc++_r)
set(_MPI_Fortran_GENERIC_COMPILER_NAMES
    mpif95
    mpif95_r
    mpf95
    mpf95_r
...
set(_MPI_GNU_Fortran_COMPILER_NAMES
    mpigfortran
    mpgfortran
    mpigfortran_r
    mpgfortran_r
...
set(_MPI_PGI_Fortran_COMPILER_NAMES
    mpipgifort
    mpipgf95
    mpipgf90
    mppgf95
...
set(_MPI_XL_CXX_COMPILER_NAMES
    mpixlcxx
    mpixlC
    mpixlc++
    mpxlcxx
    ...
    mpixlc++_r
    mpxlCC_r)
set(_MPI_XL_Fortran_COMPILER_NAMES
    mpixlf95
    mpixlf95_r
    mpxlf95
    mpxlf95_r
...
# Prepend vendor-specific compiler wrappers to the list. If we don't know the
# compiler, attempt all of them. By attempting vendor-specific compiler names
# first, we should avoid situations where the compiler wrapper stems from a
# proprietary MPI and won't know which compiler it's being used for. For
# instance, Intel MPI controls its settings via the I_MPI_CC environment
# variables if the generic name is being used. If we know which compiler we're
# working with, we can use the most specialized wrapper there is in order to
...
# Names to try for mpiexec Only mpiexec commands are guaranteed to behave as
# described in the standard, mpirun commands are not covered by the standard in
# any way whatsoever. lamexec is the executable for LAM/MPI, srun is for SLURM
# or Open MPI with SLURM support. srun -n X <executable> is however a valid
# command, so it behaves 'like' mpiexec.
...
function(_MPI_check_compiler LANG QUERY_FLAG OUTPUT_VARIABLE RESULT_VARIABLE)
  ...
  execute_process(
    COMMAND ${MPI_${LANG}_COMPILER} ${_MPI_COMPILER_WRAPPER_OPTIONS}
            ${QUERY_FLAG}
    OUTPUT_VARIABLE WRAPPER_OUTPUT
    OUTPUT_STRIP_TRAILING_WHITESPACE
    ERROR_VARIABLE WRAPPER_OUTPUT
  ...
  # library that has invalid or missing version information there would be
  # warning messages emitted by ld.so in the compiler output. In either case,
  # we'll treat the output as invalid.
  if(WRAPPER_OUTPUT
     MATCHES
     "undefined reference|unrecognized|need to set|no version information available|command not found"
  )
...
function(_MPI_interrogate_compiler LANG)
  ...
  # Define the MPICH and Intel MPI compiler variables to the compilers set in
  # CMake. It's possible to have a per-compiler configuration in these MPI
  # implementations and a particular MPICH derivate might check compiler
  # interoperability. Intel MPI in particular does this with
  # I_MPI_CHECK_COMPILER.
  file(TO_NATIVE_PATH "${CMAKE_${LANG}_COMPILER}" _MPI_UNDERLAYING_COMPILER)
  # On Windows, the Intel MPI batch scripts can only work with filnames - Full
  # paths will break them. Due to the lack of other MPICH-based wrappers for
  # Visual C++, we may treat this as default.
  if(MSVC)
    get_filename_component(_MPI_UNDERLAYING_COMPILER
                           "${_MPI_UNDERLAYING_COMPILER}" NAME)
  endif()
  if(LANG STREQUAL "C")
  ...
  # Set these two variables for Intel MPI: - I_MPI_DEBUG_INFO_STRIP: It adds
  # 'objcopy' lines to the compiler output. We support stripping them (see
  # below), but if we can avoid them in the first place, we should. -
  ...
  # little sense, since correct MPI usage from CMake already circumvenes this.
  ...
  # will also work. Open MPI also supports -show, but separates linker and
  # compiler information
  _mpi_check_compiler(${LANG} "-showme:compile" MPI_COMPILE_CMDLINE
                      MPI_COMPILER_RETURN)
  if(MPI_COMPILER_RETURN EQUAL "0")
    _mpi_check_compiler(${LANG} "-showme:link" MPI_LINK_CMDLINE
                        MPI_COMPILER_RETURN)
  ...
  # MPICH and MVAPICH offer -compile-info and -link-info. For modern versions,
  # both do the same as -show. However, for old versions, they do differ when
  # called for mpicxx and mpif90 and it's necessary to use them over -show in
  # order to find the removed MPI C++ bindings.
  if(NOT MPI_COMPILER_RETURN EQUAL "0")
    _mpi_check_compiler(${LANG} "-compile-info" MPI_COMPILE_CMDLINE
                        MPI_COMPILER_RETURN)

    if(MPI_COMPILER_RETURN EQUAL "0")
      _mpi_check_compiler(${LANG} "-link-info" MPI_LINK_CMDLINE
                          MPI_COMPILER_RETURN)
  ...
  # Cray compiler wrappers come usually without a separate mpicc/c++/ftn, but
  # offer --cray-print-opts=...
  if(NOT MPI_COMPILER_RETURN EQUAL "0")
    _mpi_check_compiler(${LANG} "--cray-print-opts=cflags" MPI_COMPILE_CMDLINE
                        MPI_COMPILER_RETURN)
  ...
  # MPICH, MVAPICH2 and Intel MPI just use "-show". Open MPI also offers this,
  # but the -showme commands are more specialized.
  if(NOT MPI_COMPILER_RETURN EQUAL "0")
  ...
  if(NOT MPI_COMPILER_RETURN EQUAL "0")
    _mpi_check_compiler(${LANG} "-showme" MPI_COMPILE_CMDLINE
                        MPI_COMPILER_RETURN)
  endif()

  if(MPI_COMPILER_RETURN EQUAL "0" AND DEFINED MPI_COMPILE_CMDLINE)
    # Intel MPI can be run with -compchk or I_MPI_CHECK_COMPILER set to 1. In
    # this case, -show will be prepended with a line to the compiler checker.
    # This is a script that performs compatibility checks and returns a non-zero
    # exit code together with an error if something fails. It has to be called
    # as "compchk.sh <arch> <compiler>". Here, <arch> is one out of 32 (i686),
    # 64 (ia64) or 32e (x86_64). The compiler is identified by filename, and can
    # be either the MPI compiler or the underlying compiler. NOTE: It is vital
    # to run this script while the environment variables are set up, otherwise
    # it can check the wrong compiler.
    if(MPI_COMPILE_CMDLINE MATCHES
       "^([^\" ]+/compchk.sh|\"[^\"]+/compchk.sh\") +([^ ]+)")
      # Now CMAKE_MATCH_1 contains the path to the compchk.sh file and
      # CMAKE_MATCH_2 the architecture flag.
      unset(COMPILER_CHECKER_OUTPUT)
      execute_process(
        COMMAND ${CMAKE_MATCH_1} ${CMAKE_MATCH_2} ${MPI_${LANG}_COMPILER}
        OUTPUT_VARIABLE COMPILER_CHECKER_OUTPUT
        OUTPUT_STRIP_TRAILING_WHITESPACE
        ERROR_VARIABLE COMPILER_CHECKER_OUTPUT
        ERROR_STRIP_TRAILING_WHITESPACE
        RESULT_VARIABLE MPI_COMPILER_RETURN)
      # If it returned a non-zero value, the check below will fail and cause the
      # interrogation to be aborted.
      if(NOT MPI_COMPILER_RETURN EQUAL "0")
        if(NOT MPI_FIND_QUIETLY)
          message(
            STATUS "Intel MPI compiler check failed: ${COMPILER_CHECKER_OUTPUT}"
          )
        endif()
      else()
  ...
  # Visual Studio parsers permit each flag prefixed by either / or -. We'll
  # normalize this to the - syntax we use for CMake purposes anyways.
  if(MSVC)
    foreach(_MPI_VARIABLE IN ITEMS COMPILE LINK)
      # The Intel MPI wrappers on Windows prefix their output with some
      # copyright boilerplate. To prevent possible problems, we discard this
      # text before proceeding with any further matching.
      string(
        REGEX
        REPLACE
          "^[^ ]+ for the Intel\\(R\\) MPI Library [^\n]+ for Windows\\*\nCopyright\\(C\\) [^\n]+, Intel Corporation\\. All rights reserved\\.\n\n"
          ""
          MPI_${_MPI_VARIABLE}_CMDLINE
          "${MPI_${_MPI_VARIABLE}_CMDLINE}")
      string(REGEX REPLACE "(^| )/" "\\1-" MPI_${_MPI_VARIABLE}_CMDLINE
                           "${MPI_${_MPI_VARIABLE}_CMDLINE}")
      string(REPLACE "-libpath:" "-LIBPATH:" MPI_${_MPI_VARIABLE}_CMDLINE
                     "${MPI_${_MPI_VARIABLE}_CMDLINE}")
    endforeach()
  endif()

  # For MSVC and cl-compatible compilers, the keyword /link indicates a point
  # after which everything following is passed to the linker. In this case, we
  # drop all prior information from the link line and treat any unknown extra
  # flags as linker flags.
  set(_MPI_FILTERED_LINK_INFORMATION FALSE)
  if(MSVC)
    if(MPI_LINK_CMDLINE MATCHES " -(link|LINK) ")
      string(REGEX REPLACE ".+-(link|LINK) +" "" MPI_LINK_CMDLINE
                           "${MPI_LINK_CMDLINE}")
      set(_MPI_FILTERED_LINK_INFORMATION TRUE)
    endif()
    string(REGEX REPLACE " +-(link|LINK) .+" "" MPI_COMPILE_CMDLINE
                         "${MPI_COMPILE_CMDLINE}")
  endif()
  ...
  if(UNIX)
    ...
    if(LANG STREQUAL "Fortran")
      # If MPICH (and derivates) didn't recognize the Fortran compiler include
      # flag during configuration, they'll return a set of three commands,
      # consisting out of a symlink command for mpif.h, the actual compiler
      # command and deletion of the created symlink. Especially with
      # M(VA)PICH-1, this appears to happen erroneously, and therefore we should
      # translate this output into an additional include directory and then drop
      # it from the output.
      if(MPI_COMPILE_CMDLINE MATCHES "^ln -s ([^\" ]+|\"[^\"]+\") mpif.h")
        get_filename_component(MPI_INCLUDE_DIRS_WORK "${CMAKE_MATCH_1}"
                               DIRECTORY)
        string(REGEX REPLACE "^ln -s ([^\" ]+|\"[^\"]+\") mpif.h\n" ""
                             MPI_COMPILE_CMDLINE "${MPI_COMPILE_CMDLINE}")
        string(REGEX REPLACE "^ln -s ([^\" ]+|\"[^\"]+\") mpif.h\n" ""
                             MPI_LINK_CMDLINE "${MPI_LINK_CMDLINE}")
        string(REGEX REPLACE "\nrm -f mpif.h$" "" MPI_COMPILE_CMDLINE
                             "${MPI_COMPILE_CMDLINE}")
        string(REGEX REPLACE "\nrm -f mpif.h$" "" MPI_LINK_CMDLINE
                             "${MPI_LINK_CMDLINE}")
      endif()
    endif()

    # If Intel MPI was configured for static linkage with -static_mpi, the
    # wrapper will by default strip debug information from resulting binaries
    # (see I_MPI_DEBUG_INFO_STRIP). Since we cannot process this information
    # into CMake logic, we need to discard the resulting objcopy commands from
    # the output.
    string(REGEX REPLACE "(^|\n)objcopy[^\n]+(\n|$)" "" MPI_COMPILE_CMDLINE
                         "${MPI_COMPILE_CMDLINE}")
    string(REGEX REPLACE "(^|\n)objcopy[^\n]+(\n|$)" "" MPI_LINK_CMDLINE
  ...
  # For Visual C++, extracting compiler options in a generic fashion isn't easy.
  # However, no MPI implementation on Windows seems to require any specific
  # ones, either.
  if(NOT MSVC)
    # Extract compile options from the compile command line.
    string(REGEX MATCHALL "(^| )-f([^\" ]+|\"[^\"]+\")" MPI_ALL_COMPILE_OPTIONS
                 "${MPI_COMPILE_CMDLINE}")

    foreach(_MPI_COMPILE_OPTION IN LISTS MPI_ALL_COMPILE_OPTIONS)
      string(REGEX REPLACE "^ " "" _MPI_COMPILE_OPTION "${_MPI_COMPILE_OPTION}")

      # Ignore -fstack-protector directives: These occur on MPICH and MVAPICH
      # when the libraries themselves were built with this flag. However, this
      # flag is unrelated to using MPI, and we won't match the accompanying
      # --param-ssp-size and -Wp,-D_FORTIFY_SOURCE flags and therefore produce
      # inconsistent results with the regularly flags. Similarly, aliasing flags
      # do not belong into our flag array. Also strip out `-framework` flags.
      if(NOT _MPI_COMPILE_OPTION MATCHES
         "^-f((no-|)(stack-protector|strict-aliasing)|PI[CE]|pi[ce]|ramework)")
        list(APPEND MPI_COMPILE_OPTIONS_WORK "${_MPI_COMPILE_OPTION}")
      endif()
    endforeach()
  endif()

  # For GNU-style compilers, it's possible to prefix includes and definitions
  # with certain flags to pass them only to the preprocessor. For CMake
  # purposes, we need to treat, but ignore such scopings. Note that we do not
  # support spaces between the arguments, i.e. -Wp,-I -Wp,/opt/mympi will not be
  # parsed correctly. This form does not seem to occur in any common MPI
  ...
  string(REGEX MATCHALL
               "(^| )${_MPI_PREPROCESSOR_FLAG_REGEX}-D *([^\" ]+|\"[^\"]+\")"
               MPI_ALL_COMPILE_DEFINITIONS "${MPI_COMPILE_CMDLINE}")

  foreach(_MPI_COMPILE_DEFINITION IN LISTS MPI_ALL_COMPILE_DEFINITIONS)
    string(REGEX REPLACE "^ ?${_MPI_PREPROCESSOR_FLAG_REGEX}-D *" ""
                         _MPI_COMPILE_DEFINITION "${_MPI_COMPILE_DEFINITION}")
  ...
  # If extracting failed to work, we'll try using -showme:incdirs. Unlike
  # before, we do this without the environment variables set up, but since only
  # MPICH derivates are affected by any of them, and -showme:... is only
  # supported by Open MPI and LAM/MPI, this isn't a concern.
  if(NOT MPI_ALL_INCLUDE_PATHS)
    _mpi_check_compiler(${LANG} "-showme:incdirs" MPI_INCDIRS_CMDLINE
                        MPI_INCDIRS_COMPILER_RETURN)
    if(MPI_INCDIRS_COMPILER_RETURN)
      separate_arguments(MPI_ALL_INCLUDE_PATHS NATIVE_COMMAND
                         "${MPI_INCDIRS_CMDLINE}")
    endif()
  endif()
  ...
  # The next step are linker flags and library directories. Here, we first take
  # the flags given in raw -L or -LIBPATH: syntax.
  ...
  foreach(_MPI_LPATH IN LISTS MPI_DIRECT_LINK_PATHS)
    string(REGEX REPLACE "(^| )${CMAKE_LIBRARY_PATH_FLAG} *" "" _MPI_LPATH
                         "${_MPI_LPATH}")
    list(APPEND MPI_ALL_LINK_PATHS "${_MPI_LPATH}")
  endforeach()

  # If the link commandline hasn't been filtered (e.g. when using MSVC and
  # /link), we need to extract the relevant parts first.
  if(NOT _MPI_FILTERED_LINK_INFORMATION)
    string(REGEX MATCHALL "(^| )(-Wl,|-Xlinker +)([^\" ]+|\"[^\"]+\")"
                 MPI_LINK_FLAGS "${MPI_LINK_CMDLINE}")

    # In this case, we could also find some indirectly given linker paths, e.g.
    # prefixed by -Xlinker or -Wl, Since syntaxes like -Wl,-L
    # -Wl,/my/path/to/lib are also valid, we parse these paths by first removing
    # -Wl, and -Xlinker from the list of filtered flags and then parse the
    # remainder of the output.
    string(REGEX REPLACE "(-Wl,|-Xlinker +)" "" MPI_LINK_FLAGS_RAW
                         "${MPI_LINK_FLAGS}")

    # Now we can parse the leftover output. Note that spaces can now be handled
    # since the above example would reduce to -L /my/path/to/lib and can be
    # extracted correctly.
    string(
      REGEX
        MATCHALL
        "^(${CMAKE_LIBRARY_PATH_FLAG},? *|--library-path=)([^\" ]+|\"[^\"]+\")"
        MPI_INDIRECT_LINK_PATHS "${MPI_LINK_FLAGS_RAW}")

    foreach(_MPI_LPATH IN LISTS MPI_INDIRECT_LINK_PATHS)
      string(REGEX REPLACE "^(${CMAKE_LIBRARY_PATH_FLAG},? *|--library-path=)"
                           "" _MPI_LPATH "${_MPI_LPATH}")
      list(APPEND MPI_ALL_LINK_PATHS "${_MPI_LPATH}")
    endforeach()
    ...
    string(
      REGEX
      REPLACE
        "(^| )(-Wl,|-Xlinker +)(${CMAKE_LIBRARY_PATH_FLAG},? *(-Wl,|-Xlinker +)?|--library-path=)([^\" ]+|\"[^\"]+\")"
        ""
        MPI_LINK_CMDLINE_FILTERED
        "${MPI_LINK_CMDLINE}")

    # Some MPI implementations pass on options they themselves were built with.
    # Since -z,noexecstack is a common hardening, we should strip it. In
    # general, the -z options should be undesirable.
    string(REGEX
           REPLACE "(^| )-Wl,-z(,[^ ]+| +-Wl,[^ ]+)" ""
                   MPI_LINK_CMDLINE_FILTERED "${MPI_LINK_CMDLINE_FILTERED}")
    string(REGEX
           REPLACE "(^| )-Xlinker +-z +-Xlinker +[^ ]+" ""
                   MPI_LINK_CMDLINE_FILTERED "${MPI_LINK_CMDLINE_FILTERED}")
    ...
    # As a next step, we assemble the linker flags extracted in a preliminary
    # flags string
    ...
    # In the filtered case, we obtain the link time flags by just stripping the
    # library paths.
    string(REGEX REPLACE "(^| )${CMAKE_LIBRARY_PATH_FLAG} *([^\" ]+|\"[^\"]+\")"
                         "" MPI_LINK_CMDLINE_FILTERED "${MPI_LINK_CMDLINE}")
  endif()

  # If we failed to extract any linker paths, we'll try using the
  # -showme:libdirs option with the MPI compiler. This will return a list of
  # folders, not a set of flags!
  if(NOT MPI_ALL_LINK_PATHS)
    _mpi_check_compiler(${LANG} "-showme:libdirs" MPI_LIBDIRS_CMDLINE
                        MPI_LIBDIRS_COMPILER_RETURN)
    if(MPI_LIBDIRS_COMPILER_RETURN)
      separate_arguments(MPI_ALL_LINK_PATHS NATIVE_COMMAND
                         "${MPI_LIBDIRS_CMDLINE}")
    endif()
  endif()

  # We need to remove potential quotes and convert the paths to CMake syntax
  # while resolving them, too.
  ...
  # Extract the set of libraries to link against from the link command line This
  # only makes sense if CMAKE_LINK_LIBRARY_FLAG is defined, i.e. a -lxxxx syntax
  ...
  if(CMAKE_LINK_LIBRARY_FLAG)
    ...
    foreach(_MPI_LIB_NAME IN LISTS MPI_LIBNAMES)
      ...
      string(REGEX REPLACE "^ ?${CMAKE_LINK_LIBRARY_FLAG}(:lib|:)?" ""
                           _MPI_LIB_NAME "${_MPI_LIB_NAME}")
      string(REPLACE "\"" "" _MPI_LIB_NAME "${_MPI_LIB_NAME}")
      list(APPEND MPI_LIB_NAMES_WORK "${_MPI_LIB_NAME}")
    endforeach()
  endif()

  # Treat linker objects given by full path, for example static libraries,
  # import libraries or shared libraries if there aren't any import libraries in
  # use on the system. Note that we do not consider CMAKE_<TYPE>_LIBRARY_PREFIX
  # intentionally here: The linker will for a given file decide how to link it
  # based on file type, not based on a prefix like 'lib'.
  ...
  string(REGEX MATCHALL "${_MPI_LIB_NAME_REGEX}" MPI_LIBNAMES
               "${MPI_LINK_CMDLINE}")
  foreach(_MPI_LIB_NAME IN LISTS MPI_LIBNAMES)
    # Do not match "-l:" flags
    string(REGEX MATCH "^ ?${CMAKE_LINK_LIBRARY_FLAG}:" _MPI_LIB_NAME_TEST
                 "${_MPI_LIB_NAME}")
  ...
  # Determine full path names for all of the libraries that one needs to link
  # against in an MPI program
  ...
  foreach(_MPI_LIB_NAME IN LISTS MPI_LIB_NAMES_WORK)
    ...
    find_library(
      MPI_${_MPI_PLAIN_LIB_NAME}_LIBRARY
    ...
    if(MPI_${_MPI_PLAIN_LIB_NAME}_LIBRARY)
      get_filename_component(_MPI_TAKEN_DIRECTORY
                             "${MPI_${_MPI_PLAIN_LIB_NAME}_LIBRARY}" DIRECTORY)
  ...
  foreach(_MPI_LIB_FULLPATH IN LISTS MPI_LIB_FULLPATHS_WORK)
    ...
    set(MPI_${_MPI_PLAIN_LIB_NAME}_LIBRARY
        "${_MPI_LIB_FULLPATH}"
        CACHE FILEPATH "Location of the ${_MPI_PLAIN_LIB_NAME} library for MPI")
    mark_as_advanced(MPI_${_MPI_PLAIN_LIB_NAME}_LIBRARY)
  endforeach()
  # Directly linked objects should be linked first in case some generic linker
  # flags are needed for them.
  if(MPI_DIRECT_LIB_NAMES_WORK)
    set(MPI_PLAIN_LIB_NAMES_WORK
        "${MPI_DIRECT_LIB_NAMES_WORK};${MPI_PLAIN_LIB_NAMES_WORK}")
  endif()

  # MPI might require pthread to work. The above mechanism wouldn't detect it,
  # but we need to link it in that case. -lpthread is covered by the normal
  # library treatment on the other hand.
  ...
  if(MPI_${LANG}_EXTRA_COMPILE_DEFINITIONS)
    list(APPEND MPI_COMPILE_DEFINITIONS_WORK
         "${MPI_${LANG}_EXTRA_COMPILE_DEFINITIONS}")
  endif()
  ...
  if(NOT MPI_${LANG}_COMPILE_OPTIONS)
    set(MPI_${LANG}_COMPILE_OPTIONS
        ${MPI_COMPILE_OPTIONS_WORK}
        CACHE STRING "MPI ${LANG} compilation options" FORCE)
  endif()
  if(NOT MPI_${LANG}_COMPILE_DEFINITIONS)
    set(MPI_${LANG}_COMPILE_DEFINITIONS
        ${MPI_COMPILE_DEFINITIONS_WORK}
        CACHE STRING "MPI ${LANG} compilation definitions" FORCE)
  endif()
  if(NOT MPI_${LANG}_COMPILER_INCLUDE_DIRS)
    set(MPI_${LANG}_COMPILER_INCLUDE_DIRS
        ${MPI_INCLUDE_DIRS_WORK}
        CACHE STRING "MPI ${LANG} compiler wrapper include directories" FORCE)
  endif()
  if(NOT MPI_${LANG}_LINK_FLAGS)
    set(MPI_${LANG}_LINK_FLAGS
        ${MPI_LINK_FLAGS_WORK}
        CACHE STRING "MPI ${LANG} linker flags" FORCE)
  endif()
  if(NOT MPI_${LANG}_LIB_NAMES)
    set(MPI_${LANG}_LIB_NAMES
...
function(_MPI_guess_settings LANG)
  ...
  # Currently only MSMPI and MPICH2 on Windows are supported, so we can skip
  # this search if we're not targeting that.
  if(WIN32)
    # MSMPI

    # The environment variables MSMPI_INC and MSMPILIB32/64 are the only ways of
    # locating the MSMPI_SDK, which is installed separately from the runtime.
    # Thus it's possible to have mpiexec but not MPI headers or import libraries
    # and vice versa.
    if(NOT MPI_GUESS_LIBRARY_NAME OR MPI_GUESS_LIBRARY_NAME STREQUAL "MSMPI")
      # We first attempt to locate the msmpi.lib. Should be find it, we'll
      # assume that the MPI present is indeed Microsoft MPI.
      ...
      if(MPI_msmpi_LIBRARY)
        # Next, we attempt to locate the MPI header. Note that for Fortran we
        # know that mpif.h is a way MSMPI can be used and therefore that header
        # has to be present.
        if(NOT MPI_${LANG}_ADDITIONAL_INCLUDE_DIRS)
          get_filename_component(MPI_MSMPI_INC_DIR "$ENV{MSMPI_INC}" REALPATH)
          set(MPI_${LANG}_ADDITIONAL_INCLUDE_DIRS
              "${MPI_MSMPI_INC_DIR}"
              CACHE STRING "MPI ${LANG} additional include directories" FORCE)
          unset(MPI_MSMPI_INC_DIR)
        endif()

        # For MSMPI, one can compile the MPI module by building the mpi.f90
        # shipped with the MSMPI SDK, thus it might be present or provided by
        # the user. Figuring out which is supported is done later on. The PGI
        # Fortran compiler for instance ships a prebuilt set of modules in its
        # own include folder. Should a user be employing PGI or have built its
        # own set and provided it via cache variables, the splitting routine
        # would have located the module files.

        # For C and C++, we're done here (MSMPI does not ship the MPI-2 C++
        # bindings) - however, for Fortran we need some extra library to glue
        # Fortran support together: MSMPI ships 2-4 Fortran libraries, each for
        # different Fortran compiler behaviors. The library names ending with a
        # c are using the cdecl calling convention, whereas those ending with an
        # s are for Fortran implementations using stdcall. Therefore, the 64-bit
        # MSMPI only ships those ending in 'c', whereas the 32-bit has both
        # variants available. The second difference is the last but one letter,
        # if it's an e(nd), the length of a string argument is passed by the
        # Fortran compiler after all other arguments on the parameter list, if
        # it's an m(ixed), it's passed immediately after the string address.

        # To summarize: - msmpifec: CHARACTER length passed after the parameter
        # list and using cdecl calling convention - msmpifmc: CHARACTER length
        # passed directly after string address and using cdecl calling
        # convention - msmpifes: CHARACTER length passed after the parameter
        # list and using stdcall calling convention - msmpifms: CHARACTER length
        # passed directly after string address and using stdcall calling
        # convention 32-bit MSMPI ships all four libraries, 64-bit MSMPI ships
        # only the first two.

        # As is, Intel Fortran and PGI Fortran both use the 'ec' variant of the
        # calling convention, whereas the old Compaq Visual Fortran compiler
        # defaulted to the 'ms' version. It's possible to make Intel Fortran use
        # the CVF calling convention using /iface:cvf, but we assume - and this
        # is also assumed in FortranCInterface - this isn't the case. It's also
        # possible to make CVF use the 'ec' variant, using
        # /iface=(cref,nomixed_str_len_arg).

        # Our strategy is now to locate all libraries, but enter msmpifec into
        # the LIB_NAMES array. Should this not be adequate it's a
        # straightforward way for a user to change the LIB_NAMES array and have
        # his library found. Still, this should not be necessary outside of
        # exceptional cases, as reasoned.
        if(LANG STREQUAL "Fortran")
          ...
          foreach(mpistrlenpos IN ITEMS e m)
            foreach(mpicallingconv IN LISTS MPI_MSMPI_CALLINGCONVS)
              find_library(
                MPI_msmpif${mpistrlenpos}${mpicallingconv}_LIBRARY
          ...
          # At this point we're *not* done. MSMPI requires an additional include
          # file for Fortran giving the value of MPI_AINT. This file is called
          # mpifptr.h located in the x64 and x86 subfolders, respectively.
          find_path(
            MPI_mpifptr_INCLUDE_DIR
            NAMES "mpifptr.h"
          ...
          if(NOT MPI_${LANG}_ADDITIONAL_INCLUDE_VARS)
            set(MPI_${LANG}_ADDITIONAL_INCLUDE_VARS
                "mpifptr"
                CACHE
                  STRING
                  "MPI ${LANG} additional include directory variables, given in the form MPI_<name>_INCLUDE_DIR."
                  FORCE)
          endif()
          mark_as_advanced(MPI_${LANG}_ADDITIONAL_INCLUDE_VARS
                           MPI_mpifptr_INCLUDE_DIR)
        else()
          if(NOT MPI_${LANG}_LIB_NAMES)
            set(MPI_${LANG}_LIB_NAMES
    ...
    # At this point there's not many MPIs that we could still consider. OpenMPI
    # 1.6.x and below supported Windows, but these ship compiler wrappers that
    # still work. The only other relevant MPI implementation without a wrapper
    # is MPICH2, which had Windows support in 1.4.1p1 and older.
    if(NOT MPI_GUESS_FOUND AND (NOT MPI_GUESS_LIBRARY_NAME
                                OR MPI_GUESS_LIBRARY_NAME STREQUAL "MPICH2"))
      ...
      # All of C, C++ and Fortran will need mpi.lib, so we'll look for this
      # first
      find_library(
        MPI_mpi_LIBRARY
        NAMES mpi
      ...
      if(MPI_mpi_LIBRARY)
        ...
        # If MPI-2 C++ bindings are requested, we need to locate cxx.lib as
        # well. Otherwise, MPICH_SKIP_MPICXX will be defined and these bindings
        # aren't needed.
        if(LANG STREQUAL "CXX" AND NOT MPI_CXX_SKIP_MPICXX)
          ...
          # For Fortran, MPICH2 provides three different libraries: fmpich2.lib
          # which uses uppercase symbols and cdecl, fmpich2s.lib which uses
          # uppercase symbols and stdcall (32-bit only), fmpich2g.lib which uses
          # lowercase symbols with double underscores and cdecl. fmpich2s.lib
          # would be useful for Compaq Visual Fortran, fmpich2g.lib has to be
          # used with GNU g77 and is also provided in the form of an .a archive
          # for MinGW and Cygwin. From our perspective, fmpich2.lib is the only
          # one we need to try, and if it doesn't work with the given Fortran
          # compiler we'd find out later on during validation
        elseif(LANG STREQUAL "Fortran")
          find_library(
            MPI_fmpich2_LIBRARY
            NAMES fmpich2
            HINTS ${MPI_MPICH_PREFIX_PATHS})
          find_library(
            MPI_fmpich2s_LIBRARY
            NAMES fmpich2s
            HINTS ${MPI_MPICH_PREFIX_PATHS})
          find_library(
            MPI_fmpich2g_LIBRARY
            NAMES fmpich2g
            HINTS ${MPI_MPICH_PREFIX_PATHS})
          mark_as_advanced(MPI_fmpich2_LIBRARY MPI_fmpich2s_LIBRARY
                           MPI_fmpich2g_LIBRARY)
        ...
        if(NOT MPI_${LANG}_ADDITIONAL_INCLUDE_DIRS)
          # For MPICH2, the include folder would be in ../include relative to
          # the library folder.
          get_filename_component(MPI_MPICH_ROOT_DIR "${MPI_mpi_LIBRARY}"
                                 DIRECTORY)
          get_filename_component(MPI_MPICH_ROOT_DIR "${MPI_MPICH_ROOT_DIR}"
                                 DIRECTORY)
          if(IS_DIRECTORY "${MPI_MPICH_ROOT_DIR}/include")
            set(MPI_${LANG}_ADDITIONAL_INCLUDE_DIRS
                "${MPI_MPICH_ROOT_DIR}/include"
                CACHE
                  STRING
                  "MPI ${LANG} additional include directory variables, given in the form MPI_<name>_INCLUDE_DIR."
                  FORCE)
...
function(_MPI_adjust_compile_definitions LANG)
  if(LANG STREQUAL "CXX")
    # To disable the C++ bindings, we need to pass some definitions since the
    # mpi.h header has to deal with both C and C++ bindings in MPI-2.
    if(MPI_CXX_SKIP_MPICXX AND NOT MPI_${LANG}_COMPILE_DEFINITIONS MATCHES
                               "SKIP_MPICXX")
      # MPICH_SKIP_MPICXX is being used in MPICH and derivatives like MVAPICH or
      # Intel MPI OMPI_SKIP_MPICXX is being used in Open MPI _MPICC_H is being
      # used for IBM Platform MPI
      list(APPEND MPI_${LANG}_COMPILE_DEFINITIONS "MPICH_SKIP_MPICXX"
           "OMPI_SKIP_MPICXX" "_MPICC_H")
      set(MPI_${LANG}_COMPILE_DEFINITIONS
          "${MPI_${LANG}_COMPILE_DEFINITIONS}"
          CACHE STRING "MPI ${LANG} compilation definitions" FORCE)
    endif()
...
macro(_MPI_assemble_libraries LANG)
  ...
  # Only for libraries do we need to check whether the compiler's linking stage
  # is separate.
  if(NOT MPI_${LANG}_COMPILER STREQUAL CMAKE_${LANG}_COMPILER
     OR NOT MPI_${LANG}_WORKS_IMPLICIT)
...
macro(_MPI_assemble_include_dirs LANG)
  set(MPI_${LANG}_INCLUDE_DIRS ${MPI_${LANG}_COMPILER_INCLUDE_DIRS}
                               ${MPI_${LANG}_ADDITIONAL_INCLUDE_DIRS})
...
macro(_MPI_split_include_dirs LANG)
  ...
  if(MPI_${LANG}_INCLUDE_PATH)
    list(APPEND MPI_${LANG}_ADDITIONAL_INCLUDE_DIRS
         "${MPI_${LANG}_INCLUDE_PATH}")
  endif()

  # We try to find the headers/modules among those paths (and system paths) For
  # C/C++, we just need to have a look for mpi.h.
  if(LANG MATCHES "^(C|CXX)$")
    find_path(MPI_${LANG}_HEADER_DIR "mpi.h"
              HINTS ${MPI_${LANG}_COMPILER_INCLUDE_DIRS}
                    ${MPI_${LANG}_ADDITIONAL_INCLUDE_DIRS})
    ...
    # Fortran is more complicated here: An implementation could provide any of
    # the Fortran 77/90/2008 APIs for MPI. For example, MSMPI only provides
    # Fortran 77 and - if mpi.f90 is built - potentially a Fortran 90 module.
  elseif(LANG STREQUAL "Fortran")
    ...
    if(MPI_${LANG}_ADDITIONAL_INCLUDE_DIRS)
      list(REMOVE_ITEM MPI_${LANG}_ADDITIONAL_INCLUDE_DIRS
           "${MPI_${LANG}_F77_HEADER_DIR}" "${MPI_${LANG}_MODULE_DIR}")
    endif()
  ...
  if(MPI_${LANG}_ADDITIONAL_INCLUDE_DIRS)
    ...
    foreach(MPI_IMPLICIT_INC_DIR IN
            LISTS CMAKE_${LANG}_IMPLICIT_LINK_DIRECTORIES)
      list(REMOVE_ITEM MPI_${LANG}_ADDITIONAL_INCLUDE_DIRS
           ${MPI_IMPLICIT_INC_DIR})
    endforeach()
  endif()
...
macro(_MPI_create_imported_target LANG)
  ...
  # When this is consumed for compiling CUDA, use '-Xcompiler' to wrap
  # '-pthread' and '-fexceptions'.
  string(
    REPLACE "-pthread"
  ...
  string(
    REPLACE
      "-fexceptions"
      "$<$<COMPILE_LANG_AND_ID:CUDA,NVIDIA>:SHELL:-Xcompiler >-fexceptions"
      _MPI_${LANG}_COMPILE_OPTIONS "${_MPI_${LANG}_COMPILE_OPTIONS}")
  set_property(
    TARGET MPI::MPI_${LANG} PROPERTY INTERFACE_COMPILE_OPTIONS
                                     "${_MPI_${LANG}_COMPILE_OPTIONS}")
  unset(_MPI_${LANG}_COMPILE_OPTIONS)

  set_property(
    TARGET MPI::MPI_${LANG} PROPERTY INTERFACE_COMPILE_DEFINITIONS
                                     "${MPI_${LANG}_COMPILE_DEFINITIONS}")

  if(MPI_${LANG}_LINK_FLAGS)
    string(REPLACE "," "$<COMMA>" _MPI_${LANG}_LINK_FLAGS
                   "${MPI_${LANG}_LINK_FLAGS}")
  ...
  # If the compiler links MPI implicitly, no libraries will be found as they're
  # contained within CMAKE_<LANG>_IMPLICIT_LINK_LIBRARIES already.
...
function(_MPI_try_staged_settings LANG MPI_TEST_FILE_NAME MODE RUN_BINARY
  ...
  if(RUN_BINARY)
    try_run(
      ...
      MPI_RESULT_${LANG}_${MPI_TEST_FILE_NAME}_${MODE} SOURCE_FROM_VAR
      "${MPI_TEST_SOURCE_FILE}" MPI_TEST_SOURCE_CONTENT
      COMPILE_DEFINITIONS ${MPI_TEST_COMPILE_DEFINITIONS} LINK_LIBRARIES
                          MPI::MPI_${LANG}
  ...
  if(NOT SUPPRESS_ERRORS)
    ...
    elseif(DEFINED MPI_RUN_RESULT_${LANG}_${MPI_TEST_FILE_NAME}_${MODE}
           AND MPI_RUN_RESULT_${LANG}_${MPI_TEST_FILE_NAME}_${MODE})
      file(
        APPEND ${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/CMakeError.log
...
macro(_MPI_check_lang_works LANG SUPPRESS_ERRORS)
  ...
  # provides: - the mpi_f08 module - *both*, the mpi module and 'mpif.h' Since
  # older MPI standards (MPI-1) did not define anything but 'mpif.h', we need to
  # check all three individually.
  if(NOT MPI_${LANG}_WORKS)
    if(LANG STREQUAL "Fortran")
      ...
      endforeach()
      # MPI-1 versions had no MPI_INTGER_KIND defined, so we need to try without
      # it. However, MPI-1 also did not define the Fortran 90 and 08 modules, so
      ...
      # might not be MPI-2 C++ support, but there can't be MPI-2 C++ support
      # without the C bindings being present, so checking for them is
      # sufficient.
...
# Some systems install various MPI implementations in separate folders in some
# MPI prefix This macro enumerates all such subfolders and adds them to the list
# of hints that will be searched.
macro(MPI_search_mpi_prefix_folder PREFIX_FOLDER)
  if(EXISTS "${PREFIX_FOLDER}")
    file(
      GLOB _MPI_folder_children
      RELATIVE "${PREFIX_FOLDER}"
      "${PREFIX_FOLDER}/*")
...
if(CMAKE_HOST_SYSTEM_NAME STREQUAL "Linux")
  ...
  # /usr/lib64/mpi/gcc/<name> We enumerate the subfolders and append each as a
  # prefix
...
elseif(CMAKE_HOST_SYSTEM_NAME STREQUAL "FreeBSD")
  ...
  # implementations will be found in /usr/local/mpi/<name>
  MPI_search_mpi_prefix_folder("/usr/local/mpi")
endif()

# Most MPI distributions have some form of mpiexec or mpirun which gives us
# something we can look for. The MPI standard does not mandate the existence of
# either, but instead only makes requirements if a distribution ships an mpiexec
# program (mpirun executables are not regulated by the standard).

# We defer searching for mpiexec binaries belonging to guesses until later. By
# doing so, mismatches between mpiexec and the MPI we found should be reduced.
...
# exists in (typically bin). This gives us a fairly reliable base directory to
# search for /bin /lib and /include from.
get_filename_component(_MPI_BASE_DIR "${MPIEXEC_EXECUTABLE}" PATH)
get_filename_component(_MPI_BASE_DIR "${_MPI_BASE_DIR}" PATH)

# According to the MPI standard, section 8.8 -n is a guaranteed, and the only
# guaranteed way to launch an MPI process using mpiexec if such a program
# exists.
...
cmake_host_system_information(RESULT _MPIEXEC_NUMPROCS
...
mark_as_advanced(MPIEXEC_EXECUTABLE MPIEXEC_NUMPROC_FLAG MPIEXEC_PREFLAGS
                 MPIEXEC_POSTFLAGS MPIEXEC_MAX_NUMPROCS)

# =============================================================================
# Backward compatibility input hacks.  Propagate the FindMPI hints to C and CXX
# if the respective new versions are not defined.  Translate the old MPI_LIBRARY
# and MPI_EXTRA_LIBRARY to respective MPI_${LANG}_LIBRARIES.
#
...
if(NOT MPI_IGNORE_LEGACY_VARIABLES)
  foreach(LANG IN ITEMS C CXX)
    ...
    # Set new vars based on their old equivalents, if the new versions are not
    # already set.
    ...
    if(MPI_${LANG}_COMPILE_FLAGS)
      separate_arguments(MPI_SEPARATE_FLAGS NATIVE_COMMAND
                         "${MPI_${LANG}_COMPILE_FLAGS}")
    ...
    # If a list of libraries was given, we'll split it into new-style cache
    # variables
    unset(MPI_${LANG}_EXTRA_LIB_NAMES)
    if(NOT MPI_${LANG}_LIB_NAMES)
      foreach(_MPI_LIB IN LISTS MPI_${LANG}_LIBRARIES MPI_LIBRARY
                                MPI_EXTRA_LIBRARY)
        if(_MPI_LIB)
...
# If the user specified a library name we assume they prefer that library over a
# wrapper. If not, they can disable skipping manually.
...
foreach(LANG IN ITEMS C CXX Fortran)
  if(CMAKE_${LANG}_COMPILER_LOADED)
    ...
    elseif(
      LANG STREQUAL "CXX"
      AND NOT MPI_CXX_SKIP_MPICXX
      AND MPICXX IN_LIST MPI_FIND_COMPONENTS)
      set(_MPI_FIND_${LANG} TRUE)
    else()
    ...
    if(LANG IN_LIST MPI_FIND_COMPONENTS)
      string(
        APPEND
        _MPI_FAIL_REASON
        "MPI component '${LANG}' was requested, but language ${LANG} is not enabled.  "
      )
    endif()
  endif()
  if(_MPI_FIND_${LANG})
    if(LANG STREQUAL "CXX" AND NOT MPICXX IN_LIST MPI_FIND_COMPONENTS)
      option(MPI_CXX_SKIP_MPICXX
    ...
    if(NOT
       (MPI_${LANG}_LIB_NAMES
        AND (MPI_${LANG}_INCLUDE_PATH
             OR MPI_${LANG}_INCLUDE_DIRS
             OR MPI_${LANG}_COMPILER_INCLUDE_DIRS)))
      ...
      if(NOT MPI_${LANG}_COMPILER AND NOT MPI_ASSUME_NO_BUILTIN_MPI)
        ...
        # compiler supports MPI on its own, which is the case on e.g. Cray
        # PrgEnv.
        _mpi_create_imported_target(${LANG})
        _mpi_check_lang_works(${LANG} TRUE)

        # If the compiler can build MPI code on its own, it functions as an MPI
        # compiler and we'll set the variable to point to it.
        if(MPI_${LANG}_WORKS)
          set(MPI_${LANG}_COMPILER
              "${CMAKE_${LANG}_COMPILER}"
              CACHE FILEPATH "MPI compiler for ${LANG}" FORCE)
      ...
      if(NOT MPI_${LANG}_COMPILER STREQUAL CMAKE_${LANG}_COMPILER
         OR NOT MPI_${LANG}_WORKS)
        set(MPI_${LANG}_WRAPPER_FOUND FALSE)
        set(MPI_PINNED_COMPILER FALSE)

        if(NOT MPI_SKIP_COMPILER_WRAPPER)
          if(MPI_${LANG}_COMPILER)
            # If the user supplies a compiler *name* instead of an absolute
            # path, assume that we need to find THAT compiler.
            if(NOT IS_ABSOLUTE "${MPI_${LANG}_COMPILER}")
              # Get rid of our default list of names and just search for the
              # name the user wants.
          ...
          # If we have an MPI base directory, we'll try all compiler names in
          # that one first. This should prevent mixing different MPI
          # environments
          if(_MPI_BASE_DIR)
            find_program(
              MPI_${LANG}_COMPILER
          ...
          # If the base directory did not help (for example because the mpiexec
          # isn't in the same directory as the compilers), we shall try
          # searching in the default paths.
          find_program(
            MPI_${LANG}_COMPILER
            NAMES ${_MPI_${LANG}_COMPILER_NAMES}
            PATH_SUFFIXES bin sbin
          ...
          if(MPI_${LANG}_COMPILER STREQUAL CMAKE_${LANG}_COMPILER)
            ...
            # interrogate it. Otherwise, MPI compilers for which CMake has
            # separate linking stages, e.g. Intel MPI on Windows where link.exe
            # is being used directly during linkage instead of
            # CMAKE_<LANG>_COMPILER will not work.
            if(NOT MPI_${LANG}_WORKS)
              set(MPI_${LANG}_WORKS_IMPLICIT FALSE)
        ...
        # have NOT found an mpic++-like compiler wrapper (previous block), and
        # we do NOT use the Cray cc/CC compiler wrappers as CC/CXX CMake
        # compiler. So as a last resort, we now interrogate cc/CC/ftn for MPI
        # flags.
        ...
        if(NOT MPI_PINNED_COMPILER AND NOT MPI_${LANG}_WRAPPER_FOUND)
          ...
          if(_MPI_PKG AND PKG_CONFIG_FOUND)
            ...
            if(MPI_${LANG}_PKG_FOUND)
              set(MPI_${LANG}_COMPILE_OPTIONS
                  ${MPI_${LANG}_PKG_CFLAGS}
                  CACHE STRING "MPI ${LANG} compilation options" FORCE)
              set(MPI_${LANG}_INCLUDE_PATH
                  ${MPI_${LANG}_PKG_INCLUDE_DIRS}
                  CACHE STRING "MPI ${LANG} include directories" FORCE)
              set(MPI_${LANG}_LINK_FLAGS
                  ${MPI_${LANG}_PKG_LDFLAGS}
                  CACHE STRING "MPI ${LANG} linker flags" FORCE)
              set(MPI_${LANG}_LIB_NAMES
                  ${MPI_${LANG}_PKG_LIBRARIES}
                  CACHE STRING "MPI ${LANG} libraries to link against" FORCE)
              foreach(_MPI_LIB IN LISTS MPI_${LANG}_LIB_NAMES)
                if(_MPI_LIB)
                  get_filename_component(_MPI_PLAIN_LIB_NAME "${_MPI_LIB}"
                                         NAME_WE)
                  get_filename_component(_MPI_LIB_NAME "${_MPI_LIB}" NAME)
                  get_filename_component(_MPI_LIB_DIR "${_MPI_LIB}" DIRECTORY)
                  find_library(
          ...
          if(NOT MPI_SKIP_GUESSING AND NOT MPI_${LANG}_PKG_FOUND)
            ...
            if(LANG STREQUAL "CXX" AND MPI_C_WRAPPER_FOUND)
              ...
              set(MPI_${LANG}_COMPILE_DEFINITIONS
                  ...
                  CACHE STRING "MPI ${LANG} compilation definitions")
              set(MPI_${LANG}_COMPILER_INCLUDE_DIRS
                  ${MPI_C_INCLUDE_DIRS}
                  CACHE STRING
                        "MPI ${LANG} compiler wrapper include directories")
              set(MPI_${LANG}_LINK_FLAGS
                  ${MPI_C_LINK_FLAGS}
    ...
    set(MPI_${LANG}_COMPILE_OPTIONS
        ""
        CACHE STRING "MPI ${LANG} compilation flags")
    set(MPI_${LANG}_COMPILE_DEFINITIONS
        ""
        CACHE STRING "MPI ${LANG} compilation definitions")
    set(MPI_${LANG}_COMPILER_INCLUDE_DIRS
        ""
        CACHE STRING "MPI ${LANG} compiler wrapper include directories")
    set(MPI_${LANG}_ADDITIONAL_INCLUDE_DIRS
        ""
    ...
    # If we've found MPI, then we'll perform additional analysis: Determine the
    # MPI version, MPI library version, supported MPI APIs (i.e. MPI-2 C++
    # bindings). For Fortran we also need to find specific parameters if we're
    # under MPI-3.
    if(MPI_${LANG}_WORKS)
      ...
      # At this point, we know the bindings present but not the MPI version or
      # anything else.
      ...
      # For Fortran, we'll want to use the most modern MPI binding to test
      # capabilities other than the Fortran parameters, since those depend on
      # the method of consumption. For C++, we can always use the C bindings,
      # and should do so, since the C++ bindings do not exist in MPI-3 whereas
      # the C bindings do, and the C++ bindings never offered any feature
      # advantage over their C counterparts.
      if(LANG STREQUAL "Fortran")
        ...
        # Another difference between C and Fortran is that we can't use the
        # preprocessor to determine whether MPI_VERSION and MPI_SUBVERSION are
        # provided. These defines did not exist in MPI 1.0 and 1.1 and therefore
        # might not exist. For C/C++, test_mpi.c will handle the MPI_VERSION
        # extraction, but for Fortran, we need mpiver.f90.
        if(NOT DEFINED MPI_${LANG}_VERSION)
          _mpi_try_staged_settings(${LANG} mpiver ${MPI_${LANG}_HIGHEST_METHOD}
                                   FALSE FALSE)
          if(MPI_RESULT_${LANG}_mpiver_${MPI_${LANG}_HIGHEST_METHOD})
            file(
              STRINGS ${MPI_BIN_FOLDER}/mpiver_${LANG}.bin _MPI_VERSION_STRING
              LIMIT_COUNT 1
              REGEX "INFO:MPI-VER")
            if(_MPI_VERSION_STRING MATCHES
              ...
              set(MPI_${LANG}_VERSION
                  "${MPI_${LANG}_VERSION_MAJOR}.${MPI_${LANG}_VERSION_MINOR}")
            endif()
          endif()
        endif()

        # Finally, we want to find out which capabilities a given interface
        # supports, compare the MPI-3 standard. This is determined by interface
        # specific parameters MPI_SUBARRAYS_SUPPORTED and
        # MPI_ASYNC_PROTECTS_NONBLOCKING and might vary between the different
        # methods of consumption.
        if(MPI_DETERMINE_Fortran_CAPABILITIES
           AND NOT MPI_Fortran_CAPABILITIES_DETERMINED)
          foreach(mpimethod IN ITEMS F08_MODULE F90_MODULE F77_HEADER)
            if(MPI_${LANG}_HAVE_${mpimethod})
              ...
              _mpi_try_staged_settings(${LANG} fortranparam_mpi ${mpimethod}
                                       TRUE FALSE)
              if(MPI_RESULT_${LANG}_fortranparam_mpi_${mpimethod}
                 AND NOT
                     "${MPI_RUN_RESULT_${LANG}_fortranparam_mpi_${mpimethod}}"
        ...
        # By the MPI-2 standard, MPI_VERSION and MPI_SUBVERSION are valid for
        # both C and C++ bindings.
        if(NOT DEFINED MPI_${LANG}_VERSION)
          file(
            STRINGS ${MPI_BIN_FOLDER}/test_mpi_${LANG}.bin _MPI_VERSION_STRING
            LIMIT_COUNT 1
            REGEX "INFO:MPI-VER")
          if(_MPI_VERSION_STRING MATCHES
             ".*INFO:MPI-VER\\[([0-9]+)\\.([0-9]+)\\].*")
      ...
      # At this point, we have dealt with determining the MPI version and
      # parameters for each Fortran method available. The one remaining issue is
      # to determine which MPI library is installed. Determining the version and
      # vendor of the MPI library is only possible via MPI_Get_library_version()
      # at runtime, and therefore we cannot do this while cross-compiling (a
      # user may still define MPI_<lang>_LIBRARY_VERSION_STRING themselves and
      # we'll attempt splitting it, which is equivalent to provide the try_run
      # output). It's also worth noting that the installed version string can
      # depend on the language, or on the system the binary runs on if MPI is
      ...
      if(MPI_DETERMINE_LIBRARY_VERSION AND NOT
        ...
        if(MPI_RESULT_${LANG}_libver_mpi_${MPI_${LANG}_HIGHEST_METHOD}
           AND MPI_RUN_RESULT_${LANG}_libver_mpi_${MPI_${LANG}_HIGHEST_METHOD}
               EQUAL "0")
          string(
            STRIP
    ...
    if(NOT MPI_${LANG}_COMPILER STREQUAL CMAKE_${LANG}_COMPILER)
      ...
      if(LANG STREQUAL "Fortran")
        # For Fortran we only need one of the module or header directories to
        # have *some* support for MPI.
      ...
      # Append the works variable now. If the settings did not work, this will
      # show up properly.
      list(APPEND MPI_${LANG}_REQUIRED_VARS "MPI_${LANG}_WORKS")
    else()
      # If the compiler worked implicitly, use its path as output. Should the
      # compiler variable be set, we also require it to work.
...
unset(_MPI_REQ_VARS)
foreach(LANG IN ITEMS C CXX Fortran)
  if((NOT MPI_FIND_COMPONENTS AND CMAKE_${LANG}_COMPILER_LOADED)
     OR LANG IN_LIST MPI_FIND_COMPONENTS)
