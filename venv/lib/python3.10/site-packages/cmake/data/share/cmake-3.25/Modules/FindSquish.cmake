...
set(SQUISH_INSTALL_DIR_STRING
    "Directory containing the bin, doc, and lib directories for Squish; this should be the root of the installation directory."
)
...
if(NOT SQUISH_INSTALL_DIR)
  ...
  foreach(dir ${SQUISH_INSTALL_DIR_SEARCH2})
    set(SQUISH_INSTALL_DIR_SEARCH ${SQUISH_INSTALL_DIR_SEARCH}
                                  "${dir}/../lib/fltk")
  endforeach()
  string(REPLACE "//" "/" SQUISH_INSTALL_DIR_SEARCH
  ...
  find_path(
    ...
    HINTS # Look for an environment variable SQUISH_INSTALL_DIR.
...
if(SQUISH_INSTALL_DIR)
  ...
  if(NOT SQUISH_CLIENT_EXECUTABLE)
    find_program(
      ...
      ${SQUISH_INSTALL_DIR}/bin/squishrunner${CMAKE_EXECUTABLE_SUFFIX}
      DOC "The ${SQUISH_CLIENT_EXECUTABLE_STRING}")
  endif()
  ...
  if(NOT SQUISH_SERVER_EXECUTABLE)
    find_program(
      ...
      ${SQUISH_INSTALL_DIR}/bin/squishserver${CMAKE_EXECUTABLE_SUFFIX}
      DOC "The ${SQUISH_SERVER_EXECUTABLE_STRING}")
  endif()
...
if(SQUISH_CLIENT_EXECUTABLE)
  ...
  execute_process(
    COMMAND "${SQUISH_CLIENT_EXECUTABLE}" --version
  ...
  if("${_squishVersionOutput}" MATCHES "([0-9]+)\\.([0-9]+)\\.([0-9]+)")
    ...
    set(SQUISH_VERSION
        "${SQUISH_VERSION_MAJOR}.${SQUISH_VERSION_MINOR}.${SQUISH_VERSION_PATCH}"
    )
  endif()
else()
...
find_package_handle_standard_args(
  Squish
  REQUIRED_VARS SQUISH_INSTALL_DIR SQUISH_CLIENT_EXECUTABLE
                SQUISH_SERVER_EXECUTABLE
...
macro(squish_v3_add_test testName testAUT testCase envVars testWraper)
  if("${SQUISH_VERSION_MAJOR}" STRGREATER "3")
    message(
      STATUS
        "Using squish_v3_add_test(), but SQUISH_VERSION_MAJOR is ${SQUISH_VERSION_MAJOR}.\nThis may not work."
    )
  endif()
  ...
  add_test(
    ...
    -P
    "${_SQUISH_MODULE_DIR}/SquishTestScript.cmake")
...
function(squish_v4_add_test testName)
  if(NOT "${SQUISH_VERSION_MAJOR}" STRGREATER "3")
    message(
      STATUS
        "Using squish_add_test(), but SQUISH_VERSION_MAJOR is ${SQUISH_VERSION_MAJOR}.\nThis may not work."
    )
  endif()
  ...
  if(_SQUISH_UNPARSED_ARGUMENTS)
    message(
      FATAL_ERROR
        "Unknown keywords given to SQUISH_ADD_TEST(): \"${_SQUISH_UNPARSED_ARGUMENTS}\""
    )
  endif()
  ...
  if(NOT _SQUISH_SUITE)
    message(
      FATAL_ERROR "Required argument SUITE not given for SQUISH_ADD_TEST()")
  endif()

  if(NOT _SQUISH_TEST)
    message(
      FATAL_ERROR "Required argument TEST not given for SQUISH_ADD_TEST()")
  endif()
  ...
  if(NOT EXISTS "${absTestSuite}")
    message(
      FATAL_ERROR
        "Could not find squish test suite ${_SQUISH_SUITE} (checked ${absTestSuite})"
    )
  endif()
  ...
  if(NOT EXISTS "${absTestCase}")
    message(
      FATAL_ERROR
        "Could not find squish testcase ${_SQUISH_TEST} (checked ${absTestCase})"
    )
  endif()
  ...
  add_test(
    NAME ${testName}
    COMMAND
      ${CMAKE_COMMAND} -V -VV "-Dsquish_version:STRING=4"
      ...
      "-Dsquish_post_command:STRING=${_SQUISH_POST_COMMAND}" -P
      "${_SQUISH_MODULE_DIR}/SquishTestScript.cmake")
  set_tests_properties(${testName} PROPERTIES FAIL_REGULAR_EXPRESSION
                                              "FAIL;FAILED;ERROR;FATAL")
endfunction()
