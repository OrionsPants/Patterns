...
foreach(LANG IN ITEMS C CXX Fortran)
  if("${CMAKE_${LANG}_COMPILER_ID}" MATCHES "Intel")
    if(NOT _IRSL_HAVE_Intel)
      # The oneAPI icx/ifx compilers are under ${os}/bin. The classic
      # icc/icpc/icl/ifort compilers may be under ${os}/bin/intel64.
      ...
      if(WIN32)
        set(_Intel_possible_redistdirs
            ...
            "${_Intel_basedir}/../../redist/${_Intel_archdir}/compiler")
      elseif(APPLE)
        set(_Intel_possible_redistdirs "${_Intel_basedir}/../../compiler/lib")
      else()
        set(_Intel_possible_redistdirs
...
if(MSVC)
  ...
  if(MSVC_VERSION EQUAL 1300)
    set(__install__libs "${SYSTEMROOT}/system32/msvcp70.dll"
                        "${SYSTEMROOT}/system32/msvcr70.dll")
  endif()

  if(MSVC_VERSION EQUAL 1310)
    set(__install__libs "${SYSTEMROOT}/system32/msvcp71.dll"
  ...
  if(MSVC_TOOLSET_VERSION EQUAL 80)
    ...
    find_path(
      MSVC_REDIST_DIR
      NAMES ${CMAKE_MSVC_ARCH}/Microsoft.VC80.CRT/Microsoft.VC80.CRT.manifest
      PATHS "${msvc_install_dir}/../../VC/redist" "${base_dir}/VC/redist")
    mark_as_advanced(MSVC_REDIST_DIR)
    set(MSVC_CRT_DIR "${MSVC_REDIST_DIR}/${CMAKE_MSVC_ARCH}/Microsoft.VC80.CRT")

    # Install the manifest that allows DLLs to be loaded from the directory
    # containing the executable.
    if(NOT CMAKE_INSTALL_DEBUG_LIBRARIES_ONLY)
      set(__install__libs
          "${MSVC_CRT_DIR}/Microsoft.VC80.CRT.manifest"
          "${MSVC_CRT_DIR}/msvcm80.dll" "${MSVC_CRT_DIR}/msvcp80.dll"
    ...
    if(CMAKE_INSTALL_DEBUG_LIBRARIES)
      ...
      set(__install__libs
          ${__install__libs} "${MSVC_CRT_DIR}/Microsoft.VC80.DebugCRT.manifest"
          "${MSVC_CRT_DIR}/msvcm80d.dll" "${MSVC_CRT_DIR}/msvcp80d.dll"
  ...
  if(MSVC_TOOLSET_VERSION EQUAL 90)
    ...
    find_path(
      MSVC_REDIST_DIR
      NAMES ${CMAKE_MSVC_ARCH}/Microsoft.VC90.CRT/Microsoft.VC90.CRT.manifest
      PATHS "${msvc_install_dir}/../../VC/redist"
    ...
    if(NOT CMAKE_INSTALL_DEBUG_LIBRARIES_ONLY)
      set(__install__libs
          ...
          "${MSVC_CRT_DIR}/msvcm90.dll" "${MSVC_CRT_DIR}/msvcp90.dll"
    ...
    if(CMAKE_INSTALL_DEBUG_LIBRARIES)
      ...
      set(__install__libs
          ${__install__libs} "${MSVC_CRT_DIR}/Microsoft.VC90.DebugCRT.manifest"
          "${MSVC_CRT_DIR}/msvcm90d.dll" "${MSVC_CRT_DIR}/msvcp90d.dll"
  ...
  if(_MSVCRT_DLL_VERSION)
    ...
    # Find the runtime library redistribution directory.
    if(vs VERSION_LESS 15
       AND DEFINED MSVC${vs}_REDIST_DIR
       AND EXISTS "${MSVC${vs}_REDIST_DIR}")
      set(MSVC_REDIST_DIR "${MSVC${vs}_REDIST_DIR}") # use old cache entry
    endif()
    if(NOT vs VERSION_LESS 15)
      set(_vs_redist_paths "")
      # The toolset and its redistributables may come with any VS version 15 or
      # newer.
      ...
      get_filename_component(
        _vs_dir
        "[HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\VisualStudio\\${vs}.0;InstallDir]"
        ABSOLUTE)
      set(programfilesx86 "ProgramFiles(x86)")
      set(_vs_redist_paths
          "${_vs_dir}/../../VC/redist" "${base_dir}/VC/redist"
    ...
    find_path(
      MSVC_REDIST_DIR
      NAMES ${CMAKE_MSVC_ARCH}/Microsoft.${MSVC_REDIST_NAME}.CRT
      PATHS ${_vs_redist_paths})
    unset(_vs_redist_paths)
    mark_as_advanced(MSVC_REDIST_DIR)
    set(MSVC_CRT_DIR
        "${MSVC_REDIST_DIR}/${CMAKE_MSVC_ARCH}/Microsoft.${MSVC_REDIST_NAME}.CRT"
    )

    if(NOT CMAKE_INSTALL_DEBUG_LIBRARIES_ONLY)
      set(__install__libs "${MSVC_CRT_DIR}/msvcp${v}.dll")
      if(NOT vs VERSION_LESS 14)
        ...
        endforeach()
        list(APPEND __install__libs "${MSVC_CRT_DIR}/vcruntime${v}.dll"
    ...
    if(CMAKE_INSTALL_DEBUG_LIBRARIES)
      ...
      set(__install__libs ${__install__libs} "${MSVC_CRT_DIR}/msvcp${v}d.dll")
      if(NOT vs VERSION_LESS 14)
        foreach(
          crt
        ...
        endforeach()
        list(APPEND __install__libs "${MSVC_CRT_DIR}/vcruntime${v}d.dll"
    ...
    if(CMAKE_INSTALL_UCRT_LIBRARIES AND NOT vs VERSION_LESS 14)
      ...
      find_path(
        WINDOWS_KITS_DIR
        NAMES Redist/${__ucrt_version}ucrt/DLLs/${CMAKE_MSVC_ARCH}/ucrtbase.dll
              Redist/ucrt/DLLs/${CMAKE_MSVC_ARCH}/ucrtbase.dll
        PATHS $ENV{CMAKE_WINDOWS_KITS_10_DIR} "${windows_kits_dir}"
      ...
      if(NOT CMAKE_INSTALL_DEBUG_LIBRARIES_ONLY)
        if(EXISTS
           "${WINDOWS_KITS_DIR}/Redist/${__ucrt_version}ucrt/DLLs/${CMAKE_MSVC_ARCH}/ucrtbase.dll"
        )
          file(
            GLOB
            __ucrt_dlls
            "${WINDOWS_KITS_DIR}/Redist/${__ucrt_version}ucrt/DLLs/${CMAKE_MSVC_ARCH}/*.dll"
          )
        else()
      ...
      if(CMAKE_INSTALL_DEBUG_LIBRARIES)
        if(EXISTS
           "${WINDOWS_KITS_DIR}/bin/${__ucrt_version}${CMAKE_MSVC_ARCH}/ucrt/ucrtbased.dll"
        )
          file(
            GLOB
            __ucrt_dlls
            "${WINDOWS_KITS_DIR}/bin/${__ucrt_version}${CMAKE_MSVC_ARCH}/ucrt/*.dll"
          )
        else()
          file(GLOB __ucrt_dlls
  ...
  if(CMAKE_INSTALL_MFC_LIBRARIES)
    if(MSVC_VERSION EQUAL 1300)
      set(__install__libs ${__install__libs} "${SYSTEMROOT}/system32/mfc70.dll")
    endif()

    if(MSVC_VERSION EQUAL 1310)
      set(__install__libs ${__install__libs} "${SYSTEMROOT}/system32/mfc71.dll")
    endif()
    ...
    if(MSVC_VERSION EQUAL 1400)
      ...
      set(MSVC_MFC_DIR
          "${MSVC_REDIST_DIR}/${CMAKE_MSVC_ARCH}/Microsoft.VC80.MFC")
      # Install the manifest that allows DLLs to be loaded from the directory
      # containing the executable.
      if(NOT CMAKE_INSTALL_DEBUG_LIBRARIES_ONLY)
        set(__install__libs
            ${__install__libs} "${MSVC_MFC_DIR}/Microsoft.VC80.MFC.manifest"
            "${MSVC_MFC_DIR}/mfc80.dll" "${MSVC_MFC_DIR}/mfc80u.dll"
            "${MSVC_MFC_DIR}/mfcm80.dll" "${MSVC_MFC_DIR}/mfcm80u.dll")
      endif()

      # include the language dll's for vs8 as well as the actual dll's
      set(MSVC_MFCLOC_DIR
          "${MSVC_REDIST_DIR}/${CMAKE_MSVC_ARCH}/Microsoft.VC80.MFCLOC")
      # Install the manifest that allows DLLs to be loaded from the directory
      # containing the executable.
      set(__install__libs
          ${__install__libs}
    ...
    if(MSVC_VERSION EQUAL 1500)
      ...
      set(MSVC_MFC_DIR
          "${MSVC_REDIST_DIR}/${CMAKE_MSVC_ARCH}/Microsoft.VC90.MFC")
      # Install the manifest that allows DLLs to be loaded from the directory
      # containing the executable.
      if(NOT CMAKE_INSTALL_DEBUG_LIBRARIES_ONLY)
        set(__install__libs
            ${__install__libs} "${MSVC_MFC_DIR}/Microsoft.VC90.MFC.manifest"
            "${MSVC_MFC_DIR}/mfc90.dll" "${MSVC_MFC_DIR}/mfc90u.dll"
            "${MSVC_MFC_DIR}/mfcm90.dll" "${MSVC_MFC_DIR}/mfcm90u.dll")
      endif()

      # include the language dll's for vs9 as well as the actual dll's
      set(MSVC_MFCLOC_DIR
          "${MSVC_REDIST_DIR}/${CMAKE_MSVC_ARCH}/Microsoft.VC90.MFCLOC")
      # Install the manifest that allows DLLs to be loaded from the directory
      # containing the executable.
      set(__install__libs
          ${__install__libs}
    ...
    if(_MFC_DLL_VERSION)
      ...
      if(NOT vs VERSION_LESS 15)
        ...
        find_path(
          MSVC_REDIST_MFC_DIR
          NAMES ${CMAKE_MSVC_ARCH}/Microsoft.${MSVC_REDIST_NAME}.MFC
          PATHS ${_MSVC_REDIST_DIRS}
          NO_DEFAULT_PATH)
      ...
      # Multi-Byte Character Set versions of MFC are available as optional addon
      # since Visual Studio 12.  So for version 12 or higher, check whether they
      # are available and exclude them if they are not.

      if(CMAKE_INSTALL_DEBUG_LIBRARIES)
        set(MSVC_MFC_DIR
            "${MSVC_REDIST_MFC_DIR}/Debug_NonRedist/${CMAKE_MSVC_ARCH}/Microsoft.${MSVC_REDIST_NAME}.DebugMFC"
        )
        set(__install__libs ${__install__libs} "${MSVC_MFC_DIR}/mfc${v}ud.dll"
                            "${MSVC_MFC_DIR}/mfcm${v}ud.dll")
        if("${v}" LESS 12 OR EXISTS "${MSVC_MFC_DIR}/mfc${v}d.dll")
          set(__install__libs ${__install__libs} "${MSVC_MFC_DIR}/mfc${v}d.dll")
        endif()
      ...
      set(MSVC_MFC_DIR
          "${MSVC_REDIST_MFC_DIR}/${CMAKE_MSVC_ARCH}/Microsoft.${MSVC_REDIST_NAME}.MFC"
      )
      if(NOT CMAKE_INSTALL_DEBUG_LIBRARIES_ONLY)
        set(__install__libs ${__install__libs} "${MSVC_MFC_DIR}/mfc${v}u.dll"
                            "${MSVC_MFC_DIR}/mfcm${v}u.dll")
        if("${v}" LESS 12 OR EXISTS "${MSVC_MFC_DIR}/mfc${v}.dll")
          set(__install__libs ${__install__libs} "${MSVC_MFC_DIR}/mfc${v}.dll")
        endif()
        if("${v}" LESS 12 OR EXISTS "${MSVC_MFC_DIR}/mfcm${v}.dll")
          set(__install__libs ${__install__libs} "${MSVC_MFC_DIR}/mfcm${v}.dll")
        endif()
      endif()

      # include the language dll's as well as the actual dll's
      set(MSVC_MFCLOC_DIR
          "${MSVC_REDIST_MFC_DIR}/${CMAKE_MSVC_ARCH}/Microsoft.${MSVC_REDIST_NAME}.MFCLOC"
      )
      set(__install__libs
          ${__install__libs}
  ...
  # MSVC 8 was the first version with OpenMP Furthermore, there is no debug
  # version of this
  if(CMAKE_INSTALL_OPENMP_LIBRARIES AND _IRSL_HAVE_MSVC)
    ...
    if(_MSOMP_DLL_VERSION)
      ...
      set(MSVC_OPENMP_DIR
          "${MSVC_REDIST_DIR}/${CMAKE_MSVC_ARCH}/Microsoft.${MSVC_REDIST_NAME}.OPENMP"
      )
  ...
  foreach(lib ${__install__libs})
    if(EXISTS ${lib})
      ...
      if(NOT CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS_NO_WARNINGS)
        ...
        # This warning indicates an incomplete Visual Studio installation or a
        # bug somewhere above here in this file. If you would like to avoid this
        # warning, fix the real problem, or set
        # CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS_NO_WARNINGS before including this
        # file.
      endif()
    endif()
  endforeach()
...
if(_IRSL_HAVE_Intel)
  ...
  if(CMAKE_INSTALL_OPENMP_LIBRARIES)
    if(WIN32)
      list(APPEND __install_libs "${_Intel_redistdir}/libiomp5md.dll"
           "${_Intel_redistdir}/libiompstubs5md.dll")
    elseif(APPLE)
      list(APPEND __install_libs "${_Intel_redistdir}/libiomp5.dylib"
           "${_Intel_redistdir}/libiompstubs5.dylib")
    else()
      list(APPEND __install_libs "${_Intel_redistdir}/libiomp5.so"
           "${_Intel_redistdir}/libiompstubs5.so")
  ...
  elseif(APPLE)
    ...
    if(CMAKE_Fortran_COMPILER_ID MATCHES Intel)
      foreach(__Intel_lib IN ITEMS libifcore.dylib libifcoremt.dylib
                                   libifport.dylib libifportmt.dylib)
    ...
    if(CMAKE_C_COMPILER_ID MATCHES Intel OR CMAKE_CXX_COMPILER_ID MATCHES Intel)
      ...
      if(_Intel_compiler_ver VERSION_GREATER_EQUAL 15)
        list(APPEND __install_libs "${_Intel_redistdir}/libistrconv.so"
             "${_Intel_redistdir}/libgfxoffload.so")
      endif()
    endif()
    if(_Intel_compiler_ver VERSION_GREATER_EQUAL 16)
      foreach(
        __Intel_lib IN
        ITEMS libioffload_host.so libioffload_host.so.5 libioffload_target.so
              libioffload_target.so.5 libmpx.so offload_main)
...
if(WATCOM)
  ...
  foreach(lib ${__install__libs})
    if(EXISTS ${lib})
      ...
      if(NOT CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS_NO_WARNINGS)
        message(WARNING "system runtime library file does not exist: '${lib}'")
        # This warning indicates an incomplete Watcom installation or a bug
        # somewhere above here in this file. If you would like to avoid this
        # warning, fix the real problem, or set
        # CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS_NO_WARNINGS before including this
        # file.
      endif()
...
# Include system runtime libraries in the installation if any are specified by
# CMAKE_INSTALL_SYSTEM_RUNTIME_LIBS.
