...
function(_cpack_nuget_variable_fallback OUTPUT_VAR_NAME NUGET_VAR_NAME)
  ...
  _cpack_nuget_debug(
    "_cpack_nuget_variable_fallback: " "OUTPUT_VAR_NAME=`${OUTPUT_VAR_NAME}`, "
    "NUGET_VAR_NAME=`${NUGET_VAR_NAME}`" "${_va_args}")

  set(_options USE_CDATA)
  set(_one_value_args LIST_GLUE)
  set(_multi_value_args FALLBACK_VARS)
  cmake_parse_arguments(PARSE_ARGV 0 _args "${_options}" "${_one_value_args}"
                        "${_multi_value_args}")

  if(CPACK_NUGET_PACKAGE_COMPONENT)
    string(TOUPPER "${CPACK_NUGET_PACKAGE_COMPONENT}"
  ...
  if(_result)
    ...
    set(${OUTPUT_VAR_NAME}
        "${_value_before}${_result}${_value_after}"
        PARENT_SCOPE)
  endif()

endfunction()

function(_cpack_nuget_variable_fallback_and_wrap_into_element ELEMENT
         NUGET_VAR_NAME)
  set(_options)
  set(_one_value_args)
  set(_multi_value_args FALLBACK_VARS ATTRIBUTES)
  cmake_parse_arguments(PARSE_ARGV 2 _args "${_options}" "${_one_value_args}"
                        "${_multi_value_args}")
  ...
  if(_value)
    set(_CPACK_NUGET_${_ELEMENT_UP}_TAG
...
function(_cpack_nuget_deprecation_warning NUGET_ELEMENT VARNAME REPLACEMENT)
  if(${VARNAME})
    if(REPLACEMENT)
      message(
        DEPRECATION
          "nuspec element `${NUGET_ELEMENT}` is deprecated in NuGet; consider replacing `${VARNAME}` with `${REPLACEMENT}`"
      )
    else()
      message(
        DEPRECATION
          "nuspec element `${NUGET_ELEMENT}` is deprecated in NuGet; consider removing `${VARNAME}`"
      )
    endif()
  endif()
endfunction()
...
function(_cpack_nuget_render_spec)
  ...
  # Warn about deprecated nuspec elements; warnings only display if variable is
  # set Note that while nuspec's "summary" element is deprecated, there is no
  # suggested replacement so (for now) no deprecation warning is shown for
  # `CPACK_NUGET_*_DESCRIPTION_SUMMARY`
  _cpack_nuget_deprecation_warning(
    "licenseUrl"
  ...
  _cpack_nuget_deprecation_warning(
    "licenseUrl"
    CPACK_NUGET_${CPACK_NUGET_PACKAGE_COMPONENT}_LICENSEURL
  ...
  _cpack_nuget_deprecation_warning(
    "iconUrl" CPACK_NUGET_${CPACK_NUGET_PACKAGE_COMPONENT}_ICONURL
    "CPACK_NUGET_${CPACK_NUGET_PACKAGE_COMPONENT}_ICON")

  # Set nuspec fields
  _cpack_nuget_variable_fallback(CPACK_NUGET_PACKAGE_VERSION VERSION
                                 FALLBACK_VARS CPACK_PACKAGE_VERSION)
  ...
  _cpack_nuget_variable_fallback_and_wrap_into_element(owners OWNERS LIST_GLUE
                                                       ",")
  _cpack_nuget_variable_fallback_and_wrap_into_element(
    projectUrl HOMEPAGE_URL FALLBACK_VARS CPACK_PACKAGE_HOMEPAGE_URL)
  ...
  # "license" takes a "type" attribute of either "file" or "expression" "file"
  # refers to a file path of a .txt or .md file relative to the installation
  # root "expression" refers to simple or compound expression of license
  # identifiers listed at https://spdx.org/licenses/ Note that only one of
  # CPACK_NUGET_PACKAGE_LICENSE_FILE_NAME and
  # CPACK_NUGET_PACKAGE_LICENSE_EXPRESSION may be specified. If both are
  # specified, CPACK_NUGET_PACKAGE_LICENSE_FILE_NAME takes precedence and
  # CPACK_NUGET_PACKAGE_LICENSE_EXPRESSION is ignored.
  if(CPACK_NUGET_PACKAGE_LICENSE_FILE_NAME)
    _cpack_nuget_variable_fallback_and_wrap_into_element(
      license LICENSE_FILE_NAME ATTRIBUTES [[type="file"]])
  elseif(CPACK_NUGET_PACKAGE_LICENSE_EXPRESSION)
    _cpack_nuget_variable_fallback_and_wrap_into_element(
      license LICENSE_EXPRESSION ATTRIBUTES [[type="expression"]])
  endif()
  ...
  _cpack_nuget_variable_fallback_and_wrap_into_element(
    summary DESCRIPTION_SUMMARY FALLBACK_VARS CPACK_PACKAGE_DESCRIPTION_SUMMARY)
  if(CPACK_NUGET_PACKAGE_REQUIRE_LICENSE_ACCEPTANCE)
    set(_CPACK_NUGET_REQUIRELICENSEACCEPTANCE_TAG
        "<requireLicenseAcceptance>true</requireLicenseAcceptance>")
  endif()
  _cpack_nuget_variable_fallback_and_wrap_into_element(releaseNotes
                                                       RELEASE_NOTES)
  ...
  foreach(_dep IN LISTS _deps)
    ...
    if(_ver)
      ...
      string(
        CONCAT _collected_deps "${_collected_deps}"
               "            <dependency id=\"${_dep}\" version=\"${_ver}\" />\n"
      )
    endif()
  endforeach()
  ...
  # Render the spec file NOTE The spec filename doesn't matter. Being included
  # into a package, NuGet will name it properly.
  _cpack_nuget_debug(
    "Rendering `${CPACK_TEMPORARY_DIRECTORY}/CPack.NuGet.nuspec` file...")
  configure_file("${CMAKE_ROOT}/Modules/Internal/CPack/CPack.NuGet.nuspec.in"
                 "${CPACK_TEMPORARY_DIRECTORY}/CPack.NuGet.nuspec" @ONLY)
endfunction()
...
if(CPACK_NUGET_ORDINAL_MONOLITIC)
  # This variable `CPACK_NUGET_ALL_IN_ONE` set by C++ code: Meaning to pack all
  # installed files into a single package
...
elseif(CPACK_NUGET_ALL_IN_ONE)
  # This variable `CPACK_NUGET_ALL_IN_ONE` set by C++ code: Meaning to pack all
  # installed components into a single package
  _cpack_nuget_debug(
    "---[Making a monolitic package from installed components]---")
  ...
  if(CPACK_NUGET_GROUPS)
    ...
    foreach(_group IN LISTS CPACK_NUGET_GROUPS)
      ...
      # Temporary set `CPACK_NUGET_PACKAGE_COMPONENT` to the group name to
      # properly collect various per group settings
      set(CPACK_NUGET_PACKAGE_COMPONENT ${_group})
      _cpack_nuget_render_spec()
      unset(CPACK_NUGET_PACKAGE_COMPONENT)
      execute_process(
        COMMAND "${NUGET_EXECUTABLE}" pack
                ${CPACK_NUGET_PACK_ADDITIONAL_OPTIONS}
        WORKING_DIRECTORY "${CPACK_TEMPORARY_DIRECTORY}"
        RESULT_VARIABLE _nuget_result)
      if(NOT _nuget_result EQUAL 0)
        message(FATAL_ERROR "Nuget pack failed")
  ...
  if(CPACK_NUGET_COMPONENTS)
    ...
    foreach(_comp IN LISTS CPACK_NUGET_COMPONENTS)
      _cpack_nuget_debug(
        "Starting to make the package for component `${_comp}`")
      # Render a spec file which includes only given component
      unset(_CPACK_NUGET_FILES_TAG)
      _cpack_nuget_make_files_tag(${_comp})
      # Temporary set `CPACK_NUGET_PACKAGE_COMPONENT` to the current component
      # name to properly collect various per group settings
...
if(NOT GEN_CPACK_OUTPUT_FILES)
  message(
    FATAL_ERROR
      "NuGet package was not generated at `${CPACK_TEMPORARY_DIRECTORY}`!")
endif()
