...
# =============================================================================
# If the user has provided ``GSL_ROOT_DIR``, use it!  Choose items found at this
# location over system locations.
if(EXISTS "$ENV{GSL_ROOT_DIR}")
  file(TO_CMAKE_PATH "$ENV{GSL_ROOT_DIR}" GSL_ROOT_DIR)
  set(GSL_ROOT_DIR
      "${GSL_ROOT_DIR}"
      CACHE PATH "Prefix for GSL installation.")
endif()
...
# =============================================================================
# As a first try, use the PkgConfig module.  This will work on many *NIX
# systems.  See :module:`findpkgconfig` This will return ``GSL_INCLUDEDIR`` and
# ``GSL_LIBDIR`` used below.
...
if(NOT GSL_VERSION)
  ...
  # 1. If gsl-config is not available, try looking in gsl/gsl_version.h
  if(NOT GSL_VERSION AND EXISTS "${GSL_INCLUDE_DIRS}/gsl/gsl_version.h")
    ...
    string(REGEX REPLACE ".*define[ ]+GSL_VERSION[ ]+\"([^\"]*)\".*" "\\1"
                         GSL_VERSION ${gsl_version_h_contents})
  endif()
...
# =============================================================================
# Register imported libraries: 1. If we can find a Windows .dll file (or if we
# can find both Debug and Release libraries), we will set appropriate target
# properties for these. 2. However, for most systems, we will only register the
# import location and include directory.
...
if(WIN32)
  ...
  string(REPLACE ".lib" ".dll" GSL_CBLAS_LIBRARY_DEBUG_DLL
                 "${GSL_CBLAS_LIBRARY_DEBUG}")
endif()
...
if(GSL_FOUND AND NOT TARGET GSL::gsl)
  if(EXISTS "${GSL_LIBRARY_DLL}" AND EXISTS "${GSL_CBLAS_LIBRARY_DLL}")
    ...
    set_target_properties(
      GSL::gslcblas
      PROPERTIES IMPORTED_LOCATION_RELEASE "${GSL_CBLAS_LIBRARY_DLL}"
    ...
    set_target_properties(
      GSL::gsl
      PROPERTIES IMPORTED_LOCATION_RELEASE "${GSL_LIBRARY_DLL}"
    ...
    if(EXISTS "${GSL_LIBRARY_DEBUG_DLL}" AND EXISTS
                                             "${GSL_CBLAS_LIBRARY_DEBUG_DLL}")
      set_property(
        TARGET GSL::gslcblas
        APPEND
      ...
      set_target_properties(
        GSL::gslcblas
        PROPERTIES IMPORTED_LOCATION_DEBUG "${GSL_CBLAS_LIBRARY_DEBUG_DLL}"
      ...
      set_property(
        ...
        PROPERTY IMPORTED_CONFIGURATIONS Debug)
      set_target_properties(
        GSL::gsl PROPERTIES IMPORTED_LOCATION_DEBUG "${GSL_LIBRARY_DEBUG_DLL}"
    ...
    set_target_properties(
      GSL::gslcblas
      PROPERTIES IMPORTED_LOCATION "${GSL_CBLAS_LIBRARY}"
                 INTERFACE_INCLUDE_DIRECTORIES "${GSL_INCLUDE_DIRS}"
                 IMPORTED_LINK_INTERFACE_LANGUAGES "C")
    set_target_properties(
      GSL::gsl
