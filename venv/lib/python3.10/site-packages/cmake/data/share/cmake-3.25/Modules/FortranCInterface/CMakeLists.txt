...
# Note that some compiler manglings cannot be invoked from C: SunPro uses
# "my_module.my_sub_" PathScale uses "MY_SUB.in.MY_MODULE"
...
if(NOT CMAKE_Fortran_COMPILER_ID MATCHES "^(PathScale|Cray)$")
  # Provide mymodule_ and my_module_ init symbols because: - PGI Fortran uses
  # module init symbols but not for: - PathScale Fortran uses module init
  # symbols but module symbols use '.in.' so we cannot provide them anyway. -
  # Cray Fortran >= 7.3.2 uses module init symbols but module symbols use
  # 'mysub$mymodule_' so we cannot provide them anyway.
...
# In case the Fortran compiler produces PIC by default make sure the C compiler
# produces PIC even if it is not its default.
...
# If IPO is enabled here, GCC gfortran >= 12.0 will obfuscate the strings of the
# return values in the compiled executable, which we use to regex match against
# later. The static libraries must be build with IPO and non-IPO objects, as
# that will ensure the verify step will operate on IPO objects, if requested by
# the system compiler flags.
