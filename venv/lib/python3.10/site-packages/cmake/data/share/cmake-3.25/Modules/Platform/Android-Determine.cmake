...
# This module detects platform-wide information about the Android target in
# order to store it in "CMakeSystem.cmake".

# Include the NDK hook. It can be used by NDK to inject necessary fixes for an
# earlier cmake.
if(CMAKE_ANDROID_NDK)
  include(${CMAKE_ANDROID_NDK}/build/cmake/hooks/pre/Android-Determine.cmake
          OPTIONAL)
endif()

# Support for NVIDIA Nsight Tegra Visual Studio Edition was previously
# implemented in the CMake VS IDE generators.  Avoid interfering with that
# functionality for now.
if(CMAKE_GENERATOR_PLATFORM STREQUAL "Tegra-Android")
  return()
endif()

# Commonly used Android toolchain files that pre-date CMake upstream support set
# CMAKE_SYSTEM_VERSION to 1.  Avoid interfering with them.
...
if(CMAKE_GENERATOR MATCHES "Visual Studio")
  if(NOT CMAKE_SYSROOT)
    ...
    configure_file(
      ${CMAKE_ROOT}/Modules/Platform/Android/VCXProjInspect.vcxproj.in
      ${CMAKE_PLATFORM_INFO_DIR}/VCXProjInspect.vcxproj @ONLY)
    cmake_host_system_information(RESULT _msbuild QUERY VS_MSBUILD_COMMAND
    )# undocumented query
    execute_process(
      COMMAND "${_msbuild}" "VCXProjInspect.vcxproj" "/p:Configuration=Debug"
              "/p:Platform=${vcx_platform}"
    ...
    unset(_msbuild)
    if(NOT CMAKE_SYSROOT AND VCXPROJ_INSPECT_OUTPUT MATCHES
                             "CMAKE_SYSROOT=([^%\r\n]+)[\r\n]")
      # Strip VS diagnostic output from the end of the line.
      string(REGEX REPLACE " \\(TaskId:[0-9]*\\)$" "" _sysroot
                           "${CMAKE_MATCH_1}")
      if(EXISTS "${_sysroot}")
...
if(CMAKE_SYSROOT)
  if(NOT IS_DIRECTORY "${CMAKE_SYSROOT}")
    message(FATAL_ERROR "Android: The specified CMAKE_SYSROOT:\n"
...
elseif(CMAKE_ANDROID_STANDALONE_TOOLCHAIN)
  ...
  elseif(IS_DIRECTORY "${_ANDROID_SYSROOT_STANDALONE_TOOLCHAIN}")
    set(CMAKE_ANDROID_STANDALONE_TOOLCHAIN
        "${_ANDROID_SYSROOT_STANDALONE_TOOLCHAIN}")
  ...
  elseif(IS_DIRECTORY "${ANDROID_STANDALONE_TOOLCHAIN}")
    file(TO_CMAKE_PATH "${ANDROID_STANDALONE_TOOLCHAIN}"
         CMAKE_ANDROID_STANDALONE_TOOLCHAIN)
  ...
  elseif(IS_DIRECTORY "$ENV{ANDROID_STANDALONE_TOOLCHAIN}")
    file(TO_CMAKE_PATH "$ENV{ANDROID_STANDALONE_TOOLCHAIN}"
         CMAKE_ANDROID_STANDALONE_TOOLCHAIN)
  endif()
...
if(CMAKE_ANDROID_STANDALONE_TOOLCHAIN)
  ...
  if(EXISTS "${CMAKE_ANDROID_STANDALONE_TOOLCHAIN}/bin/clang")
    ...
    file(
      STRINGS "${CMAKE_ANDROID_STANDALONE_TOOLCHAIN}/bin/clang"
      _ANDROID_STANDALONE_TOOLCHAIN_BIN_CLANG
      REGEX "${_ANDROID_API_LEVEL_CLANG_REGEX}"
      LIMIT_COUNT 1
      LIMIT_INPUT 65536)
    if(_ANDROID_STANDALONE_TOOLCHAIN_BIN_CLANG MATCHES
       "${_ANDROID_API_LEVEL_CLANG_REGEX}")
  ...
  if(NOT _ANDROID_STANDALONE_TOOLCHAIN_API)
    # The compiler launcher does not know __ANDROID_API__.  Assume this is not
    # unified headers and look for it in the api-level.h header.
    set(_ANDROID_API_LEVEL_H_REGEX
        "^[\t ]*#[\t ]*define[\t ]+__ANDROID_API__[\t ]+([0-9]+)")
    file(
      STRINGS
      "${CMAKE_ANDROID_STANDALONE_TOOLCHAIN}/sysroot/usr/include/android/api-level.h"
      _ANDROID_API_LEVEL_H_CONTENT
      REGEX "${_ANDROID_API_LEVEL_H_REGEX}")
...
if(NOT CMAKE_ANDROID_NDK AND NOT CMAKE_ANDROID_STANDALONE_TOOLCHAIN)
  message(
    FATAL_ERROR "Android: Neither the NDK or a standalone toolchain was found.")
endif()

if(CMAKE_ANDROID_NDK)
  # NDK >= 18 has platforms.cmake. It provides: NDK_MIN_PLATFORM_LEVEL
  # NDK_MAX_PLATFORM_LEVEL
  include("${CMAKE_ANDROID_NDK}/build/cmake/platforms.cmake" OPTIONAL
          RESULT_VARIABLE _INCLUDED_PLATFORMS)
  # NDK >= 18 has abis.cmake. It provides: NDK_KNOWN_DEVICE_ABI32S
  # NDK_KNOWN_DEVICE_ABI64S NDK >= 23 also provides: NDK_KNOWN_DEVICE_ABIS
  # NDK_ABI_<abi>_PROC NDK_ABI_<abi>_ARCH NDK_ABI_<abi>_TRIPLE
  # NDK_ABI_<abi>_LLVM_TRIPLE NDK_PROC_<processor>_ABI NDK_ARCH_<arch>_ABI
  include("${CMAKE_ANDROID_NDK}/build/cmake/abis.cmake" OPTIONAL
          RESULT_VARIABLE _INCLUDED_ABIS)
endif()

if(CMAKE_ANDROID_NDK AND EXISTS "${CMAKE_ANDROID_NDK}/source.properties")
  # Android NDK revision Possible formats: * r16, build 1234: 16.0.1234 * r16b,
  # build 1234: 16.1.1234 * r16 beta 1, build 1234: 16.0.1234-beta1
  #
  # Canary builds are not specially marked.
  file(READ "${CMAKE_ANDROID_NDK}/source.properties"
       _ANDROID_NDK_SOURCE_PROPERTIES)

  set(_ANDROID_NDK_REVISION_REGEX
      "^Pkg\\.Desc = Android NDK\nPkg\\.Revision = ([0-9]+)\\.([0-9]+)\\.([0-9]+)(-beta([0-9]+))?"
  )
  if(NOT _ANDROID_NDK_SOURCE_PROPERTIES MATCHES
     "${_ANDROID_NDK_REVISION_REGEX}")
    string(REPLACE "\n" "\n  " _ANDROID_NDK_SOURCE_PROPERTIES
                   "${_ANDROID_NDK_SOURCE_PROPERTIES}")
    message(
      FATAL_ERROR
...
if(CMAKE_ANDROID_NDK)
  ...
  elseif(CMAKE_HOST_SYSTEM_NAME STREQUAL "Windows")
    ...
    message(
      FATAL_ERROR
        "Android: Builds hosted on '${CMAKE_HOST_SYSTEM_NAME}' not supported.")
  endif()

  # Look for a unified toolchain/sysroot provided with the NDK.
  set(CMAKE_ANDROID_NDK_TOOLCHAIN_UNIFIED
      "${CMAKE_ANDROID_NDK}/toolchains/llvm/prebuilt/${CMAKE_ANDROID_NDK_TOOLCHAIN_HOST_TAG}"
  )
...
if(_INCLUDED_ABIS)
  if(NDK_KNOWN_DEVICE_ABIS)
    ...
    set(_ANDROID_KNOWN_ABIS ${NDK_KNOWN_DEVICE_ABI32S}
...
# Validate inputs.
if(CMAKE_ANDROID_ARCH_ABI AND NOT DEFINED
                              "NDK_ABI_${CMAKE_ANDROID_ARCH_ABI}_PROC")
  message(
    FATAL_ERROR
      "Android: Unknown ABI CMAKE_ANDROID_ARCH_ABI='${CMAKE_ANDROID_ARCH_ABI}'."
  )
endif()
if(CMAKE_SYSTEM_PROCESSOR AND NOT DEFINED
                              "NDK_PROC_${CMAKE_SYSTEM_PROCESSOR}_ABI")
  message(
    FATAL_ERROR
      "Android: Unknown processor CMAKE_SYSTEM_PROCESSOR='${CMAKE_SYSTEM_PROCESSOR}'."
  )
endif()
if(_ANDROID_SYSROOT_ARCH AND NOT DEFINED
                             "NDK_ARCH_${_ANDROID_SYSROOT_ARCH}_ABI")
  message(
    FATAL_ERROR
...
if(NOT CMAKE_ANDROID_ARCH_ABI)
  ...
  elseif(_INCLUDED_ABIS)
    ...
    # https://developer.android.com/ndk/guides/application_mk.html Default is
    # the oldest ARM ABI.
    ...
    foreach(config_mk IN LISTS _ANDROID_CONFIG_MKS)
      ...
      string(REPLACE "TOOLCHAIN_ABIS :=" "" _ANDROID_TOOL_ABIS
                     "${_ANDROID_TOOL_ABIS}")
      separate_arguments(_ANDROID_TOOL_ABIS UNIX_COMMAND
                         "${_ANDROID_TOOL_ABIS}")
...
if(_ANDROID_SYSROOT_ARCH AND NOT "x${_ANDROID_SYSROOT_ARCH}" STREQUAL
                             "x${CMAKE_ANDROID_ARCH}")
  message(
    FATAL_ERROR
...
if(NOT NDK_ABI_${CMAKE_ANDROID_ARCH_ABI}_PROC STREQUAL CMAKE_SYSTEM_PROCESSOR)
  message(
    FATAL_ERROR
      "Android: The specified CMAKE_ANDROID_ARCH_ABI='${CMAKE_ANDROID_ARCH_ABI}' and CMAKE_SYSTEM_PROCESSOR='${CMAKE_SYSTEM_PROCESSOR}' is not a valid combination."
  )
endif()
...
if(CMAKE_SYSTEM_VERSION)
  if(CMAKE_ANDROID_API AND NOT "x${CMAKE_ANDROID_API}" STREQUAL
                           "x${CMAKE_SYSTEM_VERSION}")
    message(
      FATAL_ERROR
  ...
  if(CMAKE_ANDROID_NDK)
    if(_INCLUDED_PLATFORMS)
      if(CMAKE_SYSTEM_VERSION GREATER NDK_MAX_PLATFORM_LEVEL
         OR CMAKE_SYSTEM_VERSION LESS NDK_MIN_PLATFORM_LEVEL)
        message(
          FATAL_ERROR
...
elseif(CMAKE_ANDROID_NDK)
  if(_INCLUDED_PLATFORMS)
    ...
    # be for this ABI.
    if(CMAKE_ANDROID_ARCH_ABI MATCHES "64(-v8a)?$" AND CMAKE_SYSTEM_VERSION
                                                       LESS 21)
      set(CMAKE_SYSTEM_VERSION 21)
    endif()
  else()
    file(
      GLOB _ANDROID_APIS_1
      RELATIVE "${CMAKE_ANDROID_NDK}/platforms"
      "${CMAKE_ANDROID_NDK}/platforms/android-[0-9]")
    file(
      GLOB _ANDROID_APIS_2
      RELATIVE "${CMAKE_ANDROID_NDK}/platforms"
      "${CMAKE_ANDROID_NDK}/platforms/android-[0-9][0-9]")
...
if(NOT CMAKE_SYSTEM_VERSION MATCHES "^[0-9]+$")
  message(
    FATAL_ERROR
      "Android: The API specified by CMAKE_SYSTEM_VERSION='${CMAKE_SYSTEM_VERSION}' is not an integer."
  )
endif()
...
elseif(DEFINED CMAKE_ANDROID_ARM_MODE)
  message(
    FATAL_ERROR
      "Android: CMAKE_ANDROID_ARM_MODE is set but is valid only for 'armeabi' architectures."
  )
endif()
...
elseif(DEFINED CMAKE_ANDROID_ARM_NEON)
  message(
    FATAL_ERROR
      "Android: CMAKE_ANDROID_ARM_NEON is set but is valid only for 'armeabi-v7a' architecture."
  )
endif()
...
message(
  STATUS
    "Android: Targeting API '${CMAKE_SYSTEM_VERSION}' with architecture '${CMAKE_ANDROID_ARCH}', ABI '${CMAKE_ANDROID_ARCH_ABI}', and processor '${CMAKE_SYSTEM_PROCESSOR}'"
)
...
# earlier cmake.
if(CMAKE_ANDROID_NDK)
  include(${CMAKE_ANDROID_NDK}/build/cmake/hooks/post/Android-Determine.cmake
          OPTIONAL)
endif()
