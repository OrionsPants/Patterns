...
macro(__determine_compiler_id_test testflags_var userflags_var)
  ...
  cmake_determine_compiler_id_match_vendor(
    "${lang}" "${COMPILER_${lang}_PRODUCED_OUTPUT}")

  if(NOT CMAKE_${lang}_COMPILER_ID)
    foreach(file ${COMPILER_${lang}_PRODUCED_FILES})
      cmake_determine_compiler_id_check(
        "${lang}" "${CMAKE_${lang}_COMPILER_ID_DIR}/${file}" "${src}")
    endforeach()
  endif()
endmacro()

# Function to compile a source file to identify the compiler.  This is used
# internally by CMake and should not be included by user code. If successful,
# sets CMAKE_<lang>_COMPILER_ID and CMAKE_<lang>_PLATFORM_ID
...
function(CMAKE_DETERMINE_COMPILER_ID lang flagvar src)
  ...
  set(CMAKE_${lang}_COMPILER_ID_DIR
      ${CMAKE_PLATFORM_INFO_DIR}/CompilerId${lang})

  # If we REQUIRE_SUCCESS, i.e. TEST_FLAGS_FIRST has the correct flags, we still
  # need to try two combinations: with COMPILER_ID_FLAGS (from user) and without
  # (see issue #21869).
  if(CMAKE_${lang}_COMPILER_ID_REQUIRE_SUCCESS)
    ...
    # Try building with no extra flags and then try each set of helper flags.
    # Stop when the compiler is identified.
    foreach(userflags "${CMAKE_${lang}_COMPILER_ID_FLAGS_LIST}" "")
  ...
  if(CMAKE_${lang}_COMPILER_ID_TOOL)
    ...
    set(CMAKE_${lang}_COMPILER
        "${CMAKE_${lang}_COMPILER_ID_TOOL}"
        PARENT_SCOPE)
  elseif(NOT CMAKE_${lang}_COMPILER)
    set(CMAKE_${lang}_COMPILER
        "CMAKE_${lang}_COMPILER-NOTFOUND"
        PARENT_SCOPE)
  endif()
  ...
  if(NOT CMAKE_${lang}_COMPILER_ID AND "${CMAKE_GENERATOR}" MATCHES
                                       "Green Hills MULTI")
  ...
  if(lang STREQUAL "Fortran" AND CMAKE_${lang}_COMPILER_ID STREQUAL "LCC")
    execute_process(
      COMMAND "${CMAKE_${lang}_COMPILER}" --version
      OUTPUT_VARIABLE output
      ERROR_VARIABLE output
  ...
  if(COMPILER_QNXNTO AND (CMAKE_${lang}_COMPILER_ID STREQUAL "GNU"
                          OR CMAKE_${lang}_COMPILER_ID STREQUAL "LCC"))
    execute_process(
  ...
  # if the format is unknown after all files have been checked, put "Unknown" in
  # the cache
  if(NOT CMAKE_EXECUTABLE_FORMAT)
    set(CMAKE_EXECUTABLE_FORMAT
        "Unknown"
        CACHE INTERNAL "Executable file format")
  endif()
  ...
  if("x${CMAKE_${lang}_COMPILER_ID}" STREQUAL "xClang"
    ...
    if("x${CMAKE_${lang}_SIMULATE_ID}" STREQUAL "xMSVC")
      ...
      set(_variant
          " with ${CMAKE_${lang}_COMPILER_FRONTEND_VARIANT}-like command-line")
    else()
      set(CMAKE_${lang}_COMPILER_FRONTEND_VARIANT "GNU")
    endif()
  ...
  if(CMAKE_${lang}_COMPILER_ID)
    ...
    if(CMAKE_${lang}_COMPILER_ARCHITECTURE_ID
       AND "x${CMAKE_${lang}_COMPILER_ID}" STREQUAL "xIAR")
      set(_archid " ${CMAKE_${lang}_COMPILER_ARCHITECTURE_ID}")
    else()
      set(_archid "")
    endif()
    message(
      STATUS "The ${lang} compiler identification is "
  ...
  if(lang STREQUAL "Fortran" AND CMAKE_${lang}_COMPILER_ID STREQUAL "XL")
    set(CMAKE_${lang}_XL_CPP
        "${CMAKE_${lang}_COMPILER_ID_CPP}"
        PARENT_SCOPE)
  endif()
  ...
  set(CMAKE_${lang}_COMPILER_ARCHITECTURE_ID
      ...
      PARENT_SCOPE)
  set(MSVC_${lang}_ARCHITECTURE_ID
      "${MSVC_${lang}_ARCHITECTURE_ID}"
      PARENT_SCOPE)
  set(CMAKE_${lang}_XCODE_ARCHS
      "${CMAKE_${lang}_XCODE_ARCHS}"
      PARENT_SCOPE)
  set(CMAKE_${lang}_CL_SHOWINCLUDES_PREFIX
      "${CMAKE_${lang}_CL_SHOWINCLUDES_PREFIX}"
      PARENT_SCOPE)
  set(CMAKE_${lang}_COMPILER_FRONTEND_VARIANT
  ...
  set(CMAKE_${lang}_COMPILER_VERSION_INTERNAL
      ...
      PARENT_SCOPE)
  set(CMAKE_${lang}_COMPILER_WRAPPER
      "${CMAKE_${lang}_COMPILER_WRAPPER}"
      PARENT_SCOPE)
  set(CMAKE_${lang}_SIMULATE_ID
      "${CMAKE_${lang}_SIMULATE_ID}"
      PARENT_SCOPE)
  set(CMAKE_${lang}_SIMULATE_VERSION
      "${CMAKE_${lang}_SIMULATE_VERSION}"
      PARENT_SCOPE)
  set(CMAKE_${lang}_STANDARD_COMPUTED_DEFAULT
      "${CMAKE_${lang}_STANDARD_COMPUTED_DEFAULT}"
      PARENT_SCOPE)
  set(CMAKE_${lang}_EXTENSIONS_COMPUTED_DEFAULT
      "${CMAKE_${lang}_EXTENSIONS_COMPUTED_DEFAULT}"
      PARENT_SCOPE)
  set(CMAKE_${lang}_COMPILER_PRODUCED_OUTPUT
      "${COMPILER_${lang}_PRODUCED_OUTPUT}"
      PARENT_SCOPE)
  set(CMAKE_${lang}_COMPILER_PRODUCED_FILES
      "${COMPILER_${lang}_PRODUCED_FILES}"
      PARENT_SCOPE)
endfunction()
...
function(CMAKE_DETERMINE_COMPILER_ID_WRITE lang src)
  ...
  compiler_id_detection(CMAKE_${lang}_COMPILER_ID_CONTENT ${lang} ID_STRING
                        VERSION_STRINGS PLATFORM_DEFAULT_COMPILER)
...
# -----------------------------------------------------------------------------
# Function to build the compiler id source file and look for output files.
...
")
  ...
  if("${CMAKE_GENERATOR}" MATCHES "Visual Studio ([0-9]+)")
    ...
    elseif(CMAKE_VS_PLATFORM_TOOLSET MATCHES
      ...
      if(CMAKE_VS_PLATFORM_NAME STREQUAL "Tegra-Android")
        ...
        if(CMAKE_VS_PLATFORM_TOOLSET_VERSION)
          ...
          if(CMAKE_VS_PLATFORM_TOOLSET_VERSION VERSION_GREATER_EQUAL "14.20")
            if(EXISTS
               "${CMAKE_GENERATOR_INSTANCE}/VC/Auxiliary/Build.${CMAKE_VS_PLATFORM_TOOLSET_VERSION}/Microsoft.VCToolsVersion.${CMAKE_VS_PLATFORM_TOOLSET_VERSION}.props"
            )
              set(id_sep ".")
            endif()
          endif()
          set(id_toolset_version_props
              "<Import Project=\"${CMAKE_GENERATOR_INSTANCE}\\VC\\Auxiliary\\Build${id_sep}${CMAKE_VS_PLATFORM_TOOLSET_VERSION}\\Microsoft.VCToolsVersion.${CMAKE_VS_PLATFORM_TOOLSET_VERSION}.props\" />"
          )
          unset(id_sep)
        endif()
      endif()
    ...
    if(CMAKE_VS_PLATFORM_TOOLSET_HOST_ARCHITECTURE)
      set(id_PreferredToolArchitecture
          "<PreferredToolArchitecture>${CMAKE_VS_PLATFORM_TOOLSET_HOST_ARCHITECTURE}</PreferredToolArchitecture>"
      )
    else()
      set(id_PreferredToolArchitecture "")
    endif()
    ...
    if(CMAKE_VS_TARGET_FRAMEWORK_VERSION)
      set(id_TargetFrameworkVersion
          "<TargetFrameworkVersion>${CMAKE_VS_TARGET_FRAMEWORK_VERSION}</TargetFrameworkVersion>"
      )
    endif()
    if(CMAKE_VS_TARGET_FRAMEWORK_IDENTIFIER)
      set(id_TargetFrameworkIdentifier
          "<TargetFrameworkIdentifier>${CMAKE_VS_TARGET_FRAMEWORK_IDENTIFIER}</TargetFrameworkIdentifier>"
      )
    endif()
    if(CMAKE_VS_TARGET_FRAMEWORK_TARGETS_VERSION)
      set(id_TargetFrameworkTargetsVersion
          "<TargetFrameworkTargetsVersion>${CMAKE_VS_TARGET_FRAMEWORK_TARGETS_VERSION}</TargetFrameworkTargetsVersion>"
      )
    endif()
    ...
    if(id_cl_var)
      set(id_PostBuildEvent_Command
          "echo CMAKE_${lang}_COMPILER=$(${id_cl_var})")
    else()
      set(id_PostBuildEvent_Command
          "for %%i in (${id_cl}) do %40echo CMAKE_${lang}_COMPILER=%%~$PATH:i")
    endif()
    set(id_Import_props "")
    ...
    if(lang STREQUAL CUDA)
      ...
      set(id_ItemDefinitionGroup_entry
          "<CudaCompile>${cuda_target}<AdditionalOptions>%(AdditionalOptions)-v</AdditionalOptions></CudaCompile>"
      )
      set(id_PostBuildEvent_Command
          [[echo CMAKE_CUDA_COMPILER=$(CudaToolkitBinDir)\nvcc.exe]])
      if(CMAKE_VS_PLATFORM_TOOLSET_CUDA_CUSTOM_DIR)
        ...
        if(EXISTS ${CMAKE_VS_PLATFORM_TOOLSET_CUDA_CUSTOM_DIR}nvcc)
          set(id_CudaToolkitCustomDir
              "<CudaToolkitCustomDir>${CMAKE_VS_PLATFORM_TOOLSET_CUDA_CUSTOM_DIR}nvcc</CudaToolkitCustomDir>"
          )
        else()
          set(id_CudaToolkitCustomDir
              "<CudaToolkitCustomDir>${CMAKE_VS_PLATFORM_TOOLSET_CUDA_CUSTOM_DIR}</CudaToolkitCustomDir>"
          )
        endif()
        if(EXISTS
           ${CMAKE_VS_PLATFORM_TOOLSET_CUDA_CUSTOM_DIR}CUDAVisualStudioIntegration
        )
          string(
            CONCAT
              id_Import_props
              "<Import Project=\"${CMAKE_VS_PLATFORM_TOOLSET_CUDA_CUSTOM_DIR}CUDAVisualStudioIntegration\\extras\\visual_studio_integration\\MSBuildExtensions\\${cuda_tools}.props\" />"
          )
          string(
            CONCAT
              id_Import_targets
              "<Import Project=\"${CMAKE_VS_PLATFORM_TOOLSET_CUDA_CUSTOM_DIR}CUDAVisualStudioIntegration\\extras\\visual_studio_integration\\MSBuildExtensions\\${cuda_tools}.targets\" />"
          )
        else()
          string(
            CONCAT
              id_Import_props
              "<Import Project=\"${CMAKE_VS_PLATFORM_TOOLSET_CUDA_CUSTOM_DIR}\\extras\\visual_studio_integration\\MSBuildExtensions\\${cuda_tools}.props\" />"
          )
          string(
            CONCAT
              id_Import_targets
              "<Import Project=\"${CMAKE_VS_PLATFORM_TOOLSET_CUDA_CUSTOM_DIR}\\extras\\visual_studio_integration\\MSBuildExtensions\\${cuda_tools}.targets\" />"
          )
        endif()
      else()
    ...
    if(CMAKE_VS_MSBUILD_COMMAND AND NOT lang STREQUAL "Fortran")
      set(command
          "${CMAKE_VS_MSBUILD_COMMAND}" "CompilerId${lang}.${ext}"
    ...
    if(command)
      ...
      set(CMAKE_${lang}_COMPILER_ID_OUTPUT
          "VS environment not known to support ${lang}")
    endif()
    ...
    if("${CMAKE_${lang}_COMPILER_ID_OUTPUT}" MATCHES
      ...
      if(EXISTS "${_comp}")
        ...
        set(CMAKE_${lang}_COMPILER_ID_TOOL
            "${_comp}"
            PARENT_SCOPE)
      endif()
    endif()
  elseif("${CMAKE_GENERATOR}" MATCHES "Xcode")
    ...
    if(CMAKE_OSX_SYSROOT)
      ...
      if(CMAKE_OSX_SYSROOT MATCHES "(^|/)[Ii][Pp][Hh][Oo][Nn][Ee]"
         OR CMAKE_OSX_SYSROOT MATCHES "(^|/)[Aa][Pp][Pp][Ll][Ee][Tt][Vv]")
    ...
    string(REGEX MATCHALL "${stdlib_regex}" all_stdlib_matches
                 "${CMAKE_CXX_FLAGS}")
    ...
    if(CMAKE_SYSTEM_NAME STREQUAL "Darwin" AND CMAKE_OSX_SYSROOT MATCHES
                                               "^$|[Mm][Aa][Cc][Oo][Ss]")
    ...
    if(DEFINED CMAKE_${lang}_COMPILER_ID_TOOL_MATCH_REGEX)
      if("${CMAKE_${lang}_COMPILER_ID_OUTPUT}" MATCHES
         "${CMAKE_${lang}_COMPILER_ID_TOOL_MATCH_REGEX}")
        set(_comp
            "${CMAKE_MATCH_${CMAKE_${lang}_COMPILER_ID_TOOL_MATCH_INDEX}}")
        if(EXISTS "${_comp}")
          set(CMAKE_${lang}_COMPILER_ID_TOOL
              "${_comp}"
              PARENT_SCOPE)
  ...
  elseif("${CMAKE_GENERATOR}" MATCHES "Green Hills MULTI")
    ...
    execute_process(
      COMMAND
        "${CMAKE_${lang}_COMPILER}" ${CMAKE_${lang}_COMPILER_ID_ARG1}
        ${userflags} ${testflags} ${CMAKE_${lang}_COMPILER_ID_FLAGS_ALWAYS}
        "${src}"
      WORKING_DIRECTORY ${CMAKE_${lang}_COMPILER_ID_DIR}
      OUTPUT_VARIABLE CMAKE_${lang}_COMPILER_ID_OUTPUT
      ERROR_VARIABLE CMAKE_${lang}_COMPILER_ID_OUTPUT
    ...
    if("${CMAKE_${lang}_COMPILER_ID_OUTPUT}" MATCHES
      ...
      if(EXISTS "${_cpp}")
        set(CMAKE_${lang}_COMPILER_ID_CPP
            "${_cpp}"
            PARENT_SCOPE)
      endif()
    endif()
  ...
  if(CMAKE_${lang}_COMPILER_ID_RESULT
     ...
     OR CMAKE_${lang}_COMPILER_ID_OUTPUT MATCHES
        "warning #5117: Bad # preprocessor line")
    # Compilation failed.
    string(
      APPEND
      _CMAKE_DETERMINE_COMPILER_ID_BUILD_MSG
...
")
    ...
    if(NOT CMAKE_${lang}_COMPILER_ID_OUTPUT MATCHES
       "warning #5117: Bad # preprocessor line")
      file(APPEND ${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/CMakeError.log
           "${MSG}")
    endif()

    # Some languages may know the correct/desired set of flags and want to fail
    # right away if they don't work. This is currently only used by CUDA.
    if(__compiler_id_require_success)
      message(FATAL_ERROR "${_CMAKE_DETERMINE_COMPILER_ID_BUILD_MSG}")
    else()
      # Build up the outputs for compiler detection attempts so that users can
      # see all set of flags tried, instead of just last
      set(_CMAKE_DETERMINE_COMPILER_ID_BUILD_MSG
          "${_CMAKE_DETERMINE_COMPILER_ID_BUILD_MSG}"
          PARENT_SCOPE)
    endif()
...
")
    ...
    # Find the executable produced by the compiler, try all files in the binary
    # dir.
...
function(CMAKE_DETERMINE_COMPILER_ID_MATCH_VENDOR lang output)
  foreach(vendor ${CMAKE_${lang}_COMPILER_ID_MATCH_VENDORS})
    if(output MATCHES
       "${CMAKE_${lang}_COMPILER_ID_MATCH_VENDOR_REGEX_${vendor}}")
...
function(CMAKE_DETERMINE_COMPILER_ID_CHECK lang file)
  ...
  if(NOT CMAKE_${lang}_COMPILER_ID)
    ...
    set(CMAKE_${lang}_COMPILER_ID_STRING_REGEX
        ".?I.?N.?F.?O.?:.?[A-Za-z0-9_]+\\[[^]]*\\]")
    ...
    # Some ADSP processors result in characters being detected as separate
    # strings
    if(CMAKE_${lang}_COMPILER_ID_STRINGS STREQUAL "")
      file(STRINGS "${file}" CMAKE_${lang}_COMPILER_ID_STRINGS LENGTH_MAXIMUM 1)
      string(REGEX REPLACE ";" "" CMAKE_${lang}_COMPILER_ID_STRING
                           "${CMAKE_${lang}_COMPILER_ID_STRINGS}")
      string(REGEX MATCHALL "${CMAKE_${lang}_COMPILER_ID_STRING_REGEX}"
                   CMAKE_${lang}_COMPILER_ID_STRINGS
                   "${CMAKE_${lang}_COMPILER_ID_STRING}")
    endif()

    # With the IAR Compiler, some strings are found twice, first time as
    # incomplete list like "?<Constant "INFO:compiler[IAR]">".  Remove the
    # incomplete copies.
    list(FILTER CMAKE_${lang}_COMPILER_ID_STRINGS EXCLUDE REGEX
         "\\?<Constant \\\"")

    # The IAR-AVR compiler uses a binary format that places a '6' character
    # (0x34) before each character in the string.  Strip out these characters
    # without removing any legitimate characters.
    if(CMAKE_${lang}_COMPILER_ID_STRINGS MATCHES "(.)I.N.F.O.:.")
      string(
        REGEX
        REPLACE "${CMAKE_MATCH_1}([^;])" "\\1"
                CMAKE_${lang}_COMPILER_ID_STRINGS
                "${CMAKE_${lang}_COMPILER_ID_STRINGS}")
    endif()
    ...
    foreach(info ${CMAKE_${lang}_COMPILER_ID_STRINGS})
      ...
      if("${info}" MATCHES "INFO:compiler_version\\[([^]\"]*)\\]")
        string(REGEX REPLACE "^0+([0-9]+)" "\\1" COMPILER_VERSION
                             "${CMAKE_MATCH_1}")
        string(REGEX REPLACE "\\.0+([0-9])" ".\\1" COMPILER_VERSION
                             "${COMPILER_VERSION}")
      endif()
      if("${info}" MATCHES "INFO:compiler_version_internal\\[([^]\"]*)\\]")
        ...
        string(REGEX REPLACE "^0+([0-9]+)" "\\1" COMPILER_VERSION_INTERNAL
      ...
      foreach(comp MAJOR MINOR PATCH TWEAK)
        foreach(
          ...
          9)
          if("${info}" MATCHES
             "INFO:compiler_version_${comp}_digit_${digit}\\[([0-9])\\]")
            set(value ${CMAKE_MATCH_1})
            math(
              EXPR COMPILER_VERSION_${comp}
              "${COMPILER_VERSION_${comp}} + ${value} * ${DIGIT_VALUE_${digit}}"
            )
  ...
  if(NOT CMAKE_EXECUTABLE_FORMAT)
    file(
      READ ${file} CMAKE_EXECUTABLE_MAGIC
      LIMIT 4
      HEX)
    ...
    # # COFF (.exe) files start with "MZ" if("${CMAKE_EXECUTABLE_MAGIC}" MATCHES
    # "4d5a....") set(CMAKE_EXECUTABLE_FORMAT "COFF" CACHE INTERNAL "Executable
    # file format") endif()
    #
...
# -----------------------------------------------------------------------------
# Function to query the compiler vendor. This uses a table with entries of the
# form list(APPEND CMAKE_${lang}_COMPILER_ID_VENDORS ${vendor})
# set(CMAKE_${lang}_COMPILER_ID_VENDOR_FLAGS_${vendor} -some-vendor-flag)
# set(CMAKE_${lang}_COMPILER_ID_VENDOR_REGEX_${vendor} "Some Vendor Output") We
# try running the compiler with the flag for each vendor and matching its
# regular expression in the output.
function(CMAKE_DETERMINE_COMPILER_ID_VENDOR lang userflags)
  ...
  if(NOT CMAKE_${lang}_COMPILER_ID_DIR)
    ...
    # CMAKE_DETERMINE_COMPILER_ID() This is done e.g. for detecting the compiler
    # ID for assemblers. Compute the directory in which to run the test and
    # Create a clean working directory.
    set(CMAKE_${lang}_COMPILER_ID_DIR
        ${CMAKE_PLATFORM_INFO_DIR}/CompilerId${lang})
    file(REMOVE_RECURSE ${CMAKE_${lang}_COMPILER_ID_DIR})
  ...
  # them to "C" so we get the expected output to match.
  ...
  foreach(vendor ${CMAKE_${lang}_COMPILER_ID_VENDORS})
    ...
    if("${output}" MATCHES "${regex}")
      ...
      set(CMAKE_${lang}_COMPILER_ID_OUTPUT
          "${output}"
          PARENT_SCOPE)
      set(CMAKE_${lang}_COMPILER_ID_VENDOR_MATCH
          "${CMAKE_MATCH_1}"
          PARENT_SCOPE)
      break()
    else()
...
function(CMAKE_DETERMINE_MSVC_SHOWINCLUDES_PREFIX lang userflags)
  # Run this MSVC-compatible compiler to detect what the /showIncludes option
  # displays.  We can use a C source even with the C++ compiler because
  # MSVC-compatible compilers handle both and show the same output.
  set(showdir ${CMAKE_BINARY_DIR}/CMakeFiles/ShowIncludes)
  file(WRITE ${showdir}/foo.h "\n")
  file(WRITE ${showdir}/main.c "#include \"foo.h\" \nint main(){}\n")
  execute_process(
    COMMAND "${CMAKE_${lang}_COMPILER}" ${CMAKE_${lang}_COMPILER_ID_ARG1}
            ${userflags} /nologo /showIncludes /c main.c
    WORKING_DIRECTORY ${showdir}
    OUTPUT_VARIABLE out
    ERROR_VARIABLE err
    RESULT_VARIABLE res ENCODING AUTO # cl prints in console output code page
  )
