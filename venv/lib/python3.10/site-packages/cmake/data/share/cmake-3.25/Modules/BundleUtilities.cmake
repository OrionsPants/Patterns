...
if(DEFINED CMAKE_GENERATOR)
  ...
  elseif(NOT _BundleUtilities_CMP0080 STREQUAL "OLD"
         AND NOT _CMP0080_SUPPRESS_WARNING)
    _warn_cmp0080()
  endif()
endif()
...
function(get_bundle_main_executable bundle result_var)
  ...
  if(EXISTS "${bundle}/Contents/Info.plist")
    set(result
        "error: no CFBundleExecutable in '${bundle}/Contents/Info.plist' file")
    ...
    foreach(line ${info_plist})
      if(line_is_main_executable)
        string(REGEX REPLACE "^.*<string>(.*)</string>.*$" "\\1"
                             bundle_executable "${line}")
        break()
      endif()
    ...
    if(NOT bundle_executable STREQUAL "")
      if(EXISTS "${bundle}/Contents/MacOS/${bundle_executable}")
        ...
        # Ultimate goal: If not in "Contents/MacOS" then scan the bundle for
        # matching files. If there is only one executable file that matches,
    ...
    #
    # More inclusive technique... (This one would work on Windows and Linux too,
    # if a developer followed the typical Mac bundle naming convention...)
    #
...
function(get_dotapp_dir exe dotapp_dir_var)
  ...
  if(s MATCHES "/.*\\.app/")
    # If there is a ".app" parent directory, ascend until we hit it: (typical of
    # a Mac bundle executable)
    #
    set(done 0)
    while(NOT ${done})
    ...
    # Otherwise use a directory containing the exe (typical of a non-bundle
    # executable on Mac, Windows or Linux)
    #
  ...
  if(EXISTS "${app}")
    ...
    if(IS_DIRECTORY "${app}")
      if(app MATCHES "\\.app$")
        ...
        message(
          STATUS "warning: *NOT* handled - directory but not .app case...")
      endif()
    else()
      ...
      if(is_executable)
        ...
        message(
          STATUS "warning: *NOT* handled - not .app dir, not executable file..."
        )
      endif()
    endif()
  else()
...
function(
  ...
  if(NOT length_before EQUAL length_after)
    ...
    if((NOT item MATCHES "\\.dylib$") AND (item MATCHES "[^/]+\\.framework/"))
      ...
      #
      string(REGEX
             REPLACE "^.*(${item_name}.framework/.*/?${item_name}).*$"
    ...
    set(${key}_ITEM
        "${item}"
        PARENT_SCOPE)
    set(${key}_RESOLVED_ITEM
        "${resolved_item}"
        PARENT_SCOPE)
    set(${key}_DEFAULT_EMBEDDED_PATH
        "${default_embedded_path}"
        PARENT_SCOPE)
    set(${key}_EMBEDDED_ITEM
        "${embedded_item}"
        PARENT_SCOPE)
    set(${key}_RESOLVED_EMBEDDED_ITEM
        "${resolved_embedded_item}"
        PARENT_SCOPE)
    set(${key}_COPYFLAG
        "${copyflag}"
...
function(get_bundle_keys app libs dirs keys_var)
  ...
  if(valid)
    ...
    # For each extra lib, accumulate a key as well and then also accumulate any
    # of its prerequisites. (Extra libs are typically dynamically loaded
    ...
    # For each executable found in the bundle, accumulate keys as we go. The
    # list of keys should be complete when all prerequisites of all binaries in
    # the bundle have been analyzed.
    #
    foreach(exe ${exes})
...
function(link_resolved_item_into_bundle resolved_item resolved_embedded_item)
  ...
  if(resolved_item_compare STREQUAL resolved_embedded_item_compare)
    message(
      STATUS "warning: resolved_item == resolved_embedded_item - not linking..."
    )
  else()
...
function(copy_resolved_item_into_bundle resolved_item resolved_embedded_item)
  ...
  if(resolved_item_compare STREQUAL resolved_embedded_item_compare)
    message(
      STATUS "warning: resolved_item == resolved_embedded_item - not copying..."
    )
  else()
...
function(copy_resolved_framework_into_bundle resolved_item
  ...
  if(resolved_item_compare STREQUAL resolved_embedded_item_compare)
    ...
    if(BU_COPY_FULL_FRAMEWORK_CONTENTS)
      ...
      get_filename_component(resolved_embedded_dir "${resolved_embedded_item}"
                             PATH)
      get_filename_component(resolved_embedded_dir
                             "${resolved_embedded_dir}/../.." ABSOLUTE)
      # message(STATUS "copying COMMAND ${CMAKE_COMMAND} -E copy_directory
      # '${resolved_dir}' '${resolved_embedded_dir}'")
      execute_process(COMMAND ${CMAKE_COMMAND} -E copy_directory
                              "${resolved_dir}" "${resolved_embedded_dir}")
    else()
      # Framework lib itself: message(STATUS "copying COMMAND ${CMAKE_COMMAND}
      # -E copy ${resolved_item} ${resolved_embedded_item}")
      execute_process(COMMAND ${CMAKE_COMMAND} -E copy "${resolved_item}"
                              "${resolved_embedded_item}")

      # Plus Resources, if they exist:
      string(REGEX REPLACE "^(.*)/[^/]+$" "\\1/Resources" resolved_resources
                           "${resolved_item}")
      string(REGEX
             REPLACE "^(.*)/[^/]+$" "\\1/Resources" resolved_embedded_resources
                     "${resolved_embedded_item}")
      if(EXISTS "${resolved_resources}")
        # message(STATUS "copying COMMAND ${CMAKE_COMMAND} -E copy_directory
        # '${resolved_resources}' '${resolved_embedded_resources}'")
        execute_process(
          COMMAND ${CMAKE_COMMAND} -E copy_directory "${resolved_resources}"
      ...
      if(NOT EXISTS "${resolved_resources}/Info.plist")
        ...
        string(REGEX
               REPLACE "^(.*)/[^/]+/[^/]+/[^/]+$" "\\1/Contents/Info.plist"
                       resolved_info_plist "${resolved_item}")
        string(REGEX
               REPLACE "^(.*)/[^/]+$" "\\1/Resources/Info.plist"
        ...
        if(EXISTS "${resolved_info_plist}")
          ...
          execute_process(
            COMMAND ${CMAKE_COMMAND} -E copy "${resolved_info_plist}"
                    "${resolved_embedded_info_plist}")
        endif()
      endif()

      # Check if framework is versioned and fix it layout
      string(REGEX REPLACE "^.*/([^/]+)/[^/]+$" "\\1" resolved_embedded_version
                           "${resolved_embedded_item}")
      string(REGEX
             REPLACE "^(.*)/[^/]+/[^/]+$" "\\1" resolved_embedded_versions
                     "${resolved_embedded_item}")
      string(
        REGEX
        REPLACE "^.*/([^/]+)/[^/]+/[^/]+$" "\\1"
      ...
      if(resolved_embedded_versions_basename STREQUAL "Versions")
        ...
        if(NOT EXISTS "${resolved_embedded_versions}/Current")
          execute_process(
            COMMAND
              ${CMAKE_COMMAND} -E create_symlink "${resolved_embedded_version}"
        ...
        string(REGEX
               REPLACE "^.*/([^/]+)$" "\\1" resolved_embedded_item_basename
                       "${resolved_embedded_item}")
        if(NOT EXISTS
        ...
        if(NOT EXISTS "${resolved_embedded_root}/Resources")
          execute_process(
            COMMAND
              ${CMAKE_COMMAND} -E create_symlink "Versions/Current/Resources"
...
function(fixup_bundle_item resolved_embedded_item exepath dirs)
  ...
  # CMAKE_INSTALL_NAME_TOOL may not be set if executed in script mode Duplicated
  # from CMakeFindBinUtils.cmake
  find_program(
  ...
  # Change this item's id and all of its references in one call to
  # install_name_tool:
  #
  if(changes)
    ...
    if(NOT "${resolved_embedded_item}" MATCHES "\\.(bat|c?sh|bash|ksh|cmd)$"
       AND NOT file_contents MATCHES "^#!")
  ...
  if(valid)
    ...
    foreach(key ${keys})
      ...
      if("${${key}_COPYFLAG}" STREQUAL "2")
        message(
          STATUS
            "${i}/${n}: linking '${${key}_RESOLVED_ITEM}' -> '${${key}_RESOLVED_EMBEDDED_ITEM}'"
        )
      ...
      if(show_status)
        ...
        message(
          STATUS "default_embedded_path='${${key}_DEFAULT_EMBEDDED_PATH}'")
        message(STATUS "embedded_item='${${key}_EMBEDDED_ITEM}'")
        message(
          STATUS "resolved_embedded_item='${${key}_RESOLVED_EMBEDDED_ITEM}'")
      ...
      elseif(${${key}_COPYFLAG})
        ...
        if(item MATCHES "[^/]+\\.framework/")
          copy_resolved_framework_into_bundle(
            "${${key}_RESOLVED_ITEM}" "${${key}_RESOLVED_EMBEDDED_ITEM}")
        else()
    ...
    foreach(key ${keys})
      ...
      if(APPLE)
        message(
          STATUS "${i}/${n}: fixing up '${${key}_RESOLVED_EMBEDDED_ITEM}'")
        ...
        message(
          STATUS
            "${i}/${n}: fix-up not required on this platform '${${key}_RESOLVED_EMBEDDED_ITEM}'"
        )
      endif()
    endforeach()
...
function(verify_bundle_prerequisites bundle result_var info_var)
  ...
  foreach(f ${file_list})
    ...
    if(NOT prereq_filename IN_LIST CFG_IGNORE_ITEM)
      ...
      # On the Mac, "embedded" and "system" prerequisites are fine... anything
      # else means the bundle's prerequisites are not verified (i.e., the bundle
      # is not really "standalone")
      #
      # On Windows (and others? Linux/Unix/...?) "local" and "system" prereqs
      # are fine...
      #
...
function(verify_bundle_symlinks bundle result_var info_var)
  ...
  # TODO: implement this function for real... Right now, it is just a stub that
