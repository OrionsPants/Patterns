...
macro(set_spec_scripts PACKAGE_NAME)
  # we should only set scripts that were provided as script announcement without
  # content inside spec file will generate unneeded dependency on shell

  set_spec_script_if_enabled(
    "post" "${PACKAGE_NAME}"
    "${RPM_SYMLINK_POSTINSTALL}\n${CPACK_RPM_SPEC_POSTINSTALL}")

  set_spec_script_if_enabled("posttrans" "${PACKAGE_NAME}"
                             "${CPACK_RPM_SPEC_POSTTRANS}")

  set_spec_script_if_enabled("postun" "${PACKAGE_NAME}"
                             "${CPACK_RPM_SPEC_POSTUNINSTALL}")

  set_spec_script_if_enabled("pre" "${PACKAGE_NAME}"
                             "${CPACK_RPM_SPEC_PREINSTALL}")

  set_spec_script_if_enabled("pretrans" "${PACKAGE_NAME}"
                             "${CPACK_RPM_SPEC_PRETRANS}")

  set_spec_script_if_enabled("preun" "${PACKAGE_NAME}"
...
function(get_unix_permissions_octal_notation PERMISSIONS_VAR RETURN_VAR)
  ...
  foreach(PERMISSION_TYPE "OWNER" "GROUP" "WORLD")
    ...
    foreach(PERMISSION ${PERMISSIONS})
      ...
      elseif("${PERMISSION}" STREQUAL "${PERMISSION_TYPE}_EXECUTE")
        math(EXPR ${PERMISSION_TYPE}_PERMISSIONS
             "${${PERMISSION_TYPE}_PERMISSIONS} + 1")
  ...
  set(${RETURN_VAR}
      "${OWNER_PERMISSIONS}${GROUP_PERMISSIONS}${WORLD_PERMISSIONS}"
      PARENT_SCOPE)
endfunction()
...
function(cpack_rpm_prepare_relocation_paths)
  ...
  elseif(CPACK_RPM_${CPACK_RPM_PACKAGE_COMPONENT_UPPER}_PACKAGE_PREFIX)
    file(TO_CMAKE_PATH
         "${CPACK_RPM_${CPACK_RPM_PACKAGE_COMPONENT_UPPER}_PACKAGE_PREFIX}"
         PATH_PREFIX)
  else()
  ...
  if(EXISTS "${WDIR}/${PATH_PREFIX}")
    if(NOT CPACK_RPM_NO_INSTALL_PREFIX_RELOCATION
       AND NOT
           CPACK_RPM_NO_${CPACK_RPM_PACKAGE_COMPONENT}_INSTALL_PREFIX_RELOCATION
       AND NOT
           CPACK_RPM_NO_${CPACK_RPM_PACKAGE_COMPONENT_UPPER}_INSTALL_PREFIX_RELOCATION
    )
      ...
      if(CPACK_RPM_PACKAGE_DEBUG)
        message(
          "CPackRPM:Debug: removing '${PATH_PREFIX}' from relocation paths")
      endif()
    endif()
  endif()
  ...
  foreach(TMP_PATH ${FILE_PATHS_})
    ...
    if(NOT TPM_PATH_FOUND_)
      message(
        AUTHOR_WARNING
          "CPackRPM:Warning: Path ${TMP_PATH} is not on one of the relocatable paths! Package will be partially relocatable."
      )
    endif()
  endforeach()

  set(RPM_USED_PACKAGE_PREFIXES
...
function(cpack_rpm_prepare_content_list)
  # get files list
  file(
    GLOB_RECURSE CPACK_RPM_INSTALL_FILES
  ...
  # CPACK_RPM_PACKAGE_PREFIX. This is achieved by building a "filter list" which
  # is passed to the find command that generates the content-list
  if(CPACK_RPM_PACKAGE_RELOCATABLE)
    # get a list of the elements in CPACK_RPM_PACKAGE_PREFIXES that are destinct
    # parent paths of other relocation paths and remove the final element (so
    # the install-prefix dir itself is not omitted from the RPM's content-list)
  ...
  if(CPACK_RPM_PACKAGE_DEBUG)
    message(
      "CPackRPM:Debug: Initial list of path to OMIT in RPM: ${_RPM_DIRS_TO_OMIT}"
    )
  endif()
  ...
  if(NOT DEFINED CPACK_RPM_EXCLUDE_FROM_AUTO_FILELIST)
    ...
    if(CPACK_RPM_EXCLUDE_FROM_AUTO_FILELIST_ADDITION)
      if(CPACK_RPM_PACKAGE_DEBUG)
        message(
          "CPackRPM:Debug: Adding ${CPACK_RPM_EXCLUDE_FROM_AUTO_FILELIST_ADDITION} to builtin omit list."
        )
      endif()
  ...
  if(CPACK_RPM_EXCLUDE_FROM_AUTO_FILELIST)
    if(CPACK_RPM_PACKAGE_DEBUG)
      message(
        "CPackRPM:Debug: CPACK_RPM_EXCLUDE_FROM_AUTO_FILELIST= ${CPACK_RPM_EXCLUDE_FROM_AUTO_FILELIST}"
      )
    endif()
  ...
  if(CPACK_RPM_PACKAGE_DEBUG)
    message(
      "CPackRPM:Debug: Final list of path to OMIT in RPM: ${_RPM_DIRS_TO_OMIT}")
  endif()

  list(REMOVE_ITEM CPACK_RPM_INSTALL_FILES ${_RPM_DIRS_TO_OMIT})

  # add man paths that will be compressed (copied from /usr/lib/rpm/brp-compress
  # - script that does the actual compressing)
  list(
    APPEND
    MAN_LOCATIONS
    "/usr/man/man.*"
  ...
  if(CPACK_RPM_ADDITIONAL_MAN_DIRS)
    if(CPACK_RPM_PACKAGE_DEBUG)
      message(
        "CPackRPM:Debug: CPACK_RPM_ADDITIONAL_MAN_DIRS= ${CPACK_RPM_ADDITIONAL_MAN_DIRS}"
      )
    endif()
  ...
  foreach(PACK_LOCATION IN LISTS CPACK_RPM_INSTALL_FILES)
    foreach(MAN_LOCATION IN LISTS MAN_LOCATIONS)
      ...
      if(PACK_LOCATION MATCHES "${MAN_LOCATION}/"
        ...
        # insert file location that covers compressed man pages even if using a
        # wildcard causes duplicates as those are handled by RPM and we still
        # keep the same file list in spec file - wildcard only represents file
        # type (e.g. .gz)
...
function(cpack_rpm_symlink_create_relocation_script PACKAGE_PREFIXES)
  ...
  foreach(SYMLINK_INDEX RANGE ${LAST_INDEX})
    ...
    set(PARTS_CNT 0)
    set(SCRIPT_PART
        "if [ \"$RPM_INSTALL_PREFIX${SYMLINK_INDEX}\" != \"${SRC_PATH}\" ]; then\n"
    )
      ...
      if(_RPM_RELOCATION_SCRIPT_${SYMLINK_INDEX}_${POINT_INDEX})
        if("${SYMLINK_INDEX}" EQUAL "${POINT_INDEX}")
          ...
          string(
            APPEND
            SCRIPT_PART
            "  if [ \"$RPM_INSTALL_PREFIX${POINT_INDEX}\" != \"${POINT_PATH}\" ]; then\n"
          )
          set(INDENT "  ")
        endif()

        foreach(RELOCATION_NO IN
                LISTS _RPM_RELOCATION_SCRIPT_${SYMLINK_INDEX}_${POINT_INDEX})
          math(EXPR PARTS_CNT ${PARTS_CNT}+1)

          math(EXPR RELOCATION_INDEX ${RELOCATION_NO}-1)
          list(GET _RPM_RELOCATION_SCRIPT_PAIRS ${RELOCATION_INDEX}
               RELOCATION_SCRIPT_PAIR)
          string(FIND "${RELOCATION_SCRIPT_PAIR}" ":" SPLIT_INDEX)

          math(EXPR SRC_PATH_END ${SPLIT_INDEX}-${SRC_PATH_LEN})
          string(SUBSTRING ${RELOCATION_SCRIPT_PAIR} ${SRC_PATH_LEN}
                           ${SRC_PATH_END} SYMLINK_)

          math(EXPR POINT_PATH_START ${SPLIT_INDEX}+1+${POINT_PATH_LEN})
          string(SUBSTRING ${RELOCATION_SCRIPT_PAIR} ${POINT_PATH_START} -1
                           POINT_)
  ...
  set(RPM_SYMLINK_POSTINSTALL
      "${SCRIPT}"
      PARENT_SCOPE)
endfunction()

function(cpack_rpm_symlink_add_for_relocation_script PACKAGE_PREFIXES SYMLINK
         SYMLINK_RELOCATION_PATHS POINT POINT_RELOCATION_PATHS)
  ...
  if(SYMLINK_PATHS_COUTN)
    foreach(SYMLINK_RELOC_PATH IN LISTS SYMLINK_RELOCATION_PATHS)
      ...
      foreach(POINT_RELOC_PATH IN LISTS POINT_RELOCATION_PATHS)
        ...
        # both paths relocated
        list(APPEND _RPM_RELOCATION_SCRIPT_${SYMLINK_INDEX}_${POINT_INDEX}
             "${PAIR_NO}")
        list(APPEND RELOCATION_VARS
             "_RPM_RELOCATION_SCRIPT_${SYMLINK_INDEX}_${POINT_INDEX}")
  ...
  foreach(VAR IN LISTS RELOCATION_VARS)
    set(${VAR}
        ...
        PARENT_SCOPE)
  endforeach()
  set(_RPM_RELOCATION_SCRIPT_PAIRS
      "${_RPM_RELOCATION_SCRIPT_PAIRS}"
      PARENT_SCOPE)
  set(REQUIRES_SYMLINK_RELOCATION_SCRIPT
      "true"
      PARENT_SCOPE)
  set(DIRECTIVE
      "%ghost "
...
function(cpack_rpm_prepare_install_files INSTALL_FILES_LIST WDIR
         PACKAGE_PREFIXES IS_RELOCATABLE)
  # Prepend directories in ${CPACK_RPM_INSTALL_FILES} with %dir This is
  # necessary to avoid duplicate files since rpmbuild does recursion on its own
  # when encountering a pathname which is a directory which is not flagged as
  # %dir
  ...
  foreach(F IN LISTS INSTALL_FILES_LIST)
    ...
    if(IS_SYMLINK "${WDIR}/${F}")
      if(IS_RELOCATABLE)
        ...
        string(SUBSTRING "${SYMLINK_POINT_}" ${SYMLINK_POINT_INDEX_}
                         ${SYMLINK_POINT_LENGTH_} SYMLINK_POINT_)

        # check if path is relative or absolute
        string(SUBSTRING "${SYMLINK_POINT_}" 0 1 SYMLINK_IS_ABSOLUTE_)

        if(${SYMLINK_IS_ABSOLUTE_} STREQUAL "/")
          # prevent absolute paths from having /../ or /./ section inside of
          # them
          get_filename_component(SYMLINK_POINT_ "${SYMLINK_POINT_}" ABSOLUTE)
        else()
          # handle relative path
          get_filename_component(
            SYMLINK_POINT_ "${SYMLINK_LOCATION_}/${SYMLINK_POINT_}" ABSOLUTE)
        endif()
        ...
        if(SYMLINK_POINT_ MATCHES "${IN_SYMLINK_POINT_REGEX}")
          ...
          cpack_rpm_symlink_get_relocation_prefixes(
            "${F}" "${PACKAGE_PREFIXES}" "SYMLINK_RELOCATIONS")
          cpack_rpm_symlink_get_relocation_prefixes(
            "${SYMLINK_POINT_WD_}" "${PACKAGE_PREFIXES}" "POINT_RELOCATIONS")

          list(LENGTH SYMLINK_RELOCATIONS SYMLINK_RELOCATIONS_COUNT)
          list(LENGTH POINT_RELOCATIONS POINT_RELOCATIONS_COUNT)
        else()
          # location pointed to is outside WDR so it should be treated as a
          # permanent symlink
          ...
          message(
            AUTHOR_WARNING
              "CPackRPM:Warning: Symbolic link '${F}' points to location that is outside packaging path! Link will possibly not be relocatable."
          )
        endif()
        ...
        if(SYMLINK_RELOCATIONS_COUNT AND POINT_RELOCATIONS_COUNT)
          ...
          foreach(SYMLINK_RELOCATION_PREFIX IN LISTS SYMLINK_RELOCATIONS)
            list(FIND POINT_RELOCATIONS "${SYMLINK_RELOCATION_PREFIX}"
                 FOUND_INDEX)
          ...
          if(NOT ${FOUND_INDEX} EQUAL -1)
            ...
            if(${SYMLINK_RELOCATIONS_COUNT} EQUAL 1
               AND ${POINT_RELOCATIONS_COUNT} EQUAL 1)
              # permanent symlink
              get_filename_component(SYMLINK_LOCATION_ "${F}" DIRECTORY)
              file(RELATIVE_PATH FINAL_PATH_ ${SYMLINK_LOCATION_}
                   ${SYMLINK_POINT_WD_})
              ...
              cpack_rpm_symlink_add_for_relocation_script(
                "${PACKAGE_PREFIXES}" "${F}" "${SYMLINK_RELOCATIONS}"
        ...
        elseif(POINT_RELOCATIONS_COUNT)
          ...
          # is not relocatable or points to non relocatable path - permanent
          # symlink
          execute_process(COMMAND "${CMAKE_COMMAND}" -E create_symlink
...
# We need to check if the binaries were compiled with debug symbols because
...
function(cpack_rpm_debugsymbol_check INSTALL_FILES WORKING_DIR)
  if(NOT CPACK_BUILD_SOURCE_DIRS)
    message(
      FATAL_ERROR
        "CPackRPM: CPACK_BUILD_SOURCE_DIRS variable is not set!"
  ...
  if(NOT CPACK_OBJDUMP_EXECUTABLE)
    message(
      FATAL_ERROR
        "CPackRPM: objdump binary could not be found!"
        " Required for debuginfo packaging. See documentation of"
        " CPACK_RPM_DEBUGINFO_PACKAGE variable for details.")
  endif()
  ...
  if(NOT DEBUGEDIT_EXECUTABLE)
    message(
      FATAL_ERROR
        "CPackRPM: debugedit binary could not be found!"
        " Required for debuginfo packaging. See documentation of"
        " CPACK_RPM_DEBUGINFO_PACKAGE variable for details.")
  endif()
  ...
  foreach(F IN LISTS INSTALL_FILES)
    ...
    execute_process(
      COMMAND "${CPACK_OBJDUMP_EXECUTABLE}" -h ${WORKING_DIR}/${F}
      ...
      if(FIND_RESULT GREATER -1)
        ...
        foreach(source_dir_ IN LISTS CPACK_BUILD_SOURCE_DIRS)
          ...
          if(source_dir_len_ LESS debuginfo_dir_len)
            message(
              FATAL_ERROR
                "CPackRPM: source dir path '${source_dir_}' is"
          ...
          execute_process(
            COMMAND
              "${DEBUGEDIT_EXECUTABLE}" -b "${source_dir_}" -d
              "${CPACK_RPM_BUILD_SOURCE_DIRS_PREFIX}/src_${index_}" -i -l
              "${CPACK_RPM_DIRECTORY}/${CPACK_PACKAGE_FILE_NAME}${CPACK_RPM_PACKAGE_COMPONENT_PART_PATH}/debugsources_add.list"
              "${WORKING_DIR}/${F}"
          ...
          foreach(source_ IN LISTS sources_)
            if(EXISTS "${source_dir_}/${source_}"
               AND NOT IS_DIRECTORY "${source_dir_}/${source_}")
        ...
        message(
          WARNING
            "CPackRPM: File: ${F} does not contain debug symbols. They will possibly be missing from debuginfo package!"
        )
      endif()

      get_file_permissions("${WORKING_DIR}/${F}" permissions_)
      if(NOT "USER_EXECUTE" IN_LIST permissions_
         AND NOT "GROUP_EXECUTE" IN_LIST permissions_
         AND NOT "WORLD_EXECUTE" IN_LIST permissions_)
        if(CPACK_RPM_INSTALL_WITH_EXEC)
          execute_process(
            COMMAND chmod a+x ${WORKING_DIR}/${F}
            RESULT_VARIABLE res_
            ERROR_VARIABLE err_
            OUTPUT_QUIET)
          ...
          if(res_)
            message(
              FATAL_ERROR
                "CPackRPM: could not apply execute permissions "
          ...
          message(
            AUTHOR_WARNING
              "CPackRPM: File: ${WORKING_DIR}/${F} does not "
  ...
  if(NOT DEFINED CPACK_RPM_DEBUGINFO_EXCLUDE_DIRS)
    ...
    if(CPACK_RPM_DEBUGINFO_EXCLUDE_DIRS_ADDITION)
      if(CPACK_RPM_PACKAGE_DEBUG)
        message(
          "CPackRPM:Debug: Adding ${CPACK_RPM_DEBUGINFO_EXCLUDE_DIRS_ADDITION} to builtin omit list."
        )
      endif()
  ...
  if(CPACK_RPM_PACKAGE_DEBUG)
    message(
      "CPackRPM:Debug: CPACK_RPM_DEBUGINFO_EXCLUDE_DIRS= ${CPACK_RPM_DEBUGINFO_EXCLUDE_DIRS}"
    )
  endif()
  ...
  set(TMP_RPM_DEBUGINFO_INSTALL
      ...
      PARENT_SCOPE)
  set(TMP_DEBUGINFO_ADDITIONAL_SOURCES
...
function(cpack_rpm_generate_package)
  # rpmbuild is the basic command for building RPM package it may be a simple
  # (symbolic) link to rpm command.
  find_program(RPMBUILD_EXECUTABLE rpmbuild)

  # Check version of the rpmbuild tool this would be easier to track bugs with
  # users and CPackRPM debug mode. We may use RPM version in order to check for
  # available version dependent features
  if(RPMBUILD_EXECUTABLE)
    execute_process(
      COMMAND ${RPMBUILD_EXECUTABLE} --version
      OUTPUT_VARIABLE _TMP_VERSION
      ERROR_QUIET OUTPUT_STRIP_TRAILING_WHITESPACE)
    string(REGEX REPLACE "^.* " "" RPMBUILD_EXECUTABLE_VERSION ${_TMP_VERSION})
    if(CPACK_RPM_PACKAGE_DEBUG)
      message(
        "CPackRPM:Debug: rpmbuild version is <${RPMBUILD_EXECUTABLE_VERSION}>")
    endif()
  endif()
  ...
  # Display lsb_release output if DEBUG mode enable This will help to diagnose
  # problem with CPackRPM because we will know on which kind of Linux we are
  if(CPACK_RPM_PACKAGE_DEBUG)
    find_program(LSB_RELEASE_EXECUTABLE lsb_release)
    if(LSB_RELEASE_EXECUTABLE)
      execute_process(
        COMMAND ${LSB_RELEASE_EXECUTABLE} -a
  ...
  # We may use RPM version in the future in order to shut down warning about
  # space in buildtree some recent RPM version should support space in different
  # places. not checked [yet].
  if(CPACK_TOPLEVEL_DIRECTORY MATCHES ".* .*")
    message(
      FATAL_ERROR
        "${RPMBUILD_EXECUTABLE} can't handle paths with spaces, use a build directory without spaces for building RPMs."
    )
  endif()

  # If rpmbuild is found we try to discover alien since we may be on non RPM
  # distro like Debian. In this case we may try to to use more advanced features
  # like generating RPM directly from DEB using alien. FIXME feature not
  # finished (yet)
  ...
  #
  # Use user-defined RPM specific variables value or generate reasonable default
  # value from CPACK_xxx generic values. The variables comes from the needed
  # (mandatory or not) values found in the RPM specification file aka ".spec"
  # file. The variables which may/should be defined are:
  #
  ...
  if(CPACK_RPM_PACKAGE_COMPONENT)
    ...
    if(NOT CPACK_RPM_MAIN_COMPONENT_UPPER STREQUAL
       CPACK_RPM_PACKAGE_COMPONENT_UPPER)
  ...
  # Replace '-' in version with '_' '-' character is  an Illegal RPM version
  # character it is illegal because it is used to separate RPM "Version" from
  # RPM "Release"
  ...
  if(NOT CPACK_RPM_PACKAGE_ARCHITECTURE)
    ...
    if(CPACK_RPM_PACKAGE_DEBUG)
      message(
        "CPackRPM:Debug: using user-specified build arch = ${CPACK_RPM_PACKAGE_ARCHITECTURE}"
      )
    endif()
  endif()
  ...
  if(CPACK_RPM_PACKAGE_COMPONENT)
    ...
    if(CPACK_RPM_PACKAGE_DEBUG)
      message(
        "CPackRPM:Debug: using component build arch = ${CPACK_RPM_PACKAGE_ARCHITECTURE}"
      )
    endif()
  endif()
  ...
  # CPACK_RPM_PACKAGE_RELEASE The RPM release is the numbering of the RPM
  # package ITSELF this is the version of the PACKAGING and NOT the version of
  # the CONTENT of the package. You may well need to generate a new RPM package
  # release without changing the version of the packaged software. This is the
  # case when the packaging is buggy (not) the software :=) If not set, 1 is a
  # good candidate
  ...
  # CPACK_RPM_PACKAGE_SOURCE The name of the source tarball in case we generate
  # a source RPM

  # CPACK_RPM_PACKAGE_DESCRIPTION The variable content may be either -
  # explicitly given by the user or - filled with the content of
  # CPACK_PACKAGE_DESCRIPTION_FILE if it is defined - set to a default value
  #
  ...
  if(CPACK_RPM_COMPRESSION_TYPE)
    if(CPACK_RPM_PACKAGE_DEBUG)
      message(
        "CPackRPM:Debug: User Specified RPM compression type: ${CPACK_RPM_COMPRESSION_TYPE}"
      )
    endif()
  ...
  if(NOT CPACK_RPM_PACKAGE_SOURCES)
    if(CPACK_PACKAGE_RELOCATABLE OR CPACK_RPM_PACKAGE_RELOCATABLE)
      ...
      if(CPACK_SET_DESTDIR AND (NOT CPACK_SET_DESTDIR STREQUAL "I_ON"))
        message(
          "CPackRPM:Warning: CPACK_SET_DESTDIR is set (=${CPACK_SET_DESTDIR}) while requesting a relocatable package (CPACK_RPM_PACKAGE_RELOCATABLE is set): this is not supported, the package won't be relocatable."
        )
        set(CPACK_RPM_PACKAGE_RELOCATABLE FALSE)
      else()
        set(CPACK_RPM_PACKAGE_PREFIX ${CPACK_PACKAGING_INSTALL_PREFIX}
        )# kept for back compatibility (provided external RPM spec files)
    ...
    if(CPACK_RPM_PACKAGE_COMPONENT)
      message(
        FATAL_ERROR
          "CPACK_RPM_PACKAGE_SOURCES parameter can not be used"
          " in combination with CPACK_RPM_PACKAGE_COMPONENT parameter!")
    endif()

    set(CPACK_RPM_PACKAGE_RELOCATABLE FALSE) # disable relocatable option if
                                             # building source RPM
  endif()
  ...
  # Check if additional fields for RPM spec header are given There may be some
  # COMPONENT specific variables as well If component specific var is not
  # provided we use the global one for each component
  foreach(
    _RPM_SPEC_HEADER
    URL
    REQUIRES
    ...
    if(DEFINED CPACK_RPM_PACKAGE_${_RPM_SPEC_HEADER})
      # Prefix can be replaced by Prefixes but the old version still works so
      # we'll ignore it for now Requires* is a special case because it gets
      # transformed to Requires(pre/post/preun/postun) Auto* is a special case
      # because the tags can not be queried by querytags rpmbuild flag
      set(special_case_tags_
          PREFIX
          REQUIRES_PRE
          REQUIRES_POST
          REQUIRES_PREUN
      ...
      if(NOT _RPM_SPEC_HEADER IN_LIST RPMBUILD_TAG_LIST
         AND NOT _RPM_SPEC_HEADER IN_LIST special_case_tags_)
        message(
          AUTHOR_WARNING "CPackRPM:Warning: ${_RPM_SPEC_HEADER} not "
    ...
    if(DEFINED CPACK_RPM_PACKAGE_${_RPM_SPEC_HEADER}_TMP)
      # Transform NAME --> Name e.g. PROVIDES --> Provides The Upper-case first
      # letter and lowercase tail is the appropriate value required in the final
      # RPM spec file.
      ...
      # The following keywords require parentheses around the "pre" or "post"
      # suffix in the final RPM spec file.
      set(SCRIPTS_REQUIREMENTS_LIST REQUIRES_PRE REQUIRES_POST REQUIRES_PREUN
                                    REQUIRES_POSTUN)
      list(FIND SCRIPTS_REQUIREMENTS_LIST ${_RPM_SPEC_HEADER}
           IS_SCRIPTS_REQUIREMENT_FOUND)
      ...
      if(CPACK_RPM_PACKAGE_DEBUG)
        message(
          "CPackRPM:Debug: User defined ${_PACKAGE_HEADER_NAME}:\n ${CPACK_RPM_PACKAGE_${_RPM_SPEC_HEADER}_TMP}"
        )
      endif()
      set(TMP_RPM_${_RPM_SPEC_HEADER}
          "${_PACKAGE_HEADER_NAME}: ${CPACK_RPM_PACKAGE_${_RPM_SPEC_HEADER}_TMP}"
      )
      unset(CPACK_RPM_PACKAGE_${_RPM_SPEC_HEADER}_TMP)
    endif()
  endforeach()

  # CPACK_RPM_SPEC_INSTALL_POST May be used to define a RPM post intallation
  # script for example setting it to "/bin/true" may prevent rpmbuild from
  # stripping binaries.
  if(CPACK_RPM_SPEC_INSTALL_POST)
    if(CPACK_RPM_PACKAGE_DEBUG)
      message(
        "CPackRPM:Debug: User defined CPACK_RPM_SPEC_INSTALL_POST = ${CPACK_RPM_SPEC_INSTALL_POST}"
      )
    endif()
    set(TMP_RPM_SPEC_INSTALL_POST
        "%define __spec_install_post ${CPACK_RPM_SPEC_INSTALL_POST}")
  endif()

  # CPACK_RPM_REQUIRES_EXCLUDE_FROM May be defined to keep the dependency
  # generator from scanning specific files or directories for deps.
  if(CPACK_RPM_REQUIRES_EXCLUDE_FROM)
    if(CPACK_RPM_PACKAGE_DEBUG)
      message(
        "CPackRPM:Debug: User defined CPACK_RPM_REQUIRES_EXCLUDE_FROM = ${CPACK_RPM_REQUIRES_EXCLUDE_FROM}"
      )
    endif()
    set(TMP_RPM_REQUIRES_EXCLUDE_FROM
        "%global __requires_exclude_from ${CPACK_RPM_REQUIRES_EXCLUDE_FROM}")
  endif()
  ...
  # CPACK_RPM_POST_TRANS_SCRIPT_FILE (or
  # CPACK_RPM_<COMPONENT>_POST_TRANS_SCRIPT_FILE) May be used to embed a post
  # installation/uninstallation/transaction script in the spec file. The
  # referred script file(s) will be read and directly put after the %post or
  # %postun or %posttrans section
  # ----------------------------------------------------------------
  ...
  # CPACK_RPM_PRE_TRANS_SCRIPT_FILE (or
  # CPACK_RPM_<COMPONENT>_PRE_TRANS_SCRIPT_FILE) May be used to embed a pre
  # installation/uninstallation/transaction script in the spec file. The
  # referred script file(s) will be read and directly put after the %pre or
  # %preun or %pretrans section
  foreach(RPM_SCRIPT_FILE_TYPE_ "INSTALL" "UNINSTALL" "TRANS")
    foreach(RPM_SCRIPT_FILE_TIME_ "PRE" "POST")
      ...
      if(CPACK_RPM_PACKAGE_COMPONENT)
        cpack_rpm_variable_fallback(
          "CPACK_RPM_${RPM_SCRIPT_FILE_TIME_}_${RPM_SCRIPT_FILE_TYPE_}_READ_FILE"
      ...
      if(CPACK_RPM_${RPM_SCRIPT_FILE_TIME_}_${RPM_SCRIPT_FILE_TYPE_}_READ_FILE)
        if(EXISTS
          ...
          message(
            "CPackRPM:Warning: CPACK_RPM_${RPM_SCRIPT_FILE_TIME_}_${RPM_SCRIPT_FILE_TYPE_}_SCRIPT_FILE <${CPACK_RPM_${RPM_SCRIPT_FILE_TIME_}_${RPM_SCRIPT_FILE_TYPE_}_READ_FILE}> does not exists - ignoring"
          )
        endif()
      else()
        # reset SPEC var value if no file has been specified (either globally or
        # component-wise)
  ...
  # CPACK_RPM_CHANGELOG_FILE May be used to embed a changelog in the spec file.
  ...
  if(CPACK_RPM_CHANGELOG_FILE)
    if(EXISTS ${CPACK_RPM_CHANGELOG_FILE})
      ...
      message(
        SEND_ERROR
          "CPackRPM:Warning: CPACK_RPM_CHANGELOG_FILE <${CPACK_RPM_CHANGELOG_FILE}> does not exists - ignoring"
      )
    endif()
  else()
  ...
  # CPACK_RPM_SPEC_MORE_DEFINE This is a generated spec rpm file spaceholder
  ...
  # it seems rpmbuild can't handle spaces in the path neither escaping (as
  # below) nor putting quotes around the path seem to help string(REGEX REPLACE
  # " " "\\\\ " CPACK_RPM_DIRECTORY "${CPACK_TOPLEVEL_DIRECTORY}")
  set(CPACK_RPM_DIRECTORY "${CPACK_TOPLEVEL_DIRECTORY}")

  cpack_rpm_prepare_content_list()

  # In component case, put CPACK_ABSOLUTE_DESTINATION_FILES_<COMPONENT> into
  # CPACK_ABSOLUTE_DESTINATION_FILES_INTERNAL otherwise, put
  # CPACK_ABSOLUTE_DESTINATION_FILES This must be done BEFORE the
  # CPACK_ABSOLUTE_DESTINATION_FILES_INTERNAL handling
  if(CPACK_RPM_PACKAGE_COMPONENT)
    if(CPACK_ABSOLUTE_DESTINATION_FILES)
      ...
      if(CPACK_RPM_PACKAGE_DEBUG)
        message(
          "CPackRPM:Debug: Handling Absolute Destination Files: <${CPACK_ABSOLUTE_DESTINATION_FILES_INTERNAL}>"
        )
  ...
  if(CPACK_RPM_PACKAGE_COMPONENT)
    ...
    if(CPACK_RPM_PACKAGE_DEBUG AND CPACK_RPM_USER_FILELIST_INTERNAL)
      message(
        "CPackRPM:Debug: Handling User Filelist: <${CPACK_RPM_USER_FILELIST_INTERNAL}>"
      )
      message("CPackRPM:Debug: in component = ${CPACK_RPM_PACKAGE_COMPONENT}")
    endif()
  ...
  # Remove those files from CPACK_ABSOLUTE_DESTINATION_FILES_INTERNAL or
  # CPACK_RPM_INSTALL_FILES, hence it must be done before these auto-generated
  # lists are processed.
  ...
  if(CPACK_ABSOLUTE_DESTINATION_FILES_INTERNAL)
    ...
    if(CPACK_RPM_PACKAGE_DEBUG)
      message(
        "CPackRPM:Debug: CPACK_RPM_ABSOLUTE_INSTALL_FILES=${CPACK_RPM_ABSOLUTE_INSTALL_FILES}"
      )
      message(
        "CPackRPM:Debug: CPACK_RPM_INSTALL_FILES=${CPACK_RPM_INSTALL_FILES}")
    endif()
  else()
  ...
  cpack_rpm_variable_fallback(
    ...
    "CPACK_RPM_DEBUGINFO_PACKAGE")
  if(CPACK_RPM_DEBUGINFO_PACKAGE OR (CPACK_RPM_DEBUGINFO_SINGLE_PACKAGE
                                     AND NOT GENERATE_SPEC_PARTS))
    cpack_rpm_variable_fallback(
    ...
    if(CPACK_RPM_DEBUGINFO_SINGLE_PACKAGE AND GENERATE_SPEC_PARTS)
      file(WRITE
           "${CPACK_RPM_ROOTDIR}/SPECS/${CPACK_RPM_PACKAGE_COMPONENT}.files"
           "${CPACK_RPM_INSTALL_FILES}")
    else()
      if(CPACK_RPM_DEBUGINFO_SINGLE_PACKAGE AND CPACK_RPM_PACKAGE_COMPONENT)
        ...
        # if there were other components/groups so we need to move files from
        # them to current component otherwise those files won't be found
        file(
          GLOB components_
          LIST_DIRECTORIES true
          RELATIVE "${CPACK_TOPLEVEL_DIRECTORY}/${CPACK_PACKAGE_FILE_NAME}"
        ...
        foreach(component_ IN LISTS components_)
          ...
          foreach(f_ IN LISTS files_for_move_)
            ...
            if(EXISTS "${src_file_}" AND EXISTS "${WDIR}/${f_}")
              execute_process(
                COMMAND ${CMAKE_COMMAND} -E compare_files "${src_file_}"
                        "${WDIR}/${f_}" RESULT_VARIABLE res_)
              if(res_)
                message(
                  FATAL_ERROR
                    "CPackRPM:Error: File on path '${WDIR}/${f_}'"
...
")
      elseif(CPACK_RPM_DEBUGINFO_SINGLE_PACKAGE)
        message(
          AUTHOR_WARNING
            "CPackRPM:Warning: debuginfo package was requested"
  ...
  if(CPACK_RPM_PACKAGE_DEBUG)
    message(
      "CPackRPM:Debug: CPACK_TOPLEVEL_DIRECTORY          = ${CPACK_TOPLEVEL_DIRECTORY}"
    )
    message(
      "CPackRPM:Debug: CPACK_TOPLEVEL_TAG                = ${CPACK_TOPLEVEL_TAG}"
    )
    message(
      "CPackRPM:Debug: CPACK_TEMPORARY_DIRECTORY         = ${CPACK_TEMPORARY_DIRECTORY}"
    )
    message(
      "CPackRPM:Debug: CPACK_OUTPUT_FILE_NAME            = ${CPACK_OUTPUT_FILE_NAME}"
    )
    message(
      "CPackRPM:Debug: CPACK_OUTPUT_FILE_PATH            = ${CPACK_OUTPUT_FILE_PATH}"
    )
    message(
      "CPackRPM:Debug: CPACK_PACKAGE_FILE_NAME           = ${CPACK_PACKAGE_FILE_NAME}"
    )
    message(
      "CPackRPM:Debug: CPACK_RPM_BINARY_SPECFILE         = ${CPACK_RPM_BINARY_SPECFILE}"
    )
    message(
      "CPackRPM:Debug: CPACK_PACKAGE_INSTALL_DIRECTORY   = ${CPACK_PACKAGE_INSTALL_DIRECTORY}"
    )
    message(
      "CPackRPM:Debug: CPACK_TEMPORARY_PACKAGE_FILE_NAME = ${CPACK_TEMPORARY_PACKAGE_FILE_NAME}"
    )
  endif()
  ...
  cpack_rpm_variable_fallback(
    "CPACK_RPM_FILE_NAME" "CPACK_RPM_${CPACK_RPM_PACKAGE_COMPONENT}_FILE_NAME"
  ...
  if(NOT CPACK_RPM_FILE_NAME STREQUAL "RPM-DEFAULT")
    if(CPACK_RPM_FILE_NAME)
      if(NOT CPACK_RPM_FILE_NAME MATCHES ".*\\.rpm")
        message(
          FATAL_ERROR
            "'${CPACK_RPM_FILE_NAME}' is not a valid RPM package file name as it must end with '.rpm'!"
        )
      endif()
    else()
      ...
      if(CPACK_RPM_MAIN_COMPONENT_UPPER STREQUAL
         CPACK_RPM_PACKAGE_COMPONENT_UPPER)
    ...
    # else example: set(CPACK_RPM_FILE_NAME
    # "${CPACK_RPM_PACKAGE_NAME}-${CPACK_RPM_PACKAGE_VERSION}-${CPACK_RPM_PACKAGE_RELEASE}-${CPACK_RPM_PACKAGE_ARCHITECTURE}.rpm")

    if(CPACK_RPM_DEBUGINFO_SINGLE_PACKAGE AND GENERATE_SPEC_PARTS)
      string(
        TOLOWER "${CPACK_RPM_PACKAGE_NAME}-${CPACK_PACKAGE_VERSION}.*\\.rpm"
                expected_filename_)

      file(WRITE
           "${CPACK_RPM_ROOTDIR}/SPECS/${CPACK_RPM_PACKAGE_COMPONENT}.rpm_name"
  ...
  if(CPACK_RPM_PACKAGE_SOURCES) # source rpm
    ...
    execute_process(
      COMMAND
        ...
        "${CPACK_RPM_DIRECTORY}/SOURCES/${archive_name_}.tar.gz"
    ...
    # releasing control to cpack to generate binary packages. Note however that
    # this doesn't prevent cpack to generate debuginfo packages when run from
    # srpm with --rebuild.
...
mv *.rpm %_rpmdir")
    ...
    file(
      WRITE ${CPACK_RPM_BINARY_SPECFILE}.in
...
")
    ...
    # We should generate a USER spec file template: - either because the user
    # asked for it : CPACK_RPM_GENERATE_USER_BINARY_SPECFILE_TEMPLATE - or the
    # user did not provide one : NOT CPACK_RPM_USER_BINARY_SPECFILE
    set(RPMBUILD_FLAGS "-bb")
    if(CPACK_RPM_GENERATE_USER_BINARY_SPECFILE_TEMPLATE
       OR NOT CPACK_RPM_USER_BINARY_SPECFILE)
...
")
    ...
    # Stop here if we were asked to only generate a template USER spec file The
    # generated file may then be used as a template by user who wants to
    # customize their own spec file.
    if(CPACK_RPM_GENERATE_USER_BINARY_SPECFILE_TEMPLATE)
      message(
        FATAL_ERROR
          "CPackRPM: STOP here Generated USER binary spec file template is: ${CPACK_RPM_BINARY_SPECFILE}.in"
      )
    endif()
  endif()

  # After that we may either use a user provided spec file or generate one using
  # appropriate variables value.
  if(CPACK_RPM_USER_BINARY_SPECFILE)
    # User may have specified SPECFILE just use it
    message(
      "CPackRPM: Will use USER specified spec file: ${CPACK_RPM_USER_BINARY_SPECFILE}"
    )
    ...
    message(
      "CPackRPM: Will use GENERATED spec file: ${CPACK_RPM_BINARY_SPECFILE}")
  ...
  if(NOT GENERATE_SPEC_PARTS) # generate package
    if(RPMBUILD_EXECUTABLE)
      ...
      execute_process(
        COMMAND
          "${RPMBUILD_EXECUTABLE}" ${RPMBUILD_FLAGS} --define
          "_topdir ${CPACK_RPM_DIRECTORY}" --buildroot
      ...
      if(CPACK_RPM_PACKAGE_DEBUG OR CPACK_RPMBUILD_EXEC_RESULT)
        ...
        message(
          "CPackRPM:Debug:    - ${CPACK_TOPLEVEL_DIRECTORY}/rpmbuild${CPACK_RPM_PACKAGE_NAME}.err"
        )
        message("CPackRPM:Debug: *** ${RPMBUILDERR} ***")
        message(
          "CPackRPM:Debug:    - ${CPACK_TOPLEVEL_DIRECTORY}/rpmbuild${CPACK_RPM_PACKAGE_NAME}.out"
        )
    ...
    if(NOT GENERATED_FILES)
      message(
        FATAL_ERROR "RPM package was not generated! ${CPACK_RPM_DIRECTORY}")
    endif()
    ...
    if(CPACK_RPM_DEBUGINFO_PACKAGE)
      ...
      if(CPACK_RPM_DEBUGINFO_FILE_NAME AND NOT CPACK_RPM_DEBUGINFO_FILE_NAME
                                           STREQUAL "RPM-DEFAULT")
        list(
          APPEND expected_filenames_
