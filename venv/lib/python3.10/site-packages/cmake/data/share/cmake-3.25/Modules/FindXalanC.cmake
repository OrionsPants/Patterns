...
function(_XalanC_GET_VERSION version_hdr)
  ...
  if(_contents)
    string(REGEX REPLACE "[^*]*#define XALAN_VERSION_MAJOR[ \t(]+([0-9]+).*"
                         "\\1" XalanC_MAJOR "${_contents}")
    string(REGEX REPLACE "[^*]*#define XALAN_VERSION_MINOR[ \t(]+([0-9]+).*"
                         "\\1" XalanC_MINOR "${_contents}")
    string(REGEX REPLACE "[^*]*#define XALAN_VERSION_REVISION[ \t(]+([0-9]+).*"
                         "\\1" XalanC_PATCH "${_contents}")
    ...
    set(XalanC_VERSION
        "${XalanC_MAJOR}.${XalanC_MINOR}.${XalanC_PATCH}"
        PARENT_SCOPE)
    set(XalanC_VERSION_MAJOR
        "${XalanC_MAJOR}"
        PARENT_SCOPE)
    set(XalanC_VERSION_MINOR
        "${XalanC_MINOR}"
        PARENT_SCOPE)
    set(XalanC_VERSION_PATCH
        "${XalanC_PATCH}"
        PARENT_SCOPE)
  else()
    message(
      FATAL_ERROR
        "Include file ${version_hdr} does not exist or does not contain expected version information"
    )
  endif()
endfunction()
...
if(XalanC_INCLUDE_DIR
   AND EXISTS "${XalanC_INCLUDE_DIR}/xalanc/Include/XalanVersion.hpp")
...
if(NOT XalanC_LIBRARY)
  ...
  find_library(
    ...
    NAMES "Xalan-C" "xalan-c" "Xalan-C_${XalanC_VERSION_MAJOR}"
          "Xalan-C_${XalanC_VERSION_MAJOR}_${XalanC_VERSION_MINOR}"
    DOC "Xalan-C++ libraries (release)")
  find_library(
    XalanC_LIBRARY_DEBUG
    NAMES "Xalan-CD" "xalan-cd" "Xalan-C_${XalanC_VERSION_MAJOR}D"
...
find_package_handle_standard_args(
  ...
  REQUIRED_VARS XalanC_LIBRARY XalanC_INCLUDE_DIR XalanC_VERSION XercesC_FOUND
  ...
  if(NOT TARGET XalanC::XalanC)
    ...
    if(XalanC_INCLUDE_DIRS)
      set_target_properties(
        XalanC::XalanC PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                  "${XalanC_INCLUDE_DIRS}")
    endif()
    if(EXISTS "${XalanC_LIBRARY}")
      set_target_properties(
        XalanC::XalanC PROPERTIES IMPORTED_LINK_INTERFACE_LANGUAGES "CXX"
                                  IMPORTED_LOCATION "${XalanC_LIBRARY}")
    endif()
    if(EXISTS "${XalanC_LIBRARY_RELEASE}")
      set_property(
        TARGET XalanC::XalanC
        APPEND
