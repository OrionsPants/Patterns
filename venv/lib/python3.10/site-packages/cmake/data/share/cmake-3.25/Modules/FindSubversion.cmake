...
if(Subversion_SVN_EXECUTABLE)
  # the subversion commands should be executed with the C locale, otherwise the
  # message (which are parsed) may be translated, Alex
  ...
  macro(Subversion_WC_INFO dir prefix)
    ...
    cmake_parse_arguments("Subversion_WC_INFO" "IGNORE_SVN_FAILURE" "" ""
                          ${ARGN})
    ...
    if(${Subversion_svn_info_result} EQUAL 0)
      string(REGEX REPLACE "^(.*\n)?URL: ([^\n]+).*" "\\2" ${prefix}_WC_URL
                           "${${prefix}_WC_INFO}")
      string(REGEX REPLACE "^(.*\n)?Repository Root: ([^\n]+).*" "\\2"
                           ${prefix}_WC_ROOT "${${prefix}_WC_INFO}")
      string(REGEX REPLACE "^(.*\n)?Revision: ([^\n]+).*" "\\2"
    ...
    elseif(NOT Subversion_WC_INFO_IGNORE_SVN_FAILURE)
      message(
        SEND_ERROR
          "Command \"${Subversion_SVN_EXECUTABLE} info ${dir}\" failed with output:\n${Subversion_svn_info_error}"
      )
    endif()
  ...
  macro(Subversion_WC_LOG dir prefix)
    # This macro can block if the certificate is not signed: svn ask you to
    # accept the certificate and wait for your answer This macro requires a svn
    # server network access (Internet most of the time) and can also be slow
    # since it access the svn server
    execute_process(
