...
function(_ep_parse_arguments f keywords name ns args)
  ...
  # upper-case letter followed by at least two more upper-case letters, numbers
  ...
  foreach(arg IN LISTS args)
    ...
    if(arg MATCHES "^[A-Z][A-Z0-9_][A-Z0-9_]+$"
       AND NOT (("x${arg}x" STREQUAL "x${key}x") AND ("x${key}x" STREQUAL
                                                      "xCOMMANDx"))
       AND NOT arg MATCHES "^(TRUE|FALSE)$")
    ...
    if(is_value)
      if(key)
        ...
        # Missing Keyword
        message(
          AUTHOR_WARNING "value '${arg}' with no previous keyword in ${f}")
      endif()
    else()
      set(key "${arg}")
    endif()
...
function(
  ...
  if(NOT GIT_VERSION_STRING VERSION_LESS 1.8.5)
    ...
    # Use `git checkout <branch>` even though this risks ambiguity with a local
    # path.  Unfortunately we cannot use `git checkout <tree-ish> --` because
    # that will not search for remote branch names, a common use case.
    set(git_checkout_explicit-- "")
  endif()
  if("${git_tag}" STREQUAL "")
  ...
  if(GIT_VERSION_STRING VERSION_LESS 2.20 OR 2.21 VERSION_LESS_EQUAL
                                             GIT_VERSION_STRING)
...
function(
  ...
  # now check for curl locals so that the local values will override the globals
  ...
  if(NOT http_headers STREQUAL "")
    foreach(header ${http_headers})
      string(PREPEND HTTP_HEADERS_ARGS "HTTPHEADER \"${header}\"\n        ")
    endforeach()
  endif()

  # Used variables: * TLS_VERIFY_CODE * TLS_CAINFO_CODE * ALGO * EXPECT_VALUE *
  # REMOTE * LOCAL * SHOW_PROGRESS * TIMEOUT_ARGS * TIMEOUT_MSG * USERPWD_ARGS *
  # HTTP_HEADERS_ARGS
  configure_file(
...
function(_ep_write_verifyfile_script script_filename LOCAL hash)
  ...
  # Used variables: * ALGO * EXPECT_VALUE * LOCAL
  configure_file(
...
function(_ep_set_directories name)
  ...
  get_property(
    log_dir
    TARGET ${name}
    PROPERTY _EP_LOG_DIR)
  if(NOT log_dir)
    get_property(
      log_dir
      TARGET ${name}
      PROPERTY _EP_STAMP_DIR)
  endif()
  ...
  elseif(IS_ABSOLUTE "${source_subdir}")
    message(
      FATAL_ERROR "External project ${name} has non-relative SOURCE_SUBDIR!")
  else()
  ...
  if(build_in_source)
    ...
    if(source_subdir)
      set_property(TARGET ${name} PROPERTY _EP_BINARY_DIR
  ...
  # directories now at configure time and ensure they exist again at build time
  # (since somebody might remove one of the required directories and try to
  # rebuild without re-running cmake). They need to exist now at makefile
...
# IMPORTANT: this MUST be a macro and not a function because of the in-place
# replacements that occur in each ${var}
#
...
function(_ep_write_initial_cache target_name script_filename
         script_initial_cache)
...
function(_ep_get_build_command name step cmd_var)
    ...
    if("${CMAKE_GENERATOR}" MATCHES "Make" AND ("${cmake_generator}" MATCHES
      ...
      if(_isMultiConfig)
        if(CMAKE_CFG_INTDIR
           ...
           AND NOT CMAKE_CFG_INTDIR MATCHES "\\$")
  ...
  if(NOT "${args}" STREQUAL "")
    # args could have empty items, so we must quote it to prevent them from
...
function(_ep_write_log_script name step cmd_var)
  ...
  if(command MATCHES "^\\$\\(MAKE\\)")
    # GNU make recognizes the string "$(MAKE)" as recursive make, so ensure that
    # it appears directly in the makefile.
...
")
  ...
  # Wrap multiple 'COMMAND' lines up into a second-level wrapper script so all
  # output can be sent to one log file.
...
")
    ...
    file(
      ...
      CONTENT "${code}")
    set(command ${CMAKE_COMMAND} "-Dmake=\${make}" "-Dconfig=\${config}" -P
                ${stamp_dir}/${name}-${step}-$<CONFIG>-impl.cmake)
  endif()
...
# On multi-config generators, provide a placeholder for a per-config subdir. On
# single-config generators, this is empty.
function(_ep_get_configuration_subdir_genex suffix_var)
  set(suffix "")
  get_property(_isMultiConfig GLOBAL PROPERTY GENERATOR_IS_MULTI_CONFIG)
  if(_isMultiConfig)
    if(CMAKE_GENERATOR STREQUAL "Xcode")
      # The Xcode generator does not support per-config sources, so use the
      # underlying build system's placeholder instead.
...
function(_ep_step_add_target name step no_deps)
  ...
  get_property(
    cmp0114
    TARGET ${name}
    PROPERTY _EP_CMP0114)
  _ep_get_step_stampfile(${name} ${step} stamp_file)
  cmake_policy(PUSH)
  if(cmp0114 STREQUAL "NEW")
    # To implement CMP0114 NEW behavior with Makefile generators, we need
    # CMP0113 NEW behavior.
  ...
  set_target_properties(
    ${name}-${step}
    PROPERTIES _EP_IS_EXTERNAL_PROJECT_STEP 1
               LABELS "${name}"
...
function(_ep_step_add_target_dependencies name step node)
  ...
  foreach(dependee IN LISTS dependees)
    ...
    if(exclude_from_main OR dependee_dependers MATCHES ";")
      # The step on which our step target depends itself has dependents in
      # multiple targes.  It needs a step target too so that there is a unique
      # place for its custom command.
...
function(ExternalProject_Add_Step name step)
  ...
  set(keywords
      ...
      LOG
      USES_TERMINAL)
  _ep_parse_arguments(ExternalProject_Add_Step "${keywords}" ${name}
                      _EP_${step}_ "${ARGN}")
  ...
  if(independent STREQUAL "")
    ...
    set_property(TARGET ${name} PROPERTY _EP_${step}_INDEPENDENT
                                         "${independent}")
  endif()
  ...
  get_property(
    dependers
    TARGET ${name}
    PROPERTY _EP_${step}_DEPENDERS)
  set_property(
    TARGET ${name}
    APPEND
    PROPERTY _EP_${step}_INTERNAL_DEPENDERS ${dependers})
  foreach(depender IN LISTS dependers)
    set_property(
      TARGET ${name}
      APPEND
      PROPERTY _EP_${depender}_INTERNAL_DEPENDEES ${step})
  ...
  get_property(
    depends
    TARGET ${name}
    PROPERTY _EP_${step}_DEPENDS)

  # Byproducts of the step.
  get_property(
    byproducts
    TARGET ${name}
    PROPERTY _EP_${step}_BYPRODUCTS)

  # Dependencies on steps.
  get_property(
    dependees
    TARGET ${name}
    PROPERTY _EP_${step}_DEPENDEES)
  set_property(
    TARGET ${name}
    APPEND
    PROPERTY _EP_${step}_INTERNAL_DEPENDEES ${dependees})
  foreach(dependee IN LISTS dependees)
    set_property(
      TARGET ${name}
      APPEND
      PROPERTY _EP_${dependee}_INTERNAL_DEPENDERS ${step})
  ...
  if(always)
    ...
    if(_isMultiConfig)
      ...
      foreach(cfg ${CMAKE_CONFIGURATION_TYPES})
        string(REPLACE "${cfgdir}" "/${cfg}" stamp_file_config "${stamp_file}")
  ...
  get_property(
    log
    TARGET ${name}
    PROPERTY _EP_${step}_LOG)
  if(command AND log)
    _ep_write_log_script(${name} ${step} command)
  endif()

  if("${command}" STREQUAL "")
    # Some generators (i.e. Xcode) will not generate a file level target if no
    # command is set, and therefore the dependencies on this target will be
    # broken. The empty command is replaced by an echo command here in order to
    # avoid this issue.
  ...
  get_property(
    ...
    TARGET ${name}
    PROPERTY _EP_INDEPENDENT_STEP_TARGETS)
    ...
    if(independent_step_targets AND cmp0114 STREQUAL "")
      ...
      if(NOT warned)
        set_property(TARGET ${name}
                     PROPERTY _EP_CMP0114_WARNED_INDEPENDENT_STEP_TARGETS 1)
        cmake_policy(GET_WARNING CMP0114 _cmp0114_warning)
        string(
          APPEND
          _cmp0114_warning
...
function(ExternalProject_Add_StepDependencies name step)
  ...
  if(NOT type STREQUAL "UTILITY")
    message(
      FATAL_ERROR "Target \"${name}\" was not generated by ExternalProject_Add."
    )
  endif()
  ...
  if(TARGET ${name}-${step})
    get_property(
      ...
      PROPERTY TYPE)
    if(NOT type STREQUAL "UTILITY")
      message(FATAL_ERROR "Target \"${name}-${step}\" was not generated by "
  ...
  # Always add file-level dependency, but add target-level dependency only if
  # the target exists for that step.
  _ep_get_step_stampfile(${name} ${step} stamp_file)
...
function(_ep_add_download_command name)
  ExternalProject_Get_Property(${name} source_dir stamp_dir download_dir
                               tmp_dir)

  get_property(
    cmd_set
    TARGET ${name}
    PROPERTY _EP_DOWNLOAD_COMMAND
    SET)
  get_property(
    cmd
    TARGET ${name}
  ...
  get_property(
    cvs_repository
    TARGET ${name}
    PROPERTY _EP_CVS_REPOSITORY)
  get_property(
    svn_repository
    TARGET ${name}
    PROPERTY _EP_SVN_REPOSITORY)
  get_property(
    git_repository
    TARGET ${name}
    PROPERTY _EP_GIT_REPOSITORY)
  get_property(
    hg_repository
    TARGET ${name}
    PROPERTY _EP_HG_REPOSITORY)
  get_property(
    url
    TARGET ${name}
    PROPERTY _EP_URL)
  ...
  elseif(svn_repository)
    ...
    get_property(
      svn_revision
      TARGET ${name}
      PROPERTY _EP_SVN_REVISION)
    get_property(
      svn_username
      TARGET ${name}
      PROPERTY _EP_SVN_USERNAME)
    get_property(
      svn_password
      TARGET ${name}
      PROPERTY _EP_SVN_PASSWORD)
    get_property(
      svn_trust_cert
      TARGET ${name}
      PROPERTY _EP_SVN_TRUST_CERT)
    get_property(
      uses_terminal
      TARGET ${name}
  ...
  elseif(git_repository)
    ...
    get_property(
      git_shallow
      TARGET ${name}
      PROPERTY _EP_GIT_SHALLOW)
    get_property(
      git_progress
      TARGET ${name}
      PROPERTY _EP_GIT_PROGRESS)
    get_property(
      git_config
      TARGET ${name}
      PROPERTY _EP_GIT_CONFIG)
    ...
    # If the repo changes, the clone script should be run again. But if only the
    # tag changes, avoid running the clone script again. Let the 'always'
    # running update step checkout the new tag.
    #
    set(extra_repo_info
...
")
    ...
    # Since git clone doesn't succeed if the non-empty source_dir exists, create
    # a cmake script to invoke as download command. The script will delete the
    # source directory and then call git clone.
    #
    _ep_write_gitclone_script(
  ...
  elseif(hg_repository)
    ...
    # Since hg clone doesn't succeed if the non-empty source_dir exists, create
    # a cmake script to invoke as download command. The script will delete the
    # source directory and then call hg clone.
    #
    _ep_write_hgclone_script(
    ...
    if(IS_DIRECTORY "${url}")
      ...
      if("${url}" MATCHES "^[a-z]+://")
        ...
        _ep_write_downloadfile_script(
          ...
          "${netrc}"
          "${netrc_file}")
        set(cmd ${CMAKE_COMMAND} -P "${download_script}" COMMAND)
        if(no_extract)
        ...
        set(comment "Performing download step (${steps}) for '${name}'")
        _ep_write_verifyfile_script("${stamp_dir}/verify-${name}.cmake"
                                    "${file}" "${hash}")
      endif()
      ...
      if(no_extract)
        if(NOT extract_timestamp STREQUAL "extract_timestamp-NOTFOUND")
          message(
            FATAL_ERROR "Cannot specify DOWNLOAD_EXTRACT_TIMESTAMP when using "
        ...
        _ep_write_extractfile_script(
          "${stamp_dir}/extract-${name}.cmake" "${name}" "${file}"
          "${source_dir}" "${options}")
        list(APPEND cmd COMMAND ${CMAKE_COMMAND} -P
...
")
    ...
    set(cmd ${HG_EXECUTABLE} pull COMMAND ${HG_EXECUTABLE} update ${hg_tag})
    set(always 1)
  endif()
...
function(_ep_extract_configure_command var name)
  ...
  if(cmd_set)
    ...
    # If there are any CMAKE_CACHE_ARGS or CMAKE_CACHE_DEFAULT_ARGS, write an
    # initial cache and use it
    get_property(
      cmake_cache_args
      TARGET ${name}
    ...
    if(cmake_generator)
      ...
      if(cmake_generator_platform)
        message(
          FATAL_ERROR "Option CMAKE_GENERATOR_PLATFORM not allowed without "
                      "CMAKE_GENERATOR.")
      endif()
      if(CMAKE_GENERATOR_PLATFORM)
        list(APPEND cmd "-A${CMAKE_GENERATOR_PLATFORM}")
...
function(_ep_add_configure_command name)
  ...
  # used, cmake args or cmake generator) then re-run the configure step. Fixes
  # issue https://gitlab.kitware.com/cmake/cmake/-/issues/10258
  #
  configure_file(
    ${CMAKE_CURRENT_FUNCTION_LIST_DIR}/ExternalProject/cfgcmd.txt.in
    ${tmp_dir}/${name}-cfgcmd.txt @ONLY)
...
function(_ep_add_install_command name)
  ...
  # With BUILD_ALWAYS+BUILD_BYPRODUCTS, Ninja restats the build step outputs and
  # may not consider this step to be out-of-date.  Explicitly mark it
  # out-of-date too.
  get_property(
    build_always
    TARGET ${name}
...
function(_ep_add_test_command name)
  ...
  get_property(
    before
    TARGET ${name}
    PROPERTY _EP_TEST_BEFORE_INSTALL)
  get_property(
    after
    TARGET ${name}
    PROPERTY _EP_TEST_AFTER_INSTALL)
  get_property(
    exclude
    TARGET ${name}
    PROPERTY _EP_TEST_EXCLUDE_FROM_MAIN)
  get_property(
    cmd_set
    TARGET ${name}
...
function(ExternalProject_Add name)
  cmake_policy(
    GET CMP0097 _EP_CMP0097 PARENT_SCOPE # undocumented, do not use outside of
                                         # CMake
  )
  cmake_policy(
    GET CMP0114 cmp0114
  ...
  if(CMAKE_XCODE_BUILD_SYSTEM VERSION_GREATER_EQUAL 12 AND NOT cmp0114 STREQUAL
                                                           "NEW")
    message(
      AUTHOR_WARNING
  ...
  cmake_policy(
    GET CMP0135 _EP_CMP0135 PARENT_SCOPE # undocumented, do not use outside of
                                         # CMake
  )
  ...
  # EXCLUDE_FROM_ALL target property. Later, if the EXCLUDE_FROM_ALL argument
  # was passed, we explicitly set it for the target.
  add_custom_target(${name} ALL DEPENDS ${complete_stamp_file})
  cmake_policy(POP)
  set_target_properties(
    ${name}
    PROPERTIES _EP_IS_EXTERNAL_PROJECT 1
               LABELS ${name}
               FOLDER "ExternalProjectTargets/${name}"
               _EP_CMP0114 "${cmp0114}")
  ...
  # The 'complete' step depends on all other steps and creates a 'done' mark.  A
  # dependent external project's 'configure' step depends on the 'done' mark so
  # that it rebuilds when this project rebuilds.  It is important that 'done' is
  # not the output of any custom command so that CMake does not propagate build
  # rules to other external project targets, which may cause problems during
  # parallel builds.  However, the Ninja generator needs to see the entire
  # dependency graph, and can cope with custom commands belonging to multiple
  # targets, so we add the 'done' mark as an output for Ninja only.
  ...
  # Set up custom build steps based on the target properties. Each step depends
  # on the previous one.
  #
  # The target depends on the output of the final step. (Already set up above in
  # the DEPENDS of the add_custom_target command.)
  #
  ...
  # Test is special in that it might depend on build, or it might depend on
  # install.
  #
