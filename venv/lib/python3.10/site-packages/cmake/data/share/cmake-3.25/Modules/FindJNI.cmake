...
if(NOT JNI_FIND_COMPONENTS)
  ...
  if(ANDROID
     AND CMAKE_SYSTEM_VERSION GREATER_EQUAL 31
     AND JVM IN_LIST JNI_FIND_COMPONENTS)
...
macro(java_append_library_directories _var)
  # Determine java arch-specific library subdir Mostly based on
  # openjdk/jdk/make/common/shared/Platform.gmk as of openjdk 1.6.0_18 + icedtea
  # patches. However, it would be much better to base the guess on the first
  # part of the GNU config.guess platform triplet.
  ...
  elseif(CMAKE_SYSTEM_PROCESSOR MATCHES "^mips")
    # mips* machines are bi-endian mostly so processor does not tell endianness
  ...
  # Sometimes ${CMAKE_SYSTEM_PROCESSOR} is added to the list to prefer current
  # value to a hardcoded list. Remove possible duplicates.
...
if(_JAVA_HOME)
  java_append_library_directories(
    JAVA_AWT_LIBRARY_DIRECTORIES ${_JAVA_HOME}/jre/lib/{libarch}
    ${_JAVA_HOME}/jre/lib ${_JAVA_HOME}/lib/{libarch} ${_JAVA_HOME}/lib
    ${_JAVA_HOME})
endif()
...
if(WIN32)
  ...
  macro(_JNI_GET_INSTALLED_VERSIONS _KIND)
    execute_process(
      COMMAND REG QUERY "HKLM\\SOFTWARE\\JavaSoft\\${_KIND}"
    ...
    if(NOT _JAVA_RESULT)
      string(
        REGEX
          MATCHALL
          "HKEY_LOCAL_MACHINE\\\\SOFTWARE\\\\JavaSoft\\\\${_KIND}\\\\[0-9._]+"
          _JNI_VERSIONS "${_JAVA_VERSIONS}")
      string(
        REGEX
        REPLACE
          "HKEY_LOCAL_MACHINE\\\\SOFTWARE\\\\JavaSoft\\\\${_KIND}\\\\([0-9._]+)"
          "\\1" _JNI_VERSIONS "${_JNI_VERSIONS}")
      if(_JNI_VERSIONS)
        ...
        foreach(_JNI_VERSION IN LISTS _JNI_VERSIONS)
          ...
          if(JNI_FIND_VERSION_EXACT AND NOT _JNI_CMAKE_VERSION MATCHES
                                        "^${JNI_FIND_VERSION}")
            continue()
          endif()
          if(DEFINED JNI_FIND_VERSION AND _JNI_CMAKE_VERSION VERSION_LESS
                                          JNI_FIND_VERSION)
            break()
          endif()
          list(
            APPEND
            _JNI_HINTS
            "[HKEY_LOCAL_MACHINE\\SOFTWARE\\JavaSoft\\${_KIND}\\${_JNI_VERSION};JavaHome]"
          )
        endforeach()
      endif()
    endif()
...
set(_JNI_JAVA_DIRECTORIES_BASE
    ...
    /usr/lib/jvm/java-6-sun-1.6.0.00 # can this one be removed according to
                                     # #8821 ? Alex
...
foreach(_java_dir IN LISTS _JNI_JAVA_DIRECTORIES_BASE)
  list(
    ...
    ${_java_dir}/lib
    ${_java_dir})
  list(APPEND _JNI_JAVA_INCLUDE_TRIES ${_java_dir}/include)
endforeach()
...
set(_JNI_NORMAL_JVM NAMES jvm PATHS ${JAVA_JVM_LIBRARY_DIRECTORIES})

set(_JNI_NORMAL_JAWT NAMES jawt PATHS ${JAVA_AWT_LIBRARY_DIRECTORIES})
...
if("${JAVA_JVM_LIBRARY};${JAVA_AWT_LIBRARY};" MATCHES
   "(/JavaVM.framework|-framework JavaVM);")
...
if(JAVA_NativeHelper_LIBRARY)
  ...
  if(NOT JAVA_JVM_LIBRARY)
    set(JAVA_JVM_LIBRARY
        "${JAVA_NativeHelper_LIBRARY}"
        CACHE FILEPATH "Alias to nativehelper library" FORCE)
...
if(ANDROID)
  ...
  if(EXISTS ${JAVA_SOURCE_PROPERTIES_FILE})
    ...
    string(
      REGEX
      REPLACE ".*Pkg\\.Revision = (([0-9]+)\\.([0-9]+)\\.([0-9]+)([^\n]+)?).*"
              "\\1" JNI_VERSION "${NDK_VERSION_CONTENTS}")
...
find_package_handle_standard_args(
  JNI
  REQUIRED_VARS ${JNI_REQUIRED_VARS} ${JNI_FPHSA_ARGS}
  HANDLE_COMPONENTS)

mark_as_advanced(JAVA_AWT_LIBRARY JAVA_JVM_LIBRARY JAVA_AWT_INCLUDE_PATH
...
if(JNI_FOUND)
  ...
  if(NOT JNI_INCLUDE_PATH2_OPTIONAL AND JAVA_INCLUDE_PATH2)
    set_property(
      TARGET JNI::JNI
      APPEND
      PROPERTY INTERFACE_INCLUDE_DIRECTORIES ${JAVA_INCLUDE_PATH2})
  endif()
  ...
  if(JNI_AWT_FOUND)
    ...
    set_property(TARGET JNI::AWT PROPERTY INTERFACE_INCLUDE_DIRECTORIES
                                          ${JAVA_AWT_INCLUDE_PATH})
    set_property(TARGET JNI::AWT PROPERTY IMPORTED_LOCATION ${JAVA_AWT_LIBRARY})
  ...
  if(JNI_JVM_FOUND OR JNI_NativeHelper_FOUND)
    ...
    if(NOT TARGET JNI::JVM)
      if(JAVA_JVM_LIBRARY AND NOT JAVA_JVM_LIBRARY STREQUAL
                              JAVA_NativeHelper_LIBRARY)
    ...
    if(NOT _JNI_JVM_TYPE STREQUAL "INTERFACE_LIBRARY")
      ...
      set_property(
        TARGET JNI::JVM
        APPEND
        PROPERTY INTERFACE_LINK_LIBRARIES JNI::NativeHelper)
    endif()
