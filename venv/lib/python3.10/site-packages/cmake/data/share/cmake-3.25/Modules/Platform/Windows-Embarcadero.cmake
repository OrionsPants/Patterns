...
if("${CMAKE_${_lang}_COMPILER_VERSION}" VERSION_LESS 6.30)
  ...
  set(_tR "-tWR -tW-") # -tWR Use DLL runtime (implies -D_RTLDLL, and '-tW'
                       # too!!)
  # Notes: - The flags affect linking so we pass them to the linker. - The flags
  # affect preprocessing so we pass them to the compiler. - Since '-tWR' implies
  # '-tW' we use '-tWR -tW-' instead. - Since '-tW-' disables '-tWD' we use
  # '-tWR -tW- -tWD' for DLLs.
else()
  set(EMBARCADERO 1)
...
foreach(t EXE SHARED MODULE)
  string(APPEND CMAKE_${t}_LINKER_FLAGS_INIT
         " ${_tM} -lS:1048576 -lSc:4098 -lH:1048576 -lHc:8192 ")
  string(APPEND CMAKE_${t}_LINKER_FLAGS_DEBUG_INIT " -v")
  string(APPEND CMAKE_${t}_LINKER_FLAGS_RELWITHDEBINFO_INIT " -v")
endforeach()

# The Borland link tool does not support multiple concurrent invocations within
...
macro(__embarcadero_language lang)
  ...
  # compile a source file into an object file place <DEFINES> outside the
  # response file because Borland refuses to parse quotes from the response
  # file.
  ...
  # place <DEFINES> outside the response file because Borland refuses to parse
  # quotes from the response file.
  ...
  # Create a shared library. First create a module and then its import library.
