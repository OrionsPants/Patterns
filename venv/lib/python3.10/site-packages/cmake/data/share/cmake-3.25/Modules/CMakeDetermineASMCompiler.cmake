...
if(NOT CMAKE_ASM${ASM_DIALECT}_COMPILER)
  ...
  if(NOT $ENV{ASM${ASM_DIALECT}} STREQUAL "")
    get_filename_component(
      CMAKE_ASM${ASM_DIALECT}_COMPILER_INIT $ENV{ASM${ASM_DIALECT}} PROGRAM
      PROGRAM_ARGS CMAKE_ASM${ASM_DIALECT}_FLAGS_ENV_INIT)
    if(CMAKE_ASM${ASM_DIALECT}_FLAGS_ENV_INIT)
      set(CMAKE_ASM${ASM_DIALECT}_COMPILER_ARG1
          "${CMAKE_ASM${ASM_DIALECT}_FLAGS_ENV_INIT}"
          CACHE STRING "Arguments to ASM${ASM_DIALECT} compiler")
    endif()
    if(NOT EXISTS ${CMAKE_ASM${ASM_DIALECT}_COMPILER_INIT})
      message(
        FATAL_ERROR
          "Could not find compiler set in environment variable ASM${ASM_DIALECT}:\n$ENV{ASM${ASM_DIALECT}}."
      )
    endif()
  endif()
  ...
  if("ASM${ASM_DIALECT}" STREQUAL "ASM") # the generic assembler support
    ...
    if(NOT CMAKE_ASM${ASM_DIALECT}_COMPILER_INIT
       AND NOT CMAKE_ASM${ASM_DIALECT}_COMPILER_LIST)
      message(
        FATAL_ERROR
          "CMAKE_ASM${ASM_DIALECT}_COMPILER_INIT or CMAKE_ASM${ASM_DIALECT}_COMPILER_LIST must be preset !"
      )
    endif()
  endif()
...
if(NOT _CMAKE_TOOLCHAIN_LOCATION)
  get_filename_component(_CMAKE_TOOLCHAIN_LOCATION
...
if(NOT CMAKE_ASM${ASM_DIALECT}_COMPILER_ID)
  ...
  set(CMAKE_ASM${ASM_DIALECT}_COMPILER_ID_VENDOR_REGEX_GNU
  ...
  set(CMAKE_ASM${ASM_DIALECT}_COMPILER_ID_VENDOR_REGEX_AppleClang
      "(Apple LLVM version)")
  ...
  set(CMAKE_ASM${ASM_DIALECT}_COMPILER_ID_VENDOR_REGEX_IntelLLVM
      "(Intel[^\n]+oneAPI)")
  ...
  set(CMAKE_ASM${ASM_DIALECT}_COMPILER_ID_VENDOR_REGEX_ARMCC
      "(ARM Compiler)|(ARM Assembler)|(Arm Compiler)")
  ...
  if("x${CMAKE_ASM${ASM_DIALECT}_COMPILER_ID}" STREQUAL "xIAR")
    # primary necessary to detect architecture, so the right archiver and linker
    # can be picked eg. "IAR Assembler V8.10.1.12857/W32 for ARM" or "IAR
    # Assembler V4.11.1.4666 for Renesas RX" Earlier versions did not provide
    # `--version`, so grep the full output to extract Assembler ID string
    ...
    if("${_compileid}" MATCHES
       "for.*(MSP430|8051|ARM|AVR|RH850|RISC-?V|RL78|RX|STM8|V850)")
  ...
  elseif("x${CMAKE_ASM${ASM_DIALECT}_COMPILER_ID}" STREQUAL "xClang")
    ...
    execute_process(
      COMMAND ${CMAKE_ASM${ASM_DIALECT}_COMPILER} -?
    ...
    set(_variant
        " with ${CMAKE_ASM${ASM_DIALECT}_COMPILER_FRONTEND_VARIANT}-like command-line"
    )
  endif()
...
if(CMAKE_ASM${ASM_DIALECT}_COMPILER_ID)
  ...
  if(CMAKE_ASM${ASM_DIALECT}_COMPILER_ARCHITECTURE_ID
     AND "x${CMAKE_ASM${ASM_DIALECT}_COMPILER_ID}" STREQUAL "xIAR")
  ...
  message(
    STATUS
      "The ASM${ASM_DIALECT} compiler identification is ${CMAKE_ASM${ASM_DIALECT}_COMPILER_ID}${_archid}${_version}${_variant}"
  )
  unset(_archid)
  unset(_version)
  unset(_variant)
...
# If we have a gas/as cross compiler, they have usually some prefix, like e.g.
# powerpc-linux-gas, arm-elf-gas or i586-mingw32msvc-gas , optionally with a
# 3-component version number at the end The other tools of the toolchain usually
# have the same prefix NAME_WE cannot be used since then this test will fail for
# names like "arm-unknown-nto-qnx6.3.0-gas.exe", where BASENAME would be
