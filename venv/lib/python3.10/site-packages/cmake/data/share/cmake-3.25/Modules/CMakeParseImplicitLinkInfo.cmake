...
#
# Note: this function is leaked/exposed by FindOpenMP and therefore needs to
# have a stable API so projects that copied `FindOpenMP` for backwards
...
function(
  ...
  set(multiValueArgs)
  cmake_parse_arguments(EXTRA_PARSE "${keywordArgs}" "${oneValueArgs}"
                        "${multiValueArgs}" ${ARGN})
  ...
  # Construct a regex to match linker lines.  It must match both the whole line
  # and just the command (argv[0]).
  set(linker_regex
      "^( *|.*[/\\])(${linker}|${startfile}|([^/\\]+-)?ld|collect2)[^/\\]*( |$)"
  )
  ...
  foreach(line IN LISTS output_lines)
    set(cmd)
    if("${line}" MATCHES "${linker_regex}" AND NOT "${line}" MATCHES
                                               "${linker_exclude_regex}")
      if(XCODE)
        # Xcode unconditionally adds a path under the project build tree and on
        # older versions it is not reported with proper quotes.  Remove it.
        string(REGEX REPLACE "([][+.*()^])" "\\\\\\1" _dir_regex
                             "${CMAKE_BINARY_DIR}")
        string(REGEX REPLACE " -[FL]${_dir_regex}/([^ ]| [^-])+( |$)" " " xline
                             "${line}")
      ...
      # check to see if the link line is comma-separated instead of space
      # separated
    ...
    if(EXTRA_PARSE_LANGUAGE
       AND ("x${CMAKE_${EXTRA_PARSE_LANGUAGE}_ID}" STREQUAL "xMSVC"
            OR "x${CMAKE_${EXTRA_PARSE_LANGUAGE}_SIMULATE_ID}" STREQUAL "xMSVC"
           ))
      set(is_msvc 1)
    endif()
    set(search_static 0)
    if("${cmd}" MATCHES "${linker_regex}")
      ...
      foreach(arg IN LISTS args)
        if(skip_value_of)
          string(APPEND log
                 "    arg [${arg}] ==> skip value of ${skip_value_of}\n")
          set(skip_value_of "")
        ...
        elseif("${arg}" STREQUAL "-lto_library")
          ...
          string(APPEND log
                 "    arg [${arg}] ==> ignore, skip following value\n")
        elseif("${arg}" MATCHES "^-l([^:].*)$")
          # Unix library.
          set(lib "${CMAKE_MATCH_1}")
  ...
  # Cleanup list of libraries and flags. We remove items that are not
  # language-specific.
  set(implicit_libs "")
  foreach(lib IN LISTS implicit_libs_tmp)
    ...
    elseif(search_static)
      # This library appears after a -Bstatic flag.  Due to ordering and
      # filtering for mixed-language link lines, we do not preserve the -Bstatic
      # flag itself.  Instead, use an absolute path. Search using a temporary
      # variable with a distinct name so that our test suite does not depend on
      # disk content.
      find_library(
        "CMAKE_${lang}_IMPLICIT_LINK_LIBRARY_${lib}" NO_CACHE
        NAMES "lib${lib}.a"
        NO_DEFAULT_PATH
        PATHS ${implicit_dirs_tmp})
  ...
  if(EXTRA_PARSE_COMPUTE_IMPLICIT_OBJECTS)
    set(${EXTRA_PARSE_COMPUTE_IMPLICIT_OBJECTS}
        "${implicit_objs}"
        PARENT_SCOPE)
  endif()
endfunction()
