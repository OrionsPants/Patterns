...
if(NOT (("${CMAKE_GENERATOR}" MATCHES "Make") OR ("${CMAKE_GENERATOR}" MATCHES
                                                  "Ninja")))
  message(
    FATAL_ERROR
      "HIP language not currently supported by \"${CMAKE_GENERATOR}\" generator"
  )
endif()
...
if(NOT CMAKE_HIP_COMPILER)
  ...
  if(NOT $ENV{HIPCXX} STREQUAL "")
    ...
    get_filename_component(CMAKE_HIP_COMPILER_INIT $ENV{HIPCXX} PROGRAM
                           PROGRAM_ARGS CMAKE_HIP_FLAGS_ENV_INIT)
    if(CMAKE_HIP_FLAGS_ENV_INIT)
      set(CMAKE_HIP_COMPILER_ARG1
          "${CMAKE_HIP_FLAGS_ENV_INIT}"
  ...
  if(NOT CMAKE_HIP_COMPILER_INIT)
    ...
    execute_process(
      COMMAND hipconfig --hipclangpath
      OUTPUT_VARIABLE _CMAKE_HIPCONFIG_CLANGPATH
      RESULT_VARIABLE _CMAKE_HIPCONFIG_RESULT)
    if(_CMAKE_HIPCONFIG_RESULT EQUAL 0 AND EXISTS
                                           "${_CMAKE_HIPCONFIG_CLANGPATH}")
...
if(NOT CMAKE_HIP_COMPILER_ROCM_ROOT AND CMAKE_HIP_COMPILER_ID STREQUAL "Clang")
  execute_process(
    COMMAND "${CMAKE_HIP_COMPILER}" -v -print-targets
  ...
  if(_CMAKE_HIP_COMPILER_RESULT EQUAL 0
     AND _CMAKE_HIP_COMPILER_STDERR MATCHES
         "Found HIP installation: *([^,]*)[,\n]")
    set(CMAKE_HIP_COMPILER_ROCM_ROOT "${CMAKE_MATCH_1}")
    file(TO_CMAKE_PATH "${CMAKE_HIP_COMPILER_ROCM_ROOT}"
...
if(CMAKE_HIP_COMPILER_SYSROOT)
  string(
    CONCAT _SET_CMAKE_HIP_COMPILER_SYSROOT
...
if(NOT DEFINED CMAKE_HIP_ARCHITECTURES)
  ...
  find_program(
    _CMAKE_HIP_ROCM_AGENT_ENUMERATOR
    NAMES rocm_agent_enumerator
    HINTS "${CMAKE_HIP_COMPILER_ROCM_ROOT}/bin" NO_CACHE)
  if(_CMAKE_HIP_ROCM_AGENT_ENUMERATOR)
    execute_process(
      COMMAND "${_CMAKE_HIP_ROCM_AGENT_ENUMERATOR}" -t GPU
    ...
    if(_CMAKE_ROCM_AGENT_ENUMERATOR_RESULT EQUAL 0)
      separate_arguments(_hip_archs NATIVE_COMMAND
                         "${_CMAKE_ROCM_AGENT_ENUMERATOR_STDOUT}")
...
configure_file(${CMAKE_ROOT}/Modules/CMakeHIPCompiler.cmake.in
               ${CMAKE_PLATFORM_INFO_DIR}/CMakeHIPCompiler.cmake @ONLY)
