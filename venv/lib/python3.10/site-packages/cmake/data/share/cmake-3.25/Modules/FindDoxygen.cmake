...
# it contains the doxygen executable in the bundle. In the versions I've seen,
# it is located in Resources, but in general, more often binaries are located in
# MacOS.
#
# NOTE: The official Doxygen.app distributed for OS X uses non-standard
# conventions. Instead of the command-line "doxygen" tool being placed in
# Doxygen.app/Contents/MacOS, "Doxywizard" is placed there instead and "doxygen"
# is placed in Contents/Resources.  This is most likely done so that something
# happens when people double-click on the Doxygen.app package. Unfortunately,
# CMake gets confused by this as when it sees the bundle it uses "Doxywizard" as
# the executable to use instead of "doxygen". Therefore to work-around this
# issue we temporarily disable the app-bundle feature, just for this CMake
# module:
#
if(APPLE)
...
# FYI: In older versions of OS X Doxygen, dot was included with the Doxygen
# bundle, but newer versions require you to download Graphviz.app which contains
# "dot" or use something like homebrew.
# ============== End OSX stuff ================
...
function(_Doxygen_get_version doxy_version result_var doxy_path)
  ...
  string(REGEX MATCH [[^[0-9]+\.[0-9]+\.[0-9]+]] sem_doxygen_version
               "${full_doxygen_version}")
...
function(_Doxygen_version_validator version_match doxy_path)
  if(NOT DEFINED Doxygen_FIND_VERSION)
    ...
    if(version_result)
      message(
        DEBUG
        "Unable to determine candidate doxygen version at ${doxy_path}: ${version_result}"
      )
    endif()
...
macro(_Doxygen_find_doxygen)
  ...
  if(DOXYGEN_EXECUTABLE)
    _doxygen_get_version(DOXYGEN_VERSION _Doxygen_version_result
                         "${DOXYGEN_EXECUTABLE}")

    if(_Doxygen_version_result)
      message(
        WARNING
          "Unable to determine doxygen version: ${_Doxygen_version_result}")
    endif()

    # Create an imported target for Doxygen
    if(NOT TARGET Doxygen::doxygen)
      add_executable(Doxygen::doxygen IMPORTED GLOBAL)
      set_target_properties(Doxygen::doxygen PROPERTIES IMPORTED_LOCATION
                                                        "${DOXYGEN_EXECUTABLE}")
    endif()
  endif()
endmacro()
...
macro(_Doxygen_find_dia)
  ...
  find_program(
    ...
    NAMES dia
    PATHS "$ENV{ProgramFiles}/Dia" "$ENV{ProgramFiles${_x86}}/Dia"
    DOC "Diagram Editor tool for use with Doxygen")
  mark_as_advanced(DOXYGEN_DIA_EXECUTABLE)

  if(DOXYGEN_DIA_EXECUTABLE)
    # The Doxyfile wants the path to the utility, not the entire path including
    # file name
    get_filename_component(DOXYGEN_DIA_PATH "${DOXYGEN_DIA_EXECUTABLE}"
                           DIRECTORY)
    if(WIN32)
    ...
    if(NOT TARGET Doxygen::dia)
      ...
      set_target_properties(Doxygen::dia PROPERTIES IMPORTED_LOCATION
                                                    "${DOXYGEN_DIA_EXECUTABLE}")
    endif()
  endif()
...
macro(_Doxygen_find_dot)
  if(WIN32)
    set(_x86 "(x86)")
    file(GLOB _Doxygen_GRAPHVIZ_BIN_DIRS "$ENV{ProgramFiles}/Graphviz*/bin"
  ...
  find_program(
    ...
    NAMES dot
    PATHS ${_Doxygen_GRAPHVIZ_BIN_DIRS}
  ...
  if(DOXYGEN_DOT_EXECUTABLE)
    ...
    if(NOT TARGET Doxygen::dot)
      ...
      set_target_properties(Doxygen::dot PROPERTIES IMPORTED_LOCATION
                                                    "${DOXYGEN_DOT_EXECUTABLE}")
    endif()
  endif()
...
macro(_Doxygen_find_mscgen)
  ...
  find_program(
    ...
    NAMES mscgen
    PATHS "$ENV{ProgramFiles}/Mscgen" "$ENV{ProgramFiles${_x86}}/Mscgen"
  ...
  if(DOXYGEN_MSCGEN_EXECUTABLE)
    # The Doxyfile wants the path to the utility, not the entire path including
    # file name
    get_filename_component(DOXYGEN_MSCGEN_PATH "${DOXYGEN_MSCGEN_EXECUTABLE}"
                           DIRECTORY)
    if(WIN32)
    ...
    if(NOT TARGET Doxygen::mscgen)
      ...
      set_target_properties(
        Doxygen::mscgen PROPERTIES IMPORTED_LOCATION
                                   "${DOXYGEN_MSCGEN_EXECUTABLE}")
    endif()
  endif()
...
if(NOT Doxygen_FIND_COMPONENTS)
  ...
  # Preserve backward compatibility: search for `dot` also if `DOXYGEN_SKIP_DOT`
...
find_package_handle_standard_args(
  ...
  VERSION_VAR DOXYGEN_VERSION HANDLE_VERSION_RANGE
  HANDLE_COMPONENTS)
...
# This allows people to substitute them directly into Doxyfile with
# configure_file().
if(DOXYGEN_FOUND)
...
if(_Doxygen_keep_backward_compat)
  ...
  # No need to keep any backward compatibility for `DOXYGEN_MSCGEN_XXX` and
  # `DOXYGEN_DIA_XXX` since they were not supported before component support was
  # added
endif()
...
if(TARGET Doxygen::doxygen)
  # If doxygen was found, use it to generate a minimal default Doxyfile. We will
  # delete this file after we have finished using it below to generate the other
  # files that doxygen_add_docs() will use.
  ...
  if(_Doxygen_tpl_result)
    message(
      FATAL_ERROR "Unable to generate Doxyfile template: ${_Doxygen_tpl_result}"
    )
...
#
  ...
  # We only need one copy of these across the whole build, since their content
  # is only dependent on the version of Doxygen being used. Therefore we always
  # put them at the top of the build tree so that they are in a predictable
  # location.
  ...
  # Get strings containing a configuration key from the template Doxyfile we
  # obtained from this version of Doxygen. Because some options are split across
  # multiple lines by ending lines with backslashes, we cannot just use
  # file(STRINGS...) with a REGEX. Instead, read lines without a REGEX so that
  # file(STRINGS...) handles the trailing backslash as a line continuation. It
  # stores multi-lines as lists, so we then have to replace the ";" list
  # separator with backslashed newlines again so that we get the original
  # content stored back as the value part.
  ...
  # Build up a Doxyfile that provides @configVar@ substitutions for each Doxygen
  # config option as well as a separate CMake script which provides the default
  # value for each of those options if the project doesn't supply them. Each
  # config option will support substitution of a CMake variable of the same name
  # except with DOXYGEN_ prepended.
...
function(doxygen_quote_value VARIABLE)
  # Quote a value of the given variable if: - VARIABLE parameter was really
  # given - the variable it names is defined and is not present in the list
  # specified by DOXYGEN_VERBATIM_VARS (if set) - the value of the named
  # variable isn't already quoted - the value has spaces
  if(VARIABLE
     AND DEFINED ${VARIABLE}
     AND NOT ${VARIABLE} MATCHES "^\".* .*\"$"
     AND ${VARIABLE} MATCHES " "
     AND NOT (DEFINED DOXYGEN_VERBATIM_VARS AND "${VARIABLE}" IN_LIST
...
function(doxygen_list_to_quoted_strings LIST_VARIABLE)
  if(LIST_VARIABLE AND DEFINED ${LIST_VARIABLE})
    ...
    unset(_sep)
    unset(_verbatim)
    # Have to test if list items should be treated as verbatim here because we
    # lose the variable name when we pass just one list item to
    # doxygen_quote_value() below
    if(DEFINED DOXYGEN_VERBATIM_VARS AND "${LIST_VARIABLE}" IN_LIST
                                         DOXYGEN_VERBATIM_VARS)
      set(_verbatim True)
    endif()
...
doxygen_add_docs() for target ${targetName}")
  ...
  # If not already defined, set some relevant defaults based on the assumption
  # that the documentation is for the whole project. Details specified in the
  # project() command will be used to populate a number of these defaults.
  ...
  if(NOT DEFINED DOXYGEN_PROJECT_NAME)
    ...
    # the title of most generated pages and in a few other places. The default
    # value is: My Project.
  ...
  if(NOT DEFINED DOXYGEN_RECURSIVE)
    ...
    # should be searched for input files as well. CMake projects generally
    # evolve to span multiple directories, so it makes more sense for this to be
    # on by default. Doxygen's default value has this setting turned off, so we
    # override it.
  ...
  if(NOT DEFINED DOXYGEN_OUTPUT_DIRECTORY)
    ...
    # path is used, Doxygen will interpret it as being relative to the location
    # where doxygen was started, but we need to run Doxygen in the source tree
    # so that relative input paths work intuitively. Therefore, we ensure that
    # the output directory is always an absolute path and if the project
    # provided a relative path, we treat it as relative to the current BINARY
    # directory so that output is not generated inside the source tree.
    set(DOXYGEN_OUTPUT_DIRECTORY "${CMAKE_CURRENT_BINARY_DIR}")
  elseif(NOT IS_ABSOLUTE "${DOXYGEN_OUTPUT_DIRECTORY}")
    get_filename_component(
      DOXYGEN_OUTPUT_DIRECTORY "${DOXYGEN_OUTPUT_DIRECTORY}" ABSOLUTE BASE_DIR
      "${CMAKE_CURRENT_BINARY_DIR}")
  endif()

  if(NOT DEFINED DOXYGEN_HAVE_DOT)
    # If you set the HAVE_DOT tag to YES then doxygen will assume the dot tool
    # is available from the path. This tool is part of Graphviz (see:
    # https://www.graphviz.org/), a graph visualization toolkit from AT&T and
    # Lucent Bell Labs. The other options in this section have no effect if this
    # option is set to NO. Doxygen's default value is: NO.
    if(Doxygen_dot_FOUND)
  ...
  if(NOT DEFINED DOXYGEN_DOT_MULTI_TARGETS)
    ...
    # (>1.8.10) support this, Doxygen disables this feature by default. This tag
  ...
  if(NOT DEFINED DOXYGEN_GENERATE_LATEX)
    ...
    # output. We only want the HTML output enabled by default, so we turn this
    # off if the project hasn't specified it.
  ...
  if(NOT DEFINED DOXYGEN_WARN_FORMAT)
    if(CMAKE_VS_MSBUILD_COMMAND OR CMAKE_VS_DEVENV_COMMAND)
      ...
      # the file (if it could be obtained via FILE_VERSION_FILTER). Doxygen's
      # default value is: $file:$line: $text
  ...
  if(DEFINED DOXYGEN_WARN_LOGFILE AND NOT IS_ABSOLUTE "${DOXYGEN_WARN_LOGFILE}")
    ...
    get_filename_component(DOXYGEN_WARN_LOGFILE "${DOXYGEN_WARN_LOGFILE}"
                           ABSOLUTE BASE_DIR "${CMAKE_CURRENT_BINARY_DIR}")
  endif()

  # Any files from the INPUT that match any of the EXCLUDE_PATTERNS will be
  # excluded from the set of input files. We provide some additional patterns to
  # prevent commonly unwanted things from CMake builds being pulled in.
  #
  # Note that the wildcards are matched against the file with absolute path, so
  # to exclude all test directories for example use the pattern */test/*
  list(
    APPEND
  ...
  # Now bring in Doxgen's defaults for those things the project has not already
  # set and we have not provided above
  include("${CMAKE_BINARY_DIR}/CMakeDoxygenDefaults.cmake" OPTIONAL)

  # Cleanup built HTMLs on "make clean" TODO Any other dirs?
  if(DOXYGEN_GENERATE_HTML)
    ...
    set_property(
      DIRECTORY
      APPEND
      PROPERTY ADDITIONAL_CLEAN_FILES "${_args_clean_html_dir}")
  endif()

  # Build up a list of files we can identify from the inputs so we can list them
  # as DEPENDS and SOURCES in the custom command/target (the latter makes them
  # display in IDEs). This must be done before we transform the various
  # DOXYGEN_... variables below because we need to process DOXYGEN_INPUT as a
  # list first.
  unset(_sources)
  foreach(_item IN LISTS DOXYGEN_INPUT)
    get_filename_component(_abs_item "${_item}" ABSOLUTE BASE_DIR
                           "${_args_WORKING_DIRECTORY}")
    get_source_file_property(_isGenerated "${_abs_item}" GENERATED)
    if(_isGenerated
       OR (EXISTS "${_abs_item}"
           AND NOT IS_DIRECTORY "${_abs_item}"
           AND NOT IS_SYMLINK "${_abs_item}"))
  ...
  # Only create the stamp file if asked to. If we don't create it, the target
