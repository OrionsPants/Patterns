...
function(ProcessorCount var)
  ...
  if(NOT count)
    ...
    if(ProcessorCount_cmd_sysctl)
      execute_process(
        COMMAND ${ProcessorCount_cmd_sysctl} -n hw.ncpu
  ...
  if(NOT count)
    # Linux (systems with nproc): Prefer nproc to getconf if available as
    # getconf may return the host CPU count in Linux containers
  ...
  if(NOT count)
    ...
    if(ProcessorCount_cmd_getconf)
      execute_process(
        COMMAND ${ProcessorCount_cmd_getconf} _NPROCESSORS_ONLN
        ERROR_QUIET OUTPUT_STRIP_TRAILING_WHITESPACE
        OUTPUT_VARIABLE count)
      # message("ProcessorCount: trying getconf
  ...
  if(NOT count)
    ...
    if(ProcessorCount_cmd_machinfo)
      execute_process(
        COMMAND ${ProcessorCount_cmd_machinfo}
        ERROR_QUIET OUTPUT_STRIP_TRAILING_WHITESPACE
        OUTPUT_VARIABLE machinfo_output)
      string(REGEX MATCHALL "Number of CPUs = ([0-9]+)" procs
                   "${machinfo_output}")
      set(count "${CMAKE_MATCH_1}")
      if(NOT count)
        string(REGEX MATCHALL "([0-9]+) logical processors" procs
                     "${machinfo_output}")
        set(count "${CMAKE_MATCH_1}")
      endif()
      # message("ProcessorCount: trying machinfo
      ...
      if(ProcessorCount_cmd_mpsched)
        execute_process(
          COMMAND ${ProcessorCount_cmd_mpsched} -s
          OUTPUT_QUIET ERROR_STRIP_TRAILING_WHITESPACE
          ERROR_VARIABLE mpsched_output)
        string(REGEX MATCHALL "Processor Count *: *([0-9]+)" procs
                     "${mpsched_output}")
        set(count "${CMAKE_MATCH_1}")
        # message("ProcessorCount: trying mpsched -s
        # '${ProcessorCount_cmd_mpsched}'")
      endif()
    endif()
  endif()
  ...
  if(NOT count)
    ...
    if(ProcessorCount_cmd_lsconf)
      execute_process(
        COMMAND ${ProcessorCount_cmd_lsconf}
        ERROR_QUIET OUTPUT_STRIP_TRAILING_WHITESPACE
        OUTPUT_VARIABLE lsconf_output)
      string(REGEX MATCHALL "Number Of Processors: ([0-9]+)" procs
                   "${lsconf_output}")
      set(count "${CMAKE_MATCH_1}")
  ...
  if(NOT count)
    ...
    if(ProcessorCount_cmd_pidin)
      execute_process(
        COMMAND ${ProcessorCount_cmd_pidin} info
        ERROR_QUIET OUTPUT_STRIP_TRAILING_WHITESPACE
  ...
  if(NOT count)
    ...
    if(ProcessorCount_cmd_psrinfo)
      execute_process(
        COMMAND ${ProcessorCount_cmd_psrinfo} -p -v
        ERROR_QUIET OUTPUT_STRIP_TRAILING_WHITESPACE
        OUTPUT_VARIABLE psrinfo_output)
      string(REGEX MATCHALL "has [0-9]+ virtual processor" procs
                   "${psrinfo_output}")
      set(count "")
      foreach(proc ${procs})
      ...
      if(ProcessorCount_cmd_uname)
        execute_process(
          COMMAND ${ProcessorCount_cmd_uname} -X
          ERROR_QUIET OUTPUT_STRIP_TRAILING_WHITESPACE
          OUTPUT_VARIABLE uname_X_output)
        string(REGEX MATCHALL "NumCPU = ([0-9]+)" procs "${uname_X_output}")
        set(count "${CMAKE_MATCH_1}")
        # message("ProcessorCount: trying uname -X
        # '${ProcessorCount_cmd_uname}'")
      endif()
    endif()
  endif()

  # Execute this code when all previously attempted methods return empty output:
  #
  if(NOT count)
  ...
  if(NOT count)
    ...
    if(ProcessorCount_cmd_sysinfo)
      execute_process(
        COMMAND ${ProcessorCount_cmd_sysinfo}
        ERROR_QUIET OUTPUT_STRIP_TRAILING_WHITESPACE
        OUTPUT_VARIABLE sysinfo_X_output)
      string(REGEX MATCHALL "\nCPU #[0-9]+:" procs "\n${sysinfo_X_output}")
      list(LENGTH procs count)
      # message("ProcessorCount: trying sysinfo
      # '${ProcessorCount_cmd_sysinfo}'")
    endif()
  endif()

  # Since cygwin builds of CMake do not define WIN32 anymore, but they still run
  # on Windows, and will still have this env var defined:
  #
  if(NOT count)
    set(count "$ENV{NUMBER_OF_PROCESSORS}")
  ...
  # Ensure an integer return (avoid inadvertently returning an empty string or
  # an error string)... If it's not a decimal integer, return 0:
  #
