...
# function to turn generator name into a version string like vs9 or vs10
function(GET_VS_VERSION_STRING generator var)
  string(REGEX REPLACE "Visual Studio ([0-9][0-9]?)($|.*)" "\\1" NUMBER
                       "${generator}")
...
if(BUILD_TESTING)
  ...
  if(EXISTS "${PROJECT_SOURCE_DIR}/DartConfig.cmake")
    ...
    # specified NIGHTLY_START_TIME. Time is specified in 24 hour format.
  ...
  if("${_update_type}" STREQUAL "cvs")
    ...
    set(CVS_UPDATE_OPTIONS
        "-d -A -P"
        CACHE STRING "Options passed to the cvs update command.")
  ...
  set(CTEST_SUBMIT_RETRY_DELAY
      5
      CACHE STRING "How long to wait between timed-out CTest submissions.")
  set(CTEST_SUBMIT_RETRY_COUNT
      3
      CACHE STRING "How many times to retry timed-out CTest submissions.")
  ...
  if(COMMAND cmake_host_system_information)
    ...
    set(SITE
        "${_ctest_hostname}"
        CACHE STRING "Name of the computer/site where compile is being run")
  ...
  if(NOT BUILDNAME)
    ...
    if(UNIX OR BORLAND)
      get_filename_component(DART_COMPILER_NAME "${DART_COMPILER}"
                             ${DART_NAME_COMPONENT})
    else()
      get_filename_component(DART_COMPILER_NAME "${CMAKE_MAKE_PROGRAM}"
                             ${DART_NAME_COMPONENT})
    endif()
  ...
  # the build command
  build_command(MAKECOMMAND_DEFAULT_VALUE CONFIGURATION
                "\${CTEST_CONFIGURATION_TYPE}")
  set(MAKECOMMAND
      ${MAKECOMMAND_DEFAULT_VALUE}
      CACHE STRING "Command to build the project")

  # the default build configuration the ctest build handler will use if there is
  # no -C arg given to ctest:
