...
# Internal use, projects must not call this directly. It is intended for use by
# FetchContent_Declare() only.
#
# Sets a content-specific global property (not meant for use outside of
# functions defined here in this file) which can later be retrieved using
# __FetchContent_getSavedDetails() with just the same content name. If there is
# already a value stored in the property, it is left unchanged and this call has
# no effect. This allows parent projects to define the content details,
# overriding anything a child project may try to set (properties are not cached
# between runs, so the first thing to set it in a build will be in control).
function(__FetchContent_declareDetails contentName)
  ...
  elseif("${FETCHCONTENT_TRY_FIND_PACKAGE_MODE}" STREQUAL "OPT_IN"
         OR NOT DEFINED FETCHCONTENT_TRY_FIND_PACKAGE_MODE)
    set(__tryFindPackage FALSE)
    set(__tryFindPackageAllowed TRUE)
  else()
    message(
      FATAL_ERROR "Unsupported value for FETCHCONTENT_TRY_FIND_PACKAGE_MODE: "
  ...
  foreach(__item IN LISTS ARGN)
    ...
    if(__item STREQUAL "FIND_PACKAGE_ARGS")
      ...
      # variable but with an empty value initially. This allows us to check at
      # the start of the loop whether to store remaining items in this variable
      # or not. Note that there could be no more args, which is still a valid
      # case because we automatically provide ${contentName} as the package name
      # and there may not need to be any further arguments.
      set(__findPackageArgs "")
      continue() # Don't store this item
    elseif(__item STREQUAL "OVERRIDE_FIND_PACKAGE")
      set(__tryFindPackageAllowed FALSE)
      # Define a separate dedicated property for find_package() to check in its
      # implementation. This will be a placeholder until FetchContent actually
      # does the population. After that, we will have created a stand-in config
      # file that find_package() will pick up instead.
      set(propertyName
          "_FetchContent_${contentNameLower}_override_find_package")
...
# Internal use, projects must not call this directly. It is intended for use by
# the FetchContent_Declare() function.
#
# Retrieves details saved for the specified content in an earlier call to
...
# Saves population details of the content, sets defaults for the SOURCE_DIR and
# BUILD_DIR.
function(FetchContent_Declare contentName)

  # Always check this even if we won't save these details. This helps projects
  # catch errors earlier. Avoid using if(... IN_LIST ...) so we don't have to
  # alter policy settings
  list(FIND ARGN OVERRIDE_FIND_PACKAGE index_OVERRIDE_FIND_PACKAGE)
  list(FIND ARGN FIND_PACKAGE_ARGS index_FIND_PACKAGE_ARGS)
  if(index_OVERRIDE_FIND_PACKAGE GREATER_EQUAL 0 AND index_FIND_PACKAGE_ARGS
                                                     GREATER_EQUAL 0)
    message(
      FATAL_ERROR
  ...
  set(oneValueArgs SVN_REPOSITORY DOWNLOAD_NO_EXTRACT
                   DOWNLOAD_EXTRACT_TIMESTAMP BINARY_DIR SOURCE_DIR)
  ...
  if(ARG_SVN_REPOSITORY)
    # Add a hash of the svn repository URL to the source dir. This works around
    # the problem where if the URL changes, the download would fail because it
    # tries to checkout/update rather than switch the old URL to the new one. We
    # limit the hash to the first 7 characters so that the source path doesn't
    # get overly long (which can be a problem on windows due to path length
    # limits).
  ...
  # The ExternalProject_Add() call in the sub-build won't see the CMP0135 policy
  # setting of our caller. Work out if that policy will be needed and explicitly
  # set the relevant option if not already provided. The condition here is
  # essentially an abbreviated version of the logic in ExternalProject's
  # _ep_add_download_command() function.
  if(NOT ARG_DOWNLOAD_NO_EXTRACT AND NOT DEFINED ARG_DOWNLOAD_EXTRACT_TIMESTAMP)
    ...
    if(urlIndex GREATER_EQUAL 0)
      ...
      if(urlIndex GREATER_EQUAL numArgs)
        message(
          FATAL_ERROR "URL keyword needs to be followed by at least one URL")
      endif()
      ...
      if(NOT IS_DIRECTORY "${firstUrl}")
        cmake_policy(
          GET CMP0135 _FETCHCONTENT_CMP0135 PARENT_SCOPE # undocumented, do not
...
# =======================================================================
# Set/get whether the specified content has been populated yet. The setter also
# records the source and binary dirs used.
# =======================================================================

# Semi-internal use. Projects must not call this directly. Dependency providers
# must call it if they satisfy a request made with the
# FETCHCONTENT_MAKEAVAILABLE_SERIAL method (that is the only permitted place to
# call it outside of the FetchContent module).
...
# Set variables in the calling scope for any of the retrievable properties. If
# no specific properties are requested, variables will be set for all
# retrievable properties.
#
# This function is intended to also be used by projects as the canonical way to
# detect whether they should call FetchContent_Populate() and pull the populated
# source into the build with add_subdirectory(), if they are using the populated
# content in that way.
...
# The value of contentName will always have been lowercased by the caller. All
# other arguments are assumed to be options that are understood by
...
function(__FetchContent_directPopulate contentName)
  ...
  set(oneValueArgs
      ...
      INSTALL_COMMAND
      TEST_COMMAND
      # We force these to be ON since we are always executing serially and we
      # want all steps to have access to the terminal in case they need input
      # from the command line (e.g. ask for a private key password) or they want
      # to provide timely progress. We silently absorb and discard these if they
  ...
  # with some convenient variables. Doing this here ensures the caller sees the
  # correct result in the case where the default values are overridden by the
  # content details set by the project.
  set(${contentName}_SOURCE_DIR
      "${ARG_SOURCE_DIR}"
      PARENT_SCOPE)
  set(${contentName}_BINARY_DIR
      "${ARG_BINARY_DIR}"
      PARENT_SCOPE)

  # The unparsed arguments may contain spaces, so build up ARG_EXTRA in such a
  # way that it correctly substitutes into the generated CMakeLists.txt file
...
")
  ...
  # Hide output if requested, but save it to a variable in case there's an error
  # so we can show the output upon failure. When not quiet, don't capture the
  # output to a variable because the user may want to see the output as it
  # happens (e.g. progress during long downloads). Combine both stdout and
  # stderr in the one capture variable so the output stays in order.
  if(ARG_QUIET)
    set(outputOptions OUTPUT_VARIABLE capturedOutput ERROR_VARIABLE
                      capturedOutput)
  else()
    set(capturedOutput)
    set(outputOptions)
  ...
  if(CMAKE_GENERATOR)
    ...
    # Likely we've been invoked via CMake's script mode where no generator is
    # set (and hence CMAKE_MAKE_PROGRAM could not be trusted even if provided).
    # We will have to rely on being able to find the default generator and build
    # tool.
    unset(subCMakeOpts)
  endif()

  set(__FETCHCONTENT_CACHED_INFO "")
  set(__passthrough_vars CMAKE_EP_GIT_REMOTE_UPDATE_STRATEGY CMAKE_TLS_VERIFY
                         CMAKE_TLS_CAINFO CMAKE_NETRC CMAKE_NETRC_FILE)
  foreach(var IN LISTS __passthrough_vars)
    if(DEFINED ${var})
      # Embed directly in the generated CMakeLists.txt file to avoid making the
      # cmake command line excessively long. It also makes debugging and testing
      # easier.
...
")
  ...
  # Create and build a separate CMake project to carry out the population. If
  ...
  # something not findable on the PATH. We also ensured above that the Debug
  # config will be defined for multi-config generators.
  configure_file(
    "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/FetchContent/CMakeLists.cmake.in"
    "${ARG_SUBBUILD_DIR}/CMakeLists.txt")
  execute_process(
    COMMAND ${CMAKE_COMMAND} ${subCMakeOpts} .
    RESULT_VARIABLE result ${outputOptions}
...
# Populate the specified content using details stored from an earlier call to
# FetchContent_Declare().
function(FetchContent_Populate contentName)

  if(NOT contentName)
    message(
      FATAL_ERROR "Empty contentName not allowed for FetchContent_Populate()")
  endif()

  string(TOLOWER ${contentName} contentNameLower)

  if(ARGN)
    # This is the direct population form with details fully specified as part of
    # the call, so we already have everything we need
    __fetchcontent_directpopulate(
      ${contentNameLower}
      SUBBUILD_DIR
      "${CMAKE_CURRENT_BINARY_DIR}/${contentNameLower}-subbuild"
      SOURCE_DIR
      "${CMAKE_CURRENT_BINARY_DIR}/${contentNameLower}-src"
      BINARY_DIR
      "${CMAKE_CURRENT_BINARY_DIR}/${contentNameLower}-build"
    ...
    set(${contentNameLower}_SOURCE_DIR
        ...
        PARENT_SCOPE)
    set(${contentNameLower}_BINARY_DIR
        "${${contentNameLower}_BINARY_DIR}"
        PARENT_SCOPE)

    # Don't set global properties, or record that we did this population, since
    # this was a direct call outside of the normal declared details form. We
    # only want to save values in the global properties for content that honors
    # the hierarchical details mechanism so that projects are not robbed of the
    # ability to override details set in nested projects.
    return()
  endif()

  # No details provided, so assume they were saved from an earlier call to
  # FetchContent_Declare(). Do a check that we haven't already populated this
  # content before in case the caller forgot to check.
  ...
  if(FETCHCONTENT_SOURCE_DIR_${contentNameUpper})
    # The source directory has been explicitly provided in the cache, so no
    # population is required. The build directory may still be specified by the
    # declared details though.
    ...
    if(NOT IS_ABSOLUTE "${FETCHCONTENT_SOURCE_DIR_${contentNameUpper}}")
      ...
      message(
        WARNING
          "Relative source directory specified. This is not safe, "
    ...
    if(savedDetails_BINARY_DIR)
      ...
      set(${contentNameLower}_BINARY_DIR
          "${FETCHCONTENT_BASE_DIR}/${contentNameLower}-build")
    endif()
  ...
  elseif(FETCHCONTENT_FULLY_DISCONNECTED)
    ...
    cmake_parse_arguments(savedDetails "" "SOURCE_DIR;BINARY_DIR" ""
                          ${contentDetails})
    ...
    if(savedDetails_SOURCE_DIR)
      ...
      set(${contentNameLower}_SOURCE_DIR
          "${FETCHCONTENT_BASE_DIR}/${contentNameLower}-src")
    endif()
    ...
    # Support both a global "disconnect all updates" and a per-content update
    # test (either one being set disables updates for this content).
    option(
    ...
    if(FETCHCONTENT_UPDATES_DISCONNECTED
       OR FETCHCONTENT_UPDATES_DISCONNECTED_${contentNameUpper})
  ...
  fetchcontent_setpopulated(
    ${contentName} SOURCE_DIR "${${contentNameLower}_SOURCE_DIR}" BINARY_DIR
    "${${contentNameLower}_BINARY_DIR}")

  # Pass variables back to the caller. The variables passed back here must match
  # what FetchContent_GetProperties() sets when it is called with just the
  # content name.
  set(${contentNameLower}_SOURCE_DIR
      "${${contentNameLower}_SOURCE_DIR}"
      PARENT_SCOPE)
  set(${contentNameLower}_BINARY_DIR
      "${${contentNameLower}_BINARY_DIR}"
      PARENT_SCOPE)
  set(${contentNameLower}_POPULATED
      True
      PARENT_SCOPE)
...
function(__FetchContent_setupFindPackageRedirection contentName)
  ...
  string(TOLOWER ${contentName} contentNameLower)
  get_property(
    wantFindPackage GLOBAL
    PROPERTY _FetchContent_${contentNameLower}_find_package_args
    DEFINED)
  ...
  # We write out dep-config.cmake and dep-config-version.cmake file name forms
  # here because they are forced to lowercase. FetchContent dependency names are
  # case-insensitive, but find_package() config files are only case-insensitive
  # for the -config and -config-version forms, not the Config and ConfigVersion
  # forms.
  set(inFileDir ${CMAKE_CURRENT_FUNCTION_LIST_DIR}/FetchContent)
  set(configFilePrefix1
      "${CMAKE_FIND_PACKAGE_REDIRECTS_DIR}/${contentName}Config")
  set(configFilePrefix2
      "${CMAKE_FIND_PACKAGE_REDIRECTS_DIR}/${contentNameLower}-config")
  if(NOT EXISTS "${configFilePrefix1}.cmake" AND NOT EXISTS
                                                 "${configFilePrefix2}.cmake")
    configure_file(${inFileDir}/package-config.cmake.in
                   "${configFilePrefix2}.cmake" @ONLY)
  endif()
  if(NOT EXISTS "${configFilePrefix1}Version.cmake"
     AND NOT EXISTS "${configFilePrefix2}-version.cmake")
  ...
  # find_package() from delegating to FetchContent and let it find these config
  # files through its normal processing.
  set(propertyName "${prefix}_override_find_package")
  set(GLOBAL PROPERTY ${propertyName} FALSE)
  set(${contentName}_DIR
      "${CMAKE_FIND_PACKAGE_REDIRECTS_DIR}"
      CACHE INTERNAL "Redirected by FetchContent")

endfunction()

# Arguments are assumed to be the names of dependencies that have been declared
# previously and should be populated. It is not an error if any of them have
# already been populated (they will just be skipped in that case). The command
# is implemented as a macro so that the variables defined by the
# FetchContent_GetProperties() and FetchContent_Populate() calls will be
...
macro(FetchContent_MakeAvailable)
  ...
  get_property(__cmake_providerCommand GLOBAL
               PROPERTY __FETCHCONTENT_MAKEAVAILABLE_SERIAL_PROVIDER)
  foreach(__cmake_contentName IN ITEMS ${ARGV})
    ...
    # overrides everything else and we shouldn't try to use find_package() or a
    # dependency provider.
    ...
    if("${FETCHCONTENT_SOURCE_DIR_${__cmake_contentNameUpper}}" STREQUAL "")
      ...
      if(NOT "${__cmake_providerCommand}" STREQUAL ""
         AND NOT DEFINED __cmake_fcProvider_${__cmake_contentNameLower})
        message(
          VERBOSE
        ...
        __fetchcontent_getsaveddetails(${__cmake_contentName}
                                       __cmake_contentDetails)
        set(__cmake_providerArgs "FETCHCONTENT_MAKEAVAILABLE_SERIAL"
                                 "${__cmake_contentName}")
        ...
        get_property(
          __cmake_addfpargs GLOBAL
          PROPERTY _FetchContent_${contentNameLower}_find_package_args
          DEFINED)
        if(__cmake_addfpargs)
          get_property(
            __cmake_fpargs GLOBAL
            PROPERTY _FetchContent_${contentNameLower}_find_package_args)
        ...
        # need preserved.
        list(APPEND __cmake_fcCurrentVarsStack ${__cmake_contentName}
             ${__cmake_contentNameLower})

        set(__cmake_fcProvider_${__cmake_contentNameLower} YES)
        cmake_language(EVAL CODE
                       "${__cmake_providerCommand}(${__cmake_providerArgs})")

        list(POP_BACK __cmake_fcCurrentVarsStack __cmake_contentNameLower
             __cmake_contentName)
      ...
      # Check if we've been asked to try find_package() first, even if we have
      # already populated this dependency. If we previously tried to use
      # find_package() for this and it succeeded, those things might no longer
      # be in scope, so we have to do it again.
      get_property(
        __cmake_haveFpArgs GLOBAL
        PROPERTY _FetchContent_${__cmake_contentNameLower}_find_package_args
        DEFINED)
      if(__cmake_haveFpArgs)
        unset(__cmake_haveFpArgs)
        message(
          VERBOSE
          "Trying find_package(${__cmake_contentName} ...) before FetchContent")
        get_property(
          __cmake_fpArgs GLOBAL
          PROPERTY _FetchContent_${__cmake_contentNameLower}_find_package_args)

        # This call could lead to FetchContent_MakeAvailable() being called for
        # a nested dependency and it may occur in the current variable scope. We
        # have to save/restore the variables we need to preserve.
        list(APPEND __cmake_fcCurrentNameStack ${__cmake_contentName}
             ${__cmake_contentNameLower})
        find_package(${__cmake_contentName} ${__cmake_fpArgs})
        list(POP_BACK __cmake_fcCurrentNameStack __cmake_contentNameLower
             __cmake_contentName)
        unset(__cmake_fpArgs)
    ...
    if(NOT ${__cmake_contentNameLower}_POPULATED)
      ...
      # Only try to call add_subdirectory() if the populated content can be
      # treated that way. Protecting the call with the check allows this
      # function to be used for projects that just want to ensure the content
      # exists, such as to provide content at a known location. We check the
      # saved details for an optional SOURCE_SUBDIR which can be used in the
      # same way as its meaning for ExternalProject. It won't matter if it was
      # passed through to the ExternalProject sub-build, since it would have
      # been ignored there.
      ...
      if("${__cmake_contentDetails}" STREQUAL "")
        message(
          FATAL_ERROR
            "No details have been set for content: ${__cmake_contentName}")
      endif()
      cmake_parse_arguments(__cmake_arg "SYSTEM" "SOURCE_SUBDIR" ""
                            ${__cmake_contentDetails})
      ...
      if(EXISTS ${__cmake_srcdir}/CMakeLists.txt)
        if(__cmake_arg_SYSTEM)
          add_subdirectory(${__cmake_srcdir}
                           ${${__cmake_contentNameLower}_BINARY_DIR} SYSTEM)
        else()
          add_subdirectory(${__cmake_srcdir}
                           ${${__cmake_contentNameLower}_BINARY_DIR})
        endif()
      endif()
  ...
  string(SUBSTRING "${__cmake_original_verify_setting}" 12 -1
