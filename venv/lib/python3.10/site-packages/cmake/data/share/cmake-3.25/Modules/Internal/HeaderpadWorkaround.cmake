...
# Do NOT include this module directly into any of your code. It is used by the
# try_compile() implementation to work around a specific issue with conflicting
# flags when building for Apple platforms.
if(NOT APPLE)
  return()
endif()
...
function(__cmake_internal_workaround_headerpad_flag_conflict _LANG)
  ...
  set(regex
      "(^| )(-fembed-bitcode(-marker|=(all|bitcode|marker))?|-bundle_bitcode)($| )"
  )
  set(remove_headerpad NO)

  # Check arbitrary flags that the user or project has set. These compiler flags
  # get added to the linker command line.
  ...
  # results in them being added to add_definitions(). Those don't end up on the
  # linker command line, so we don't need to check them here.

  if(remove_headerpad)
    foreach(
      flag IN
      ITEMS CMAKE_${_LANG}_LINK_FLAGS
