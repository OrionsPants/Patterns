...
# Get build settings for the current target/config/SDK by running `xcodebuild
# -sdk ... -showBuildSettings` and parsing it's output
function(_ios_install_combined_get_build_setting sdk variable resultvar)
  ...
  if(NOT output MATCHES " ${variable} = ([^\n]*)")
    if("${variable}" STREQUAL "VALID_ARCHS")
      # VALID_ARCHS may be unset by user for given SDK (e.g. for build without
      # simulator).
      set("${resultvar}"
          ""
          PARENT_SCOPE)
...
# Make both arch lists a disjoint set by preferring the current SDK (starting
# with Xcode 12 arm64 is available as device and simulator arch on iOS)
function(_ios_install_combined_prune_common_archs corr_sdk corr_archs_var
         this_archs_var)
  list(REMOVE_ITEM ${corr_archs_var} ${${this_archs_var}})

  string(REPLACE ";" " " printable "${${corr_archs_var}}")
  _ios_install_combined_message(
    "Architectures (${corr_sdk}) after pruning: ${printable}")
...
# function will run 'lipo -info' and parse output. Result will be returned as a
# CMake list.
...
function(_ios_install_combined_build sdk)
  ...
  execute_process(
    COMMAND "${CMAKE_COMMAND}" --build . --target "${CURRENT_TARGET}" --config
            ${CURRENT_CONFIG} -- -sdk "${sdk}"
    WORKING_DIRECTORY "${CMAKE_BINARY_DIR}"
    RESULT_VARIABLE result)
...
# Remove given architecture from file. This step needed only in rare cases when
# target was built in "unusual" way. Emit warning message.
...
function(_ios_install_combined_detect_associated_sdk corr_sdk_var)
  ...
  if("${SUPPORTED_PLATFORMS}" STREQUAL "")
    _ios_install_combined_get_build_setting(${PLATFORM_NAME}
...
#
# Preconditions: * Target already installed at ${destination} for the
# ${PLATFORM_NAME} platform
#
# This function will: * Run build for the lacking platform, i.e. opposite to the
# ${PLATFORM_NAME} * Fuse both libraries by running lipo
function(ios_install_combined target destination)
  ...
  _ios_install_combined_prune_common_archs("${corr_sdk}" corr_valid_archs
                                           this_valid_archs)

  # Return if there are no valid architectures for the SDK. (note that library
  # already installed)
  ...
  _ios_install_combined_get_build_setting("${corr_sdk}" "EXECUTABLE_PATH"
                                          corr_executable_path)
