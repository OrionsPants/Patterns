...
function(__armclang_set_processor_list lang out_var)
  execute_process(
    COMMAND "${CMAKE_${lang}_COMPILER}"
            --target=${CMAKE_${lang}_COMPILER_TARGET} -mcpu=list
...
function(__armclang_set_arch_list lang out_var)
  execute_process(
    COMMAND "${CMAKE_${lang}_COMPILER}"
            --target=${CMAKE_${lang}_COMPILER_TARGET} -march=list
...
function(__armlink_set_cpu_list lang out_var)
  if(__CMAKE_ARMClang_USING_armlink)
    ...
    set(__linker_wrapper_flags --target=${CMAKE_${lang}_COMPILER_TARGET}
                               -Xlinker)
  endif()

  execute_process(
    COMMAND "${CMAKE_LINKER}" ${__linker_wrapper_flags} --cpu=list
...
macro(__compiler_armclang lang)
  ...
  # CMAKE_SYSTEM_PROCESSOR and CMAKE_SYSTEM_ARCH are not sufficient because they
  # provide no information of additional CPU features needed in
  # `-mcpu=<name>[+[no]<feature>+...]`. The automatic setting of compile and
  # link options is deprecated and projects should specify their own.
  ...
  if(NOT "x${CMAKE_ARMClang_CMP0123}x" STREQUAL "xNEWx")
    ...
    if(NOT CMAKE_SYSTEM_PROCESSOR AND NOT CMAKE_SYSTEM_ARCH)
      message(
        FATAL_ERROR
          "  CMAKE_SYSTEM_PROCESSOR or CMAKE_SYSTEM_ARCH must be set for ARMClang\n"
          "  Supported processor: ${CMAKE_${lang}_COMPILER_PROCESSOR_LIST}\n"
          "  Supported Architecture: ${CMAKE_${lang}_COMPILER_ARCH_LIST}")
    else()
      __armclang_check_processor(
        "${CMAKE_SYSTEM_ARCH}" "${CMAKE_${lang}_COMPILER_ARCH_LIST}"
        _CMAKE_${lang}_CHECK_ARCH_RESULT)
      if(_CMAKE_${lang}_CHECK_ARCH_RESULT)
        string(APPEND CMAKE_${lang}_FLAGS_INIT " -march=${CMAKE_SYSTEM_ARCH}")
        set(__march_flag_set TRUE)
      endif()
      __armclang_check_processor(
        "${CMAKE_SYSTEM_PROCESSOR}" "${CMAKE_${lang}_COMPILER_PROCESSOR_LIST}"
        _CMAKE_${lang}_CHECK_PROCESSOR_RESULT)
      ...
      if(NOT __march_flag_set AND NOT __mcpu_flag_set)
        message(
          FATAL_ERROR
            "At least one of the variables CMAKE_SYSTEM_PROCESSOR or CMAKE_SYSTEM_ARCH must be set for ARMClang\n"
            "Supported processor: ${CMAKE_${lang}_COMPILER_PROCESSOR_LIST}\n"
            "  Supported Architecture: ${CMAKE_${lang}_COMPILER_ARCH_LIST}")
      endif()
    ...
    __armclang_check_processor(
      "${CMAKE_SYSTEM_PROCESSOR}" "${CMAKE_LINKER_CPU_LIST}"
      _CMAKE_CHECK_LINK_CPU_RESULT)
  ...
  set(CMAKE_${lang}_LINK_EXECUTABLE
      "<CMAKE_LINKER> <CMAKE_${lang}_LINK_FLAGS> <LINK_FLAGS> <LINK_LIBRARIES> <OBJECTS> -o <TARGET> ${__CMAKE_ARMClang_USING_armlink_WRAPPER} --list=<TARGET_BASE>.map"
  )
  set(CMAKE_${lang}_CREATE_STATIC_LIBRARY
      "<CMAKE_AR> --create -cr <TARGET> <LINK_FLAGS> <OBJECTS>")
  set(CMAKE_${lang}_ARCHIVE_CREATE
      "<CMAKE_AR> --create -cr <TARGET> <LINK_FLAGS> <OBJECTS>")
  set(CMAKE_${lang}_RESPONSE_FILE_LINK_FLAG
      "${__CMAKE_ARMClang_USING_armlink_WRAPPER} --via=")
