...
# Setup variables used for Green Hills MULTI generator -- Allow users to
# override these values.

if(CMAKE_GENERATOR MATCHES "Green Hills MULTI")

  # Set the project primaryTarget value If not set then primaryTarget will be
  # determined by the generator
  if((NOT DEFINED GHS_PRIMARY_TARGET) OR (DEFINED CACHE{GHS_PRIMARY_TARGET}))
    set(GHS_PRIMARY_TARGET
        "IGNORE"
        CACHE STRING "GHS MULTI primaryTarget")
  ...
  if((NOT DEFINED GHS_TOOLSET_ROOT) OR (DEFINED CACHE{GHS_TOOLSET_ROOT}))
    ...
    set(GHS_TOOLSET_ROOT
        "${_ts_root}"
        CACHE PATH "GHS platform toolset root directory")
  ...
  if((NOT DEFINED GHS_CUSTOMIZATION) OR (DEFINED CACHE{GHS_CUSTOMIZATION}))
    set(GHS_CUSTOMIZATION
        ""
        CACHE FILEPATH "optional GHS customization")
  ...
  if((NOT DEFINED GHS_GPJ_MACROS) OR (DEFINED CACHE{GHS_GPJ_MACROS}))
    set(GHS_GPJ_MACROS
        ""
        CACHE STRING
              "optional GHS macros generated in the .gpjs for legacy reasons")
    mark_as_advanced(GHS_GPJ_MACROS)
  endif()

endif()

# If project primaryTarget not set then set target platform name. -- May be used
# by the generator when determining the primaryTarget.
if(NOT GHS_PRIMARY_TARGET)
  if((NOT DEFINED GHS_TARGET_PLATFORM) OR (DEFINED CACHE{GHS_TARGET_PLATFORM}))
    set(GHS_TARGET_PLATFORM
        "integrity"
        CACHE STRING "GHS MULTI target platform")
...
if((NOT DEFINED GHS_OS_ROOT) OR (DEFINED CACHE{GHS_OS_ROOT}))
  ...
  set(GHS_OS_ROOT
      "${_os_root}"
      CACHE PATH "GHS platform OS search root directory")
  unset(_os_root)
  mark_as_advanced(GHS_OS_ROOT)
endif()

# Search for GHS_OS_DIR if not set by user and is known to be required
if(GHS_PRIMARY_TARGET MATCHES "integrity" OR GHS_TARGET_PLATFORM MATCHES
                                             "integrity")
...
if(_ghs_os_dir AND NOT DEFINED GHS_OS_DIR)
  if(EXISTS ${GHS_OS_ROOT})
    ...
    file(
      GLOB GHS_CANDIDATE_OS_DIRS
      LIST_DIRECTORIES true
      RELATIVE ${GHS_OS_ROOT}
      ${GHS_OS_ROOT}/*)
    file(
      GLOB GHS_CANDIDATE_OS_FILES
