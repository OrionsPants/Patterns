...
function(_XercesC_GET_VERSION version_hdr)
  file(STRINGS ${version_hdr} _contents
       REGEX "^[ \t]*#define XERCES_VERSION_.*")
  if(_contents)
    string(REGEX REPLACE ".*#define XERCES_VERSION_MAJOR[ \t]+([0-9]+).*" "\\1"
                         XercesC_MAJOR "${_contents}")
    string(REGEX REPLACE ".*#define XERCES_VERSION_MINOR[ \t]+([0-9]+).*" "\\1"
                         XercesC_MINOR "${_contents}")
    string(REGEX REPLACE ".*#define XERCES_VERSION_REVISION[ \t]+([0-9]+).*"
                         "\\1" XercesC_PATCH "${_contents}")
    ...
    set(XercesC_VERSION
        ...
        PARENT_SCOPE)
    set(XercesC_VERSION_MAJOR
        "${XercesC_MAJOR}"
        PARENT_SCOPE)
    set(XercesC_VERSION_MINOR
        "${XercesC_MINOR}"
        PARENT_SCOPE)
    set(XercesC_VERSION_PATCH
        "${XercesC_PATCH}"
        PARENT_SCOPE)
  else()
    message(
...
if(XercesC_INCLUDE_DIR
   AND EXISTS "${XercesC_INCLUDE_DIR}/xercesc/util/XercesVersion.hpp")
...
if(NOT XercesC_LIBRARY)
  ...
  find_library(
    ...
    NAMES "xerces-cd" "xerces-c_${XercesC_VERSION_MAJOR}D"
  ...
  if(NOT TARGET XercesC::XercesC)
    ...
    if(XercesC_INCLUDE_DIRS)
      set_target_properties(
        XercesC::XercesC PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                    "${XercesC_INCLUDE_DIRS}")
    endif()
    if(EXISTS "${XercesC_LIBRARY}")
      set_target_properties(
        XercesC::XercesC PROPERTIES IMPORTED_LINK_INTERFACE_LANGUAGES "CXX"
