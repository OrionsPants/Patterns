...
get_property(
  _fsPkgTypeIsSet GLOBAL
  PROPERTY FeatureSummary_PKG_TYPES
  SET)
if(NOT _fsPkgTypeIsSet)
  set_property(GLOBAL PROPERTY FeatureSummary_PKG_TYPES RUNTIME OPTIONAL
                               RECOMMENDED REQUIRED)
endif()

get_property(
  _fsReqPkgTypesIsSet GLOBAL
  PROPERTY FeatureSummary_REQUIRED_PKG_TYPES
  SET)
...
get_property(
  _fsDefaultPkgTypeIsSet GLOBAL
  PROPERTY FeatureSummary_DEFAULT_PKG_TYPE
  SET)
...
function(_FS_GET_FEATURE_SUMMARY _property _var _includeQuiet)
  ...
  get_property(_fsDefaultPkgType GLOBAL
               PROPERTY FeatureSummary_DEFAULT_PKG_TYPE)
  ...
  foreach(_currentFeature ${_EnabledFeatures})
    ...
    if(NOT _currentType)
      ...
      if("${_defaultInPkgTypes}" STREQUAL "-1")
        string(REGEX REPLACE ";([^;]+)$" " and \\1" _fsPkgTypes_msg
                             "${_fsPkgTypes}")
        string(REPLACE ";" ", " _fsPkgTypes_msg "${_fsPkgTypes_msg}")
        message(
          FATAL_ERROR
            "Bad package property type ${_fsDefaultPkgType} used in global property FeatureSummary_DEFAULT_PKG_TYPE. "
    ...
    if("${_type}" STREQUAL ANY OR "${_type}" STREQUAL "${_currentType}")
      ...
      get_property(_fsReqPkgTypes GLOBAL
                   PROPERTY FeatureSummary_REQUIRED_PKG_TYPES)
      ...
      get_property(_isTransitiveDepend GLOBAL
                   PROPERTY _CMAKE_${_currentFeature}_TRANSITIVE_DEPENDENCY)
      ...
      if(includeThisOne)
        ...
        get_property(_info GLOBAL
                     PROPERTY _CMAKE_${_currentFeature}_REQUIRED_VERSION)
        if(_info)
          string(APPEND _currentFeatureText " (required version ${_info})")
        endif()
        get_property(_info GLOBAL
                     PROPERTY _CMAKE_${_currentFeature}_DESCRIPTION)
        if(_info)
...
function(FEATURE_SUMMARY)
  ...
  set(options APPEND INCLUDE_QUIET_PACKAGES FATAL_ON_MISSING_REQUIRED_PACKAGES
              QUIET_ON_EMPTY DEFAULT_DESCRIPTION)
  set(oneValueArgs FILENAME VAR DESCRIPTION)
  set(multiValueArgs WHAT)

  cmake_parse_arguments(_FS "${options}" "${oneValueArgs}" "${multiValueArgs}"
                        ${_FIRST_ARG} ${ARGN})

  if(_FS_UNPARSED_ARGUMENTS)
    message(
      FATAL_ERROR
        "Unknown keywords given to FEATURE_SUMMARY(): \"${_FS_UNPARSED_ARGUMENTS}\""
    )
  endif()
  ...
  if(NOT _FS_WHAT)
    message(
      FATAL_ERROR
        "The call to FEATURE_SUMMARY() doesn't set the required WHAT argument.")
  endif()

  if(_FS_DEFAULT_DESCRIPTION AND DEFINED _FS_DESCRIPTION)
    message(
      WARNING "DEFAULT_DESCRIPTION option discarded since DESCRIPTION is set.")
    set(_FS_DEFAULT_DESCRIPTION 0)
  endif()

  set(validWhatParts "ENABLED_FEATURES" "DISABLED_FEATURES" "PACKAGES_FOUND"
                     "PACKAGES_NOT_FOUND")
  ...
  foreach(_fsPkgType ${_fsPkgTypes})
    ...
    get_property(
      _fsPkgTypeDescriptionIsSet GLOBAL
      PROPERTY FeatureSummary_${_fsPkgType}_DESCRIPTION
      SET)
    if(_fsPkgTypeDescriptionIsSet)
      get_property(_fsPkgTypeDescription GLOBAL
                   PROPERTY FeatureSummary_${_fsPkgType}_DESCRIPTION)
    endif()
    set(title_${_fsPkgType}_PACKAGES_FOUND
        "The following ${_fsPkgTypeDescription} have been found:")
    set(title_${_fsPkgType}_PACKAGES_NOT_FOUND
        "The following ${_fsPkgTypeDescription} have not been found:")
  endforeach()
      ...
      foreach(part ${_FS_WHAT})
        ...
        if(NOT "${indexInList}" STREQUAL "-1")
          ...
          if("${part}" STREQUAL "ALL")
            message(
              FATAL_ERROR
                "The WHAT argument of FEATURE_SUMMARY() contains ALL, which cannot be combined with other values."
            )
          else()
            message(
              FATAL_ERROR
                "The WHAT argument of FEATURE_SUMMARY() contains ${part}, which is not a valid value."
            )
          endif()
        endif()
      endforeach()
...
function(SET_PACKAGE_PROPERTIES _name _props)
  if(NOT "${_props}" STREQUAL "PROPERTIES")
    message(
      FATAL_ERROR
        "PROPERTIES keyword is missing in SET_PACKAGE_PROPERTIES() call.")
  endif()
  ...
  cmake_parse_arguments(_SPP "${options}" "${oneValueArgs}" "${multiValueArgs}"
                        ${ARGN})

  if(_SPP_UNPARSED_ARGUMENTS)
    message(
      FATAL_ERROR
        "Unknown keywords given to SET_PACKAGE_PROPERTIES(): \"${_SPP_UNPARSED_ARGUMENTS}\""
    )
  endif()
  ...
  if(_SPP_DESCRIPTION)
    ...
    if(_info AND NOT "${_info}" STREQUAL "${_SPP_DESCRIPTION}")
      message(
        STATUS
          "Warning: Property DESCRIPTION for package ${_name} already set to \"${_info}\", overriding it with \"${_SPP_DESCRIPTION}\""
      )
    endif()
  ...
  if(_SPP_URL)
    ...
    if(_info AND NOT "${_info}" STREQUAL "${_SPP_URL}")
      message(
        STATUS
          "Warning: Property URL already set to \"${_info}\", overriding it with \"${_SPP_URL}\""
      )
    endif()
  ...
  # handle the PURPOSE: use APPEND, since there can be multiple purposes for one
  # package inside a project
  if(_SPP_PURPOSE)
  ...
  if(DEFINED _SPP_TYPE)
    # Supported types are listed in FeatureSummary_PKG_TYPES according to their
    # priority
    get_property(_fsPkgTypes GLOBAL PROPERTY FeatureSummary_PKG_TYPES)
    ...
    if("${_typeIndexInList}" STREQUAL "-1")
      ...
      message(
        FATAL_ERROR
          "Bad package property type ${_SPP_TYPE} used in SET_PACKAGE_PROPERTIES(). "
          "Valid types are ${_fsPkgTypes_msg}.")
    endif()
...
function(SET_FEATURE_INFO)
  message(
    DEPRECATION "SET_FEATURE_INFO is deprecated. Use ADD_FEATURE_INFO instead.")
