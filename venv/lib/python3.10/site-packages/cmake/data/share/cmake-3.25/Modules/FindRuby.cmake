...
foreach(UPPER RUBY_EXECUTABLE RUBY_LIBRARY RUBY_INCLUDE_DIR
...
# 'printf("%s",Config::CONFIG@<:@"sitelibdir"@:>@)'` Ruby_LIBDIR=`$RUBY -r
# rbconfig -e 'printf("%s",Config::CONFIG@<:@"libdir"@:>@)'`
...
# Determine the list of possible names of the ruby executable depending on which
# version of ruby is required
...
# Set name of possible executables, ignoring the minor Eg: 2.1.1 => from ruby31
# to ruby21 included 2.1   => from ruby31 to ruby21 included 2     => from
# ruby31 to ruby20 included empty => from ruby31 to ruby18 included
if(NOT Ruby_FIND_VERSION_EXACT)
  ...
  foreach(_ruby_version RANGE 31 18 -1)
    ...
    if(NOT "${_ruby_major_version}${_ruby_minor_version}" VERSION_LESS
...
if(DEFINED ENV{MY_RUBY_HOME})
  ...
  if(DEFINED Ruby_FIND_VIRTUALENV)
    if(NOT Ruby_FIND_VIRTUALENV MATCHES "^(FIRST|ONLY|STANDARD)$")
      message(
        AUTHOR_WARNING
          "FindRuby: ${Ruby_FIND_VIRTUALENV}: invalid value for 'Ruby_FIND_VIRTUALENV'. 'FIRST', 'ONLY' or 'STANDARD' expected. 'FIRST' will be used instead."
      )
  ...
  if(DEFINED Ruby_FIND_VIRTUALENV)
    message(
      "Environment variable MY_RUBY_HOME isn't set, defaulting back to Ruby_FIND_VIRTUALENV=STANDARD"
    )
  endif()
...
function(_RUBY_VALIDATE_INTERPRETER)
  ...
  if(_RVI_CHECK_EXISTS AND NOT EXISTS "${Ruby_EXECUTABLE}")
    ...
    set(_Ruby_Interpreter_REASON_FAILURE
        "Cannot find the interpreter \"${Ruby_EXECUTABLE}\"")
    set_property(CACHE Ruby_EXECUTABLE PROPERTY VALUE
                                                "Ruby_EXECUTABLE-NOTFOUND")
    return()
  endif()

  # Check the version it returns executable found must have a specific version
  execute_process(
  ...
  if(result
     OR (_RVI_EXACT AND NOT version VERSION_EQUAL expected_version)
     OR (version VERSION_LESS expected_version))
    # interpreter not usable or has wrong major version
    if(result)
      set(_Ruby_Interpreter_REASON_FAILURE
          "Cannot use the interpreter \"${Ruby_EXECUTABLE}\"")
    else()
      set(_Ruby_Interpreter_REASON_FAILURE
          "Wrong major version for the interpreter \"${Ruby_EXECUTABLE}\"")
    endif()
    set_property(CACHE Ruby_EXECUTABLE PROPERTY VALUE
                                                "Ruby_EXECUTABLE-NOTFOUND")
    return()
...
while(1)
  ...
  if(_Ruby_FIND_VIRTUALENV MATCHES "^(FIRST|ONLY)$")
    ...
    find_program(
      Ruby_EXECUTABLE
      NAMES ${_Ruby_POSSIBLE_EXECUTABLE_NAMES} NAMES_PER_DIR
  ...
  if(Ruby_EXECUTABLE)
    ...
    if(NOT _Ruby_POSSIBLE_EXECUTABLE_NAMES)
      ...
      # Otherwise, continue with the remaining list. Make sure that we clear the
      # cached variable.
...
if(Ruby_EXECUTABLE AND NOT Ruby_VERSION_MAJOR)
  function(_RUBY_CONFIG_VAR RBVAR OUTVAR)
    execute_process(
      COMMAND ${Ruby_EXECUTABLE} -r rbconfig -e
              "print RbConfig::CONFIG['${RBVAR}']"
  ...
  execute_process(
    COMMAND ${Ruby_EXECUTABLE} -r vendor-specific -e "print 'true'"
    OUTPUT_VARIABLE Ruby_HAS_VENDOR_RUBY
    ERROR_QUIET)
  ...
  # again
  set(Ruby_VERSION_MAJOR
      ${Ruby_VERSION_MAJOR}
      CACHE PATH "The Ruby major version" FORCE)
  set(Ruby_VERSION_MINOR
      ${Ruby_VERSION_MINOR}
      CACHE PATH "The Ruby minor version" FORCE)
  set(Ruby_VERSION_PATCH
      ${Ruby_VERSION_PATCH}
      CACHE PATH "The Ruby patch version" FORCE)
  set(Ruby_ARCH_DIR
      ${Ruby_ARCH_DIR}
      CACHE PATH "The Ruby arch dir" FORCE)
  set(Ruby_HDR_DIR
      ${Ruby_HDR_DIR}
      CACHE PATH "The Ruby header dir (1.9+)" FORCE)
  set(Ruby_ARCHHDR_DIR
      ${Ruby_ARCHHDR_DIR}
      CACHE PATH "The Ruby arch header dir (2.0+)" FORCE)
  set(Ruby_POSSIBLE_LIB_DIR
      ${Ruby_POSSIBLE_LIB_DIR}
      CACHE PATH "The Ruby lib dir" FORCE)
  set(Ruby_RUBY_LIB_DIR
      ${Ruby_RUBY_LIB_DIR}
      CACHE PATH "The Ruby ruby-lib dir" FORCE)
  set(Ruby_SITEARCH_DIR
      ${Ruby_SITEARCH_DIR}
      CACHE PATH "The Ruby site arch dir" FORCE)
  set(Ruby_SITELIB_DIR
      ${Ruby_SITELIB_DIR}
      CACHE PATH "The Ruby site lib dir" FORCE)
  set(Ruby_HAS_VENDOR_RUBY
      ${Ruby_HAS_VENDOR_RUBY}
      CACHE BOOL "Vendor Ruby is available" FORCE)
  set(Ruby_VENDORARCH_DIR
      ${Ruby_VENDORARCH_DIR}
      CACHE PATH "The Ruby vendor arch dir" FORCE)
  set(Ruby_VENDORLIB_DIR
      ${Ruby_VENDORLIB_DIR}
      CACHE PATH "The Ruby vendor lib dir" FORCE)
...
# In case Ruby_EXECUTABLE could not be executed (e.g. cross compiling) try to
# detect which version we found. This is not too good.
if(Ruby_EXECUTABLE AND NOT Ruby_VERSION_MAJOR)
  ...
  if(${Ruby_EXECUTABLE} MATCHES "ruby2")
    ...
    string(REGEX_REPLACE ${Ruby_EXECUTABLE} "ruby2\\.?([0-7])" "\\1"
           Ruby_VERSION_MINOR)
  endif()
  # check whether we found 3.[0-1].x
  if(${Ruby_EXECUTABLE} MATCHES "ruby3")
    set(Ruby_VERSION_MAJOR 3)
    string(REGEX_REPLACE ${Ruby_EXECUTABLE} "ruby3\\.?([0-1])" "\\1"
           Ruby_VERSION_MINOR)
  endif()
...
if(Ruby_VERSION_MAJOR)
  set(Ruby_VERSION
      "${Ruby_VERSION_MAJOR}.${Ruby_VERSION_MINOR}.${Ruby_VERSION_PATCH}")
  set(_Ruby_VERSION_SHORT "${Ruby_VERSION_MAJOR}.${Ruby_VERSION_MINOR}")
  set(_Ruby_VERSION_SHORT_NODOT "${Ruby_VERSION_MAJOR}${Ruby_VERSION_MINOR}")
  set(_Ruby_NODOT_VERSION
      "${Ruby_VERSION_MAJOR}${Ruby_VERSION_MINOR}${Ruby_VERSION_PATCH}")
  set(_Ruby_NODOT_VERSION_ZERO_PATCH
      "${Ruby_VERSION_MAJOR}${Ruby_VERSION_MINOR}0")
endif()

# FIXME: Currently we require both the interpreter and development components to
# be found in order to use either.  See issue #20474.
find_path(
  Ruby_INCLUDE_DIR
  NAMES ruby.h
  HINTS ${Ruby_HDR_DIR} ${Ruby_ARCH_DIR}
        /usr/lib/ruby/${_Ruby_VERSION_SHORT}/i586-linux-gnu/)

set(Ruby_INCLUDE_DIRS ${Ruby_INCLUDE_DIR})

# if ruby > 1.8 is required or if ruby > 1.8 was found, search for the config.h
# dir
if(Ruby_FIND_VERSION VERSION_GREATER_EQUAL "1.9"
   OR Ruby_VERSION VERSION_GREATER_EQUAL "1.9"
   OR Ruby_HDR_DIR)
  find_path(
    Ruby_CONFIG_INCLUDE_DIR
    NAMES ruby/config.h config.h
    HINTS ${Ruby_HDR_DIR}/${Ruby_ARCH} ${Ruby_ARCH_DIR} ${Ruby_ARCHHDR_DIR})
...
if(WIN32)
  ...
  set(_Ruby_POSSIBLE_VERSION_SUFFICES
...
if(_Ruby_DEBUG_OUTPUT)
  ...
  message(
    STATUS "_Ruby_POSSIBLE_EXECUTABLE_NAMES: ${_Ruby_POSSIBLE_EXECUTABLE_NAMES}"
  )
  ...
  message(
    STATUS
      "Found Ruby_VERSION: \"${Ruby_VERSION}\" , short: \"${_Ruby_VERSION_SHORT}\", nodot: \"${_Ruby_VERSION_SHORT_NODOT}\""
  )
...
mark_as_advanced(Ruby_EXECUTABLE Ruby_LIBRARY Ruby_INCLUDE_DIR
                 Ruby_CONFIG_INCLUDE_DIR)

# Set some variables for compatibility with previous version of this file (no
# need to provide a CamelCase version of that...)
