...
# Features for LINK_LIBRARY generator expression check linker capabilities
if(NOT DEFINED _CMAKE_LINKER_PUSHPOP_STATE_SUPPORTED)
  ...
  if(__linker_help MATCHES "--push-state" AND __linker_help MATCHES
                                              "--pop-state")
    set(_CMAKE_LINKER_PUSHPOP_STATE_SUPPORTED
        TRUE
        CACHE INTERNAL "linker supports push/pop state")
  else()
    set(_CMAKE_LINKER_PUSHPOP_STATE_SUPPORTED
        FALSE
        CACHE INTERNAL "linker supports push/pop state")
...
# Features for LINK_GROUP generator expression RESCAN: request the linker to
# rescan static libraries until there is no pending undefined symbols
...
macro(__windows_compiler_gnu lang)
  ...
  # Initialize C link type selection flags.  These flags are used when building
  # a shared library, shared module, or executable that links to other libraries
  # to select whether to use the static or shared versions of the libraries.
  ...
  if("${_ver}" MATCHES "\\(GCC\\) 3\\.")
    if("${lang}" STREQUAL "Fortran")
      # The GNU Fortran compiler reports an error: no input files; unwilling to
      # write output files when the response file is passed with "-Wl,@".
  ...
  # Support very long lists of object files. TODO: check for which gcc versions
  # this is still needed, not needed for gcc >= 4.4. Ninja generator doesn't
  # support this work around.
  if("${CMAKE_${lang}_RESPONSE_FILE_LINK_FLAG}" STREQUAL "@"
    ...
    foreach(rule CREATE_SHARED_MODULE CREATE_SHARED_LIBRARY LINK_EXECUTABLE)
      # The gcc/collect2/ld toolchain does not use response files internally so
      # we cannot pass long object lists.  Instead pass the object file list in
      # a response file to the archiver to put them in a temporary archive. Hand
      # the archive to the linker.
      string(
        REPLACE
          "<OBJECTS>"
          "-Wl,--whole-archive <OBJECT_DIR>/objects.a -Wl,--no-whole-archive"
...
macro(__windows_compiler_gnu_abi lang)
  if(CMAKE_NO_GNUtoMS)
    ...
    option(CMAKE_GNUtoMS
           "Convert GNU import libraries to MS format (requires Visual Studio)"
           OFF)
  endif()

  if(CMAKE_GNUtoMS AND NOT CMAKE_GNUtoMS_LIB)
    # Find MS development environment setup script for this architecture. We
    # need to use the MS Librarian tool (lib.exe). Find the most recent version
    # available.

    # Query the VS Installer tool for locations of VS 2017 and above.
    set(_vs_installer_paths "")
    foreach(vs RANGE 17 15 -1) # change the first number to the largest
                               # supported version
    ...
    elseif("${CMAKE_SIZEOF_VOID_P}" EQUAL 8)
      find_program(
        CMAKE_GNUtoMS_VCVARS
        NAMES vcvars64.bat vcvarsamd64.bat
    ...
    if(CMAKE_GNUtoMS_VCVARS)
      ...
      configure_file(${CMAKE_ROOT}/Modules/Platform/GNUtoMS_lib.bat.in
                     ${CMAKE_GNUtoMS_LIB})
    else()
      message(
        WARNING
  ...
  if(CMAKE_GNUtoMS)
    ...
    set(CMAKE_${lang}_GNUtoMS_RULE
        " -Wl,--output-def,<TARGET_NAME>.def"
