...
function(android_add_test_data test_name)
  # As the names suggest, oneValueArgs lists the arguments that specify a single
  # value, while multiValueArgs can contain one or more values.
  set(keywordArgs)
  set(oneValueArgs FILES_DEST LIBS_DEST DEVICE_OBJECT_STORE DEVICE_TEST_DIR)
  set(multiValueArgs FILES LIBS NO_LINK_REGEX)

  # For example, if you called this function with FILES </path/to/file> then
  # this path would be stored in the variable AST_FILES. The AST prefix stands
  # for the name of this function (android_add_test_data).
  cmake_parse_arguments(AST "${keywordArgs}" "${oneValueArgs}"
                        "${multiValueArgs}" ${ARGN})
  if(NOT AST_DEVICE_TEST_DIR)
    message(
      FATAL_ERROR
        "-- You must specify the location of the on device test directory.")
  endif()
  if(NOT AST_DEVICE_OBJECT_STORE)
    message(
      FATAL_ERROR
        "-- You must specify the location of the on device object store.")
  endif()
  ...
  foreach(alg ${hash_algs})
    string(REGEX REPLACE "([^ ;]+)\\.${alg}" "DATA{\\1}" AST_FILES
                         "${AST_FILES}")
  endforeach()
  ...
  if(${data_files_found} GREATER "-1")
    ...
    ExternalData_expand_arguments(${DATA_TARGET_NAME} extern_data_output
                                  ${AST_FILES})
  ...
  foreach(path ${AST_FILES})
    foreach(output ${extern_data_output})
      if(${output} STREQUAL ${path})
        # Check if a destination was specified.  If not, we copy by default into
        # this project's binary directory, preserving its relative path.
        if(AST_${VAR}_DEST)
        ...
        add_custom_command(
          ...
          POST_BUILD DEPENDS ${extern_data_source}
          COMMAND ${CMAKE_COMMAND} -E copy_if_different ${extern_data_source}
                  ${DEST}/${extern_data_basename})
      endif()
    endforeach()
  endforeach()
  ...
  if(ANDROID)
    ...
    add_test(
      NAME ${test_name}
      COMMAND
        ${CMAKE_COMMAND} "-Darg_files_dest=${AST_FILES_DEST}"
        ...
        "-Darg_files=${processed_FILES}" "-Darg_libs=${AST_LIBS}"
        "-Darg_src_dir=${CMAKE_CURRENT_SOURCE_DIR}" -P
        ${CMAKE_CURRENT_FUNCTION_LIST_DIR}/AndroidTestUtilities/PushToAndroidDevice.cmake
    )
  endif()
endfunction()
