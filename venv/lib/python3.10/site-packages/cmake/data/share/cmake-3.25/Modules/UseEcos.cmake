...
if(NOT ECOSCONFIG_EXECUTABLE)
  message(
    SEND_ERROR
      "ecosconfig was not found. Either include it in the system path or set it manually using ccmake."
  )
else()
...
if(NOT EXISTS $ENV{ECOS_REPOSITORY}/ecos.db)
  message(
    SEND_ERROR
      "The environment variable ECOS_REPOSITORY is not set correctly. Set it to the directory which contains the file ecos.db"
  )
else()
...
if(NOT TCL_TCLSH)
  message(
    SEND_ERROR
      "The TCL tclsh was not found. Please install TCL, it is required for building eCos applications."
  )
else()
  message(STATUS "tlcsh found: ${TCL_TCLSH}")
endif()

# add the globale include-diretories usage: ECOS_ADD_INCLUDE_DIRECTORIES()
...
# we want to compile for the xscale processor, in this case the following macro
# has to be called usage: ECOS_USE_ARM_ELF_TOOLS()
...
# since the actual sources are located one level upwards a "../" has to be
# prepended in front of every source file call the following macro to achieve
# this, the first parameter is the name of the new list of source files with
# adjusted paths, followed by all source files usage:
# ECOS_ADJUST_DIRECTORY(adjusted_SRCS ${my_srcs})
macro(ECOS_ADJUST_DIRECTORY _target_FILES)
  foreach(_current_FILE ${ARGN})
    get_filename_component(_abs_FILE ${_current_FILE} ABSOLUTE)
    if(NOT ${_abs_FILE} STREQUAL ${_current_FILE})
      get_filename_component(
        _abs_FILE ${CMAKE_CURRENT_SOURCE_DIR}/../${_current_FILE} ABSOLUTE)
    endif()
...
# creates the dependency from all source files on the ecos target.ld, adds the
# command for compiling ecos
macro(ECOS_ADD_TARGET_LIB)
  ...
  set_source_files_properties(
    ${ARGN} PROPERTIES OBJECT_DEPENDS
                       ${CMAKE_CURRENT_BINARY_DIR}/ecos/install/lib/target.ld)

  add_custom_command(
    OUTPUT ${CMAKE_CURRENT_BINARY_DIR}/ecos/install/lib/target.ld
    COMMAND
      sh -c \"make -C ${CMAKE_CURRENT_BINARY_DIR}/ecos || exit -1\; if [ -e
      ${CMAKE_CURRENT_BINARY_DIR}/ecos/install/lib/target.ld ] \; then touch
      ${CMAKE_CURRENT_BINARY_DIR}/ecos/install/lib/target.ld\; fi\"
    DEPENDS ${CMAKE_CURRENT_BINARY_DIR}/ecos/makefile)

  add_custom_command(
    OUTPUT ${CMAKE_CURRENT_BINARY_DIR}/ecos/makefile
    COMMAND
      sh -c \" cd ${CMAKE_CURRENT_BINARY_DIR}/ecos\; ${ECOSCONFIG_EXECUTABLE}
      --config=${CMAKE_CURRENT_SOURCE_DIR}/ecos/${ECOS_CONFIG_FILE} tree || exit
      -1\;\"
    DEPENDS ${CMAKE_CURRENT_SOURCE_DIR}/ecos/${ECOS_CONFIG_FILE})

  add_custom_target(
    ecos
    make -C ${CMAKE_CURRENT_BINARY_DIR}/ecos/
    DEPENDS ${CMAKE_CURRENT_BINARY_DIR}/ecos/makefile)
endmacro()
...
# macro for creating an executable ecos application the first parameter is the
# name of the executable, the second is the list of all source files (where the
# path has been adjusted beforehand by calling ECOS_ADJUST_DIRECTORY() usage:
# ECOS_ADD_EXECUTABLE(my_app ${adjusted_SRCS})
macro(ECOS_ADD_EXECUTABLE _exe_NAME)
  # definitions, valid for all ecos projects the optimization and "-g" for
  # debugging has to be enabled in the project-specific CMakeLists.txt
  ...
  set(CMAKE_CXX_LINK_EXECUTABLE
      "<CMAKE_CXX_COMPILER> <CMAKE_CXX_LINK_FLAGS> <OBJECTS> -o <TARGET> ${_ecos_EXTRA_LIBS} -nostdlib -nostartfiles -L${CMAKE_CURRENT_BINARY_DIR}/ecos/install/lib -Ttarget.ld ${ECOS_LD_MCPU}"
  )
  set(CMAKE_C_LINK_EXECUTABLE
      "<CMAKE_C_COMPILER> <CMAKE_C_LINK_FLAGS> <OBJECTS> -o <TARGET> ${_ecos_EXTRA_LIBS} -nostdlib -nostartfiles -L${CMAKE_CURRENT_BINARY_DIR}/ecos/install/lib -Ttarget.ld ${ECOS_LD_MCPU}"
  )
  # some strict compiler flags
  set(CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -Wstrict-prototypes")
  set(CMAKE_CXX_FLAGS
      "${CMAKE_CXX_FLAGS} -Woverloaded-virtual -fno-rtti -Wctor-dtor-privacy -fno-strict-aliasing -fno-exceptions"
  )
  ...
  add_custom_command(
    ...
    POST_BUILD
    COMMAND
      ${ECOS_ARCH_PREFIX}objcopy ARGS -O binary
      ${CMAKE_CURRENT_BINARY_DIR}/${_exe_NAME}.elf
  ...
  add_custom_command(
    ...
    POST_BUILD
    COMMAND
      ${ECOS_ARCH_PREFIX}objcopy ARGS -O srec
      ${CMAKE_CURRENT_BINARY_DIR}/${_exe_NAME}.elf
  ...
  add_custom_target(
    ecosclean ${CMAKE_COMMAND} -DECOS_DIR=${CMAKE_CURRENT_BINARY_DIR}/ecos/ -P
              ${ECOS_CMAKE_MODULE_DIR}/ecos_clean.cmake)
  add_custom_target(
    normalclean
  ...
  add_custom_target(
    ...
    COMMAND echo -e \"\\n--- Symbols sorted by address ---\\n\" >
            ${CMAKE_CURRENT_BINARY_DIR}/${_exe_NAME}.lst
    COMMAND
      ${ECOS_ARCH_PREFIX}nm -S -C -n
      ${CMAKE_CURRENT_BINARY_DIR}/${_exe_NAME}.elf >>
      ${CMAKE_CURRENT_BINARY_DIR}/${_exe_NAME}.lst
    COMMAND echo -e \"\\n--- Symbols sorted by size ---\\n\" >>
            ${CMAKE_CURRENT_BINARY_DIR}/${_exe_NAME}.lst
    COMMAND
      ${ECOS_ARCH_PREFIX}nm -S -C -r --size-sort
      ${CMAKE_CURRENT_BINARY_DIR}/${_exe_NAME}.elf >>
      ${CMAKE_CURRENT_BINARY_DIR}/${_exe_NAME}.lst
    COMMAND echo -e \"\\n--- Full assembly listing ---\\n\" >>
            ${CMAKE_CURRENT_BINARY_DIR}/${_exe_NAME}.lst
    COMMAND
      ${ECOS_ARCH_PREFIX}objdump -S -x -d -C
      ${CMAKE_CURRENT_BINARY_DIR}/${_exe_NAME}.elf >>
      ${CMAKE_CURRENT_BINARY_DIR}/${_exe_NAME}.lst)
