...
function(_readFile file)
  ...
  set(_compiler_id_version_compute_${CompilerId}
      ...
      PARENT_SCOPE)
  set(_compiler_id_simulate_${CompilerId}
      ${_compiler_id_simulate}
      PARENT_SCOPE)
  set(_compiler_id_pp_test_${CompilerId}
...
function(compiler_id_detection outvar lang)
  ...
  if(NOT "x${lang}" STREQUAL "xFortran"
     AND NOT "x${lang}" STREQUAL "xCSharp"
  ...
  if(files)
    ...
    cmake_parse_arguments(CID "${options}" "${oneValueArgs}"
                          "${multiValueArgs}" ${ARGN})
    if(CID_UNPARSED_ARGUMENTS)
      message(
        FATAL_ERROR "Unrecognized arguments: \"${CID_UNPARSED_ARGUMENTS}\"")
    endif()

    # Order is relevant here. For example, compilers which pretend to be GCC
    # must appear before the actual GCC.
    if("x${lang}" STREQUAL "xCXX")
      list(APPEND ordered_compilers Comeau)
    endif()
    list(
      APPEND
      ordered_compilers
    ...
    if("x${lang}" STREQUAL "xC")
      list(APPEND ordered_compilers TinyCC Bruce)
    endif()
    list(APPEND ordered_compilers SCO ARMCC AppleClang ARMClang)
    list(
      APPEND
      ordered_compilers
      Clang
      ...
      IAR)
    if("x${lang}" STREQUAL "xC")
      list(APPEND ordered_compilers SDCC)
    endif()
    ...
    if(CID_ID_DEFINE)
      foreach(Id ${ordered_compilers})
        string(APPEND CMAKE_${lang}_COMPILER_ID_CONTENT
               "# define ${CID_PREFIX}COMPILER_IS_${Id} 0\n")
      endforeach()
      # Hard-code definitions for compilers that are no longer supported.
      string(APPEND CMAKE_${lang}_COMPILER_ID_CONTENT
             "# define ${CID_PREFIX}COMPILER_IS_MIPSpro 0\n")
    endif()
   ...
   Version date components:   YYYY=Year, MM=Month,   DD=Day  */\n")
    ...
    foreach(Id ${ordered_compilers})
      ...
      if(CID_ID_STRING)
        ...
        string(APPEND id_content
               "# define ${CID_PREFIX}COMPILER_ID \"${Id}\"${SIMULATE_BLOCK}")
      endif()
      if(CID_ID_DEFINE)
