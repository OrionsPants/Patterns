...
if(QT_QT_LIBRARY)
  if(Qt4_FIND_REQUIRED)
    message(
      FATAL_ERROR
        "Qt3 and Qt4 cannot be used together in one project.  If switching to Qt4, the CMakeCache.txt needs to be cleaned."
    )
  else()
...
macro(_QT4_ADJUST_LIB_VARS _camelCaseBasename)
  ...
  # name would change here, this would lead to the imported Qt4-library targets
  # not being resolved by cmake anymore.
  if(QT_${basename}_LIBRARY_RELEASE OR QT_${basename}_LIBRARY_DEBUG)
    ...
    if(NOT TARGET Qt4::${_camelCaseBasename})
        ...
        if(QT_USE_FRAMEWORKS AND EXISTS ${_location}/${_camelCaseBasename})
          set_property(
            ...
            PROPERTY IMPORTED_LOCATION_RELEASE
                     "${_location}/${_camelCaseBasename}")
        else()
          set_property(TARGET Qt4::${_camelCaseBasename}
                       PROPERTY IMPORTED_LOCATION_RELEASE "${_location}")
        endif()
      endif()
      ...
      if(QT_${basename}_LIBRARY_DEBUG)
        ...
        if(QT_USE_FRAMEWORKS AND EXISTS ${_location}/${_camelCaseBasename})
          ...
          set_property(TARGET Qt4::${_camelCaseBasename}
                       PROPERTY IMPORTED_LOCATION_DEBUG "${_location}")
        endif()
      endif()
      set_property(
        TARGET Qt4::${_camelCaseBasename}
        PROPERTY INTERFACE_INCLUDE_DIRECTORIES "${QT_${basename}_INCLUDE_DIR}")
      string(REGEX REPLACE "^QT" "" _stemname ${basename})
      set_property(TARGET Qt4::${_camelCaseBasename}
                   PROPERTY INTERFACE_COMPILE_DEFINITIONS "QT_${_stemname}_LIB")
    endif()
    ...
    # imported targets will be stored in the dependency files on disk. This
    ...
    if(QT_USE_IMPORTED_TARGETS)
      ...
      if(QT_${basename}_LIBRARY_DEBUG AND QT_${basename}_LIBRARY_RELEASE)
        ...
        if(_isMultiConfig OR CMAKE_BUILD_TYPE)
          set(QT_${basename}_LIBRARY
              optimized ${QT_${basename}_LIBRARY_RELEASE} debug
      ...
      set(QT_${basename}_LIBRARY
          ...
          CACHE STRING "The Qt ${basename} library" FORCE)
...
function(_QT4_QUERY_QMAKE VAR RESULT)
  execute_process(
    COMMAND "${QT_QMAKE_EXECUTABLE}" -query ${VAR}
...
function(_QT4_FIND_QMAKE QMAKE_NAMES QMAKE_RESULT VERSION_RESULT)
  ...
  if(NOT QT_QMAKE_EXECUTABLE OR NOT "${major}" EQUAL 4)
    ...
    if(NOT ${major} EQUAL 4)
      # Restore possibly found qmake and it's version; these are used later in
      # error message if incorrect version is found
      set(QT_QMAKE_EXECUTABLE
          "${curr_qmake}"
          CACHE FILEPATH "" FORCE)
...
if(QT_QMAKE_EXECUTABLE
   AND QTVERSION VERSION_GREATER 3
   AND QTVERSION VERSION_LESS 5)
  ...
  if(Qt5Core_FOUND)
    ...
    # If FindQt4 has already put the Qt 4 path in the cache, the unset() command
    # 'unhides' the (correct) cache variable.
  ...
  # ask qmake for the mkspecs directory we do this first because QT_LIBINFIX
  # might be set
  if(NOT QT_MKSPECS_DIR OR QT_QMAKE_CHANGED)
    _qt4_query_qmake(QMAKE_MKSPECS qt_mkspecs_dirs)
    # do not replace : on windows as it might be a drive letter and windows
    # should already use ; as a separator
    if(NOT WIN32)
  ...
  # ask qmake for the library dir as a hint, then search for QtCore library and
  # use that as a reference for finding the others and for setting
  # QT_LIBRARY_DIR
  if(NOT (QT_QTCORE_LIBRARY_RELEASE OR QT_QTCORE_LIBRARY_DEBUG)
     OR QT_QMAKE_CHANGED)
    ...
    if(NOT QT_QTCORE_LIBRARY_RELEASE AND MSVC)
      if(EXISTS ${QT_LIBRARY_DIR_TMP}/libqtmain.a)
        message(
          FATAL_ERROR
            "It appears you're trying to use Visual Studio with Qt built by MinGW.  Those compilers do not produce code compatible with each other."
        )
      endif()
    endif()
  ...
  if(QT_QTCORE_LIBRARY_RELEASE)
    ...
    set(QT_LIBRARY_DIR
        ${QT_LIBRARY_DIR_TMP}
        CACHE INTERNAL "Qt library dir" FORCE)
  ...
  elseif(QT_QTCORE_LIBRARY_DEBUG)
    ...
    if(NOT Qt4_FIND_QUIETLY)
      message(
        WARNING
          ...
          "\"${QT_LIBRARY_DIR_TMP}\" " "but QtCore could not be found there.  "
    ...
    if(Qt4_FIND_REQUIRED)
      message(
        FATAL_ERROR
          "Could NOT find QtCore. Check ${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/CMakeError.log for more details."
      )
    else()
      return()
    endif()
  ...
  if(QT_LIBRARY_DIR
    ...
    if(QT_QTCORE_INCLUDE_DIR)
      ...
      message(
        "Warning: QT_QMAKE_EXECUTABLE reported QT_INSTALL_HEADERS as ${qt_headers}"
      )
      message(
        "Warning: But QtCore couldn't be found.  Qt must NOT be installed correctly."
      )
    endif()
  endif()
  ...
  if(QT_HEADERS_DIR)
    if(QT_USE_FRAMEWORKS)
      # Qt/Mac frameworks has two include dirs. One is the framework include for
      # which CMake will add a -F flag and the other is an include dir for
      # non-framework Qt modules
  ...
  if(QT_LIBRARY_DIR
    ...
    if(CMAKE_CROSSCOMPILING OR NOT qt_plugins_dir)
      ...
      set(QT_PLUGINS_DIR
          ${qt_plugins_dir}
          CACHE PATH "The location of the Qt plugins")
    endif()
  endif()

  # ask qmake for the translations directory
  if(QT_LIBRARY_DIR
     AND NOT QT_TRANSLATIONS_DIR
     OR QT_QMAKE_CHANGED)
  ...
  if(QT_LIBRARY_DIR
    ...
    if(CMAKE_CROSSCOMPILING OR NOT qt_imports_dir)
      ...
      set(QT_IMPORTS_DIR
          ${qt_imports_dir}
          CACHE PATH "The location of the Qt imports")
    endif()
  endif()

  # Make variables changeable to the advanced user
  mark_as_advanced(QT_LIBRARY_DIR QT_DOC_DIR QT_MKSPECS_DIR QT_PLUGINS_DIR
                   QT_TRANSLATIONS_DIR)
  ...
  if(Q_WS_MAC)
    ...
    check_cxx_symbol_exists(QT_MAC_USE_COCOA "QtCore/qconfig.h"
                            QT_MAC_USE_COCOA)
  endif()
  ...
  foreach(QT_MODULE ${QT_MODULES})
    string(TOUPPER ${QT_MODULE} _upper_qt_module)
    find_path(
      QT_${_upper_qt_module}_INCLUDE_DIR ${QT_MODULE}
      PATHS ${QT_HEADERS_DIR}/${QT_MODULE}
    ...
    if(${QT_MODULE} STREQUAL "phonon")
      find_path(
        QT_${_upper_qt_module}_INCLUDE_DIR phonondefs.h
        PATHS ${QT_HEADERS_DIR}/${QT_MODULE}
              ${QT_LIBRARY_DIR}/${QT_MODULE}.framework/Headers
        NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
    endif()
  ...
  find_path(
    ...
    PATHS ${QT_HEADERS_DIR}/QtDesigner
  ...
  find_path(
    ...
    PATHS ${QT_HEADERS_DIR}/QtAssistant
  ...
  foreach(QT_MODULE ${QT_MODULES})
    string(TOUPPER ${QT_MODULE} _upper_qt_module)
    find_library(
      QT_${_upper_qt_module}_LIBRARY_RELEASE
      NAMES ${QT_MODULE}${QT_LIBINFIX} ${QT_MODULE}${QT_LIBINFIX}4
      PATHS ${QT_LIBRARY_DIR}
      NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
    find_library(
      QT_${_upper_qt_module}_LIBRARY_DEBUG
      NAMES ${QT_MODULE}${QT_LIBINFIX}_debug ${QT_MODULE}${QT_LIBINFIX}d
            ${QT_MODULE}${QT_LIBINFIX}d4
      PATHS ${QT_LIBRARY_DIR}
      NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
    if(QT_${_upper_qt_module}_LIBRARY_RELEASE MATCHES
       "/${QT_MODULE}\\.framework$")
      if(NOT EXISTS "${QT_${_upper_qt_module}_LIBRARY_RELEASE}/${QT_MODULE}")
        ...
        set(QT_${_upper_qt_module}_LIBRARY_RELEASE
            "QT_${_upper_qt_module}_LIBRARY_RELEASE-NOTFOUND"
            CACHE FILEPATH "Path to a library." FORCE)
      endif()
    endif()
    if(QT_${_upper_qt_module}_LIBRARY_DEBUG MATCHES
       "/${QT_MODULE}\\.framework$")
      if(NOT EXISTS "${QT_${_upper_qt_module}_LIBRARY_DEBUG}/${QT_MODULE}")
        ...
        set(QT_${_upper_qt_module}_LIBRARY_DEBUG
            "QT_${_upper_qt_module}_LIBRARY_DEBUG-NOTFOUND"
            CACHE FILEPATH "Path to a library." FORCE)
      endif()
    endif()
  endforeach()
  ...
  find_library(
    ...
    NAMES QtDesignerComponents${QT_LIBINFIX} QtDesignerComponents${QT_LIBINFIX}4
    PATHS ${QT_LIBRARY_DIR}
    NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
  find_library(
    QT_QTDESIGNERCOMPONENTS_LIBRARY_DEBUG
    NAMES QtDesignerComponents${QT_LIBINFIX}_debug
          QtDesignerComponents${QT_LIBINFIX}d
          QtDesignerComponents${QT_LIBINFIX}d4
    PATHS ${QT_LIBRARY_DIR}
    NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
  ...
  if(Q_WS_WIN)
    find_library(
      ...
      NAMES qtmain${QT_LIBINFIX}
      PATHS ${QT_LIBRARY_DIR}
      NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
    find_library(
      QT_QTMAIN_LIBRARY_DEBUG
      NAMES qtmain${QT_LIBINFIX}d
      PATHS ${QT_LIBRARY_DIR}
      NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
  endif()
  ...
  find_library(
    ...
    NAMES QtAssistantClient${QT_LIBINFIX} QtAssistantClient${QT_LIBINFIX}4
    PATHS ${QT_LIBRARY_DIR}
    NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
  find_library(
    QT_QTASSISTANTCLIENT_LIBRARY_DEBUG
    NAMES QtAssistantClient${QT_LIBINFIX}_debug QtAssistantClient${QT_LIBINFIX}d
          QtAssistantClient${QT_LIBINFIX}d4
    PATHS ${QT_LIBRARY_DIR}
    NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
  ...
  find_library(
    ...
    NAMES QtAssistantClient${QT_LIBINFIX} QtAssistantClient${QT_LIBINFIX}4
          QtAssistant${QT_LIBINFIX} QtAssistant${QT_LIBINFIX}4
    PATHS ${QT_LIBRARY_DIR}
    NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
  find_library(
  ...
  find_library(
    ...
    NAMES QtCLucene${QT_LIBINFIX} QtCLucene${QT_LIBINFIX}4
    PATHS ${QT_LIBRARY_DIR}
    NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
  find_library(
    QT_QTCLUCENE_LIBRARY_DEBUG
    NAMES QtCLucene${QT_LIBINFIX}_debug QtCLucene${QT_LIBINFIX}d
          QtCLucene${QT_LIBINFIX}d4
    PATHS ${QT_LIBRARY_DIR}
    NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
  if(Q_WS_MAC
     AND QT_QTCORE_LIBRARY_RELEASE
     AND NOT QT_QTCLUCENE_LIBRARY_RELEASE)
    find_library(
      QT_QTCLUCENE_LIBRARY_RELEASE
      NAMES QtCLucene${QT_LIBINFIX}
      PATHS ${QT_LIBRARY_DIR})
  endif()
  ...
  macro(_qt4_add_target_depends_internal _QT_MODULE _PROPERTY)
    if(TARGET Qt4::${_QT_MODULE})
      foreach(_DEPEND ${ARGN})
        ...
        if(_VALID_DEPENDS)
          set_property(
            TARGET Qt4::${_QT_MODULE}
            APPEND
            PROPERTY ${_PROPERTY} "${_VALID_DEPENDS}")
        endif()
        set(_VALID_DEPENDS)
      endforeach()
    endif()
  ...
  macro(_qt4_add_target_depends _QT_MODULE)
    if(TARGET Qt4::${_QT_MODULE})
      ...
      foreach(_config ${_configs})
        _qt4_add_target_depends_internal(
          ${_QT_MODULE} IMPORTED_LINK_INTERFACE_LIBRARIES_${_config} ${ARGN})
      endforeach()
      set(_configs)
    endif()
  endmacro()
  ...
  macro(_qt4_add_target_private_depends _QT_MODULE)
    if(TARGET Qt4::${_QT_MODULE})
      ...
      foreach(_config ${_configs})
        _qt4_add_target_depends_internal(
          ${_QT_MODULE} IMPORTED_LINK_DEPENDENT_LIBRARIES_${_config} ${ARGN})
      endforeach()
      set(_configs)
    endif()
  endmacro()

  # Set QT_xyz_LIBRARY variable and add library include path to QT_INCLUDES
  _qt4_adjust_lib_vars(QtCore)
  set_property(
    TARGET Qt4::QtCore
    APPEND
    PROPERTY INTERFACE_INCLUDE_DIRECTORIES "${QT_MKSPECS_DIR}/default"
             ${QT_INCLUDE_DIR})
  set_property(
    TARGET Qt4::QtCore
    APPEND
  ...
  set_property(TARGET Qt4::QtCore PROPERTY INTERFACE_QT_MAJOR_VERSION 4)
  set_property(
    TARGET Qt4::QtCore
    APPEND
    PROPERTY COMPATIBLE_INTERFACE_STRING QT_MAJOR_VERSION)
  ...
  if(Q_WS_WIN)
    ...
    if(QT_QAXSERVER_FOUND)
      set_property(TARGET Qt4::QAxServer PROPERTY INTERFACE_QT4_NO_LINK_QTMAIN
                                                  ON)
      set_property(
        TARGET Qt4::QAxServer
        APPEND
        PROPERTY COMPATIBLE_INTERFACE_BOOL QT4_NO_LINK_QTMAIN)
    endif()

    _qt4_adjust_lib_vars(QAxContainer)
  endif()

  # Only public dependencies are listed here. Eg, QtDBus links to QtXml, but
  # users of QtDBus do not need to link to QtXml because QtDBus only uses it
  # internally, not in public headers. Everything depends on QtCore, but that is
  ...
  if(TARGET Qt4::Qt3Support)
    ...
    set_property(
      TARGET Qt4::Qt3Support
      APPEND
      PROPERTY INTERFACE_COMPILE_DEFINITIONS QT3_SUPPORT)
  endif()
  ...
  # ############################################################################
  #
  # Check the executables of Qt ( moc, uic, rcc )
  #
  # ############################################################################
  ...
  _find_qt4_program(QT_LUPDATE_EXECUTABLE Qt4::lupdate lupdate-qt4 lupdate4
                    lupdate)
  _find_qt4_program(QT_LRELEASE_EXECUTABLE Qt4::lrelease lrelease-qt4 lrelease4
                    lrelease)
  _find_qt4_program(
    QT_QCOLLECTIONGENERATOR_EXECUTABLE Qt4::qcollectiongenerator
    qcollectiongenerator-qt4 qcollectiongenerator)
  _find_qt4_program(QT_DESIGNER_EXECUTABLE Qt4::designer designer-qt4 designer4
                    designer)
  _find_qt4_program(QT_LINGUIST_EXECUTABLE Qt4::linguist linguist-qt4 linguist4
                    linguist)

  if(NOT TARGET Qt4::qmake)
    add_executable(Qt4::qmake IMPORTED)
    set_property(TARGET Qt4::qmake PROPERTY IMPORTED_LOCATION
                                            ${QT_QMAKE_EXECUTABLE})
  endif()
  ...
  set(QT_QTGUI_PLUGINS
      qtaccessiblewidgets
      ${QT_IMAGEFORMATS_PLUGINS}
      ${QT_DECORATIONS_PLUGINS}
      ${QT_GRAPHICSDRIVERS_PLUGINS}
      ${QT_GRAPHICSSYSTEMS_PLUGINS}
      ${QT_INPUTMETHODS_PLUGINS}
  ...
  # find_library works better than find_file but we need to set prefixes to only
  # match plugins
  foreach(QT_PLUGIN_TYPE ${QT_PLUGIN_TYPES})
    string(TOUPPER ${QT_PLUGIN_TYPE} _upper_qt_plugin_type)
    set(QT_${_upper_qt_plugin_type}_PLUGINS_DIR
    ...
    foreach(QT_PLUGIN ${QT_${_upper_qt_plugin_type}_PLUGINS})
      ...
      if(QT_IS_STATIC)
        find_library(
          QT_${_upper_qt_plugin}_LIBRARY_RELEASE
          NAMES ${QT_PLUGIN}${QT_LIBINFIX} ${QT_PLUGIN}${QT_LIBINFIX}4
          PATHS ${QT_${_upper_qt_plugin_type}_PLUGINS_DIR}
          NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
        find_library(
          QT_${_upper_qt_plugin}_LIBRARY_DEBUG
          NAMES ${QT_PLUGIN}${QT_LIBINFIX}_debug ${QT_PLUGIN}${QT_LIBINFIX}d
                ${QT_PLUGIN}${QT_LIBINFIX}d4
          PATHS ${QT_${_upper_qt_plugin_type}_PLUGINS_DIR}
          NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
        _qt4_adjust_lib_vars(${QT_PLUGIN})
      else()
        # find_library works easier/better than find_file but we need to set
        # suffixes to only match plugins
        ...
        find_library(
          QT_${_upper_qt_plugin}_PLUGIN_RELEASE
          NAMES ${QT_PLUGIN}${QT_LIBINFIX} ${QT_PLUGIN}${QT_LIBINFIX}4
          PATHS ${QT_${_upper_qt_plugin_type}_PLUGINS_DIR}
          NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
        find_library(
          ...
          PATHS ${QT_${_upper_qt_plugin_type}_PLUGINS_DIR}
          NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
        mark_as_advanced(QT_${_upper_qt_plugin}_PLUGIN_RELEASE
                         QT_${_upper_qt_plugin}_PLUGIN_DEBUG)
...
if(Qt4_FIND_COMPONENTS)
  ...
  # if components specified in find_package(), make sure each of those pieces
  # were found
  set(_QT4_FOUND_REQUIRED_VARS QT_QMAKE_EXECUTABLE QT_MOC_EXECUTABLE
                               QT_RCC_EXECUTABLE QT_INCLUDE_DIR QT_LIBRARY_DIR)
  foreach(component ${Qt4_FIND_COMPONENTS})
    string(TOUPPER ${component} _COMPONENT)
    if(${_COMPONENT} STREQUAL "QTMAIN")
      if(Q_WS_WIN)
        set(_QT4_FOUND_REQUIRED_VARS ${_QT4_FOUND_REQUIRED_VARS}
                                     QT_${_COMPONENT}_LIBRARY)
      endif()
    else()
      set(_QT4_FOUND_REQUIRED_VARS
          ${_QT4_FOUND_REQUIRED_VARS} QT_${_COMPONENT}_INCLUDE_DIR
          QT_${_COMPONENT}_LIBRARY)
    endif()
  endforeach()
  ...
  # if no components specified, we'll make a default set of required variables
  # to say Qt is found
  set(_QT4_FOUND_REQUIRED_VARS
      QT_QMAKE_EXECUTABLE
      QT_MOC_EXECUTABLE
...
if(NOT QT_VERSION_MAJOR EQUAL 4)
  set(VERSION_MSG
      "Found unsuitable Qt version \"${QTVERSION}\" from ${QT_QMAKE_EXECUTABLE}"
  )
  set(Qt4_FOUND FALSE)
  if(Qt4_FIND_REQUIRED)
