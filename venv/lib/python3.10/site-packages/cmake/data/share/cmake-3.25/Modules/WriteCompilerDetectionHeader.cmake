...
function(_load_compiler_variables CompilerId lang)
  include(
    "${CMAKE_ROOT}/Modules/Compiler/${CompilerId}-${lang}-FeatureTests.cmake"
    OPTIONAL)
  set(_cmake_oldestSupported_${CompilerId}
      ${_cmake_oldestSupported}
      PARENT_SCOPE)
  foreach(feature ${ARGN})
    set(_cmake_feature_test_${CompilerId}_${feature}
        ${_cmake_feature_test_${feature}}
        PARENT_SCOPE)
  endforeach()
  include(
    "${CMAKE_ROOT}/Modules/Compiler/${CompilerId}-${lang}-DetermineCompiler.cmake"
    OPTIONAL
    RESULT_VARIABLE determinedCompiler)
  if(NOT determinedCompiler)
    include(
      "${CMAKE_ROOT}/Modules/Compiler/${CompilerId}-DetermineCompiler.cmake"
      OPTIONAL)
  endif()
  set(_compiler_id_version_compute_${CompilerId}
      ${_compiler_id_version_compute}
      PARENT_SCOPE)
endfunction()

macro(_simpledefine FEATURE_NAME FEATURE_TESTNAME FEATURE_STRING
      FEATURE_DEFAULT_STRING)
...
function(write_compiler_detection_header file_keyword file_arg prefix_keyword
         prefix_arg)
  if(NOT "x${file_keyword}" STREQUAL "xFILE")
    message(
      FATAL_ERROR "write_compiler_detection_header: FILE parameter missing.")
  endif()
  if(NOT "x${prefix_keyword}" STREQUAL "xPREFIX")
    message(
      FATAL_ERROR "write_compiler_detection_header: PREFIX parameter missing.")
  endif()
  ...
  cmake_parse_arguments(_WCD "${options}" "${oneValueArgs}" "${multiValueArgs}"
                        ${ARGN})

  if(NOT _WCD_COMPILERS)
    message(
      FATAL_ERROR
        "Invalid arguments.  write_compiler_detection_header requires at least one compiler."
    )
  endif()
  if(NOT _WCD_FEATURES AND NOT _WCD_BARE_FEATURES)
    message(
      FATAL_ERROR
        "Invalid arguments.  write_compiler_detection_header requires at least one feature."
    )
  endif()
  ...
  if(_WCD_VERSION VERSION_LESS _min_version)
    set(err
        "VERSION compatibility for write_compiler_detection_header is set to ${_WCD_VERSION}, which is too low."
    )
    string(APPEND err "  It must be set to at least ${_min_version}.  ")
    string(
      APPEND
      err
      "  Either set the VERSION parameter to the write_compiler_detection_header function, or update"
    )
    string(
      APPEND
      err
      " your minimum required CMake version with the cmake_minimum_required command."
    )
  ...
  if(_WCD_OUTPUT_DIR)
    ...
    if(NOT idx EQUAL 0)
      message(
        FATAL_ERROR
          "The compiler-specific output directory must be within the same directory as the main file."
      )
    endif()
  ...
  set(compilers GNU Clang AppleClang MSVC SunPro Intel)
...
#  error This file may only be included from ${main_file_name}
  ...
  foreach(_lang ${_langs})
    ...
    foreach(compiler ${target_compilers})
      ...
      if(_WCD_OUTPUT_FILES_VAR)
        set(compile_file_name
            "${_WCD_OUTPUT_DIR}${prefix_arg}_COMPILER_INFO_${compiler}_${_lang}.h"
        )
...
#    endif\n")
      ...
      string(CONFIGURE "${_compiler_id_version_compute_${compiler}}"
                       VERSION_BLOCK @ONLY)
...
#  endif\n")
    ...
    foreach(feature ${${_lang}_features})
      ...
      if(feature STREQUAL cxx_static_assert)
        ...
        set(def_fallback
            "enum { ${prefix_arg}_STATIC_ASSERT_JOIN(${prefix_arg}StaticAssertEnum, __LINE__) = sizeof(${prefix_arg}StaticAssert<X>) }"
        )
        string(
          APPEND
          file_content
...
\n")
  ...
  if(_WCD_OUTPUT_FILES_VAR)
    foreach(compiler ${_WCD_COMPILERS})
      foreach(_lang ${_langs})
        if(compiler_file_content_${compiler}_${_lang})
          ...
          string(APPEND CMAKE_CONFIGURABLE_FILE_CONTENT
                 "${compiler_file_content_${compiler}_${_lang}}")
      ...
      endforeach()
    endforeach()
    set(${_WCD_OUTPUT_FILES_VAR}
        ${${_WCD_OUTPUT_FILES_VAR}}
        PARENT_SCOPE)
  endif()
  ...
  configure_file("${CMAKE_ROOT}/Modules/CMakeConfigurableFile.in" "${file_arg}"
                 @ONLY)
endfunction()
