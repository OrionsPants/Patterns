...
function(ctest_coverage_collect_gcov)
  ...
  cmake_parse_arguments(GCOV "${options}" "${oneValueArgs}" "${multiValueArgs}"
                        "" ${ARGN})
  if(NOT DEFINED GCOV_TARBALL)
    message(
      FATAL_ERROR "TARBALL must be specified. for ctest_coverage_collect_gcov")
  endif()
  ...
  elseif(GCOV_TARBALL_COMPRESSION AND NOT GCOV_TARBALL_COMPRESSION MATCHES
                                      "^(GZIP|BZIP2|XZ|ZSTD|FROM_EXT)$")
    message(
      FATAL_ERROR "TARBALL_COMPRESSION must be one of OFF, GZIP, "
  ...
  if(GCOV_GLOB)
    ...
    if(${len} GREATER 0)
      file(
        GLOB_RECURSE lfiles
        RELATIVE ${binary_dir}
        "${binary_dir}/Labels.json")
      list(APPEND gcda_files ${gfiles})
      list(APPEND label_files ${lfiles})
    endif()
  else()
    # look for gcda files in the target directories this will be faster and only
    # look where the files will be
  ...
  # run gcov, this will produce the .gcov files in the current working directory
  if(NOT DEFINED GCOV_GCOV_OPTIONS)
    set(GCOV_GCOV_OPTIONS -b -x)
  endif()
  if(GCOV_QUIET)
    set(coverage_out_opts OUTPUT_QUIET ERROR_QUIET)
  else()
    set(coverage_out_opts OUTPUT_FILE "${coverage_dir}/gcov.log" ERROR_FILE
                          "${coverage_dir}/gcov.log")
  endif()
  execute_process(
    COMMAND ${gcov_command} ${GCOV_GCOV_OPTIONS} ${gcda_files}
    RESULT_VARIABLE res
    WORKING_DIRECTORY ${coverage_dir} ${coverage_out_opts})
  ...
  if(NOT "${res}" EQUAL 0)
    if(NOT GCOV_QUIET)
      message(
        STATUS "Error running gcov: ${res}, see\n  ${coverage_dir}/gcov.log")
    endif()
  endif()
  # create json file with project information
  file(
    WRITE ${coverage_dir}/data.json
    "{
    \"Source\": \"${source_dir}\",
    \"Binary\": \"${binary_dir}\"
}")
  ...
  file(
    GLOB_RECURSE unfiltered_gcov_files
    RELATIVE ${binary_dir}
    "${coverage_dir}/*.gcov")

  # if CTEST_EXTRA_COVERAGE_GLOB was specified we search for files that might be
  # uncovered
  ...
  foreach(gcov_file ${unfiltered_gcov_files})
    file(
      STRINGS ${binary_dir}/${gcov_file} first_line
      LIMIT_COUNT 1
      ENCODING UTF-8)
    ...
    elseif(NOT GCOV_QUIET)
      message(
        STATUS "Could not determine source file corresponding to: ${gcov_file}")
    endif()
    ...
    foreach(exclude_entry IN LISTS CTEST_CUSTOM_COVERAGE_EXCLUDE)
      if(source_file MATCHES "${exclude_entry}")
        ...
        if(NOT GCOV_QUIET)
          message(
            "Excluding coverage for: ${source_file} which matches ${exclude_entry}"
          )
        endif()
    ...
    foreach(uncovered_file IN LISTS uncovered_files)
      get_filename_component(resolved_uncovered_file "${uncovered_file}"
                             ABSOLUTE)
  ...
  foreach(uncovered_file ${uncovered_files})
    ...
    foreach(exclude_entry IN LISTS CTEST_CUSTOM_COVERAGE_EXCLUDE)
      if(uncovered_file MATCHES "${exclude_entry}")
        ...
        if(NOT GCOV_QUIET)
          message(
            "Excluding coverage for: ${uncovered_file} which matches ${exclude_entry}"
          )
        endif()
        break()
      endif()
    ...
    file(COPY ${uncovered_file}
         DESTINATION ${binary_dir}/uncovered/${relative_path})
    if(relative_path)
      list(APPEND uncovered_files_for_tar
           uncovered/${relative_path}/${filename})
    else()
      list(APPEND uncovered_files_for_tar uncovered/${filename})
    endif()
  endforeach()

  # tar up the coverage info with the same date so that the md5 sum will be the
  # same for the tar file independent of file time stamps
  string(REPLACE ";" "\n" gcov_files "${gcov_files}")
  string(REPLACE ";" "\n" label_files "${label_files}")
  string(REPLACE ";" "\n" uncovered_files_for_tar "${uncovered_files_for_tar}")
  file(
    WRITE "${coverage_dir}/coverage_file_list.txt"
    "${gcov_files}
...
")
  ...
  execute_process(
    COMMAND
      ...
      "--mtime=1970-01-01 0:0:0 UTC" "--format=gnutar"
