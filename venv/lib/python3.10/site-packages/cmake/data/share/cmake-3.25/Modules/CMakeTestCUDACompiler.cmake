...
if(CMAKE_CUDA_ABI_COMPILED)
  ...
  set(CMAKE_CUDA_COMPILER_WORKS TRUE)
  message(
    STATUS "Check for working CUDA compiler: ${CMAKE_CUDA_COMPILER} - skipped")

  # Run the test binary to detect the native architectures.
  execute_process(
    COMMAND "${CMAKE_PLATFORM_INFO_DIR}/CMakeDetermineCompilerABI_CUDA.bin"
  ...
  if(_CUDA_ARCHS_RESULT EQUAL 0)
    if("$ENV{CMAKE_CUDA_ARCHITECTURES_NATIVE_CLAMP}")
      # Undocumented hook used by CMake's CI. Clamp native architecture to
      # version range supported by this CUDA.
...
# selected cuda compiler can actually compile and link the most basic of
...
if(NOT CMAKE_CUDA_COMPILER_WORKS)
  ...
  string(
    CONCAT
      __TestCompiler_testCudaCompilerSource "#ifndef __CUDACC__\n"
  ...
  try_compile(
    CMAKE_CUDA_COMPILER_WORKS SOURCE_FROM_VAR
    main.cu __TestCompiler_testCudaCompilerSource
...
list(REMOVE_ITEM CMAKE_CUDA_IMPLICIT_LINK_LIBRARIES
     ${CMAKE_CUDA_IMPLICIT_LINK_LIBRARIES_EXCLUDE})

if(CMAKE_CUDA_COMPILER_ID STREQUAL "NVIDIA")
  # Remove the CUDA Toolkit include directories from the set of implicit system
  # include directories. This resolves the issue that NVCC doesn't specify these
  # includes as SYSTEM includes when compiling device code, and sometimes they
  # contain headers that generate warnings, so let users mark them as SYSTEM
  # explicitly
