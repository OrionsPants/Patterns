...
macro(matlab_get_version_from_release_name release_name version_name)
  ...
  string(REGEX MATCHALL "${release_name}=([0-9]+\\.?[0-9]*)" _matched
  ...
  if(NOT _matched STREQUAL "")
    ...
    message(
      WARNING "[MATLAB] The release name ${release_name} is not registered")
  endif()
  unset(_matched)
...
# extracts all the supported release names (R2017k...) of Matlab internal use
...
# extracts all the supported versions of Matlab internal use
...
#]=======================================================================]
function(matlab_extract_all_installed_versions_from_registry win64
         matlab_versions)

  if(NOT CMAKE_HOST_WIN32)
    message(
      FATAL_ERROR
        "[MATLAB] This macro can only be called by a windows host (call to reg.exe)"
    )
  endif()
  ...
  foreach(_installation_type IN ITEMS "MATLAB" "MATLAB Runtime"
    ...
    execute_process(
      COMMAND
        reg query
        "HKEY_LOCAL_MACHINE\\SOFTWARE\\Mathworks\\${_installation_type}" /f * /k
        ${APPEND_REG}
    ...
    if(resultMatlab EQUAL 0)
      ...
      string(REGEX MATCHALL "${_installation_type}\\\\([0-9]+(\\.[0-9]+)?)"
                   matlab_versions_regex ${varMatlab})

      foreach(match IN LISTS matlab_versions_regex)
        string(REGEX MATCH "${_installation_type}\\\\(([0-9]+)(\\.([0-9]+))?)"
                     current_match ${match})
...
macro(extract_matlab_versions_from_registry_brute_force matlab_versions)
  ...
  # this is a manual population of the versions we want to look for this can be
  # done as is, but preferably with the call to matlab_get_supported_versions
  # and variable

  # populating the versions we want to look for set(matlab_supported_versions)

  # # Matlab 7 set(matlab_major 7) foreach(current_matlab_minor RANGE 4 20)
  # list(APPEND matlab_supported_versions
  # "${matlab_major}.${current_matlab_minor}")
  # endforeach(current_matlab_minor)

  # # Matlab 8 set(matlab_major 8) foreach(current_matlab_minor RANGE 0 5)
  # list(APPEND matlab_supported_versions
  # "${matlab_major}.${current_matlab_minor}")
  # endforeach(current_matlab_minor)

  # # taking into account the possible additional versions provided by the user
  # if(DEFINED MATLAB_ADDITIONAL_VERSIONS) list(APPEND matlab_supported_versions
  # MATLAB_ADDITIONAL_VERSIONS) endif()
...
function(matlab_get_all_valid_matlab_roots_from_registry matlab_versions
  ...
  foreach(_matlab_current_version ${matlab_versions})
    ...
    if(EXISTS "${current_MATLAB_ROOT}")
      list(APPEND _matlab_roots_list "MATLAB" ${_matlab_current_version}
           ${current_MATLAB_ROOT})
    endif()
  ...
  foreach(_matlab_current_version ${matlab_versions})
    ...
    # remove the dot
    string(REPLACE "." "" _matlab_current_version_without_dot
                   "${_matlab_current_version}")

    if(EXISTS "${current_MATLAB_ROOT}")
      list(APPEND _matlab_roots_list "MCR" ${_matlab_current_version}
           "${current_MATLAB_ROOT}/v${_matlab_current_version_without_dot}")
    endif()
...
function(matlab_get_mex_suffix matlab_root mex_suffix)
  ...
  # sufficient for all win32 distributions. there is also
  # CMAKE_EXECUTABLE_SUFFIX that could be tweaked
  set(mexext_suffix "")
  if(WIN32)
    list(APPEND mexext_suffix ".bat")
  endif()

  # we first try without suffix, since cmake does not understand a list with one
  ...
  foreach(current_mexext_suffix IN LISTS mexext_suffix)
    if(NOT DEFINED Matlab_MEXEXTENSIONS_PROG OR NOT Matlab_MEXEXTENSIONS_PROG)
      ...
      find_program(
        Matlab_MEXEXTENSIONS_PROG "mexext${current_mexext_suffix}"
        PATHS ${matlab_root}/bin
        DOC "Matlab MEX extension provider"
        NO_DEFAULT_PATH)
    endif()
  ...
  if(MATLAB_FIND_DEBUG)
    message(
      STATUS
        "[MATLAB] Determining mex files extensions from '${matlab_root}/bin' with program '${Matlab_MEXEXTENSIONS_PROG}'"
    )
  endif()
  ...
  if((NOT Matlab_MEXEXTENSIONS_PROG) OR (NOT EXISTS ${Matlab_MEXEXTENSIONS_PROG}
    ...
    if(MATLAB_FIND_DEBUG)
      message(
        WARNING
          "[MATLAB] Cannot found mexext program. Matlab root is ${matlab_root}")
    endif()
  ...
  execute_process(
    ...
    OUTPUT_STRIP_TRAILING_WHITESPACE ${devnull})
  ...
  if(MATLAB_FIND_DEBUG)
    message(
      STATUS
        "[MATLAB] '${Matlab_MEXEXTENSIONS_PROG}' : determined extension '${_matlab_mex_extension}' and error string is '${_matlab_mex_extension_error}'"
    )
  endif()

  unset(Matlab_MEXEXTENSIONS_PROG CACHE)
  set(${mex_suffix}
      ${_matlab_mex_extension}
      PARENT_SCOPE)
endfunction()
...
function(matlab_get_version_from_matlab_run matlab_binary_program
  ...
  if(MATLAB_FIND_DEBUG)
    message(
      STATUS
        "[MATLAB] Determining the version of Matlab from ${matlab_binary_program}"
    )
  endif()
  ...
  if(EXISTS "${_matlab_temporary_folder}/matlabVersionLog.cmaketmp")
    if(MATLAB_FIND_DEBUG)
      message(
        STATUS
          "[MATLAB] Removing previous ${_matlab_temporary_folder}/matlabVersionLog.cmaketmp file"
      )
    endif()
    file(REMOVE "${_matlab_temporary_folder}/matlabVersionLog.cmaketmp")
  endif()

  # the log file is needed since on windows the command executes in a new window
  # and it is not possible to get back the answer of Matlab the -wait command is
  # needed on windows, otherwise the call returns immediately after the program
  # launches itself.
  if(WIN32)
  ...
  # we first try to run a simple program using the -r option, and then we use
  # the -batch option that is supported and recommended since R2019a
  set(_matlab_get_version_failed_with_r_option FALSE)

  # timeout set to 120 seconds, in case it does not start note as said before
  # OUTPUT_VARIABLE cannot be used in a platform independent manner however, not
  # setting it would flush the output of Matlab in the current console (unix
  # variant)
  execute_process(
    COMMAND
      "${matlab_binary_program}" -nosplash -nojvm ${_matlab_additional_commands}
      -logfile "matlabVersionLog.cmaketmp" -nodesktop -nodisplay -r
      "version, exit"
    ...
    TIMEOUT 120
    WORKING_DIRECTORY "${_matlab_temporary_folder}" ${devnull})

  if(_matlab_result_version_call MATCHES "timeout")
    if(MATLAB_FIND_DEBUG)
      message(
        WARNING "[MATLAB] Unable to determine the version of Matlab."
                " Matlab call with -r option timed out after 120 seconds.")
    endif()
    set(_matlab_get_version_failed_with_r_option TRUE)
  endif()

  if(NOT ${_matlab_get_version_failed_with_r_option}
     AND ${_matlab_result_version_call})
    if(MATLAB_FIND_DEBUG)
      message(
        WARNING
          "[MATLAB] Unable to determine the version of Matlab. Matlab call with -r option returned with error ${_matlab_result_version_call}."
      )
    endif()
    set(_matlab_get_version_failed_with_r_option TRUE)
  elseif(NOT ${_matlab_get_version_failed_with_r_option}
         AND NOT EXISTS "${_matlab_temporary_folder}/matlabVersionLog.cmaketmp")
    if(MATLAB_FIND_DEBUG)
      message(
        WARNING
          "[MATLAB] Unable to determine the version of Matlab. The log file does not exist."
      )
    endif()
    set(_matlab_get_version_failed_with_r_option TRUE)
  endif()
  ...
  if(_matlab_get_version_failed_with_r_option)
    execute_process(
      COMMAND
        "${matlab_binary_program}" -nosplash -nojvm
        ${_matlab_additional_commands} -logfile "matlabVersionLog.cmaketmp"
        -nodesktop -nodisplay -batch "version, exit"
    ...
    if(${_matlab_result_version_call_batch})
      if(MATLAB_FIND_DEBUG)
        message(
          WARNING
            "[MATLAB] Command executed \"${matlab_binary_program}\" -nosplash -nojvm ${_matlab_additional_commands} -logfile \"matlabVersionLog.cmaketmp\" -nodesktop -nodisplay -batch \"version, exit\""
        )
        message(
          WARNING
            "_matlab_version_from_cmd_dummy_batch (OUTPUT_VARIABLE): ${_matlab_version_from_cmd_dummy_batch}"
        )
        message(
          WARNING
            "_matlab_result_version_call_batch (RESULT_VARIABLE): ${_matlab_result_version_call_batch}"
        )
        message(
          WARNING
            "_matlab_result_version_call_error_batch (ERROR_VARIABLE): ${_matlab_result_version_call_error_batch}"
        )
        message(
          WARNING
            "[MATLAB] Unable to determine the version of Matlab. Matlab call with -batch option returned with error ${_matlab_result_version_call_batch}."
        )
      endif()
      return()
    elseif(NOT ${_matlab_get_version_failed_with_r_option}
           AND NOT EXISTS
               "${_matlab_temporary_folder}/matlabVersionLog.cmaketmp")
      if(MATLAB_FIND_DEBUG)
        message(
          WARNING
            "[MATLAB] Unable to determine the version of Matlab. The log file does not exist."
  ...
  if(NOT EXISTS "${_matlab_temporary_folder}/matlabVersionLog.cmaketmp")
    # last resort check as some HPC with "module load matlab" not enacted fail
    # to catch in earlier checks and error CMake configure even if
    # find_package(Matlab) is not REQUIRED
    if(MATLAB_FIND_DEBUG)
      message(
        WARNING
          "[MATLAB] Unable to determine the version of Matlab. The version log file does not exist."
      )
    endif()
    return()
  endif()
  ...
  file(READ "${_matlab_temporary_folder}/matlabVersionLog.cmaketmp"
       _matlab_version_from_cmd)
  file(REMOVE "${_matlab_temporary_folder}/matlabVersionLog.cmaketmp")
  ...
  if(index EQUAL -1)
    ...
    if(MATLAB_FIND_DEBUG)
      message(
        WARNING
          "[MATLAB] Cannot find the version of Matlab returned by the run.")
    endif()
    ...
    string(REGEX MATCHALL "ans[\r\n\t ]*=[\r\n\t ]*'?([0-9]+(\\.[0-9]+)?)"
                 matlab_versions_regex ${substring_ans})
    foreach(match IN LISTS matlab_versions_regex)
    ...
    set(${matlab_list_versions}
        ${matlab_list_of_all_versions_tmp}
        PARENT_SCOPE)
...
function(matlab_add_unit_test)
  ...
  # The option to run a batch program with MATLAB changes depending on the
  # MATLAB version For MATLAB before R2019a (9.6), the only supported option is
  # -r, afterwords the suggested option is -batch as -r is deprecated
  ...
  add_test(
    NAME ${${prefix}_NAME}
    COMMAND
      ${CMAKE_COMMAND} "-Dtest_name=${${prefix}_NAME}"
      ...
      "-Dmaut_BATCH_OPTION=${maut_BATCH_OPTION}" -P
      ${_FindMatlab_SELF_DIR}/MatlabTestsRedirect.cmake ${${prefix}_TEST_ARGS}
...
function(matlab_add_mex)
  ...
  if(NOT WIN32)
    # we do not need all this on Windows pthread options
    if(CMAKE_CXX_COMPILER_LOADED)
      check_cxx_compiler_flag(-pthread HAS_MINUS_PTHREAD)
    elseif(CMAKE_C_COMPILER_LOADED)
      check_c_compiler_flag(-pthread HAS_MINUS_PTHREAD)
    endif()
    # we should use try_compile instead, the link flags are discarded from this
    # compiler_flag function. check_cxx_compiler_flag(-Wl,--exclude-libs,ALL
  ...
  if(NOT Matlab_VERSION_STRING VERSION_LESS "9.1") # For 9.1 (R2016b) and newer,
                                                   # add version source file
    # Add the correct version file depending on which languages are enabled in
    # the project
    ...
    elseif(CMAKE_CXX_COMPILER_LOADED)
      # If C is not enabled, check if CXX is enabled and use the .cpp file to
      # avoid that the .c file is silently ignored
      set(MEX_VERSION_FILE
      ...
      message(
        WARNING
          "[MATLAB] matlab_add_mex requires that at least C or CXX are enabled languages"
      )
    endif()
  endif()

  # For 9.4 (R2018a) and newer, add API macro. Add it for unknown versions too,
  # just in case.
  if(NOT Matlab_VERSION_STRING VERSION_LESS "9.4" OR Matlab_VERSION_STRING
                                                     STREQUAL "unknown")
    if(${${prefix}_R2018a})
  ...
  if(${prefix}_EXECUTABLE)
    add_executable(
      ${${prefix}_NAME}
      ${_option_EXCLUDE_FROM_ALL} ${${prefix}_SRC} ${MEX_VERSION_FILE}
      ${${prefix}_DOCUMENTATION} ${${prefix}_UNPARSED_ARGUMENTS})
  else()
    if(${prefix}_MODULE)
      set(type MODULE)
    else()
    ...
    add_library(
      ${${prefix}_NAME}
      ${type} ${_option_EXCLUDE_FROM_ALL} ${${prefix}_SRC} ${MEX_VERSION_FILE}
      ${${prefix}_DOCUMENTATION} ${${prefix}_UNPARSED_ARGUMENTS})
  endif()
  ...
  if(NOT ${prefix}_NO_IMPLICIT_LINK_TO_MATLAB_LIBRARIES)
    ...
    target_link_libraries(${${prefix}_NAME} ${Matlab_MEX_LIBRARY}
                          ${Matlab_MX_LIBRARY})
  endif()
  ...
  if(NOT ${${prefix}_DOCUMENTATION} STREQUAL "")
    ...
    add_custom_command(
      TARGET ${${prefix}_NAME}
      PRE_BUILD
      COMMAND ${CMAKE_COMMAND} -E copy_if_different ${${prefix}_DOCUMENTATION}
              $<TARGET_FILE_DIR:${${prefix}_NAME}>/${output_name}.m
      COMMENT
        "[MATLAB] Copy ${${prefix}_NAME} documentation file into the output folder"
    )
  endif() # documentation
  ...
  if(WIN32)
    ...
    if(MSVC)
      ...
      if(NOT Matlab_VERSION_STRING VERSION_LESS "9.1") # For 9.1 (R2016b) and
                                                       # newer, export version
      ...
      set_property(
        TARGET ${${prefix}_NAME}
        APPEND
        PROPERTY LINK_FLAGS ${_link_flags})

    endif() # No other compiler currently supported on Windows.

    set_target_properties(
      ${${prefix}_NAME} PROPERTIES DEFINE_SYMBOL
                                   "DLL_EXPORT_SYM=__declspec(dllexport)")

  else()

    if(Matlab_VERSION_STRING VERSION_LESS "9.1") # For versions prior to 9.1
                                                 # (R2016b)
      set(_ver_map_files ${Matlab_EXTERN_LIBRARY_DIR}/mexFunction.map)
    else() # For 9.1 (R2016b) and newer
      set(_ver_map_files
          ${Matlab_EXTERN_LIBRARY_DIR}/c_exportsmexfileversion.map)
    endif()

    if(NOT Matlab_VERSION_STRING VERSION_LESS "9.5") # For 9.5 (R2018b) (and
                                                     # newer?)
      ...
      # compiling with -fvisibility=hidden, the symbol `mexFunction` cannot be
      # exported. Reading the source code for <mex.h>, it seems that the
      # preprocessor macro `MW_NEEDS_VERSION_H` needs to be defined for
      # `__attribute__((visibility("default")))` to be added in front of the
      # declaration of `mexFunction`. In previous versions of MATLAB this was
      # not the case, there `DLL_EXPORT_SYM` needed to be defined. Adding
      # `-fvisibility=hidden` to the `mex` command causes the build to fail.
    ...
    if(APPLE)
      ...
      if(Matlab_HAS_CPP_API)
        list(APPEND _ver_map_files
             ${Matlab_EXTERN_LIBRARY_DIR}/cppMexFunction.map) # This one doesn't
                                                              # exist on Linux
        set(_link_flags
            "${_link_flags} -Wl,-U,_mexCreateMexFunction -Wl,-U,_mexDestroyMexFunction -Wl,-U,_mexFunctionAdapter"
        )
        # On MacOS, the MEX command adds the above, without it the link breaks
        # because we indiscriminately use "cppMexFunction.map" even for C API
        # MEX-files.
      endif()
...
# (internal) Used to get the version of matlab, using caching. This basically
# transforms the output of the root list, with possible unknown version, to a
# version This can possibly run Matlab for extracting the version.
function(_Matlab_get_version_from_root matlab_root matlab_or_mcr
         matlab_known_version matlab_final_version)

  # if the version is not trivial, we query matlab (if not MCR) for that we keep
  # track of the location of matlab that induced this version if(NOT DEFINED
  # Matlab_PROG_VERSION_STRING_AUTO_DETECT)
  # set(Matlab_PROG_VERSION_STRING_AUTO_DETECT "" CACHE INTERNAL "internal
  # matlab location for the discovered version") endif()

  if(NOT matlab_known_version STREQUAL "NOTFOUND")
    # the version is known, we just return it
    set(${matlab_final_version}
        ${matlab_known_version}
        PARENT_SCOPE)
    set(Matlab_VERSION_STRING_INTERNAL
        ${matlab_known_version}
        CACHE INTERNAL "Matlab version (automatically determined)" FORCE)
    return()
  endif()
  ...
  if(matlab_or_mcr STREQUAL "UNKNOWN")
    ...
    if(EXISTS "${matlab_root}/appdata/version.xml")
      # we inspect the application version.xml file that contains the product
      # information
      file(STRINGS "${matlab_root}/appdata/version.xml" productinfo_string
           NEWLINE_CONSUME)
      string(REGEX MATCH
                   "<installedProductData.*displayedString=\"([a-zA-Z ]+)\".*/>"
                   product_reg_match ${productinfo_string})
    ...
    if(MATLAB_FIND_DEBUG)
      message(
        WARNING "[MATLAB] '${matlab_root}' contains the '${matlab_or_mcr}'")
    endif()
  endif()

  # UNKNOWN is the default behavior in case we - have an erroneous matlab_root -
  ...
  if(matlab_or_mcr STREQUAL "MATLAB" OR matlab_or_mcr STREQUAL "UNKNOWN")
    ...
    if(NOT _matlab_current_program)
      ...
      if(matlab_root AND EXISTS ${matlab_root})
        set(_find_matlab_options PATHS ${matlab_root} ${matlab_root}/bin
                                 NO_DEFAULT_PATH)
      endif()

      find_program(_matlab_current_program matlab ${_find_matlab_options}
    ...
    if(NOT _matlab_current_program OR NOT EXISTS ${_matlab_current_program})
      ...
      if(MATLAB_FIND_DEBUG)
        message(
          WARNING
            "[MATLAB] Cannot find the main matlab program under ${matlab_root}")
      endif()
    ...
    get_filename_component(_matlab_main_real_path_tmp
                           "${_matlab_current_program}" REALPATH)
    unset(_matlab_current_program)
    unset(_matlab_current_program CACHE)

    # is it the same as the previous one?
    if(_matlab_main_real_path_tmp STREQUAL
       Matlab_PROG_VERSION_STRING_AUTO_DETECT)
      set(${matlab_final_version}
    ...
    matlab_get_version_from_matlab_run(
      "${Matlab_PROG_VERSION_STRING_AUTO_DETECT}" matlab_list_of_all_versions)
    ...
    if((list_of_all_versions_length GREATER 1) AND MATLAB_FIND_DEBUG)
      message(
        WARNING
          "[MATLAB] Found several versions, taking the first one (versions found ${matlab_list_of_all_versions})"
      )
    endif()
  ...
  elseif(EXISTS "${matlab_root}/VersionInfo.xml")
    # MCR we cannot run anything in order to extract the version. We assume that
    # the file VersionInfo.xml exists under the MatlabRoot, we look for it and
    # extract the version from there
    set(_matlab_version_tmp "unknown")
    file(STRINGS "${matlab_root}/VersionInfo.xml" versioninfo_string
         NEWLINE_CONSUME)

    if(versioninfo_string)
      # parses "<version>9.2.0.538062</version>"
      string(REGEX MATCH "<version>(.*)</version>" version_reg_match
                   ${versioninfo_string})
    ...
    set(Matlab_VERSION_STRING_INTERNAL
        "${_matlab_version_tmp}"
        CACHE INTERNAL "Matlab (MCR) version (automatically determined)" FORCE)
  endif() # Matlab or MCR
...
function(_Matlab_find_instances_win32 matlab_roots)
  # On WIN32, we look for Matlab installation in the registry if unsuccessful,
  # we look for all known revision and filter the existing ones.
  ...
  if(NOT _matlab_versions_from_registry)
    ...
    extract_matlab_versions_from_registry_brute_force(
      _matlab_versions_from_registry)
  endif()

  # filtering the results with the registry keys
  matlab_get_all_valid_matlab_roots_from_registry(
    "${_matlab_versions_from_registry}" _matlab_possible_roots)
  set(${matlab_roots}
      ${_matlab_possible_roots}
      PARENT_SCOPE)
...
function(_Matlab_find_instances_osx matlab_roots)
  ...
  # on mac, we look for the /Application paths this corresponds to the behavior
  # on Windows. On Linux, we do not have any other guess.
  matlab_get_supported_releases(_matlab_releases)
  if(MATLAB_FIND_DEBUG)
    message(
      STATUS
        "[MATLAB] Matlab supported versions ${_matlab_releases}. If more version should be supported "
  ...
  foreach(_matlab_current_release IN LISTS _matlab_releases)
    matlab_get_version_from_release_name("${_matlab_current_release}"
                                         _matlab_current_version)
    string(REPLACE "." "" _matlab_current_version_without_dot
                   "${_matlab_current_version}")
    ...
    if(EXISTS ${_matlab_base_path})
      if(MATLAB_FIND_DEBUG)
        message(
          STATUS
            "[MATLAB] Found version ${_matlab_current_release} (${_matlab_current_version}) in ${_matlab_base_path}"
        )
      endif()
      list(APPEND _matlab_possible_roots "MATLAB" ${_matlab_current_version}
           ${_matlab_base_path})
    endif()

    # Checks MCR
    set(_mcr_path
        "/Applications/MATLAB/MATLAB_Runtime/v${_matlab_current_version_without_dot}"
    )
    if(EXISTS "${_mcr_path}")
      if(MATLAB_FIND_DEBUG)
        message(
          STATUS
            "[MATLAB] Found MCR version ${_matlab_current_release} (${_matlab_current_version}) in ${_mcr_path}"
        )
      endif()
      list(APPEND _matlab_possible_roots "MCR" ${_matlab_current_version}
           ${_mcr_path})
    endif()
...
function(_Matlab_find_instances_from_path matlab_roots)
  ...
  # At this point, we have no other choice than trying to find it from PATH. If
  # set by the user, this won't change.
  find_program(_matlab_main_tmp NAMES matlab)
  ...
  if(_matlab_main_tmp)
    ...
    # resolve symlinks
    get_filename_component(_matlab_current_location "${_matlab_main_tmp}"
                           REALPATH)

    # get the directory (the command below has to be run twice) this will be the
    # matlab root
    get_filename_component(_matlab_current_location
                           "${_matlab_current_location}" DIRECTORY)
    get_filename_component(
      _matlab_current_location "${_matlab_current_location}" DIRECTORY
    )# Matlab should be in bin

    # We found the Matlab program
    list(APPEND _matlab_possible_roots "MATLAB" "NOTFOUND"
         ${_matlab_current_location})

    # we remove this from the CACHE
    unset(_matlab_main_tmp CACHE)
  else()
    find_program(_matlab_mex_tmp NAMES mex)
    if(_matlab_mex_tmp)
      # we then populate the list of roots, with empty version
      if(MATLAB_FIND_DEBUG)
        message(
          STATUS "[MATLAB] mex compiler found from PATH: ${_matlab_mex_tmp}")
      endif()

      # resolve symlinks
      get_filename_component(_mex_current_location "${_matlab_mex_tmp}"
                             REALPATH)

      # get the directory (the command below has to be run twice) this will be
      # the matlab root
      get_filename_component(_mex_current_location "${_mex_current_location}"
                             DIRECTORY)
      get_filename_component(
        _mex_current_location "${_mex_current_location}" DIRECTORY) # Matlab
                                                                    # Runtime
                                                                    # mex
                                                                    # compiler
                                                                    # should be
      ...
      # We found the Matlab program
      list(APPEND _matlab_possible_roots "MCR" "NOTFOUND"
           ${_mex_current_location})
...
if(Matlab_ROOT_DIR)
  ...
  if(NOT EXISTS "${Matlab_ROOT_DIR}")
    ...
    if(MATLAB_FIND_DEBUG)
      message(
        WARNING
          "[MATLAB] the specified path for Matlab_ROOT_DIR does not exist (${Matlab_ROOT_DIR})"
      )
    endif()
  else()
    # NOTFOUND indicates the code below to search for the version automatically
    if("${Matlab_VERSION_STRING_INTERNAL}" STREQUAL "")
      list(APPEND _matlab_possible_roots "UNKNOWN" "NOTFOUND"
           ${Matlab_ROOT_DIR}) # empty version, empty MCR/Matlab indication
    else()
      list(APPEND _matlab_possible_roots "UNKNOWN"
           ${Matlab_VERSION_STRING_INTERNAL} ${Matlab_ROOT_DIR}) # cached
                                                                 # version
    endif()
  endif()
else()

  # if the user does not specify the possible installation root, we look for one
  # installation using the appropriate heuristics. There is apparently no
  # standard way on Linux.
  if(CMAKE_HOST_WIN32)
...
if(_numbers_of_matlab_roots GREATER 0)
  ...
  elseif(DEFINED Matlab_FIND_VERSION)
    ...
    foreach(_matlab_root_index RANGE 1 ${_numbers_of_matlab_roots} 3)
      list(GET _matlab_possible_roots ${_matlab_root_index}
    ...
    if(_numbers_of_matlab_roots GREATER 3 AND MATLAB_FIND_DEBUG)
      message(
        WARNING
          "[MATLAB] Found several distributions of Matlab. Setting the current version to ${Matlab_VERSION_STRING} (located ${Matlab_ROOT_DIR})."
...
# check if the root changed wrt. the previous defined one, if so clear all the
# cached variables for being able to reconfigure properly
if(DEFINED Matlab_ROOT_DIR_LAST_CACHED)
  ...
  if(NOT Matlab_ROOT_DIR_LAST_CACHED STREQUAL Matlab_ROOT_DIR)
    set(_Matlab_cached_vars
        ...
        # Matlab_PROG_VERSION_STRING_AUTO_DETECT Matlab_VERSION_STRING_INTERNAL
    )
...
set(Matlab_ROOT_DIR_LAST_CACHED
    ${Matlab_ROOT_DIR}
    CACHE INTERNAL "last Matlab root dir location")
set(Matlab_ROOT_DIR
    ${Matlab_ROOT_DIR}
    CACHE PATH "Matlab installation root path" FORCE)

# Fix the version, in case this one is NOTFOUND
_matlab_get_version_from_root("${Matlab_ROOT_DIR}" "${Matlab_Or_MCR}"
                              ${Matlab_VERSION_STRING} Matlab_VERSION_STRING)

if(MATLAB_FIND_DEBUG)
  message(
    STATUS
      "[MATLAB] Current version is ${Matlab_VERSION_STRING} located ${Matlab_ROOT_DIR}"
  )
endif()

# MATLAB 9.4 (R2018a) and newer have a new C++ API This API pulls additional
# required libraries.
...
if(MATLAB_FIND_DEBUG)
  message(
    STATUS
      "[MATLAB] [DEBUG]_matlab_lib_prefix_for_search = ${_matlab_lib_prefix_for_search} | _matlab_lib_dir_for_search = ${_matlab_lib_dir_for_search}"
  )
endif()

# internal This small stub around find_library is to prevent any pollution of
# CMAKE_FIND_LIBRARY_PREFIXES in the global scope. This is the function to be
# used below instead of the find_library directives.
function(_Matlab_find_library _matlab_library_prefix)
  set(CMAKE_FIND_LIBRARY_PREFIXES ${CMAKE_FIND_LIBRARY_PREFIXES}
                                  ${_matlab_library_prefix})
  find_library(${ARGN})
endfunction()

set(_matlab_required_variables)

# Order is as follow: - unconditionally required libraries/headers first - then
# library components - then program components
...
if(Matlab_Or_MCR STREQUAL "MATLAB" OR Matlab_Or_MCR STREQUAL "UNKNOWN")
  _matlab_find_library(${_matlab_lib_prefix_for_search} Matlab_MEX_LIBRARY mex
                       PATHS ${_matlab_lib_dir_for_search} NO_DEFAULT_PATH)
  ...
  _matlab_find_library(${_matlab_lib_prefix_for_search} Matlab_MX_LIBRARY mx
                       PATHS ${_matlab_lib_dir_for_search} NO_DEFAULT_PATH)
...
if("ENG_LIBRARY" IN_LIST Matlab_FIND_COMPONENTS)
  _matlab_find_library(${_matlab_lib_prefix_for_search} Matlab_ENG_LIBRARY eng
                       PATHS ${_matlab_lib_dir_for_search} NO_DEFAULT_PATH)
...
if("MAT_LIBRARY" IN_LIST Matlab_FIND_COMPONENTS)
  _matlab_find_library(${_matlab_lib_prefix_for_search} Matlab_MAT_LIBRARY mat
                       PATHS ${_matlab_lib_dir_for_search} NO_DEFAULT_PATH)
...
if("SIMULINK" IN_LIST Matlab_FIND_COMPONENTS)
  find_path(
    Matlab_SIMULINK_INCLUDE_DIR simstruc.h
...
# internal This small stub permits to add imported targets for the found MATLAB
# libraries
function(_Matlab_add_imported_target _matlab_library_variable_name
         _matlab_library_target_name)
  if(Matlab_${_matlab_library_variable_name}_LIBRARY)
    if(NOT TARGET Matlab::${_matlab_library_target_name})
      ...
      set_target_properties(
        ...
        PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${Matlab_INCLUDE_DIRS}"
                   IMPORTED_LOCATION
                   "${Matlab_${_matlab_library_variable_name}_LIBRARY}")
      if(_matlab_library_target_name STREQUAL "mex"
         OR _matlab_library_target_name STREQUAL "eng"
         OR _matlab_library_target_name STREQUAL "mat")
        set_target_properties(Matlab::${_matlab_library_target_name}
                              PROPERTIES INTERFACE_LINK_LIBRARIES Matlab::mx)
      endif()
    endif()
  endif()
endfunction()
