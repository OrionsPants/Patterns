...
# Version 1.6 (CMake 3.0) * Create targets for each library * Do not link
# libfreetype Version 1.5 (CMake 2.8.12) * 14236: Detect gthread library Detect
# pangocairo on windows Detect pangocairo with gtk module instead of with gtkmm
# * 14259: Use vc100 libraries with VS 11 * 14260: Export a GTK2_DEFINITIONS
# variable to set /vd2 when appropriate (i.e. MSVC) * Use the optimized/debug
# syntax for _LIBRARY and _LIBRARIES variables when appropriate. A new set of
# _RELEASE variables was also added. * Remove GTK2_SKIP_MARK_AS_ADVANCED option,
# as now the variables are marked as advanced by SelectLibraryConfigurations *
# Detect gmodule, pangoft2 and pangoxft libraries Version 1.4 (10/4/2012) (CMake
# 2.8.10) * 12596: Missing paths for FindGTK2 on NetBSD * 12049: Fixed detection
# of GTK include files in the lib folder on multiarch systems. Version 1.3
# (11/9/2010) (CMake 2.8.4) * 11429: Add support for detecting GTK2 built with
# Visual Studio 10. Thanks to Vincent Levesque for the patch. Version 1.2
# (8/30/2010) (CMake 2.8.3) * Merge patch for detecting gdk-pixbuf library
# (split off from core GTK in 2.21).  Thanks to Vincent Untz for the patch and
# Ricardo Cruz for the heads up. Version 1.1 (8/19/2010) (CMake 2.8.3) * Add
# support for detecting GTK2 under macports (thanks to Gary Kramlich) Version
# 1.0 (8/12/2010) (CMake 2.8.3) * Add support for detecting new pangommconfig.h
# header file (Thanks to Sune Vuorela & the Debian Project for the patch) * Add
# support for detecting fontconfig.h header * Call find_package(Freetype) since
# it's required * Add support for allowing users to add additional library
# directories via the GTK2_ADDITIONAL_SUFFIXES variable (kind of a future-kludge
# in case the GTK developers change versions on any of the directories in the
# future). Version 0.8 (1/4/2010) * Get module working under MacOSX fink by
# adding /sw/include, /sw/lib to PATHS and the gobject library Version 0.7
# (3/22/09) * Checked into CMake CVS * Added versioning support * Module now
# defaults to searching for GTK if COMPONENTS not specified. * Added HKCU prior
# to HKLM registry key and GTKMM specific environment variable as per mailing
# list discussion. * Added lib64 to include search path and a few other search
# paths where GTK may be installed on Unix systems. * Switched to lowercase
# CMake commands * Prefaced internal variables with _GTK2 to prevent collision *
# Changed internal macros to functions * Enhanced documentation Version 0.6
# (1/8/08) Added GTK2_SKIP_MARK_AS_ADVANCED option Version 0.5 (12/19/08) Second
# release to cmake mailing list

# =============================================================
# _GTK2_GET_VERSION Internal function to parse the version number in
...
function(_GTK2_GET_VERSION _OUT_major _OUT_minor _OUT_micro _gtkversion_hdr)
  file(STRINGS ${_gtkversion_hdr} _contents
       REGEX "#define GTK_M[A-Z]+_VERSION[ \t]+")
  if(_contents)
...
# =============================================================
# _GTK2_SIGCXX_GET_VERSION Internal function to parse the version number in
# sigc++config.h _OUT_major = Major version number _OUT_minor = Minor version
# number _OUT_micro = Micro version number _sigcxxversion_hdr = Header file to
# parse
# =============================================================

function(_GTK2_SIGCXX_GET_VERSION _OUT_major _OUT_minor _OUT_micro
         _sigcxxversion_hdr)
  file(STRINGS ${_sigcxxversion_hdr} _contents
       REGEX "#define SIGCXX_M[A-Z]+_VERSION[ \t]+")
  if(_contents)
    string(REGEX REPLACE ".*#define SIGCXX_MAJOR_VERSION[ \t]+([0-9]+).*" "\\1"
                         ${_OUT_major} "${_contents}")
    string(REGEX REPLACE ".*#define SIGCXX_MINOR_VERSION[ \t]+([0-9]+).*" "\\1"
                         ${_OUT_minor} "${_contents}")
    string(REGEX REPLACE ".*#define SIGCXX_MICRO_VERSION[ \t]+([0-9]+).*" "\\1"
                         ${_OUT_micro} "${_contents}")
...
# =============================================================
# _GTK2_FIND_INCLUDE_DIR Internal function to find the GTK include directories
# _var = variable to set (_INCLUDE_DIR is appended) _hdr = header file to look
# for
# =============================================================
function(_GTK2_FIND_INCLUDE_DIR _var _hdr)
  ...
  # these directories: /usr/include/<pkg> /usr/lib/<pkg>/include
  #
  ...
  if(CMAKE_LIBRARY_ARCHITECTURE)
    ...
    if(GTK2_DEBUG)
      message(
        STATUS "Adding ${_gtk2_arch_dir} to search path for multiarch support")
    endif()
  endif()
  find_path(
    GTK2_${_var}_INCLUDE_DIR ${_hdr}
    PATHS ${_gtk2_arch_dir}
          /usr/local/libx32
          /usr/local/lib64
          /usr/local/lib
          /usr/libx32
  ...
  if(GTK2_${_var}_INCLUDE_DIR)
    set(GTK2_INCLUDE_DIRS
        ${GTK2_INCLUDE_DIRS} ${GTK2_${_var}_INCLUDE_DIR}
        PARENT_SCOPE)
  endif()
...
# =============================================================
# _GTK2_FIND_LIBRARY Internal function to find libraries packaged with GTK2 _var
# = library variable to create (_LIBRARY is appended)
# =============================================================
function(_GTK2_FIND_LIBRARY _var _lib _expand_vc _append_version)
  ...
  if(GTK2_DEBUG)
    message(
      STATUS
        "[FindGTK2.cmake:${CMAKE_CURRENT_LIST_LINE}] "
  ...
  if(_expand_vc AND MSVC)
    ...
    if(MSVC_TOOLSET_VERSION LESS 110)
      ...
      # Up to gtkmm-win 2.22.0-2 there are no vc110 libraries but vc100 can be
      # used
  ...
  find_library(
    GTK2_${_var}_LIBRARY_RELEASE
    NAMES ${_lib_list}
    PATHS /opt/gnome/lib /usr/openwin/lib $ENV{GTKMM_BASEPATH}/lib
          [HKEY_CURRENT_USER\\SOFTWARE\\gtkmm\\2.4;Path]/lib
          [HKEY_LOCAL_MACHINE\\SOFTWARE\\gtkmm\\2.4;Path]/lib)
  ...
  set(GTK2_${_var}_LIBRARY
      ${GTK2_${_var}_LIBRARY}
      PARENT_SCOPE)
  set(GTK2_${_var}_FOUND
      ${GTK2_${_var}_FOUND}
      PARENT_SCOPE)
  ...
  if(GTK2_DEBUG)
    ...
    message(
      STATUS "[FindGTK2.cmake:${CMAKE_CURRENT_LIST_LINE}]     "
  ...
  if(TARGET GTK2::${_basename})
    ...
    foreach(_config ${_configs})
      _gtk2_add_target_depends_internal(
        ${_var} IMPORTED_LINK_INTERFACE_LIBRARIES_${_config} ${ARGN})
    endforeach()
  endif()
endfunction()
  ...
  if(TARGET GTK2::${_basename})
    foreach(_include ${ARGN})
      set_property(
        TARGET GTK2::${_basename}
        APPEND
        PROPERTY INTERFACE_INCLUDE_DIRECTORIES "${_include}")
    endforeach()
  endif()
endfunction()

# =============================================================
# _GTK2_ADD_TARGET Internal function to create targets for GTK2 _var = target to
# create
# =============================================================
function(_GTK2_ADD_TARGET _var)
  ...
  cmake_parse_arguments(
    _${_var} "" "" "GTK2_DEPENDS;GTK2_OPTIONAL_DEPENDS;OPTIONAL_INCLUDES"
    ${ARGN})
  ...
  if(GTK2_${_var}_FOUND)
    if(NOT TARGET GTK2::${_basename})
      ...
      if(GTK2_${_var}_LIBRARY_RELEASE)
        ...
        set_property(
          TARGET GTK2::${_basename} PROPERTY IMPORTED_LOCATION_RELEASE
                                             "${GTK2_${_var}_LIBRARY_RELEASE}")
      endif()
      ...
      if(GTK2_${_var}_LIBRARY_DEBUG)
        ...
        set_property(
          TARGET GTK2::${_basename} PROPERTY IMPORTED_LOCATION_DEBUG
                                             "${GTK2_${_var}_LIBRARY_DEBUG}")
      endif()
      ...
      if(GTK2_${_var}_INCLUDE_DIR)
        set_property(
          TARGET GTK2::${_basename}
          APPEND
          PROPERTY INTERFACE_INCLUDE_DIRECTORIES "${GTK2_${_var}_INCLUDE_DIR}")
      endif()

      if(GTK2_${_var}CONFIG_INCLUDE_DIR
         AND NOT "x${GTK2_${_var}CONFIG_INCLUDE_DIR}" STREQUAL
             "x${GTK2_${_var}_INCLUDE_DIR}")
        set_property(
          TARGET GTK2::${_basename}
          APPEND
          PROPERTY INTERFACE_INCLUDE_DIRECTORIES
                   "${GTK2_${_var}CONFIG_INCLUDE_DIR}")
      endif()

      if(GTK2_DEFINITIONS)
        set_property(
          TARGET GTK2::${_basename} PROPERTY INTERFACE_COMPILE_DEFINITIONS
                                             "${GTK2_DEFINITIONS}")
      endif()
...
if(GTK2_FIND_VERSION)
  ...
  if(GTK2_GTK_INCLUDE_DIR)
    _gtk2_get_version(GTK2_MAJOR_VERSION GTK2_MINOR_VERSION GTK2_PATCH_VERSION
    ...
    if(GTK2_FIND_VERSION_EXACT)
      ...
      if(GTK2_VERSION VERSION_EQUAL GTK2_FIND_VERSION
         OR GTK2_VERSION VERSION_GREATER GTK2_FIND_VERSION)
        set(GTK2_FAILED_VERSION_CHECK false)
      endif()
    endif()
  else()
  ...
  if(GTK2_FAILED_VERSION_CHECK)
    if(GTK2_FIND_REQUIRED AND NOT GTK2_FIND_QUIETLY)
      if(GTK2_FIND_VERSION_EXACT)
        message(
          FATAL_ERROR
            "GTK2 version check failed.  Version ${GTK2_VERSION} was found, version ${GTK2_FIND_VERSION} is needed exactly."
        )
      else()
        message(
          FATAL_ERROR
            "GTK2 version check failed.  Version ${GTK2_VERSION} was found, at least version ${GTK2_FIND_VERSION} is required"
        )
      endif()
    endif()
...
#
# On MSVC, according to https://wiki.gnome.org/gtkmm/MSWindows, the /vd2 flag
# needs to be passed to the compiler in order to use gtkmm
#
if(MSVC)
...
foreach(_GTK2_component ${GTK2_FIND_COMPONENTS})
  if(_GTK2_component STREQUAL "gtk")
    ...
    _gtk2_add_target(PANGO GTK2_DEPENDS gobject glib GTK2_OPTIONAL_DEPENDS
                     harfbuzz)
  ...
  elseif(_GTK2_component STREQUAL "gtkmm")
    ...
    if(GTK2_SIGC++CONFIG_INCLUDE_DIR)
      _gtk2_sigcxx_get_version(
        GTK2_SIGC++_VERSION_MAJOR GTK2_SIGC++_VERSION_MINOR
...
foreach(_GTK2_component ${GTK2_FIND_COMPONENTS})
  ...
  elseif(_GTK2_component STREQUAL "glade")
    find_package_handle_standard_args(
      GTK2_${_COMPONENT_UPPER} "The glade library was not found."
      GTK2_GLADE_LIBRARY GTK2_GLADE_INCLUDE_DIR)
  elseif(_GTK2_component STREQUAL "glademm")
    find_package_handle_standard_args(
      GTK2_${_COMPONENT_UPPER} "The glademm library was not found."
      GTK2_GLADEMM_LIBRARY GTK2_GLADEMM_INCLUDE_DIR
