...
# CPack script for creating Debian package Author: Mathieu Malaterre
#
...
function(get_component_package_name var component)
  ...
  if(CPACK_DEBIAN_${component_upcase}_PACKAGE_NAME)
    string(TOLOWER "${CPACK_DEBIAN_${component_upcase}_PACKAGE_NAME}"
                   package_name)
  else()
...
function(cpack_deb_format_package_description TEXT OUTPUT_VAR)
  ...
  # If the description ends with a newline (e.g. typically if it was read from a
  # file) the last line will be empty. We drop it here, otherwise it would be
  # replaced by a `.` which would lead to the package violating the
  ...
  foreach(_line IN LISTS _lines)
    ...
    if(NOT _line_strip)
      ...
      # Prepend the normal lines w/ a single space. If the line already starts
      # w/ at least one space, it'll become _verbatim_ (assuming it supposed to
      # be verbatim in the original text).
...
function(cpack_deb_prepare_package_vars)
  # CPACK_DEBIAN_PACKAGE_SHLIBDEPS If specify OFF, only user depends are used
  ...
  set(DBGSYMDIR
      "${CPACK_TOPLEVEL_DIRECTORY}/${CPACK_PACKAGE_FILE_NAME}${CPACK_DEB_PACKAGE_COMPONENT_PART_PATH}-dbgsym"
  )
  ...
  if(CPACK_DEB_PACKAGE_COMPONENT)
    ...
    if(DEFINED ${_component_shlibdeps_var})
      ...
      if(CPACK_DEBIAN_PACKAGE_DEBUG)
        message(
          "CPackDeb Debug: component '${CPACK_DEB_PACKAGE_COMPONENT}' dpkg-shlibdeps set to ${CPACK_DEBIAN_PACKAGE_SHLIBDEPS}"
        )
      endif()
    endif()
  endif()
  ...
  if(CPACK_DEBIAN_PACKAGE_SHLIBDEPS
     OR CPACK_DEBIAN_PACKAGE_GENERATE_SHLIBS
     OR CPACK_DEBIAN_DEBUGINFO_PACKAGE)
    # Generating binary list - Get type of all install files
    file(
      GLOB_RECURSE FILE_PATHS_
    ...
    foreach(FILE_ IN LISTS FILE_PATHS_)
      ...
      if(NOT FILE_RESULT_ EQUAL 0)
        message(
          FATAL_ERROR
            "CPackDeb: execution of command: '${FILE_EXECUTABLE} ./${FILE_}' failed with exit code: ${FILE_RESULT_}"
        )
      endif()
      list(APPEND CPACK_DEB_INSTALL_FILES "${INSTALL_FILE_}")
    endforeach()

    # Only dynamically linked ELF files are included Extract only file name
    # infront of ":"
  ...
  if(CPACK_DEBIAN_DEBUGINFO_PACKAGE AND CPACK_DEB_UNSTRIPPED_FILES)
    ...
    file(RELATIVE_PATH _DBGSYM_ROOT "${CPACK_TEMPORARY_DIRECTORY}"
         "${DBGSYMDIR}")
    foreach(_FILE IN LISTS CPACK_DEB_UNSTRIPPED_FILES)
      ...
      if(NOT READELF_RESULT EQUAL 0)
        message(
          FATAL_ERROR
            "CPackDeb: readelf: '${READELF_ERROR}';\n"
            "executed command: '${CPACK_READELF_EXECUTABLE} -n ${_FILE}'")
      endif()
      if(READELF_OUTPUT MATCHES
         "Build ID: ([0-9a-zA-Z][0-9a-zA-Z])([0-9a-zA-Z]*)")
      ...
      set(_FILE_DBGSYM
          ${_DBGSYM_ROOT}/usr/lib/debug/.build-id/${_BUILD_ID_START}/${_BUILD_ID_REMAINING}.debug
      )
      get_filename_component(_OUT_DIR "${_FILE_DBGSYM}" DIRECTORY)
      file(MAKE_DIRECTORY "${CPACK_TEMPORARY_DIRECTORY}/${_OUT_DIR}")
      execute_process(
        COMMAND ${CPACK_OBJCOPY_EXECUTABLE} --only-keep-debug "${_FILE}"
                "${_FILE_DBGSYM}"
        WORKING_DIRECTORY "${CPACK_TEMPORARY_DIRECTORY}"
      ...
      execute_process(
        COMMAND ${CPACK_OBJCOPY_EXECUTABLE} --add-gnu-debuglink=${_FILE_DBGSYM}
                ${_FILE}
        WORKING_DIRECTORY "${CPACK_TEMPORARY_DIRECTORY}"
        OUTPUT_VARIABLE OBJCOPY_OUTPUT
        RESULT_VARIABLE OBJCOPY_RESULT
  ...
  if(CPACK_DEBIAN_PACKAGE_SHLIBDEPS)
    ...
    if(SHLIBDEPS_EXECUTABLE)
      ...
      execute_process(
        COMMAND ${CMAKE_COMMAND} -E env LC_ALL=C ${SHLIBDEPS_EXECUTABLE}
                --version
        OUTPUT_VARIABLE _TMP_VERSION
        ERROR_QUIET OUTPUT_STRIP_TRAILING_WHITESPACE)
      if(_TMP_VERSION MATCHES
         "dpkg-shlibdeps version ([0-9]+\\.[0-9]+\\.[0-9]+)")
      ...
      if(CPACK_DEBIAN_PACKAGE_DEBUG)
        message(
          "CPackDeb Debug: dpkg-shlibdeps --version output is '${_TMP_VERSION}'"
        )
        message(
          "CPackDeb Debug: dpkg-shlibdeps version is <${SHLIBDEPS_EXECUTABLE_VERSION}>"
        )
      endif()
      ...
      if(CONTAINS_EXECUTABLE_FILES_)
        ...
        # Create blank control file for running dpkg-shlibdeps There might be
        # some other way to invoke dpkg-shlibdeps without creating this file but
        # standard debian package should not have anything that can collide with
        # this file or directory
        file(MAKE_DIRECTORY ${CPACK_TEMPORARY_DIRECTORY}/debian)
        file(WRITE ${CPACK_TEMPORARY_DIRECTORY}/debian/control "")

        # Create a DEBIAN directory so that dpkg-shlibdeps can find the package
        # dir when resolving $ORIGIN.
        file(MAKE_DIRECTORY "${CPACK_TEMPORARY_DIRECTORY}/DEBIAN")
        ...
        if(CPACK_DEBIAN_PACKAGE_SHLIBDEPS_PRIVATE_DIRS)
          ...
          if(DEFINED SHLIBDEPS_EXECUTABLE_VERSION
             AND SHLIBDEPS_EXECUTABLE_VERSION VERSION_GREATER_EQUAL 1.17.0)
            ...
            message(
              WARNING
                "CPackDeb: dkpg-shlibdeps is too old. \"CPACK_DEBIAN_PACKAGE_SHLIBDEPS_PRIVATE_DIRS\" is therefore ignored."
            )
          endif()
        endif()

        # Execute dpkg-shlibdeps --ignore-missing-info : allow dpkg-shlibdeps to
        # run even if some libs do not belong to a package -l<dir>: make
        # dpkg-shlibdeps also search in this directory for (private) shared
        # library dependencies -O : print to STDOUT
        execute_process(
          COMMAND ${SHLIBDEPS_EXECUTABLE} ${PRIVATE_SEARCH_DIR_OPTIONS}
        ...
        # E2K OSL 6.0.1 and prior has broken dpkg-shlibdeps. CPack will deal
        # with that (mocking SHLIBDEPS_OUTPUT), but inform user of this.
        if("${SHLIBDEPS_ERROR}" MATCHES
           "unknown gcc system type e2k.*, falling back to default")
          message(
            WARNING
        ...
        if(CPACK_DEBIAN_PACKAGE_DEBUG)
          message(
            "CPackDeb Debug: Found dependency: ${CPACK_DEBIAN_PACKAGE_AUTO_DEPENDS} from output ${SHLIBDEPS_OUTPUT}"
          )
        endif()

        # Remove blank control file Might not be safe if package actual contain
        # file or directory named debian
        file(REMOVE_RECURSE "${CPACK_TEMPORARY_DIRECTORY}/debian")

        # remove temporary directory that was created only for dpkg-shlibdeps
        # execution
        file(REMOVE_RECURSE "${CPACK_TEMPORARY_DIRECTORY}/DEBIAN")
      else()
        if(CPACK_DEBIAN_PACKAGE_DEBUG)
          message(
            AUTHOR_WARNING
              "CPackDeb Debug: Using only user-provided depends because package does not contain executable files that link to shared libraries."
          )
        endif()
      endif()
    else()
  ...
  if(DEFINED CPACK_DEBIAN_PACKAGE_RELEASE OR DEFINED CPACK_DEBIAN_PACKAGE_EPOCH)
    ...
    if(NOT CPACK_DEBIAN_PACKAGE_VERSION MATCHES
       "^([0-9]+:)?[0-9][A-Za-z0-9.+~-]*$")
      message(
        AUTHOR_WARNING
  ...
  if(CPACK_DEB_PACKAGE_COMPONENT
     AND CPACK_DEBIAN_${_local_component_name}_PACKAGE_ARCHITECTURE)
    set(CPACK_DEBIAN_PACKAGE_ARCHITECTURE
        "${CPACK_DEBIAN_${_local_component_name}_PACKAGE_ARCHITECTURE}")
  elseif(NOT CPACK_DEBIAN_PACKAGE_ARCHITECTURE)
    # There is no such thing as i686 architecture on debian, you should use i386
    # instead $ dpkg --print-architecture
    find_program(DPKG_CMD dpkg)
    if(NOT DPKG_CMD)
      message(
        STATUS "CPackDeb: Can not find dpkg in your path, default to i386.")
  ...
  # Source: (optional) in case several packages are constructed from a unique
  # source (multipackaging), the source may be indicated as well. The source
  # might contain a version if the generated package version is different from
  # the source version
  if(NOT CPACK_DEBIAN_PACKAGE_SOURCE)
    set(CPACK_DEBIAN_PACKAGE_SOURCE "")
  endif()

  # have a look at get_property(result GLOBAL PROPERTY ENABLED_FEATURES), this
  # returns the successful find_package() calls, maybe this can help Depends:
  # You should set: DEBIAN_PACKAGE_DEPENDS TODO: automate 'objdump -p | grep
  # NEEDED'

  # if per-component variable, overrides the global
  # CPACK_DEBIAN_PACKAGE_${variable_type_} automatic dependency discovery will
  ...
  if(CPACK_DEB_PACKAGE_COMPONENT)
    foreach(
      ...
      if(DEFINED ${_component_var})
        ...
        if(CPACK_DEBIAN_PACKAGE_DEBUG)
          message(
            "CPackDeb Debug: component '${_local_component_name}' ${value_type_} "
    ...
    if(CPACK_DEBIAN_ENABLE_COMPONENT_DEPENDS)
      ...
      foreach(_PACK IN LISTS CPACK_COMPONENT_${_local_component_name}_DEPENDS)
        ...
        list(PREPEND COMPONENT_DEPENDS
      ...
      if(COMPONENT_DEPENDS)
        ...
        list(JOIN CPACK_DEBIAN_PACKAGE_DEPENDS ", "
             CPACK_DEBIAN_PACKAGE_DEPENDS)
      endif()
    endif()
  endif()

  # at this point, the CPACK_DEBIAN_PACKAGE_DEPENDS is properly set to the
  # minimal dependency of the package Append automatically discovered
  # dependencies .
  ...
  if(NOT CPACK_DEBIAN_PACKAGE_MAINTAINER)
    if(NOT CPACK_PACKAGE_CONTACT)
      message(
        FATAL_ERROR
          "CPackDeb: Debian package requires a maintainer for a package, set CPACK_PACKAGE_CONTACT or CPACK_DEBIAN_PACKAGE_MAINTAINER"
      )
    endif()
    set(CPACK_DEBIAN_PACKAGE_MAINTAINER ${CPACK_PACKAGE_CONTACT})
  endif()

  # Description: (mandatory) Try package description first
  ...
  if(NOT CPACK_DEBIAN_PACKAGE_DESCRIPTION
     ...
     AND NOT CPACK_PACKAGE_DESCRIPTION_FILE STREQUAL
  ...
  if(NOT CPACK_DEBIAN_PACKAGE_DESCRIPTION)
    ...
    # Ok, description has set. According to the `Debian Policy Manual`_ the
    # first line is a package summary.  Try to get it as well... See also:
    # https://www.debian.org/doc/debian-policy/ch-controlfields.html#description
  elseif(CPACK_PACKAGE_DESCRIPTION_SUMMARY
         AND NOT CPACK_PACKAGE_DESCRIPTION_SUMMARY STREQUAL
             CPACK_DEFAULT_PACKAGE_DESCRIPTION_SUMMARY)
    # Merge summary w/ the detailed description
    string(PREPEND CPACK_DEBIAN_PACKAGE_DESCRIPTION
           "${CPACK_PACKAGE_DESCRIPTION_SUMMARY}\n")
  endif()
  # assert(CPACK_DEBIAN_PACKAGE_DESCRIPTION)

  # Make sure description is properly formatted
  cpack_deb_format_package_description("${CPACK_DEBIAN_PACKAGE_DESCRIPTION}"
                                       CPACK_DEBIAN_PACKAGE_DESCRIPTION)
  ...
  # Recommends: You should set: CPACK_DEBIAN_PACKAGE_RECOMMENDS

  # Suggests: You should set: CPACK_DEBIAN_PACKAGE_SUGGESTS

  # CPACK_DEBIAN_PACKAGE_CONTROL_EXTRA This variable allow advanced user to add
  # custom script to the control.tar.gz (inside the .deb archive) Typical
  # examples are: - conffiles - postinst - postrm - prerm Usage:
  ...
  if(CPACK_DEB_PACKAGE_COMPONENT)
    ...
    foreach(VAR_NAME_ IN ITEMS PACKAGE_CONTROL_EXTRA
    ...
    endforeach()
    get_component_package_name(CPACK_DEBIAN_PACKAGE_NAME
                               ${_local_component_name})
  endif()
    ...
    if(CPACK_READELF_EXECUTABLE)
      foreach(_FILE IN LISTS CPACK_DEB_SHARED_OBJECT_FILES)
        ...
        if(libname AND DEFINED soversion)
          ...
          message(
            AUTHOR_WARNING
              "Shared library '${_FILE}' is missing soname or soversion. Library will not be added to DEBIAN/shlibs control file."
          )
        endif()
      endforeach()
  ...
  cpack_deb_variable_fallback(
    "CPACK_DEBIAN_FILE_NAME" "CPACK_DEBIAN_${_local_component_name}_FILE_NAME"
  ...
  if(CPACK_DEBIAN_FILE_NAME)
    if(CPACK_DEBIAN_FILE_NAME STREQUAL "DEB-DEFAULT")
      ...
      if(NOT CPACK_DEBIAN_FILE_NAME MATCHES ".*\\.(deb|ipk)")
        message(
          FATAL_ERROR
            "'${CPACK_DEBIAN_FILE_NAME}' is not a valid DEB package file name as it must end with '.deb' or '.ipk'!"
        )
      endif()

      set(CPACK_OUTPUT_FILE_NAME "${CPACK_DEBIAN_FILE_NAME}")
      string(REGEX
             REPLACE "\.deb$" "-dbgsym.ddeb" CPACK_DBGSYM_OUTPUT_FILE_NAME
                     "${CPACK_DEBIAN_FILE_NAME}")
    endif()
    ...
    string(REGEX REPLACE "\.deb$" "-dbgsym.ddeb" CPACK_DBGSYM_OUTPUT_FILE_NAME
  ...
  if(CPACK_DEBIAN_PACKAGE_DEBUG)
    message(
      "CPackDeb:Debug: CPACK_TOPLEVEL_DIRECTORY          = '${CPACK_TOPLEVEL_DIRECTORY}'"
    )
    message(
      "CPackDeb:Debug: CPACK_TOPLEVEL_TAG                = '${CPACK_TOPLEVEL_TAG}'"
    )
    message(
      "CPackDeb:Debug: CPACK_TEMPORARY_DIRECTORY         = '${CPACK_TEMPORARY_DIRECTORY}'"
    )
    message(
      "CPackDeb:Debug: CPACK_OUTPUT_FILE_NAME            = '${CPACK_OUTPUT_FILE_NAME}'"
    )
    message(
      "CPackDeb:Debug: CPACK_OUTPUT_FILE_PATH            = '${CPACK_OUTPUT_FILE_PATH}'"
    )
    message(
      "CPackDeb:Debug: CPACK_PACKAGE_FILE_NAME           = '${CPACK_PACKAGE_FILE_NAME}'"
    )
    message(
      "CPackDeb:Debug: CPACK_PACKAGE_INSTALL_DIRECTORY   = '${CPACK_PACKAGE_INSTALL_DIRECTORY}'"
    )
    message(
      "CPackDeb:Debug: CPACK_TEMPORARY_PACKAGE_FILE_NAME = '${CPACK_TEMPORARY_PACKAGE_FILE_NAME}'"
    )
    message(
      "CPackDeb:Debug: CPACK_DEBIAN_PACKAGE_CONTROL_STRICT_PERMISSION = '${CPACK_DEBIAN_PACKAGE_CONTROL_STRICT_PERMISSION}'"
    )
    message(
  ...
  # For debian source packages: debian/control
  # http://www.debian.org/doc/debian-policy/ch-controlfields.html#s-sourcecontrolfiles

  # .dsc
  # http://www.debian.org/doc/debian-policy/ch-controlfields.html#s-debiansourcecontrolfiles

  # Builds-Depends: if(NOT CPACK_DEBIAN_PACKAGE_BUILDS_DEPENDS)
  # set(CPACK_DEBIAN_PACKAGE_BUILDS_DEPENDS "debhelper (>> 5.0.0),
  # libncurses5-dev, tcl8.4" ) endif()

  # move variables to parent scope so that they may be used to create debian
  # package
  set(GEN_CPACK_OUTPUT_FILE_NAME
      "${CPACK_OUTPUT_FILE_NAME}"
      PARENT_SCOPE)
  set(GEN_CPACK_TEMPORARY_PACKAGE_FILE_NAME
      "${CPACK_TEMPORARY_PACKAGE_FILE_NAME}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_PACKAGE_NAME
      "${CPACK_DEBIAN_PACKAGE_NAME}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_PACKAGE_VERSION
      "${CPACK_DEBIAN_PACKAGE_VERSION}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_PACKAGE_SECTION
      "${CPACK_DEBIAN_PACKAGE_SECTION}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_PACKAGE_PRIORITY
      "${CPACK_DEBIAN_PACKAGE_PRIORITY}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_PACKAGE_ARCHITECTURE
      "${CPACK_DEBIAN_PACKAGE_ARCHITECTURE}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_PACKAGE_MAINTAINER
  ...
  set(GEN_CPACK_DEBIAN_PACKAGE_DEPENDS
      "${CPACK_DEBIAN_PACKAGE_DEPENDS}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_ARCHIVE_TYPE
      "${CPACK_DEBIAN_ARCHIVE_TYPE}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_COMPRESSION_TYPE
      "${CPACK_DEBIAN_COMPRESSION_TYPE}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_PACKAGE_RECOMMENDS
      "${CPACK_DEBIAN_PACKAGE_RECOMMENDS}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_PACKAGE_SUGGESTS
      "${CPACK_DEBIAN_PACKAGE_SUGGESTS}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_PACKAGE_HOMEPAGE
      "${CPACK_DEBIAN_PACKAGE_HOMEPAGE}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_PACKAGE_PREDEPENDS
      "${CPACK_DEBIAN_PACKAGE_PREDEPENDS}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_PACKAGE_ENHANCES
      "${CPACK_DEBIAN_PACKAGE_ENHANCES}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_PACKAGE_BREAKS
      "${CPACK_DEBIAN_PACKAGE_BREAKS}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_PACKAGE_CONFLICTS
      "${CPACK_DEBIAN_PACKAGE_CONFLICTS}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_PACKAGE_PROVIDES
      "${CPACK_DEBIAN_PACKAGE_PROVIDES}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_PACKAGE_REPLACES
      "${CPACK_DEBIAN_PACKAGE_REPLACES}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_PACKAGE_SHLIBS
      "${CPACK_DEBIAN_PACKAGE_SHLIBS_LIST}"
      PARENT_SCOPE)
  ...
  set(GEN_CPACK_DEBIAN_PACKAGE_SOURCE
      ...
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_GENERATE_POSTINST
      "${CPACK_DEBIAN_GENERATE_POSTINST}"
      PARENT_SCOPE)
  set(GEN_CPACK_DEBIAN_GENERATE_POSTRM
      "${CPACK_DEBIAN_GENERATE_POSTRM}"
      PARENT_SCOPE)
  set(GEN_WDIR
      "${WDIR}"
      PARENT_SCOPE)

  set(GEN_CPACK_DEBIAN_DEBUGINFO_PACKAGE
      "${CPACK_DEBIAN_DEBUGINFO_PACKAGE}"
      PARENT_SCOPE)
  if(BUILD_IDS)
    set(GEN_DBGSYMDIR
        "${DBGSYMDIR}"
        PARENT_SCOPE)
    set(GEN_CPACK_DBGSYM_OUTPUT_FILE_NAME
