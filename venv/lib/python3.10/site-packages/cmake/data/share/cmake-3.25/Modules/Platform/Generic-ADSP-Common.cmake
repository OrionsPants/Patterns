# support for the Analog Devices toolchain for their DSPs Raphael Cotty"
# <raphael.cotty (AT) googlemail.com>
#
# it supports three architectures: Blackfin TS (TigerShark) 21k (Sharc 21xxx)
...
if(NOT ADSP)
  ...
  # if ADSP_PROCESSOR has not been set, but CMAKE_SYSTEM_PROCESSOR has, assume
  # that this is the processor name to use for the compiler
  ...
  if(CMAKE_C_COMPILER MATCHES ccblkfn
     OR CMAKE_CXX_COMPILER MATCHES ccblkfn
     OR CMAKE_ASM_COMPILER MATCHES easmBLKFN)
  ...
  if(CMAKE_C_COMPILER MATCHES ccts
     OR CMAKE_CXX_COMPILER MATCHES ccts
     OR CMAKE_ASM_COMPILER MATCHES easmTS)
    if(NOT ADSP_PROCESSOR)
  ...
  if(CMAKE_C_COMPILER MATCHES cc21k
     OR CMAKE_CXX_COMPILER MATCHES cc21k
     OR CMAKE_ASM_COMPILER MATCHES easm21k)
    if(NOT ADSP_PROCESSOR)
  ...
  macro(ADSP_GENERATE_VDK VDK_GENERATED_HEADER VDK_GENERATED_SOURCE
        VDK_KERNEL_SUPPORT_FILE)
    add_custom_command(
      OUTPUT ${VDK_GENERATED_HEADER} ${VDK_GENERATED_SOURCE}
      COMMAND
        ${ADSP_VDKGEN_EXECUTABLE} ${VDK_KERNEL_SUPPORT_FILE} -proc
        ${ADSP_PROCESSOR} -si-revision ${ADSP_PROCESSOR_SILICIUM_REVISION} -MM
  ...
  # BOOT_MODE: prom, flash, spi, spislave, UART, TWI, FIFO FORMAT: hex, ASCII,
  # binary, include WIDTH: 8, 16
  macro(ADSP_CREATE_LOADER_FILE TARGET_NAME BOOT_MODE FORMAT WIDTH)
    add_custom_command(
      TARGET ${TARGET_NAME}
      POST_BUILD
      COMMAND
        ${ADSP_ELFLOADER_EXECUTABLE}
        ${EXECUTABLE_OUTPUT_PATH}/${TARGET_NAME}.dxe -proc ${ADSP_PROCESSOR}
        -si-revision ${ADSP_PROCESSOR_SILICIUM_REVISION} -b ${BOOT_MODE} -f
        ${FORMAT} -width ${WIDTH} -o
        ${EXECUTABLE_OUTPUT_PATH}/${TARGET_NAME}.ldr
      COMMENT "Building the loader file")
  endmacro()

  macro(ADSP_CREATE_LOADER_FILE_INIT TARGET_NAME BOOT_MODE FORMAT WIDTH
        INITIALIZATION_FILE)
    add_custom_command(
      TARGET ${TARGET_NAME}
      POST_BUILD
      COMMAND
        ...
        -si-revision ${ADSP_PROCESSOR_SILICIUM_REVISION} -b ${BOOT_MODE} -f
        ${FORMAT} -width ${WIDTH} -o
        ${EXECUTABLE_OUTPUT_PATH}/${TARGET_NAME}.ldr -init
        ${INITIALIZATION_FILE}
      COMMENT "Building the loader file")
  endmacro()
