...
if(NOT CMAKE_CROSSCOMPILING
   ...
   AND CMAKE_HOST_SYSTEM_PROCESSOR MATCHES "^(arm64|x86_64)$")
  execute_process(
    COMMAND sysctl -q hw.optional.arm64
    OUTPUT_VARIABLE _sysctl_stdout
    ERROR_VARIABLE _sysctl_stderr
    RESULT_VARIABLE _sysctl_result)
  # When building on an Apple Silicon host, we need to explicitly specify the
  # architecture to the toolchain since it will otherwise guess the architecture
  # based on that of the build system tool. Set an *internal variable* to tell
  # the generators to do this.
...
#
string(REGEX REPLACE "^([0-9]+\\.[0-9]+).*$" "\\1" _CURRENT_OSX_VERSION
                     "${CURRENT_OSX_VERSION}")
...
# configure.
if(CMAKE_SYSTEM_NAME STREQUAL "Darwin" AND _CURRENT_OSX_VERSION VERSION_GREATER
                                           10.3)
...
elseif(NOT "x$ENV{SDKROOT}" STREQUAL "x" AND (NOT "x$ENV{SDKROOT}" MATCHES "/"
                                              OR IS_DIRECTORY "$ENV{SDKROOT}"))
...
elseif(
  ...
  if(_CMAKE_OSX_SDKS_DIR)
    ...
    file(
      GLOB _CMAKE_OSX_SDKS
      RELATIVE "${_CMAKE_OSX_SDKS_DIR}"
...
# Set cache variable - end user may change this during ccmake or cmake-gui
# configure. Choose the type based on the current value.
...
function(_apple_resolve_supported_archs_for_sdk_from_system_lib sdk_path ret
         ret_failed)
  # Detect the supported SDK architectures by inspecting the main libSystem
  # library.
  ...
  # Newer SDKs ship text based dylib stub files which contain the architectures
  # supported by the library in text form.
  if(EXISTS "${system_lib_tbd_path}")
    file(STRINGS "${system_lib_tbd_path}" tbd_lines
         REGEX "^(archs|targets): +\\[.+\\]")
    if(NOT tbd_lines)
      set(${ret_failed}
          TRUE
    ...
    # The tbd architectures line looks like the following: archs:           [
    # armv7, armv7s, arm64, arm64e ] or for version 4 TBD files: targets: [
    # armv7-ios, armv7s-ios, arm64-ios, arm64e-ios ]
    list(GET tbd_lines 0 first_arch_line)
    string(REGEX REPLACE "(archs|targets): +\\[ (.+) \\]" "\\2"
                         arches_comma_separated "${first_arch_line}")
  ...
  elseif(EXISTS "${system_lib_dylib_path}")
    # Old SDKs (Xcode < 7) ship dylib files, use lipo to inspect the supported
    # architectures. Can't use -archs because the option is not available in
    ...
    if(_failed
       OR NOT lipo_output
       OR NOT lipo_output MATCHES
          "(Non-fat file:|Architectures in the fat file:)")
      set(${ret_failed}
          TRUE
          PARENT_SCOPE)
      return()
    ...
    # The lipo output looks like the following: Non-fat file: <path> is
    # architecture: i386 Architectures in the fat file: <path> are: i386 x86_64
    string(REGEX REPLACE "^(.+)is architecture:(.+)" "\\2"
                         arches_space_separated "${lipo_output}")
    string(REGEX REPLACE "^(.+)are:(.+)" "\\2" arches_space_separated
                         "${arches_space_separated}")

    # Need to clean up the arches, with Xcode 4.6.3 the output of lipo -info
    # contains some additional info, e.g. Architectures in the fat file: <path>
    # are: armv7 (cputype (12) cpusubtype (11))
    string(REGEX REPLACE "\\(.+\\)" "" arches_space_separated
                         "${arches_space_separated}")
    ...
    message(
      WARNING "No way to find architectures for given sdk_path '${sdk_path}'")
    set(${ret_failed}
        TRUE
        PARENT_SCOPE)
  endif()
...
# Handle multi-arch sysroots. Do this before CMAKE_OSX_SYSROOT is transformed
# into a path, so that we know the sysroot name.
function(_apple_resolve_multi_arch_sysroots)
  ...
  if(CMAKE_OSX_SYSROOT STREQUAL "macosx")
    # macOS doesn't have a simulator sdk / sysroot, so there is no need to
    # handle per-sdk arches.
    return()
  endif()
  ...
  if(IS_DIRECTORY "${CMAKE_OSX_SYSROOT}")
    if(NOT CMAKE_OSX_SYSROOT STREQUAL _CMAKE_OSX_SYSROOT_DEFAULT)
      message(
        WARNING
          "Can not resolve multi-arch sysroots with CMAKE_OSX_SYSROOT set to path (${CMAKE_OSX_SYSROOT})"
      )
    endif()
    return()
  endif()
  ...
  foreach(sdk ${_sdks})
    ...
    if(_failed)
      # Failure to extract supported architectures for an SDK means that the
      # installed SDK is old and does not provide such information (SDKs that
      # come with Xcode >= 10.x started providing the information). In such a
      # case, return early, and handle multi-arch builds the old way (no per-sdk
      # arches).
      return()
    endif()
