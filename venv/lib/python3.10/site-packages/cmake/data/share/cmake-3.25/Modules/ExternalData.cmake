...
function(ExternalData_add_target target)
  ...
  configure_file(${_ExternalData_SELF_DIR}/ExternalData_config.cmake.in
                 ${config} @ONLY)
  ...
  # name to avoid repeating custom commands with the same output in this
  # directory. Repeating custom commands with the same output across directories
  # or across targets in the same directory may be a race, but this is likely
  # okay because we use atomic replacement of output files.
  #
  ...
  foreach(entry IN LISTS data_local)
    ...
    if(NOT DEFINED "_ExternalData_FILE_${file}")
      ...
      get_property(
        added
        DIRECTORY
        PROPERTY "_ExternalData_FILE_${file}")
      if(NOT added)
        ...
        add_custom_command(
          ...
          OUTPUT "${file}"
          COMMAND
            ${CMAKE_COMMAND} -Drelative_top=${CMAKE_BINARY_DIR} -Dfile=${file}
            -Dname=${name} -DExternalData_ACTION=local
            -DExternalData_SHOW_PROGRESS=${_ExternalData_add_target_SHOW_PROGRESS}
            -DExternalData_CONFIG=${config} -P ${_ExternalData_SELF}
    ...
    if(NOT DEFINED "_ExternalData_FILE_${file}")
      ...
      if(NOT added)
        ...
        add_custom_command(
          ...
          COMMAND
            ${CMAKE_COMMAND} -Drelative_top=${CMAKE_BINARY_DIR} -Dfile=${file}
            -Dname=${name} -Dexts=${exts} -DExternalData_ACTION=fetch
            -DExternalData_SHOW_PROGRESS=${_ExternalData_add_target_SHOW_PROGRESS}
            -DExternalData_CONFIG=${config} -P ${_ExternalData_SELF}
...
function(ExternalData_expand_arguments target outArgsVar)
  ...
  set(outArgs "")
  # This list expansion un-escapes semicolons in list element values so we must
  # re-escape them below anywhere a new list expansion will occur.
...
set(_ExternalData_REGEX_ALGO
    "MD5|SHA1|SHA224|SHA256|SHA384|SHA512|SHA3_224|SHA3_256|SHA3_384|SHA3_512")
set(_ExternalData_REGEX_EXT
...
function(_ExternalData_arg target arg options var_file)
  ...
  if(external)
    ...
    set_property(GLOBAL APPEND PROPERTY _ExternalData_${target}_FETCH
                                        "${external}")
    set_property(GLOBAL APPEND PROPERTY _ExternalData_${target}_LOCAL
                                        "${internal}")
    set("${var_file}"
        "${top_bin}/${reldata}"
        PARENT_SCOPE)
...
macro(_ExternalData_arg_series)
  ...
  if(ExternalData_SERIES_PARSE)
    ...
    elseif(NOT "x${ExternalData_SERIES_PARSE}" MATCHES
           "^x\\([^()]*\\)\\([^()]*\\)\\$$")
      message(
        FATAL_ERROR
  ...
  # Glob files that might match the series. Then match base, number, and
  # extension.
...
function(_ExternalData_arg_find_files glob pattern regex)
  ...
  foreach(entry IN LISTS globbed)
    ...
    if("x${relname}" MATCHES "^x${regex}$" # matches
      ...
      if(IS_DIRECTORY "${top_src}/${entry}")
        ...
        if(alg)
          if(NOT "${external_${externals_count}_file_name}" STREQUAL
             "${file}|${name}")
  ...
  set(have_original
      "${have_original}"
      PARENT_SCOPE)
  set(have_original_as_dir
      "${have_original_as_dir}"
      PARENT_SCOPE)
endfunction()
...
function(_ExternalData_link_or_copy src dst)
  ...
  if(UNIX AND NOT ExternalData_NO_SYMLINKS)
    ...
    if(relative_top)
      ...
      if(NOT IS_ABSOLUTE "${relsrc}"
         AND NOT "${relsrc}" MATCHES "^\\.\\./"
         AND NOT IS_ABSOLUTE "${reldst}"
         AND NOT "${reldst}" MATCHES "^\\.\\./")
...
function(_ExternalData_download_file url file err_var msg_var)
  ...
  while(retry)
    ...
    file(
      ...
      STATUS status
      LOG log
      ${inactivity_timeout} ${absolute_timeout} ${show_progress_args})
    list(GET status 0 err)
    list(GET status 1 msg)
    if(err)
      if("${msg}" MATCHES "HTTP response code said error" AND "${log}" MATCHES
                                                              "error: 503")
...
function(
  ...
  foreach(url_template IN LISTS ExternalData_URL_TEMPLATES)
    ...
    string(REGEX REPLACE "((https?|ftp)://)([^@]+@)?(.*)" "\\1\\4" secured_url
                         "${url}")
    message(STATUS "Fetching \"${secured_url}\"")
    if(url MATCHES "^ExternalDataCustomScript://([A-Za-z_][A-Za-z0-9_]*)/(.*)$")
      _externaldata_custom_fetch("${CMAKE_MATCH_1}" "${CMAKE_MATCH_2}" "${tmp}"
                                 err errMsg)
    else()
  ...
  set(success 1)
  if(found)
    # Atomically create the object.  If we lose a race with another process, do
    # not replace it.  Content-addressing ensures it has what we expect.
...
if("${ExternalData_ACTION}" STREQUAL "fetch")
  ...
  if(NOT succeeded)
    foreach(ii RANGE 0 ${exts_range})
      list(GET hash_list ${ii} hash)
      list(GET algo_list ${ii} algo)
      _externaldata_download_object("${name}" "${hash}" "${algo}" obj succeeded
                                    algoErrorMsg)
