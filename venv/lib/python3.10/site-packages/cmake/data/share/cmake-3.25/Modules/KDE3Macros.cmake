...
#
# this file contains the following macros: KDE3_ADD_DCOP_SKELS
# KDE3_ADD_DCOP_STUBS KDE3_ADD_MOC_FILES KDE3_ADD_UI_FILES KDE3_ADD_KCFG_FILES
# KDE3_AUTOMOC KDE3_INSTALL_LIBTOOL_FILE KDE3_CREATE_FINAL_FILE KDE3_ADD_KPART
# KDE3_ADD_KDEINIT_EXECUTABLE KDE3_ADD_EXECUTABLE

# neundorf@kde.org

include(AddFileDependencies)

# create the kidl and skeletion file for dcop stuff usage:
# KDE_ADD_COP_SKELS(foo_SRCS ${dcop_headers})
macro(KDE3_ADD_DCOP_SKELS _sources)
  foreach(_current_FILE ${ARGN})
    ...
    if(NOT HAVE_${_basename}_KIDL_RULE)
      ...
      add_custom_command(
        OUTPUT ${_kidl}
        COMMAND ${KDE3_DCOPIDL_EXECUTABLE} ARGS ${_tmp_FILE} > ${_kidl}
        DEPENDS ${_tmp_FILE})
    ...
    if(NOT HAVE_${_basename}_SKEL_RULE)
      ...
      add_custom_command(
        OUTPUT ${_skel}
        COMMAND ${KDE3_DCOPIDL2CPP_EXECUTABLE} ARGS --c++-suffix cpp
                --no-signals --no-stub ${_kidl}
        DEPENDS ${_kidl})
...
macro(KDE3_ADD_KCFG_FILES _sources)
  foreach(_current_FILE ${ARGN})
    ...
    add_custom_command(
      OUTPUT ${_src_FILE}
      COMMAND ${KDE3_KCFGC_EXECUTABLE} ARGS
              ${CMAKE_CURRENT_SOURCE_DIR}/${_kcfg_FILE} ${_tmp_FILE}
...
# create the moc files and add them to the list of sources usage:
# KDE_ADD_MOC_FILES(foo_SRCS ${moc_headers})
macro(KDE3_ADD_MOC_FILES _sources)
  foreach(_current_FILE ${ARGN})
    ...
    add_custom_command(
      OUTPUT ${_moc}
      COMMAND ${QT_MOC_EXECUTABLE} ARGS ${_tmp_FILE} -o ${_moc}
      DEPENDS ${_tmp_FILE})
...
# sources usage: KDE_ADD_UI_FILES(foo_SRCS ${ui_files})
macro(KDE3_ADD_UI_FILES _sources)
  foreach(_current_FILE ${ARGN})
    ...
    add_custom_command(
      OUTPUT ${_header}
      COMMAND
        ${QT_UIC_EXECUTABLE} ARGS -L ${KDE3_LIB_DIR}/kde3/plugins/designer
        -nounload -o ${_header} ${CMAKE_CURRENT_SOURCE_DIR}/${_current_FILE}
      DEPENDS ${_tmp_FILE})
    ...
    add_custom_command(
      ...
      COMMAND
        ...
        -DKDE_UIC_FILE:FILEPATH=${_tmp_FILE} -DKDE_UIC_CPP_FILE:FILEPATH=${_src}
        -DKDE_UIC_H_FILE:FILEPATH=${_header} -P ${KDE3_MODULE_DIR}/kde3uic.cmake
      DEPENDS ${_header})

    add_custom_command(
      OUTPUT ${_moc}
      COMMAND ${QT_MOC_EXECUTABLE} ARGS ${_header} -o ${_moc}
      DEPENDS ${_header})
...
macro(KDE3_AUTOMOC)
  ...
  foreach(_current_FILE ${ARGN})
    ...
    # if "SKIP_AUTOMOC" is set to true, we will not handle this file here. here.
    # this is required to make bouic work correctly: we need to add generated
    # .cpp files to the sources (to compile them), but we cannot let automoc
    # handle them, as the .cpp files don't exist yet when cmake is run for the
    # very first time on them -> however the .cpp files might exist at a later
    # run. at that time we need to skip them, so that we don't add two different
...
macro(_KDE3_ADD_ICON_INSTALL_RULE _install_SCRIPT _install_PATH _group
  ...
  # if the string doesn't match the pattern, the result is the full string, so
  ...
  if(NOT ${_group} STREQUAL ${_install_NAME})
    ...
    install(
      FILES ${_orig_NAME}
      DESTINATION ${_install_PATH}/${_icon_GROUP}/
...
macro(KDE3_INSTALL_ICONS _theme)
  ...
  foreach(_current_ICON ${_icons})
    string(REGEX REPLACE "^.*/[a-zA-Z]+([0-9]+)\\-([a-z]+)\\-(.+\\.png)$" "\\1"
                         _size "${_current_ICON}")
    string(REGEX REPLACE "^.*/[a-zA-Z]+([0-9]+)\\-([a-z]+)\\-(.+\\.png)$" "\\2"
                         _group "${_current_ICON}")
    string(REGEX REPLACE "^.*/[a-zA-Z]+([0-9]+)\\-([a-z]+)\\-(.+\\.png)$" "\\3"
                         _name "${_current_ICON}")
    _kde3_add_icon_install_rule(
      ${CMAKE_CURRENT_BINARY_DIR}/install_icons.cmake
      ${_defaultpath}/${_theme}/${_size}x${_size} ${_group} ${_current_ICON}
      ${_name})
  endforeach()
  ...
  foreach(_current_ICON ${_icons})
    ...
    _kde3_add_icon_install_rule(
      ${CMAKE_CURRENT_BINARY_DIR}/install_icons.cmake
      ${_defaultpath}/${_theme}/scalable ${_group} ${_current_ICON} ${_name})
  endforeach()
...
macro(KDE3_ADD_KPART _target_NAME _with_PREFIX)
  ...
  # if (KDE3_ENABLE_FINAL) KDE3_CREATE_FINAL_FILE(${_target_NAME}_final.cpp
  # ${_first_SRC} ${ARGN}) add_library(${_target_NAME} MODULE
  # ${_target_NAME}_final.cpp) else ()
...
macro(KDE3_ADD_KDEINIT_EXECUTABLE _target_NAME)
  ...
  # if (KDE3_ENABLE_FINAL) KDE3_CREATE_FINAL_FILE(${_target_NAME}_final.cpp
  # ${ARGN}) add_library(kdeinit_${_target_NAME} SHARED
  # ${_target_NAME}_final.cpp) else ()
...
macro(KDE3_ADD_EXECUTABLE _target_NAME)
  ...
  # if (KDE3_ENABLE_FINAL) KDE3_CREATE_FINAL_FILE(${_target_NAME}_final.cpp
  # ${ARGN}) add_executable(${_target_NAME} ${_target_NAME}_final.cpp) else ()
