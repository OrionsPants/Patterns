...
elseif(APPLE)
  ...
  find_library(OPENGL_glu_LIBRARY OpenGL
               DOC "GLU library for OS X (usually same as OpenGL library)")
  find_path(OPENGL_INCLUDE_DIR OpenGL/gl.h DOC "Include for OpenGL on OS X")
  list(APPEND _OpenGL_REQUIRED_VARS OPENGL_INCLUDE_DIR)

  list(APPEND _OpenGL_CACHE_VARS OPENGL_INCLUDE_DIR OPENGL_gl_LIBRARY
       OPENGL_glu_LIBRARY)
else()
  if(CMAKE_SYSTEM_NAME MATCHES "HP-UX")
    # Handle HP-UX cases where we only want to find OpenGL in either hpux64 or
    # hpux32 depending on if we're doing a 64 bit build.
    if(CMAKE_SIZEOF_VOID_P EQUAL 4)
      set(_OPENGL_LIB_PATH /opt/graphics/OpenGL/lib/hpux32/)
    else()
      set(_OPENGL_LIB_PATH /opt/graphics/OpenGL/lib/hpux64/
  ...
  # The first line below is to make sure that the proper headers are used on a
  # Linux machine with the NVidia drivers installed. They replace Mesa with
  # NVidia's own library but normally do not install headers and that causes the
  # linking to fail since the compiler finds the Mesa headers but NVidia's
  # library. Make sure the NVIDIA directory comes BEFORE the others. - Atanas
  # Georgiev <atanas@cs.columbia.edu>
  find_path(OPENGL_INCLUDE_DIR GL/gl.h /usr/share/doc/NVIDIA_GLX-1.0/include
            /usr/openwin/share/include /opt/graphics/OpenGL/include
  ...
  find_path(OPENGL_xmesa_INCLUDE_DIR GL/xmesa.h
            /usr/share/doc/NVIDIA_GLX-1.0/include /usr/openwin/share/include
            /opt/graphics/OpenGL/include)
  list(APPEND _OpenGL_CACHE_VARS OPENGL_INCLUDE_DIR OPENGL_GLX_INCLUDE_DIR
       OPENGL_EGL_INCLUDE_DIR OPENGL_xmesa_INCLUDE_DIR)
  ...
  find_library(
    OPENGL_glu_LIBRARY
    NAMES GLU MesaGLU
    PATHS ${OPENGL_gl_LIBRARY} /opt/graphics/OpenGL/lib /usr/openwin/lib
          /usr/shlib)

  list(APPEND _OpenGL_CACHE_VARS OPENGL_opengl_LIBRARY OPENGL_glx_LIBRARY
       OPENGL_egl_LIBRARY OPENGL_glu_LIBRARY)
  ...
  elseif(OpenGL_FIND_COMPONENTS)
    # No preference was explicitly specified, but the caller did request at
    # least one GLVND component.  Prefer GLVND for legacy GL.
  ...
  if("x${OpenGL_GL_PREFERENCE}x" STREQUAL "xGLVNDx"
    ...
    # We can provide legacy GL using GLVND libraries. Do not use any legacy GL
    # library.
    set(OPENGL_gl_LIBRARY "")
  else()
    # We cannot provide legacy GL using GLVND libraries. Search for the legacy
    # GL library.
    find_library(
      OPENGL_gl_LIBRARY
      NAMES GL MesaGL
      PATHS /opt/graphics/OpenGL/lib /usr/openwin/lib /usr/shlib
            ${_OPENGL_LIB_PATH}
      PATH_SUFFIXES libglvnd)
  ...
  # now we append the libraries as appropriate.  The complicated logic basically
  # comes down to "use libOpenGL when we can, and add in specific context
  # mechanisms when requested, or we need them to preserve the previous default
  # where glx is always available."
...
# or libGL is found. Using libGL with libEGL is never okay, though; we handle
# that case later.
...
if(CMAKE_FIND_PACKAGE_NAME STREQUAL "GLU")
  # FindGLU include()'s this module. It's an old pattern, but rather than trying
...
find_package_handle_standard_args(
  OpenGL
  REQUIRED_VARS ${_OpenGL_REQUIRED_VARS}
  HANDLE_COMPONENTS)
...
if(OPENGL_FOUND)
  ...
  if(OPENGL_opengl_LIBRARY AND NOT TARGET OpenGL::OpenGL)
    if(IS_ABSOLUTE "${OPENGL_opengl_LIBRARY}")
      ...
      set_target_properties(
        OpenGL::OpenGL PROPERTIES IMPORTED_LOCATION "${OPENGL_opengl_LIBRARY}")
    else()
      add_library(OpenGL::OpenGL INTERFACE IMPORTED)
      set_target_properties(
        OpenGL::OpenGL PROPERTIES IMPORTED_LIBNAME "${OPENGL_opengl_LIBRARY}")
    endif()
    set_target_properties(
      OpenGL::OpenGL PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                "${OPENGL_INCLUDE_DIR}")
  endif()

  # ::GLX is a GLVND library, and thus Linux-only: we don't bother checking for
  # a framework version of this library.
  if(OpenGL_GLX_FOUND
  ...
  if(OPENGL_gl_LIBRARY AND NOT TARGET OpenGL::GL)
    ...
    if(IS_ABSOLUTE "${OPENGL_gl_LIBRARY}")
      ...
      if(OPENGL_gl_LIBRARY MATCHES "/([^/]+)\\.framework$")
        ...
        set_target_properties(OpenGL::GL PROPERTIES IMPORTED_LOCATION
                                                    "${_gl_fw}")
      else()
        set_target_properties(OpenGL::GL PROPERTIES IMPORTED_LOCATION
                                                    "${OPENGL_gl_LIBRARY}")
      ...
      set_target_properties(OpenGL::GL PROPERTIES IMPORTED_LIBNAME
                                                  "${OPENGL_gl_LIBRARY}")
    endif()
  ...
  # ::EGL is a GLVND library, and thus Linux-only: we don't bother checking for
  # a framework version of this library. Note we test for OpenGL::OpenGL as a
  # target.  When this module is updated to support GLES, we would additionally
  # want to check for the hypothetical GLES target and enable EGL if either
  # ::GLES or ::OpenGL is created.
  ...
  if(OPENGL_GLU_FOUND AND NOT TARGET OpenGL::GLU)
    if(IS_ABSOLUTE "${OPENGL_glu_LIBRARY}")
      ...
      if(OPENGL_glu_LIBRARY MATCHES "/([^/]+)\\.framework$")
        ...
        set_target_properties(OpenGL::GLU PROPERTIES IMPORTED_LOCATION
                                                     "${_glu_fw}")
      else()
        set_target_properties(OpenGL::GLU PROPERTIES IMPORTED_LOCATION
                                                     "${OPENGL_glu_LIBRARY}")
      ...
      set_target_properties(OpenGL::GLU PROPERTIES IMPORTED_LIBNAME
                                                   "${OPENGL_glu_LIBRARY}")
    endif()
    set_target_properties(OpenGL::GLU PROPERTIES INTERFACE_LINK_LIBRARIES
                                                 OpenGL::GL)
  endif()
