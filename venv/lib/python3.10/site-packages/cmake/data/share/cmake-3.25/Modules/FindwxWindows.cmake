...
# before calling this. -major update for wx 2.6.2 and monolithic build option.
# (10/2005)
#
# STATUS tested with: cmake 1.6.7, Linux (Suse 7.3), wxWindows 2.4.0, gcc 2.95
# cmake 1.6.7, Linux (Suse 8.2), wxWindows 2.4.0, gcc 3.3 cmake 1.6.7, Linux
# (Suse 8.2), wxWindows 2.4.1-patch1,  gcc 3.3 cmake 1.6.7, MS Windows XP home,
# wxWindows 2.4.1, MS Visual Studio .net 7 2002 (static build) cmake 2.0.5 on
# Windows XP and Suse Linux 9.2 cmake 2.0.6 on Windows XP and Suse Linux 9.2,
# wxWidgets 2.6.2 MONOLITHIC build cmake 2.2.2 on Windows XP, MS Visual Studio
# .net 2003 7.1 wxWidgets 2.6.2 MONOLITHIC build
#
# TODO -OPTION for unicode builds -further testing of DLL linking under MS WIN32
...
if(WIN32_STYLE_FIND)
  ...
  # Windows specific:
  #
  # candidates for root/base directory of wxwindows should have subdirs include
  # and lib containing include/wx/wx.h fix the root dir to avoid mixing of
  # headers/libs from different versions/builds:

  # WX supports monolithic and multiple smaller libs (since 2.5.x), we prefer
  # monolithic for now. monolithic = WX is built as a single big library e.g.
  # compile on WIN32 as  "nmake -f makefile.vc MONOLITHIC=1 BUILD=debug SHARED=0
  # USE_OPENGL=1" (JW)
  ...
  find_path(
    WXWINDOWS_ROOT_DIR include/wx/wx.h
    HINTS
      ENV
      WXWIN
    ...
    PATHS C:/wxWidgets-2.6.2 D:/wxWidgets-2.6.2 C:/wxWidgets-2.6.1
          D:/wxWidgets-2.6.1 C:/wxWindows-2.4.2 D:/wxWindows-2.4.2)
  # message("DBG found WXWINDOWS_ROOT_DIR: ${WXWINDOWS_ROOT_DIR}")

  # find libs for combination of static/shared with release/debug be careful if
  # you add something here, avoid mixing of headers and libs of different wx
  # versions, there may be multiple WX versions installed.
  set(WXWINDOWS_POSSIBLE_LIB_PATHS "${WXWINDOWS_ROOT_DIR}/lib")
  ...
  if(WXWINDOWS_USE_MONOLITHIC)
    ...
    find_library(
      ...
      PATHS "${WXWINDOWS_ROOT_DIR}/lib/vc_lib" ${WXWINDOWS_POSSIBLE_LIB_PATHS}
    ...
    find_library(
      ...
      NAMES wxmsw26
            ...
            wx23_2
            wx22_9
      PATHS "${WXWINDOWS_ROOT_DIR}/lib/vc_dll" ${WXWINDOWS_POSSIBLE_LIB_PATHS}
    ...
    #
    # required for WXWINDOWS_USE_GL gl lib is always build separate:
    #
    find_library(
    ...
    # option(WXWINDOWS_SEPARATE_LIBS_BUILD "Is wxWindows build with separate
    # libs?" OFF)

    # HACK: This is very dirty. because the libs of a particular version are
    # explicitly listed and NOT searched/verified. TODO:  Really search for each
    # lib, then decide for monolithic x debug x shared x GL (=16 combinations)
    # for at least 18 libs -->  about 288 combinations thus we need a different
    # approach so solve this correctly ...

    message(
      STATUS
        "Warning: You are trying to use wxWidgets without monolithic build (WXWINDOWS_SEPARATE_LIBS_BUILD). This is a HACK, libraries are not verified! (JW)."
    )

    set(WXWINDOWS_STATIC_LIBS
        ${WXWINDOWS_STATIC_LIBS}
        wxbase26
        wxbase26_net
        wxbase26_odbc
        wxbase26_xml
  ...
  #
  # now we should have found all WX libs available on the system. let the user
  # decide which of the available onse to use.
  #

  # if there is at least one shared lib available let user choose whether to use
  # shared or static wxwindows libs
  ...
  # opengl/glu: TODO/FIXME: better use FindOpenGL.cmake here assume release
  # versions of glu an dopengl, here.
  if(WXWINDOWS_USE_GL)
    set(WXWINDOWS_LIBRARIES ${WXWINDOWS_LIBRARIES} opengl32 glu32)
  endif()
  ...
  if(WXWINDOWS_USE_SHARED_LIBS)
    # message("DBG wxWindows use shared lib selected.") assume that both builds
    # use the same setup(.h) for simplicity

    # shared: both wx (debug and release) found? assume that both builds use the
    # same setup(.h) for simplicity
    if(WXWINDOWS_SHARED_DEBUG_LIBRARY AND WXWINDOWS_SHARED_LIBRARY)
      ...
      set(WXWINDOWS_LIBRARIES
          ${WXWINDOWS_LIBRARIES} debug ${WXWINDOWS_SHARED_DEBUG_LIBRARY}
          optimized ${WXWINDOWS_SHARED_LIBRARY})
      if(WXWINDOWS_USE_GL)
        set(WXWINDOWS_LIBRARIES
            ${WXWINDOWS_LIBRARIES} debug ${WXWINDOWS_SHARED_DEBUG_LIBRARY_GL}
    ...
    # static: both wx (debug and release) found? assume that both builds use the
    # same setup(.h) for simplicity
    if(WXWINDOWS_STATIC_DEBUG_LIBRARY AND WXWINDOWS_STATIC_LIBRARY)
      # message("DBG wx static: debug and optimized found.")
      find_path(
        WXWINDOWS_INCLUDE_DIR_SETUPH wx/setup.h ${WXWINDOWS_ROOT_DIR}/lib/mswd
        ${WXWINDOWS_ROOT_DIR}/lib/msw ${WXWINDOWS_ROOT_DIR}/lib/vc_lib/mswd
        ${WXWINDOWS_ROOT_DIR}/lib/vc_lib/msw)
      set(WXWINDOWS_LIBRARIES
          ${WXWINDOWS_LIBRARIES} debug ${WXWINDOWS_STATIC_DEBUG_LIBRARY}
          optimized ${WXWINDOWS_STATIC_LIBRARY})
      if(WXWINDOWS_USE_GL)
        set(WXWINDOWS_LIBRARIES
            ${WXWINDOWS_LIBRARIES} debug ${WXWINDOWS_STATIC_DEBUG_LIBRARY_GL}
  ...
  # not necessary in wxWindows 2.4.1 and 2.6.2 but it may fix a previous bug,
  # see
  ...
  # Find the include directories for wxwindows the first, build specific for
  # wx/setup.h was determined before. add inc dir for general for "wx/wx.h"
  find_path(WXWINDOWS_INCLUDE_DIR wx/wx.h "${WXWINDOWS_ROOT_DIR}/include")
  # append the build specific include dir for wx/setup.h:
  if(WXWINDOWS_INCLUDE_DIR_SETUPH)
    set(WXWINDOWS_INCLUDE_DIR ${WXWINDOWS_INCLUDE_DIR}
                              ${WXWINDOWS_INCLUDE_DIR_SETUPH})
  endif()
  ...
  if(UNIX_STYLE_FIND)
    ...
    # UNIX/Linux specific:
    #
    # use backquoted wx-config to query and set flags and libs: 06/2003 Jan
    # Woetzel
    #

    option(WXWINDOWS_USE_SHARED_LIBS
           "Use shared versions (.so) of wxWindows libraries" ON)
    mark_as_advanced(WXWINDOWS_USE_SHARED_LIBS)
    ...
    find_program(
      ...
      HINTS ENV WXWIN $ENV{WXWIN}/bin
      PATHS ../wx/bin ../../wx/bin)

    # check whether wx-config was found:
    if(CMAKE_WXWINDOWS_WXCONFIG_EXECUTABLE)

      # use shared/static wx lib? remember: always link shared to use systems GL
      # etc. libs (no static linking, just link *against* static .a libs)
      if(WXWINDOWS_USE_SHARED_LIBS)
        ...
        if(NOT APPLE) # -isystem seem sto be unsuppored on Mac
          if(CMAKE_COMPILER_IS_GNUCC AND CMAKE_COMPILER_IS_GNUCXX)
            if(CMAKE_CXX_COMPILER MATCHES g\\+\\+)
              set(CMAKE_WXWINDOWS_CXX_FLAGS
                  "`${CMAKE_WXWINDOWS_WXCONFIG_EXECUTABLE} --cxxflags|sed -e s/-I/-isystem/g`"
              )
            else()
              set(CMAKE_WXWINDOWS_CXX_FLAGS
                  "`${CMAKE_WXWINDOWS_WXCONFIG_EXECUTABLE} --cxxflags`")
            endif()
          endif()
        endif()
      endif()
      ...
      set(WXWINDOWS_LIBRARIES
          "`${CMAKE_WXWINDOWS_WXCONFIG_EXECUTABLE} ${WX_CONFIG_ARGS_LIBS}`")
      ...
      # rpath:
      exec_program(
        ${CMAKE_WXWINDOWS_WXCONFIG_EXECUTABLE} ARGS
        ${WX_CONFIG_ARGS_LIBS}
        OUTPUT_VARIABLE WX_CONFIG_LIBS)

      # extract linkdirs (-L) for rpath use regular expression to match wildcard
      # equivalent "-L*<endchar>" with <endchar> is a space or a semicolon
      string(REGEX MATCHALL "[-][L]([^ ;])+"
                   WXWINDOWS_LINK_DIRECTORIES_WITH_PREFIX "${WX_CONFIG_LIBS}")
      # message("DBG
      # WXWINDOWS_LINK_DIRECTORIES_WITH_PREFIX=${WXWINDOWS_LINK_DIRECTORIES_WITH_PREFIX}")
      ...
      if(WXWINDOWS_LINK_DIRECTORIES_WITH_PREFIX)
        string(REGEX REPLACE "[-][L]" ";" WXWINDOWS_LINK_DIRECTORIES
                             ${WXWINDOWS_LINK_DIRECTORIES_WITH_PREFIX})
        # message("DBG
        # WXWINDOWS_LINK_DIRECTORIES=${WXWINDOWS_LINK_DIRECTORIES}")
      endif()
      ...
      mark_as_advanced(CMAKE_WXWINDOWS_CXX_FLAGS WXWINDOWS_INCLUDE_DIR
                       WXWINDOWS_LIBRARIES CMAKE_WXWINDOWS_WXCONFIG_EXECUTABLE)

      # we really need wx-config...
    else()
      message(
        STATUS
          "Cannot find wx-config anywhere on the system. Please put the file into your path or specify it in CMAKE_WXWINDOWS_WXCONFIG_EXECUTABLE."
      )
    ...
    message(
      STATUS
        "FindwxWindows.cmake:  Platform unknown/unsupported by FindwxWindows.cmake. It's neither WIN32 nor UNIX"
    )
  endif()
endif()
