...
function(__java_copy_file src dest comment)
  add_custom_command(
    OUTPUT ${dest}
    COMMAND ${CMAKE_COMMAND} -E copy_if_different ARGS ${src} ${dest}
    DEPENDS ${src}
    COMMENT ${comment}
    VERBATIM)
endfunction()
...
function(__java_copy_resource_namespaces VAR DEST JAVA_RESOURCE_FILES
         JAVA_RESOURCE_FILES_RELATIVE)
  ...
  foreach(_item IN LISTS VAR)
    if(NOT _ns_ID)
      if(NOT _item STREQUAL "NAMESPACE")
        message(
          FATAL_ERROR "UseJava: Expecting \"NAMESPACE\", got\t\"${_item}\"")
        return()
      endif()
    endif()
    ...
    if(NOT _ns_VAL)
      # we're expecting the next token to be a namespace value whatever it is,
      # we're treating it like a namespace
      set(_ns_VAL "${_item}")
      continue()
    endif()
    ...
    __java_copy_file(${_abs_file_name} ${DEST}/${_dest_resource_file_name}
  ...
  set(${JAVA_RESOURCE_FILES}
      "${RESOURCE_FILES_LIST}"
      PARENT_SCOPE)
  set(${JAVA_RESOURCE_FILES_RELATIVE}
      "${RELATIVE_RESOURCE_FILES_LIST}"
      PARENT_SCOPE)
endfunction()

# define helper scripts
set(_JAVA_EXPORT_TARGETS_SCRIPT
    ${CMAKE_CURRENT_LIST_DIR}/UseJava/javaTargets.cmake.in)
...
function(add_jar _TARGET_NAME)
  ...
  cmake_parse_arguments(PARSE_ARGV 1 _add_jar "${options}" "${oneValueArgs}"
                        "${multiValueArgs}")
  ...
  # This *should* still work if <resources1>... are included without a named
  # RESOURCES argument.  In that case, the old behavior of potentially
  ...
  if(_add_jar_GENERATE_NATIVE_HEADERS)
    # Raise an error if JDK version is less than 1.8 because javac -h is not
    # supported by earlier versions.
    if(Java_VERSION VERSION_LESS 1.8)
      message(
        FATAL_ERROR
          "ADD_JAR: GENERATE_NATIVE_HEADERS is not supported with this version of Java."
      )
    endif()

    unset(_GENERATE_NATIVE_HEADERS_OUTPUT_DESC)

    cmake_parse_arguments(_add_jar_GENERATE_NATIVE_HEADERS "" "" "DESTINATION"
                          ${_add_jar_GENERATE_NATIVE_HEADERS})
    if(NOT _add_jar_GENERATE_NATIVE_HEADERS_UNPARSED_ARGUMENTS)
      message(
        FATAL_ERROR
          "ADD_JAR: GENERATE_NATIVE_HEADERS: missing required argument.")
    endif()
    ...
    if(length GREATER 1)
      ...
      message(
        FATAL_ERROR
          "ADD_JAR: GENERATE_NATIVE_HEADERS: ${_add_jar_GENERATE_NATIVE_HEADERS_UNPARSED_ARGUMENTS}: unexpected argument(s)."
      )
    endif()
    if(NOT _add_jar_GENERATE_NATIVE_HEADERS_DESTINATION)
      set(_add_jar_GENERATE_NATIVE_HEADERS_DESTINATION
          "${CMAKE_CURRENT_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/${_TARGET_NAME}.dir/native_headers"
      )
    else()
      list(LENGTH _add_jar_GENERATE_NATIVE_HEADERS_DESTINATION length)
      if(NOT length EQUAL 1)
        cmake_parse_arguments(
          _add_jar_GENERATE_NATIVE_HEADERS_DESTINATION "" "BUILD;INSTALL" ""
          "${_add_jar_GENERATE_NATIVE_HEADERS_DESTINATION}")
        if(_add_jar_GENERATE_NATIVE_HEADERS_DESTINATION_UNPARSED_ARGUMENTS)
          message(
            FATAL_ERROR
              "ADD_JAR: GENERATE_NATIVE_HEADERS: DESTINATION: ${_add_jar_GENERATE_NATIVE_HEADERS_DESTINATION_UNPARSED_ARGUMENTS}: unexpected argument(s)."
          )
        endif()
        if(NOT _add_jar_GENERATE_NATIVE_HEADERS_DESTINATION_INSTALL)
          message(
            FATAL_ERROR
              "ADD_JAR: GENERATE_NATIVE_HEADERS: DESTINATION: INSTALL sub-option is required."
          )
        endif()
        if(NOT _add_jar_GENERATE_NATIVE_HEADERS_DESTINATION_BUILD)
          set(_add_jar_GENERATE_NATIVE_HEADERS_DESTINATION_BUILD
              "${CMAKE_CURRENT_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/${_TARGET_NAME}.dir/native_headers"
          )
        endif()
        set(_add_jar_GENERATE_NATIVE_HEADERS_DESTINATION
            "${_add_jar_GENERATE_NATIVE_HEADERS_DESTINATION_BUILD}")
        set(_GENERATE_NATIVE_HEADERS_OUTPUT_DESC
            "$<BUILD_INTERFACE:${_add_jar_GENERATE_NATIVE_HEADERS_DESTINATION_BUILD}>"
            "$<INSTALL_INTERFACE:${_add_jar_GENERATE_NATIVE_HEADERS_DESTINATION_INSTALL}>"
        )
      endif()
    endif()

    set(_GENERATE_NATIVE_HEADERS_TARGET
        ${_add_jar_GENERATE_NATIVE_HEADERS_UNPARSED_ARGUMENTS})
    set(_GENERATE_NATIVE_HEADERS_OUTPUT_DIR
        "${_add_jar_GENERATE_NATIVE_HEADERS_DESTINATION}")
  ...
  set(CMAKE_JAVA_INCLUDE_PATH
      ${CMAKE_JAVA_INCLUDE_PATH} ${CMAKE_CURRENT_SOURCE_DIR}
      ${CMAKE_JAVA_OBJECT_OUTPUT_PATH} ${CMAKE_JAVA_LIBRARY_OUTPUT_PATH})
  ...
  if(_add_jar_OUTPUT_NAME AND _add_jar_VERSION)
    set(_JAVA_TARGET_OUTPUT_NAME
        "${_add_jar_OUTPUT_NAME}-${_add_jar_VERSION}.jar")
  ...
  foreach(_JAVA_SOURCE_FILE IN LISTS _JAVA_SOURCE_FILES)
    ...
    elseif(_JAVA_EXT STREQUAL "")
      ...
      __java_copy_file(
        ${CMAKE_CURRENT_SOURCE_DIR}/${_JAVA_SOURCE_FILE}
        ${CMAKE_JAVA_CLASS_OUTPUT_PATH}/${_JAVA_SOURCE_FILE}
        "Copying ${_JAVA_SOURCE_FILE} to the build directory")
      list(APPEND _JAVA_RESOURCE_FILES
           ${CMAKE_JAVA_CLASS_OUTPUT_PATH}/${_JAVA_SOURCE_FILE})
  ...
  if(_add_jar_RESOURCES) # Process RESOURCES if it exists
    __java_copy_resource_namespaces(
      "${_add_jar_RESOURCES}" ${CMAKE_JAVA_CLASS_OUTPUT_PATH}
      _JAVA_RESOURCE_FILES _JAVA_RESOURCE_FILES_RELATIVE)
  endif()
  ...
  foreach(_JAVA_INCLUDE_JAR IN LISTS _add_jar_INCLUDE_JARS)
    if(TARGET ${_JAVA_INCLUDE_JAR})
      ...
      if(_JAVA_JAR_PATH)
        ...
        message(
          SEND_ERROR
            "add_jar: INCLUDE_JARS target ${_JAVA_INCLUDE_JAR} is not a jar")
      endif()
    else()
      string(APPEND CMAKE_JAVA_INCLUDE_PATH_FINAL
  ...
  if(_JAVA_COMPILE_FILES OR _JAVA_COMPILE_FILELISTS)
    ...
    add_custom_command(
      ...
      COMMAND
        ...
        -DCMAKE_JAR_CLASSES_PREFIX=${CMAKE_JAR_CLASSES_PREFIX} -P
        ${CMAKE_CURRENT_FUNCTION_LIST_DIR}/UseJava/ClearClassFiles.cmake
      COMMAND
        ${Java_JAVAC_EXECUTABLE} ${CMAKE_JAVA_COMPILE_FLAGS} -classpath
        "${CMAKE_JAVA_INCLUDE_PATH_FINAL}" -d ${CMAKE_JAVA_CLASS_OUTPUT_PATH}
        ${_GENERATE_NATIVE_HEADERS} ${_JAVA_SOURCES_FILELISTS}
      COMMAND ${CMAKE_COMMAND} -E touch
              ${CMAKE_JAVA_CLASS_OUTPUT_PATH}/java_compiled_${_TARGET_NAME}
      DEPENDS ${_JAVA_COMPILE_FILES} ${_JAVA_COMPILE_FILELISTS}
              ${_JAVA_COMPILE_DEPENDS} ${_JAVA_SOURCES_FILE}
    ...
    add_custom_command(
      ...
      COMMAND
        ...
        -DCMAKE_JAVA_CLASS_OUTPUT_PATH=${CMAKE_JAVA_CLASS_OUTPUT_PATH}
        -DCMAKE_JAR_CLASSES_PREFIX=${CMAKE_JAR_CLASSES_PREFIX} -P
        ${CMAKE_CURRENT_FUNCTION_LIST_DIR}/UseJava/ClassFilelist.cmake
  ...
  if(CMAKE_JNI_TARGET)
    add_custom_command(
      ...
      COMMAND
        ${Java_JAR_EXECUTABLE} -cf${_ENTRY_POINT_OPTION}${_MANIFEST_OPTION}
        ${_JAVA_JAR_OUTPUT_PATH} ${_ENTRY_POINT_VALUE} ${_MANIFEST_VALUE}
        ${_JAVA_RESOURCE_FILES_RELATIVE} @java_class_filelist
      COMMAND
        ${CMAKE_COMMAND} -D_JAVA_TARGET_DIR=${_add_jar_OUTPUT_DIR}
        -D_JAVA_TARGET_OUTPUT_NAME=${_JAVA_TARGET_OUTPUT_NAME}
        -D_JAVA_TARGET_OUTPUT_LINK=${_JAVA_TARGET_OUTPUT_LINK} -P
  ...
  set_property(TARGET ${_TARGET_NAME} PROPERTY INSTALL_FILES
                                               ${_JAVA_JAR_OUTPUT_PATH})

  if(_JAVA_TARGET_OUTPUT_LINK)
    set_property(
      TARGET ${_TARGET_NAME}
      PROPERTY INSTALL_FILES ${_JAVA_JAR_OUTPUT_PATH}
               ${_add_jar_OUTPUT_DIR}/${_JAVA_TARGET_OUTPUT_LINK})

    if(CMAKE_JNI_TARGET)
      set_property(
        TARGET ${_TARGET_NAME}
        PROPERTY JNI_SYMLINK ${_add_jar_OUTPUT_DIR}/${_JAVA_TARGET_OUTPUT_LINK})
    endif()
  endif()

  set_property(TARGET ${_TARGET_NAME} PROPERTY JAR_FILE
                                               ${_JAVA_JAR_OUTPUT_PATH})

  set_property(TARGET ${_TARGET_NAME} PROPERTY CLASSDIR
                                               ${CMAKE_JAVA_CLASS_OUTPUT_PATH})

  if(_GENERATE_NATIVE_HEADERS)
    # create an INTERFACE library encapsulating include directory for generated
    # headers
    add_library(${_GENERATE_NATIVE_HEADERS_TARGET} INTERFACE)
    target_include_directories(
      ${_GENERATE_NATIVE_HEADERS_TARGET}
      INTERFACE "${_GENERATE_NATIVE_HEADERS_OUTPUT_DESC}" ${JNI_INCLUDE_DIRS})
    set_property(
      TARGET ${_GENERATE_NATIVE_HEADERS_TARGET}
      PROPERTY NATIVE_HEADERS_DIRECTORY
               "${_GENERATE_NATIVE_HEADERS_OUTPUT_DIR}")
    ...
    set_property(
      DIRECTORY
      APPEND
      PROPERTY ADDITIONAL_CLEAN_FILES "${_GENERATE_NATIVE_HEADERS_OUTPUT_DIR}")
  endif()
endfunction()
...
function(INSTALL_JAR _TARGET_NAME)
  if(ARGC EQUAL 2)
    ...
    cmake_parse_arguments(_install_jar "" "DESTINATION;COMPONENT" "" ${ARGN})
    if(_install_jar_DESTINATION)
      set(_DESTINATION ${_install_jar_DESTINATION})
    else()
      message(
        SEND_ERROR
          "install_jar: ${_TARGET_NAME}: DESTINATION must be specified.")
    endif()
  ...
  set_property(TARGET ${_TARGET_NAME} PROPERTY INSTALL_DESTINATION
                                               ${_DESTINATION})
  ...
  if(__FILES)
    ...
    message(
      SEND_ERROR
        "install_jar: The target ${_TARGET_NAME} is not known in this scope.")
  endif()
endfunction()
...
function(INSTALL_JNI_SYMLINK _TARGET_NAME)
  if(ARGC EQUAL 2)
    ...
    cmake_parse_arguments(_install_jni_symlink "" "DESTINATION;COMPONENT" ""
                          ${ARGN})
    if(_install_jni_symlink_DESTINATION)
      ...
      message(
        SEND_ERROR
          "install_jni_symlink: ${_TARGET_NAME}: DESTINATION must be specified."
      )
    endif()
  ...
  if(__SYMLINK)
    ...
    message(
      SEND_ERROR
        "install_jni_symlink: The target ${_TARGET_NAME} is not known in this scope."
    )
  endif()
endfunction()
...
function(find_jar VARIABLE)
  ...
  set(_jar_files)
  set(_jar_versions)
  set(_jar_paths /usr/share/java/ /usr/local/share/java/ ${Java_JAR_PATHS})
  set(_jar_doc "NOTSET")
...
function(create_javadoc _target)
  ...
  add_custom_target(
    ${_target}_javadoc ALL
    COMMAND ${Java_JAVADOC_EXECUTABLE} ${_javadoc_options} ${_javadoc_files}
            ${_javadoc_packages}
    WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR})

  install(DIRECTORY ${_javadoc_builddir} DESTINATION ${_javadoc_installpath})
endfunction()
...
function(create_javah)
  if(Java_VERSION VERSION_GREATER_EQUAL 10)
    message(
      FATAL_ERROR
        "create_javah: not supported with this Java version. Use add_jar(GENERATE_NATIVE_HEADERS) instead."
    )
  elseif(Java_VERSION VERSION_GREATER_EQUAL 1.8)
    message(
      DEPRECATION
        "create_javah: this command will no longer be supported starting with version 10 of JDK. Update your project by using command add_jar(GENERATE_NATIVE_HEADERS) instead."
    )
  endif()

  cmake_parse_arguments(
    _create_javah "" "TARGET;GENERATED_FILES;OUTPUT_NAME;OUTPUT_DIR"
    "CLASSES;CLASSPATH;DEPENDS" ${ARGN})

  # ckeck parameters
  if(NOT _create_javah_TARGET AND NOT _create_javah_GENERATED_FILES)
    message(
      FATAL_ERROR "create_javah: TARGET or GENERATED_FILES must be specified.")
  endif()
  if(_create_javah_OUTPUT_NAME AND _create_javah_OUTPUT_DIR)
    message(
      FATAL_ERROR
        "create_javah: OUTPUT_NAME and OUTPUT_DIR are mutually exclusive.")
  endif()
  ...
  if(_create_javah_CLASSPATH)
    # CLASSPATH can specify directories, jar files or targets created with
    # add_jar command
    set(_classpath)
    foreach(_path IN LISTS _create_javah_CLASSPATH)
      if(TARGET ${_path})
        ...
        if(_jar_path)
          ...
          message(
            SEND_ERROR "create_javah: CLASSPATH target ${_path} is not a jar.")
        endif()
      elseif(EXISTS "${_path}")
        ...
        message(
          SEND_ERROR "create_javah: CLASSPATH entry ${_path} does not exist.")
      endif()
    endforeach()
  ...
  if(_create_javah_OUTPUT_NAME)
    ...
    get_filename_component(_create_javah_OUTPUT_DIR
                           "${_create_javah_OUTPUT_NAME}" DIRECTORY)
    get_filename_component(_create_javah_OUTPUT_DIR
                           "${_create_javah_OUTPUT_DIR}" ABSOLUTE)
  endif()
  ...
  add_custom_command(
    ...
    COMMAND "${Java_JAVAH_EXECUTABLE}" ${_javah_options} -jni
            ${_create_javah_CLASSES} ${_create_javah_DEPENDS}
...
function(export_jars)
  ...
  cmake_parse_arguments(_export_jars "" "FILE;NAMESPACE" "TARGETS" ${ARGN})
  ...
  configure_file(${_JAVA_EXPORT_TARGETS_SCRIPT} ${_export_jars_FILE} @ONLY)
endfunction()

function(install_jar_exports)
  # Parse and validate arguments
  cmake_parse_arguments(
    _install_jar_exports "" "FILE;DESTINATION;COMPONENT;NAMESPACE" "TARGETS"
    ${ARGN})
  ...
  if(IS_ABSOLUTE "${_install_jar_exports_DESTINATION}")
    file(RELATIVE_PATH _relpath ${_install_jar_exports_DESTINATION}
  ...
  configure_file(${_JAVA_EXPORT_TARGETS_SCRIPT}
                 ${_tmpdir}/${_install_jar_exports_FILE} @ONLY)
  install(
    FILES ${_tmpdir}/${_install_jar_exports_FILE}
