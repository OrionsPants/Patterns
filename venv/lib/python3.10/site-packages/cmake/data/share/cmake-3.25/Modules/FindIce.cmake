...
# The Ice checks are contained in a function due to the large number of
...
function(_Ice_FIND)
  ...
  foreach(ver ${ice_versions})
    string(REGEX MATCH "^([0-9]+)\\.([0-9]+)\$" two_digit_version_match
                 "${ver}")
    if(two_digit_version_match)
      string(REGEX REPLACE "^([0-9]+)\\.([0-9]+)\$" "\\1\\2" two_digit_version
                           "${ver}")
      list(APPEND ice_suffix_versions "${two_digit_version}")
    endif()
  endforeach()

  # Set up search paths, taking compiler into account.  Search Ice_HOME, with
  # ICE_HOME in the environment as a fallback if unset.
  if(Ice_HOME)
  ...
  if(vcvers)
    ...
    foreach(vcver IN LISTS vcvers)
      # Earlier Ice (3.3) builds don't use vcnnn subdirectories, but are
      # harmless to check.
      list(APPEND ice_binary_suffixes "bin/vc${vcver}${_x64}" "bin/vc${vcver}")
      list(APPEND ice_debug_library_suffixes "lib/vc${vcver}${_x64}"
           "lib/vc${vcver}")
      list(APPEND ice_release_library_suffixes "lib/vc${vcver}${_x64}"
           "lib/vc${vcver}")
    endforeach()
  endif()
  # Generic 64-bit and 32-bit directories
  list(APPEND ice_binary_suffixes "bin${_x64}" "bin")
  list(APPEND ice_debug_library_suffixes "libx32" "${_lib64}" "lib${_x64}"
       "lib")
  list(APPEND ice_release_library_suffixes "libx32" "${_lib64}" "lib${_x64}"
       "lib")
  if(vcvers)
  ...
  # On Windows, look in the registry for install locations.  Different versions
  # of Ice install support different compiler versions.
  if(vcvers)
    ...
    foreach(ice_version ${ice_versions})
      # Ice 3.3 releases use a Visual Studio year suffix and value is enclosed
      # in double quotes, though only the leading quote is returned by
      ...
      if(vcyear)
        ...
        if(ice_location AND NOT ("${ice_location}" STREQUAL "/registry"
                                 OR "${ice_location}" STREQUAL "/"))
          string(REGEX REPLACE "^\"(.*)\"?$" "\\1" ice_location
                               "${ice_location}")
  ...
  if(Ice_SLICE2CPP_EXECUTABLE)
    ...
    execute_process(
      COMMAND ${Ice_SLICE2CPP_EXECUTABLE} --version
    ...
    # Make short version
    string(REGEX REPLACE "^(.*)\\.[^.]*$" "\\1" Ice_VERSION_SLICE2CPP_SHORT
                         "${Ice_VERSION_SLICE2CPP_FULL}")
    set(Ice_VERSION
        "${Ice_VERSION_SLICE2CPP_FULL}"
        PARENT_SCOPE)
  endif()
  ...
  set(Freeze_INCLUDE_DIR
      "${Freeze_INCLUDE_DIR}"
      PARENT_SCOPE)

  # In common use on Linux, MacOS X (homebrew) and FreeBSD; prefer
  # version-specific dir
  list(APPEND ice_slice_paths /usr/local/share /usr/share)
  list(APPEND ice_slice_suffixes "Ice-${Ice_VERSION_SLICE2CPP_FULL}/slice"
       "Ice-${Ice_VERSION_SLICE2CPP_SHORT}/slice" "ice/slice" Ice)

  # Find slice directory
  find_path(
    Ice_SLICE_DIR
    NAMES "Ice/Connection.ice"
    HINTS ${ice_roots} ${ice_slice_paths}
  ...
  foreach(component ${Ice_FIND_COMPONENTS})
    ...
    if("${CMAKE_CXX_COMPILER_ID}" STREQUAL "MSVC" OR "${CMAKE_CXX_SIMULATE_ID}"
                                                     STREQUAL "MSVC")
      string(REGEX MATCH ".+\\+\\+11$" component_library_cpp11
                   "${component_library}")
      if(component_library_cpp11)
        string(REGEX REPLACE "^(.+)(\\+\\+11)$" "\\1" component_library
                             "${component_library}")
      endif()
    ...
    find_library(
      "${component_cache_release}" ${component_library_release_names}
      HINTS ${ice_roots}
      PATH_SUFFIXES ${ice_release_library_suffixes}
      DOC "Ice ${component} library (release)")
    find_library(
      "${component_cache_debug}" ${component_library_debug_names}
      HINTS ${ice_roots}
  ...
  endforeach()
  set(_Ice_REQUIRED_LIBS_FOUND
      "${Ice_REQUIRED_LIBS_FOUND}"
      PARENT_SCOPE)
  set(Ice_LIBRARY
      "${Ice_LIBRARY}"
      PARENT_SCOPE)
  ...
  if(Ice_DEBUG)
    ...
    message(
      STATUS "ICE slice path search order: ${ice_roots} ${ice_slice_paths}")
    ...
    message(
      STATUS "ICE release library suffixes: ${ice_release_library_suffixes}")
    message(STATUS "----------------")
  endif()
endfunction()
...
find_package_handle_standard_args(
  Ice
  FOUND_VAR Ice_FOUND
  REQUIRED_VARS Ice_SLICE2CPP_EXECUTABLE Ice_INCLUDE_DIR Ice_SLICE_DIR
                Ice_LIBRARY _Ice_REQUIRED_LIBS_FOUND
    ...
    if(${_Ice_component_found})
      ...
      if(NOT TARGET ${_Ice_imported_target})
        ...
        set_target_properties(
          ${_Ice_imported_target} PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                             "${Ice_INCLUDE_DIRS}")
        if(EXISTS "${${_Ice_component_cache}}")
          set_target_properties(
            ${_Ice_imported_target}
