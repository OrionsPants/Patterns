...
# This module is used by the Makefile generator to determine the following
# variables: CMAKE_SYSTEM_NAME - on unix this is uname -s, for windows it is
# Windows CMAKE_SYSTEM_VERSION - on unix this is uname -r, for windows it is
# empty CMAKE_SYSTEM - ${CMAKE_SYSTEM}-${CMAKE_SYSTEM_VERSION}, for windows:
# ${CMAKE_SYSTEM}
#
# Expected uname -s output:
#
# AIX                           AIX BSD/OS                        BSD/OS FreeBSD
# FreeBSD HP-UX                         HP-UX Linux Linux GNU/kFreeBSD
# GNU/kFreeBSD NetBSD NetBSD OpenBSD                       OpenBSD OFS/1
# (Digital Unix) OSF1 SCO OpenServer 5              SCO_SV SCO UnixWare 7
# UnixWare SCO UnixWare (pre release 7)  UNIX_SV SCO XENIX Xenix Solaris SunOS
# SunOS                         SunOS Tru64                         Tru64 Ultrix
# ULTRIX cygwin CYGWIN_NT-5.1 MSYS MSYS_NT-6.1 MacOSX Darwin
...
if(CMAKE_HOST_UNIX)
  ...
  if(CMAKE_UNAME)
    ...
    elseif(CMAKE_HOST_SYSTEM_NAME MATCHES "Darwin")
      ...
      elseif(DEFINED ENV{CMAKE_APPLE_SILICON_PROCESSOR})
        set(_CMAKE_APPLE_SILICON_PROCESSOR
            "$ENV{CMAKE_APPLE_SILICON_PROCESSOR}")
      else()
        set(_CMAKE_APPLE_SILICON_PROCESSOR "")
      endif()
      if(_CMAKE_APPLE_SILICON_PROCESSOR)
        if(";${_CMAKE_APPLE_SILICON_PROCESSOR};" MATCHES "^;(arm64|x86_64);$")
          ...
          if(NOT _sysctl_result EQUAL 0 OR NOT _sysctl_stdout MATCHES
                                           "hw.optional.arm64: 1")
            set(_CMAKE_APPLE_SILICON_PROCESSOR "")
          endif()
    ...
    set(CMAKE_UNAME
        ${CMAKE_UNAME}
        CACHE INTERNAL "uname command")
    # processor may have double quote in the name, and that needs to be removed
    string(REPLACE "\"" "" CMAKE_HOST_SYSTEM_PROCESSOR
                   "${CMAKE_HOST_SYSTEM_PROCESSOR}")
    string(REPLACE "/" "_" CMAKE_HOST_SYSTEM_PROCESSOR
                   "${CMAKE_HOST_SYSTEM_PROCESSOR}")
  endif()
else()
  if(CMAKE_HOST_WIN32)
...
# if a toolchain file is used, the user wants to cross compile. in this case
# read the toolchain file and keep the CMAKE_HOST_SYSTEM_* variables around so
# they can be used in CMakeLists.txt. In all other cases, the host and target
# platform are the same.
if(CMAKE_TOOLCHAIN_FILE)
  # at first try to load it as path relative to the directory from which cmake
  # has been run
  ...
  if(NOT _INCLUDED_TOOLCHAIN_FILE)
    ...
    include("${CMAKE_TOOLCHAIN_FILE}" OPTIONAL
            RESULT_VARIABLE _INCLUDED_TOOLCHAIN_FILE)
  endif()
  ...
  if(_INCLUDED_TOOLCHAIN_FILE)
    ...
    message(
      FATAL_ERROR "Could not find toolchain file: ${CMAKE_TOOLCHAIN_FILE}")
  endif()
endif()

# if CMAKE_SYSTEM_NAME is here already set, either it comes from a toolchain
# file or it was set via -DCMAKE_SYSTEM_NAME=... if that's the case, assume we
# are crosscompiling
...
# this file is also executed from cpack, then we don't need to generate these
# files in this case there is no CMAKE_BINARY_DIR
if(CMAKE_BINARY_DIR)
  ...
  # so settings done there are also available if they don't go in the cache and
  # in try_compile()
  ...
  # configure variables set in this file for fast reload, the template file is
  # defined at the top of this file
  configure_file(${CMAKE_ROOT}/Modules/CMakeSystem.cmake.in
                 ${CMAKE_PLATFORM_INFO_DIR}/CMakeSystem.cmake @ONLY)
