...
function(is_file_executable file result_var)
  ...
  if(WIN32 AND NOT UNIX)
    ...
    # to determine ${result_var}. In 99%+? practical cases, the exe name match
    # will be sufficient...
    #
  endif()

  # Use the information returned from the Unix shell command "file" to determine
  # if ${file_full} should be considered an executable file...
  #
  ...
  if(UNIX)
    ...
    if(file_cmd)
      ...
      # Replace the name of the file in the output with a placeholder token (the
      # string " _file_full_ ") so that just in case the path name of the file
      # contains the word "text" or "executable" we are not fooled into thinking
      # "the wrong thing" because the file name matches the other 'file' command
      ...
      # Also detect position independent executables on Linux, where "file"
      # gives "shared object ... (uses shared libraries)"
      ...
      # "file" version 5.22 does not print "(used shared libraries)" but uses
      # "interpreter"
...
function(gp_item_default_embedded_path item default_embedded_path_var)
  ...
  # On Windows and Linux, "embed" prerequisites in the same directory as the
  # executable by default:
  #
  set(path "@executable_path")

  # On the Mac, relative to the executable depending on the type of the thing we
  # are embedding:
  #
  if(APPLE)
    #
    # The assumption here is that all executables in the bundle will be in
    # same-level-directories inside the bundle. The parent directory of an
    # executable inside the bundle should be MacOS or a sibling of MacOS and all
    # embedded paths returned from here will begin with "@executable_path/../"
    # and will work from all executables in all such same-level-directories
    # inside the bundle.
    #
...
function(gp_resolve_item context item exepath dirs resolved_item_var)
  ...
  if(NOT resolved)
    if(item MATCHES "^@loader_path")
      #
      # @loader_path references are assumed relative to the PATH of the given
      # "context" (presumably another library)
      #
  ...
  if(NOT resolved)
    if(item MATCHES "^@rpath")
      #
      # @rpath references are relative to the paths built into the binaries with
      # -rpath We handle this case like we do for other Unixes
      #
      string(REPLACE "@rpath/" "" norpath_item "${item}")

      set(ri "ri-NOTFOUND")
      find_file(ri "${norpath_item}" ${exepath} ${dirs} ${rpaths}
                NO_DEFAULT_PATH)
      if(ri)
  ...
  if(NOT resolved)
    if(item MATCHES "[^/]+\\.framework/")
      set(fw "fw-NOTFOUND")
      find_file(fw "${item}" "~/Library/Frameworks" "/Library/Frameworks"
  ...
  # Provide a hook so that projects can override item resolution by whatever
  # logic they choose:
  #
...
")
    ...
    # warning: cannot resolve item '${item}'
    #
    # possible problems: need more directories? need to use
    # InstallRequiredSystemLibraries? run in install tree instead of build tree?
    #
    # context='${context}' item='${item}' exepath='${exepath}' dirs='${dirs}'
    # resolved_item_var='${resolved_item_var}'
    # ******************************************************************************
    # ")
  endif()

  set(${resolved_item_var}
      "${resolved_item}"
      PARENT_SCOPE)
endfunction()
...
function(gp_resolved_file_type original_file file exepath dirs type_var)
  ...
  if(NOT IS_ABSOLUTE "${original_file}")
    message(
      STATUS
        "warning: gp_resolved_file_type expects absolute full path for first arg original_file"
    )
  endif()
  if(IS_ABSOLUTE "${original_file}")
    get_filename_component(original_file "${original_file}" ABSOLUTE
    )# canonicalize path
  endif()
      ...
      if(UNIX)
        ...
        if(CYGPATH_EXECUTABLE)
          ...
          if(NOT env_rv STREQUAL "0")
            message(
              FATAL_ERROR
                "${CYGPATH_EXECUTABLE} -W failed: ${env_rv}\n${env_ev}")
          endif()
          execute_process(
            COMMAND ${CYGPATH_EXECUTABLE} -S
            RESULT_VARIABLE env_rv
          ...
          if(NOT env_rv STREQUAL "0")
            message(
              FATAL_ERROR
                "${CYGPATH_EXECUTABLE} -S failed: ${env_rv}\n${env_ev}")
          endif()
  ...
  if(NOT is_embedded)
    if(NOT IS_ABSOLUTE "${resolved_file}")
      if(lower MATCHES "^(msvc|api-ms-win-|vcruntime)[^/]+dll" AND is_system)
        message(
          STATUS
            "info: non-absolute msvc file '${file}' returning type '${type}'")
      else()
        message(
          STATUS
...
function(gp_file_type original_file file type_var)
  if(NOT IS_ABSOLUTE "${original_file}")
    message(
      STATUS
        "warning: gp_file_type expects absolute full path for first arg original_file"
    )
  endif()
...
function(
  ...
  if(NOT gp_cmd)
    message(
      STATUS
        "warning: could not find '${gp_tool}' - cannot analyze prerequisites..."
    )
    return()
  endif()
  ...
  elseif(gp_tool MATCHES "objdump(\\.exe)?$")
    ...
    set(gp_regex_fallback "")
    set(gp_regex_cmp_count 1)
    # objdump generates copious output so we create a grep filter to pre-filter
    # results
    if(WIN32)
    ...
    if(gp_grep_cmd)
      set(gp_cmd_maybe_filter COMMAND ${gp_grep_cmd} "${gp_grep_cmd_arg}"
                              "^[[:blank:]]*DLL Name: ")
    endif()
  else()
    ...
    message(
      STATUS
        "CMake function get_prerequisites needs more code to handle '${gp_tool}'"
    )
  ...
  if(gp_tool MATCHES "dumpbin$")
    ...
    get_filename_component(gp_cmd_dlls_dir "${gp_cmd_dir}/../../Common7/IDE"
                           ABSOLUTE)
  ...
  # Track new prerequisites at each new level of recursion. Start with an empty
  # list at each level:
  #
  set(unseen_prereqs)
  ...
  if(gp_tool MATCHES "dumpbin$")
    # Exclude delay load dependencies under windows (they are listed in dumpbin
    # output after the message below)
    string(FIND "${gp_cmd_ov}"
  ...
  foreach(candidate ${candidates})
    if("${candidate}" MATCHES "${gp_regex}")
      ...
      if(gp_regex_error AND "${candidate}" MATCHES "${gp_regex_error}")
        string(REGEX REPLACE "${gp_regex_fallback}" "\\1" raw_item
                             "${candidate}")
      else()
      ...
      if(gp_regex_cmp_count GREATER 1)
        string(REGEX REPLACE "${gp_regex}" "\\2" raw_compat_version
                             "${candidate}")
      ...
      if(gp_regex_cmp_count GREATER 2)
        string(REGEX REPLACE "${gp_regex}" "\\3" raw_current_version
                             "${candidate}")
        string(REGEX REPLACE "^([0-9]+)\\.([0-9]+)\\.([0-9]+)$" "\\1"
      ...
      if(add_item)
        ...
        if(${recurse})
          ...
          # prerequisites...
          #
          # But first: resolve its name to an absolute full path name such that
          # the analysis tools can simply accept it as input.
          #
          if(NOT list_length_before_append EQUAL list_length_after_append)
            gp_resolve_item("${target}" "${item}" "${exepath}" "${dirs}"
                            resolved_item "${rpaths}")
            if(EXISTS "${resolved_item}")
              # Recurse only if we could resolve the item. Otherwise the
              # prerequisites_var list will be cleared
...
function(list_prerequisites_by_glob glob_arg glob_exp)
  ...
  message(
    STATUS
      "List prerequisites of executables matching ${glob_arg} '${glob_exp}'")
  message(STATUS "")
