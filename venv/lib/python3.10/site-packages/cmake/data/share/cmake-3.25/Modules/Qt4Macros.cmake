...
function(QT4_CREATE_MOC_COMMAND infile outfile moc_flags moc_options moc_target)
  ...
  # limit Pass the parameters in a file.  Set the working directory to be that
  # containing the parameters file and reference it by just the file name.  This
  # is necessary because the moc tool on MinGW builds does not seem to handle
  # spaces in the path to the file given with the @ syntax.
  ...
  if(moc_target)
    ...
    set(targetdefines
        "$<$<BOOL:${targetdefines}>:-D$<JOIN:${targetdefines},\n-D>\n>")
  ...
  add_custom_command(
    ...
    DEPENDS ${infile} ${_moc_parameters_file} ${_moc_working_dir}
    VERBATIM)
endfunction()
...
macro(QT4_GENERATE_MOC infile outfile)
  ...
  qt4_create_moc_command(${abs_infile} ${_outfile} "${moc_flags}" ""
                         "${moc_target}")
  set_property(SOURCE ${outfile} PROPERTY SKIP_AUTOMOC TRUE) # don't run automoc
                                                             # on this file
  set_property(SOURCE ${outfile} PROPERTY SKIP_AUTOUIC TRUE) # don't run autouic
                                                             # on this file
endmacro()
...
macro(QT4_WRAP_CPP outfiles)
  ...
  foreach(it ${moc_files})
    ...
    qt4_create_moc_command(${it} ${outfile} "${moc_flags}" "${moc_options}"
                           "${moc_target}")
    set_property(
      SOURCE ${outfile} PROPERTY SKIP_AUTOMOC TRUE) # don't run automoc on this
                                                    # file
    set_property(
      SOURCE ${outfile} PROPERTY SKIP_AUTOUIC TRUE) # don't run autouic on this
                                                    # file
...
macro(QT4_WRAP_UI outfiles)
  ...
  foreach(it ${ui_files})
    ...
    add_custom_command(
      OUTPUT ${outfile}
      COMMAND Qt4::uic ARGS ${ui_options} -o ${outfile} ${infile}
...
macro(QT4_ADD_RESOURCES outfiles)
  ...
  foreach(it ${rcc_files})
    ...
    if(EXISTS "${infile}")
      ...
      # The .qrc file does not exist (yet). Let's add a dependency and hope that
    ...
    add_custom_command(
      OUTPUT ${outfile}
      COMMAND Qt4::rcc ARGS ${rcc_options} -name ${outfilename} -o ${outfile}
              ${infile}
      MAIN_DEPENDENCY ${infile}
...
macro(QT4_ADD_DBUS_INTERFACE _sources _interface _basename)
  ...
  set_property(SOURCE ${_impl} PROPERTY SKIP_AUTOMOC TRUE) # don't run automoc
                                                           # on this file
  set_property(SOURCE ${_impl} PROPERTY SKIP_AUTOUIC TRUE) # don't run autouic
                                                           # on this file
...
macro(QT4_ADD_DBUS_ADAPTOR _sources _xml_file _include _parentClass
  ...
  if(_optionalBasename)
    ...
    string(REGEX REPLACE "(.*[/\\.])?([^\\.]+)\\.xml" "\\2adaptor" _basename
                         ${_infile})
    string(TOLOWER ${_basename} _basename)
  endif()
  ...
  if(_optionalClassName)
    add_custom_command(
      OUTPUT "${_impl}" "${_header}"
      COMMAND Qt4::qdbusxml2cpp -m -a ${_basename} -c ${_optionalClassName} -i
              ${_include} -l ${_parentClass} ${_infile}
      DEPENDS ${_infile}
      VERBATIM)
  else()
    add_custom_command(
      OUTPUT "${_impl}" "${_header}"
      COMMAND Qt4::qdbusxml2cpp -m -a ${_basename} -i ${_include} -l
              ${_parentClass} ${_infile}
      DEPENDS ${_infile}
      VERBATIM)
  endif()
...
macro(QT4_AUTOMOC)
  ...
  foreach(_current_FILE ${ARGN})
    ...
    # if "SKIP_AUTOMOC" is set to true, we will not handle this file here. This
    # is required to make uic work correctly: we need to add generated .cpp
    # files to the sources (to compile them), but we cannot let automoc handle
    # them, as the .cpp files don't exist yet when cmake is run for the very
    # first time on them -> however the .cpp files might exist at a later run.
    # at that time we need to skip them, so that we don't add two different
    # rules for the same moc file
    get_property(
      _skip
      SOURCE ${_abs_FILE}
...
macro(QT4_CREATE_TRANSLATION _qm_files)
  ...
  foreach(_ts_file ${_my_tsfiles})
    if(_my_sources)
      ...
      set(_ts_pro
          ${CMAKE_CURRENT_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/${_ts_name}_lupdate.pro
      )
      set(_pro_srcs)
      ...
      file(
        GENERATE
        OUTPUT ${_ts_pro}
        CONTENT "SOURCES =${_pro_srcs}\nINCLUDEPATH =${_pro_includes}\n")
    endif()
    add_custom_command(
      OUTPUT ${_ts_file}
      COMMAND Qt4::lupdate ARGS ${_lupdate_options} ${_ts_pro} ${_my_dirs} -ts
              ${_ts_file}
...
macro(QT4_ADD_TRANSLATION _qm_files)
  foreach(_current_FILE ${ARGN})
    ...
    # everything before the last dot has to be considered the file name
    # (including other dots)
    ...
    add_custom_command(
      OUTPUT ${qm}
      COMMAND Qt4::lrelease ARGS ${_abs_FILE} -qm ${qm}
      DEPENDS ${_abs_FILE}
      VERBATIM)
  ...
  foreach(_module ${modules})
    ...
    if(_ucmodule STREQUAL QAXCONTAINER OR _ucmodule STREQUAL QAXSERVER)
      if(NOT QT_Q${_ucmodule}_FOUND)
        message(
          FATAL_ERROR
            "Can not use \"${_module}\" module which has not yet been found.")
      endif()
      ...
      if("${_ucmodule}" STREQUAL "MAIN")
        message(
          FATAL_ERROR "Can not use \"${_module}\" module with qt4_use_modules.")
      endif()
    endif()
    ...
    set_property(
      TARGET ${_target}
      APPEND
      PROPERTY INCLUDE_DIRECTORIES ${${_targetPrefix}_INCLUDE_DIR}
               ${QT_HEADERS_DIR} ${QT_MKSPECS_DIR}/default)
    set_property(
      TARGET ${_target}
      APPEND
      PROPERTY COMPILE_DEFINITIONS ${${_targetPrefix}_COMPILE_DEFINITIONS})
  endforeach()
endfunction()
