...
# we don't know anything about cursesw, so only ncurses may be ncursesw
if(NOT CURSES_NEED_WIDE)
  ...
  # Also, if we are searching for wide curses - we are actually searching for
  # ncurses, we don't know about any other unicode version.
...
if(CURSES_NCURSES_LIBRARY AND ((NOT CURSES_CURSES_LIBRARY)
                               OR CURSES_NEED_NCURSES))
  set(CURSES_USE_NCURSES TRUE)
endif()
# http://cygwin.com/ml/cygwin-announce/2010-01/msg00002.html cygwin ncurses
# stopped providing curses.h symlinks see above message.  Cygwin is an ncurses
# package, so force ncurses on cygwin if the curses.h is missing
...
# Not sure the logic is correct here. If NCurses is required, use the function
# wsyncup() to check if the library has NCurses functionality (at least this is
# where it breaks on NetBSD). If wsyncup is in curses, use this one. If not, try
# to find ncurses and check if this has the symbol. Once the ncurses library is
# found, search the ncurses.h header first, but some web pages also say that
# even with ncurses there is not always a ncurses.h:
# http://osdir.com/ml/gnome.apps.mc.devel/2002-06/msg00029.html So at first try
# ncurses.h, if not found, try to find curses.h under the same prefix as the
# library was found, if still not found, try curses.h with the default search
# paths.
if(CURSES_CURSES_LIBRARY AND CURSES_NEED_NCURSES)
  include(${CMAKE_CURRENT_LIST_DIR}/CMakePushCheckState.cmake)
  cmake_push_check_state()
  set(CMAKE_REQUIRED_QUIET ${Curses_FIND_QUIETLY})
  check_library_exists("${CURSES_CURSES_LIBRARY}" wsyncup ""
                       CURSES_CURSES_HAS_WSYNCUP)

  if(CURSES_NCURSES_LIBRARY AND NOT CURSES_CURSES_HAS_WSYNCUP)
    check_library_exists("${CURSES_NCURSES_LIBRARY}" wsyncup ""
...
if(CURSES_USE_NCURSES)
  ...
  check_library_exists("${CURSES_NCURSES_LIBRARY}" cbreak ""
                       CURSES_NCURSES_HAS_CBREAK)
  check_library_exists("${CURSES_NCURSES_LIBRARY}" nodelay ""
  ...
  if(NOT CURSES_NCURSES_HAS_CBREAK OR NOT CURSES_NCURSES_HAS_NODELAY)
    ...
    mark_as_advanced(CURSES_EXTRA_LIBRARY)
  endif()
else()
  get_filename_component(_cursesLibDir "${CURSES_CURSES_LIBRARY}" PATH)
  get_filename_component(_cursesParentDir "${_cursesLibDir}" PATH)

  # We can't find anything with CURSES_NEED_WIDE because we know only about
  # ncursesw unicode curses version
  if(NOT CURSES_NEED_WIDE)
    find_path(
      CURSES_INCLUDE_PATH
      NAMES curses.h
...
find_package_handle_standard_args(Curses DEFAULT_MSG CURSES_LIBRARY
                                  CURSES_INCLUDE_PATH)

mark_as_advanced(CURSES_INCLUDE_PATH CURSES_CURSES_LIBRARY
