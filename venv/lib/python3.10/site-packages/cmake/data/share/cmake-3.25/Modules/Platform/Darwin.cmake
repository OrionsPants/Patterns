...
# Darwin versions: 6.x == Mac OSX 10.2 (Jaguar) 7.x == Mac OSX 10.3 (Panther)
# 8.x == Mac OSX 10.4 (Tiger) 9.x == Mac OSX 10.5 (Leopard) 10.x == Mac OSX 10.6
# (Snow Leopard) 11.x == Mac OSX 10.7 (Lion) 12.x == Mac OSX 10.8 (Mountain
# Lion)
string(REGEX REPLACE "^([0-9]+)\\.([0-9]+).*$" "\\1" DARWIN_MAJOR_VERSION
                     "${CMAKE_SYSTEM_VERSION}")
string(REGEX REPLACE "^([0-9]+)\\.([0-9]+).*$" "\\2" DARWIN_MINOR_VERSION
                     "${CMAKE_SYSTEM_VERSION}")

# Do not use the "-Wl,-search_paths_first" flag with the OSX 10.2 compiler. Done
# this way because it is too early to do a TRY_COMPILE.
...
# More desirable, but does not work: include(CheckCXXCompilerFlag)
# CHECK_CXX_COMPILER_FLAG("-Wl,-search_paths_first"
...
foreach(lang C CXX OBJC OBJCXX)
  ...
  if(HAVE_FLAG_SEARCH_PATHS_FIRST)
    set(CMAKE_${lang}_LINK_FLAGS
        "-Wl,-search_paths_first ${CMAKE_${lang}_LINK_FLAGS}")
  endif()
...
if("${_CURRENT_OSX_VERSION}" VERSION_LESS "10.5")
  # Need to list dependent shared libraries on link line.  When building with
  # -isysroot (for universal binaries), the linker always looks for dependent
  # libraries under the sysroot.  Listing them on the link line works around the
  # problem.
...
set(CMAKE_LINK_LIBRARY_USING_NEEDED_FRAMEWORK
    "LINKER:-needed_framework,<LIBRARY>")
set(CMAKE_LINK_LIBRARY_USING_NEEDED_FRAMEWORK_SUPPORTED TRUE)

set(CMAKE_LINK_LIBRARY_USING_REEXPORT_FRAMEWORK
    "LINKER:-reexport_framework,<LIBRARY>")
...
set(CMAKE_LINK_LIBRARY_USING_NEEDED_LIBRARY
    "PATH{LINKER:-needed_library <LIBRARY>}NAME{LINKER:-needed-l<LIBRARY>}")
set(CMAKE_LINK_LIBRARY_USING_NEEDED_LIBRARY_SUPPORTED TRUE)

set(CMAKE_LINK_LIBRARY_USING_REEXPORT_LIBRARY
    "PATH{LINKER:-reexport_library <LIBRARY>}NAME{LINKER:-reexport-l<LIBRARY>}")
set(CMAKE_LINK_LIBRARY_USING_REEXPORT_LIBRARY_SUPPORTED TRUE)

set(CMAKE_LINK_LIBRARY_USING_WEAK_LIBRARY
    "PATH{LINKER:-weak_library <LIBRARY>}NAME{LINKER:-weak-l<LIBRARY>}")
...
# Older OS X linkers do not report their framework search path with -v but "man
# ld" documents the following locations.
...
set(CMAKE_SYSTEM_FRAMEWORK_PATH ~/Library/Frameworks)
if(_CMAKE_OSX_SYSROOT_PATH)
  list(
    APPEND
  ...
  if(EXISTS ${_CMAKE_OSX_SYSROOT_PATH}/usr/lib)
    list(INSERT CMAKE_PLATFORM_IMPLICIT_LINK_DIRECTORIES 0
         ${_CMAKE_OSX_SYSROOT_PATH}/usr/lib)
  endif()
  if(EXISTS ${_CMAKE_OSX_SYSROOT_PATH}/usr/local/lib)
    list(INSERT CMAKE_PLATFORM_IMPLICIT_LINK_DIRECTORIES 0
         ${_CMAKE_OSX_SYSROOT_PATH}/usr/local/lib)
  endif()
endif()
...
if(CMAKE_OSX_SYSROOT)
  ...
  if(NOT _IN_TC
     AND NOT IS_SYMLINK "${CMAKE_OSX_SYSROOT}/Library/Frameworks"
...
set(CMAKE_SYSTEM_APPBUNDLE_PATH ${_apps_paths})
unset(_apps_paths)
...
if(CMAKE_SYSTEM_NAME STREQUAL "Darwin" AND CMAKE_SYSTEM_PROCESSOR STREQUAL
                                           "arm64")
  list(PREPEND CMAKE_SYSTEM_PREFIX_PATH /opt/homebrew # Brew on Apple Silicon
  )
endif()
...
if(_CMAKE_OSX_SYSROOT_PATH)
  ...
  if(EXISTS ${_CMAKE_OSX_SYSROOT_PATH}/usr/local/include)
    ...
    foreach(lang C CXX OBJC OBJCXX Swift)
      list(APPEND _CMAKE_${lang}_IMPLICIT_INCLUDE_DIRECTORIES_INIT
           ${_CMAKE_OSX_SYSROOT_PATH}/usr/local/include)
    endforeach()
  endif()
endif()
list(APPEND CMAKE_SYSTEM_PREFIX_PATH /sw # Fink
     /opt/local # MacPorts
)
