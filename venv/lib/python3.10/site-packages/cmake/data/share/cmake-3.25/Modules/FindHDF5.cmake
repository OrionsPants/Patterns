...
function(_HDF5_invoke_compiler language output_var return_value_var version_var
         is_parallel_var)
  ...
  # Verify that the compiler wrapper can actually compile: sometimes the
  # compiler wrapper exists, but not the compiler.  E.g. Miniconda / Anaconda
  # Python
  execute_process(
  ...
  if(return_value AND NOT HDF5_FIND_QUIETLY)
    ...
    execute_process(
      COMMAND ${HDF5_${language}_COMPILER_EXECUTABLE} -show ${lib_type_args}
              ${test_file}
      WORKING_DIRECTORY ${scratch_dir}
      OUTPUT_VARIABLE output
      ERROR_VARIABLE output
    ...
    if(return_value AND NOT HDF5_FIND_QUIETLY)
      message(
        STATUS "Unable to determine HDF5 ${language} flags from HDF5 wrapper.")
    endif()
    execute_process(
    ...
    string(REGEX MATCH "HDF5 Version: ([a-zA-Z0-9\\.\\-]*)" version
                 "${config_output}")
    if(version)
...
function(_HDF5_select_imported_config target imported_conf)
  # We will first assign the value to a local variable _imported_conf, then
  # assign it to the function argument at the end.
  get_target_property(_imported_conf ${target}
  ...
  if(NOT _imported_conf)
    ...
    # Find the imported configuration that we prefer. We do this by making list
    # of configurations in order of preference, starting with
    # ${CMAKE_BUILD_TYPE} and ending with the first imported_conf
...
if(NOT HDF5_FOUND AND NOT HDF5_NO_FIND_PACKAGE_CONFIG_FILE)
  find_package(HDF5 QUIET NO_MODULE HINTS ${HDF5_ROOT} ${_HDF5_SEARCH_OPTS})
  if(HDF5_FOUND)
    if(HDF5_FIND_DEBUG)
      message(
        STATUS
    ...
    set(HDF5_LIBRARIES)
    if(NOT TARGET hdf5
       AND NOT TARGET hdf5-static
       AND NOT TARGET hdf5-shared)
    ...
    foreach(_lang ${HDF5_LANGUAGE_BINDINGS})
      ...
      # Older versions of hdf5 don't have a static/shared suffix so if we detect
      # that occurrence clear the suffix
      if(_suffix AND NOT TARGET ${HDF5_${_lang}_TARGET}${_suffix})
        if(NOT TARGET ${HDF5_${_lang}_TARGET})
          # can't find this component with or without the suffix so bail out,
          # and let the following locate HDF5
      ...
      # Find library for this target. Complicated as on Windows with a DLL, we
      # need to search for the import-lib.
      _hdf5_select_imported_config(${HDF5_${_lang}_TARGET}${_suffix}
                                   _hdf5_imported_conf)
      get_target_property(_hdf5_lang_location ${HDF5_${_lang}_TARGET}${_suffix}
                          IMPORTED_IMPLIB_${_hdf5_imported_conf})
      if(NOT _hdf5_lang_location)
        # no import lib, just try LOCATION
        get_target_property(
          _hdf5_lang_location ${HDF5_${_lang}_TARGET}${_suffix}
          LOCATION_${_hdf5_imported_conf})
        if(NOT _hdf5_lang_location)
          get_target_property(_hdf5_lang_location
      ...
      if(HDF5_FIND_HL)
        get_target_property(
          _hdf5_lang_hl_location ${HDF5_${_lang}_HL_TARGET}${_suffix}
          IMPORTED_IMPLIB_${_hdf5_imported_conf})
        if(NOT _hdf5_lang_hl_location)
          ...
          if(NOT _hdf5_hl_lang_location)
            get_target_property(_hdf5_hl_lang_location
  ...
  foreach(_lang IN LISTS HDF5_LANGUAGE_BINDINGS)
    ...
    if(_lang STREQUAL "C")
      _hdf5_test_regular_compiler_c(
        HDF5_${_lang}_COMPILER_NO_INTERROGATE HDF5_${_lang}_VERSION
    ...
    elseif(_lang STREQUAL "Fortran")
      _hdf5_test_regular_compiler_fortran(HDF5_${_lang}_COMPILER_NO_INTERROGATE
                                          HDF5_${_lang}_IS_PARALLEL)
    else()
      continue()
    endif()
    if(HDF5_${_lang}_COMPILER_NO_INTERROGATE)
      if(HDF5_FIND_DEBUG)
        message(
          STATUS "HDF5: Using hdf5 compiler wrapper for all ${_lang} compiling")
      endif()
      set(HDF5_${_lang}_FOUND TRUE)
      ...
      # If this language isn't using the wrapper, then try to seed the search
      # options with the wrapper
      find_program(
        HDF5_${_lang}_COMPILER_EXECUTABLE
      ...
      if(HDF5_${_lang}_COMPILER_EXECUTABLE)
        _hdf5_invoke_compiler(
          ${_lang} HDF5_${_lang}_COMPILE_LINE HDF5_${_lang}_RETURN_VALUE
          HDF5_${_lang}_VERSION HDF5_${_lang}_IS_PARALLEL)
        if(HDF5_${_lang}_RETURN_VALUE EQUAL 0)
          if(HDF5_FIND_DEBUG)
            message(
              STATUS
                "HDF5: Using hdf5 compiler wrapper to determine ${_lang} configuration"
            )
          endif()
          _hdf5_parse_compile_line(
            HDF5_${_lang}_COMPILE_LINE HDF5_${_lang}_INCLUDE_DIRS
            HDF5_${_lang}_DEFINITIONS HDF5_${_lang}_LIBRARY_DIRS
            HDF5_${_lang}_LIBRARY_NAMES HDF5_${_lang}_HL_LIBRARY_NAMES)
          set(HDF5_${_lang}_LIBRARIES)
          ...
          if(HDF5_FIND_HL)
            ...
            foreach(_lib IN LISTS HDF5_${_lang}_HL_LIBRARY_NAMES)
              ...
              unset(_HDF5_SEARCH_NAMES_LOCAL)
              if(HDF5_${_lang}_LIBRARY_${_lib})
                list(APPEND HDF5_${_lang}_HL_LIBRARIES
                     ${HDF5_${_lang}_LIBRARY_${_lib}})
              else()
    ...
    if(HDF5_${_lang}_VERSION)
      ...
      elseif(NOT HDF5_VERSION VERSION_EQUAL HDF5_${_lang}_VERSION)
        message(
          WARNING
            "HDF5 Version found for language ${_lang}, ${HDF5_${_lang}_VERSION} is different than previously found version ${HDF5_VERSION}"
        )
      endif()
    endif()
    if(DEFINED HDF5_${_lang}_IS_PARALLEL)
      ...
      elseif(NOT HDF5_IS_PARALLEL AND HDF5_${_lang}_IS_PARALLEL)
        message(
          WARNING
            "HDF5 found for language ${_lang} is parallel but previously found language is not parallel."
        )
      elseif(HDF5_IS_PARALLEL AND NOT HDF5_${_lang}_IS_PARALLEL)
        message(
          WARNING
            "HDF5 found for language ${_lang} is not parallel but previously found language is parallel."
        )
      endif()
    endif()
  endforeach()
...
elseif(NOT HDF5_FOUND AND NOT _HDF5_NEED_TO_SEARCH)
  # Compiler wrappers aren't being used by the build but were found and used to
  # determine necessary include and library flags
...
find_program(
  ...
  NAMES h5diff
  HINTS ${HDF5_ROOT}
  PATH_SUFFIXES bin Bin ${_HDF5_SEARCH_OPTS}
...
if(NOT HDF5_FOUND)
  ...
  set(HDF5_Fortran_HL_LIBRARY_NAMES
      hdf5_hl_fortran hdf5hl_fortran ${HDF5_C_HL_LIBRARY_NAMES}
  ...
  foreach(_lang IN LISTS HDF5_LANGUAGE_BINDINGS)
    ...
    foreach(LIB IN LISTS HDF5_${_lang}_LIBRARY_NAMES)
      if(HDF5_USE_STATIC_LIBRARIES)
        # According to bug 1643 on the CMake bug tracker, this is the preferred
        # method for searching for a static library. See
        # https://gitlab.kitware.com/cmake/cmake/-/issues/1643.  We search first
        # for the full static library name, but fall back to a generic search on
        # the name if the static search fails.
        set(THIS_LIBRARY_SEARCH_DEBUG
            ...
            ${LIB}_D-static
            ${LIB}_debug-static)
        set(THIS_LIBRARY_SEARCH_RELEASE lib${LIB}.a lib${LIB}
                                        lib${LIB}-static.a ${LIB}-static)
      else()
        set(THIS_LIBRARY_SEARCH_DEBUG
            ${LIB}d ${LIB}_D ${LIB}_debug ${LIB}d-shared ${LIB}_D-shared
            ${LIB}_debug-shared)
      ...
      find_library(
        ...
        HINTS ${HDF5_ROOT}
        PATH_SUFFIXES lib Lib ${_lib_suffixes} ${_HDF5_SEARCH_OPTS})
      find_library(
    ...
    # Append the libraries for this language binding to the list of all required
    # libraries.
    list(APPEND HDF5_LIBRARIES ${HDF5_${_lang}_LIBRARIES})
    ...
    if("${_lang}" STREQUAL "Fortran")
      ...
      if(NOT _hdf5_main_library STREQUAL "")
        ...
        if(CMAKE_Fortran_COMPILER_ID STREQUAL "GNU" OR CMAKE_Fortran_COMPILER_ID
                                                       STREQUAL "LCC")
          get_filename_component(_hdf5_library_dir "${_hdf5_main_library}"
                                 DIRECTORY)
    ...
    if(HDF5_FIND_HL)
      foreach(LIB IN LISTS HDF5_${_lang}_HL_LIBRARY_NAMES)
        if(HDF5_USE_STATIC_LIBRARIES)
          ...
          # preferred method for searching for a static library. See
          # https://gitlab.kitware.com/cmake/cmake/-/issues/1643.  We search
          # first for the full static library name, but fall back to a generic
          # search on the name if the static search fails.
          set(THIS_LIBRARY_SEARCH_DEBUG
              lib${LIB}d.a
              lib${LIB}_debug.a
              lib${LIB}d
              ...
              lib${LIB}d-static
              lib${LIB}_D-static
              lib${LIB}_debug-static)
          set(THIS_LIBRARY_SEARCH_RELEASE lib${LIB}.a lib${LIB}
                                          lib${LIB}-static.a lib${LIB}-static)
        else()
          set(THIS_LIBRARY_SEARCH_DEBUG
              ${LIB}d ${LIB}_D ${LIB}_debug ${LIB}d-shared ${LIB}_D-shared
              ${LIB}_debug-shared)
  ...
  foreach(_dir IN LISTS HDF5_INCLUDE_DIRS)
    foreach(_hdr "${_dir}/H5pubconf.h" "${_dir}/H5pubconf-64.h"
                 "${_dir}/H5pubconf-32.h")
      if(EXISTS "${_hdr}")
        file(STRINGS "${_hdr}" HDF5_HAVE_PARALLEL_DEFINE
        ...
        file(STRINGS "${_hdr}" HDF5_VERSION_DEFINE
...
# If HDF5_REQUIRED_VARS is empty at this point, then it's likely that something
# external is trying to explicitly pass already found locations
...
if(HDF5_FOUND)
  if(NOT TARGET HDF5::HDF5)
    ...
    set_target_properties(
      HDF5::HDF5
      PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${HDF5_INCLUDE_DIRS}"
    ...
    if(NOT TARGET "hdf5::${hdf5_target_name}")
      if(HDF5_COMPILER_NO_INTERROGATE)
        ...
        if(_hdf5_location_release)
          set_property(
            TARGET "hdf5::${hdf5_target_name}"
            APPEND
            PROPERTY IMPORTED_CONFIGURATIONS RELEASE)
          set_property(
            TARGET "hdf5::${hdf5_target_name}"
            PROPERTY IMPORTED_LOCATION_RELEASE "${_hdf5_location_release}")
        endif()
        if(_hdf5_location_debug)
          ...
          set_property(
            TARGET "hdf5::${hdf5_target_name}"
            PROPERTY IMPORTED_LOCATION_DEBUG "${_hdf5_location_debug}")
        endif()
        if(NOT _hdf5_location_release AND NOT _hdf5_location_debug)
          set_property(TARGET "hdf5::${hdf5_target_name}"
                       PROPERTY IMPORTED_LOCATION "${_hdf5_location}")
        endif()
        if(_hdf5_libtype STREQUAL "SHARED")
          set_property(
            TARGET "hdf5::${hdf5_target_name}"
            APPEND
            PROPERTY INTERFACE_COMPILE_DEFINITIONS H5_BUILT_AS_DYNAMIC_LIB)
        elseif(_hdf5_libtype STREQUAL "STATIC")
          set_property(
            TARGET "hdf5::${hdf5_target_name}"
            APPEND
            PROPERTY INTERFACE_COMPILE_DEFINITIONS H5_BUILT_AS_STATIC_LIB)
        endif()
        unset(_hdf5_definitions)
        unset(_hdf5_libtype)
    ...
    if(NOT TARGET "hdf5::${hdf5_target_name}")
      if(HDF5_COMPILER_NO_INTERROGATE)
        ...
        set_target_properties(
          "hdf5::${hdf5_target_name}"
          PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                     "${HDF5_${hdf5_lang}_HL_INCLUDE_DIRS}"
                     INTERFACE_COMPILE_DEFINITIONS "${_hdf5_definitions}")
      else()
        if(DEFINED "HDF5_${hdf5_target_name}_LIBRARY")
          set(_hdf5_location "${HDF5_${hdf5_target_name}_LIBRARY}")
        ...
        elseif(DEFINED "HDF5_${hdf5_lang}_LIBRARY_${hdf5_target_name}")
          set(_hdf5_location "${HDF5_${hdf5_lang}_LIBRARY_${hdf5_target_name}}")
        elseif(hdf5_alt_target_name
               AND DEFINED "HDF5_${hdf5_lang}_LIBRARY_${hdf5_alt_target_name}")
          set(_hdf5_location
              "${HDF5_${hdf5_lang}_LIBRARY_${hdf5_alt_target_name}}")
        else()
          # Error if we still don't have the location.
          message(
            SEND_ERROR
  ...
  if(HDF5_DIFF_EXECUTABLE AND NOT TARGET hdf5::h5diff)
    ...
    set_target_properties(hdf5::h5diff PROPERTIES IMPORTED_LOCATION
                                                  "${HDF5_DIFF_EXECUTABLE}")
  endif()
endif()
