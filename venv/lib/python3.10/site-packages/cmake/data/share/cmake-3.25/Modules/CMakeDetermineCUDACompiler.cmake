...
if(NOT
  ...
  message(
    FATAL_ERROR
      "CUDA language not currently supported by \"${CMAKE_GENERATOR}\" generator"
  )
endif()
...
if(${CMAKE_GENERATOR} MATCHES "Visual Studio")
  if(DEFINED ENV{CUDAHOSTCXX} OR DEFINED CMAKE_CUDA_HOST_COMPILER)
    message(
      WARNING
        "Visual Studio does not support specifying CUDAHOSTCXX or CMAKE_CUDA_HOST_COMPILER. Using the C++ compiler provided by Visual Studio."
    )
  endif()
else()
  if(NOT CMAKE_CUDA_COMPILER)
    ...
    if(NOT $ENV{CUDACXX} STREQUAL "")
      get_filename_component(CMAKE_CUDA_COMPILER_INIT $ENV{CUDACXX} PROGRAM
                             PROGRAM_ARGS CMAKE_CUDA_FLAGS_ENV_INIT)
      if(CMAKE_CUDA_FLAGS_ENV_INIT)
        set(CMAKE_CUDA_COMPILER_ARG1
            "${CMAKE_CUDA_FLAGS_ENV_INIT}"
            CACHE STRING "Arguments to CUDA compiler")
      endif()
...
if(NOT CMAKE_CUDA_COMPILER_ID_RUN)
  ...
  if(${CMAKE_GENERATOR} MATCHES "Visual Studio")
    # We will not know CMAKE_CUDA_COMPILER until the main compiler id step below
    # extracts it, but we do know that the compiler id will be NVIDIA.
    set(CMAKE_CUDA_COMPILER_ID "NVIDIA")
  else()
    # We determine the vendor to help with find the toolkit and use the right
    # flags for detection right away. The main compiler identification is still
    # needed below to extract other information.
    list(APPEND CMAKE_CUDA_COMPILER_ID_VENDORS NVIDIA Clang)
    set(CMAKE_CUDA_COMPILER_ID_VENDOR_REGEX_NVIDIA
    ...
    # CMAKE_CUDA_COMPILER_TOOLKIT_VERSION and CMAKE_CUDA_COMPILER_LIBRARY_ROOT
    # in CMakeCUDACompiler.cmake so FindCUDAToolkit can avoid searching on
    # future runs and the toolkit is the same. This is very similar to
    # FindCUDAToolkit, but somewhat simplified since we can issue fatal errors
    ...
    if(CMAKE_CUDA_COMPILER_ID STREQUAL "NVIDIA")
      ...
      # Search using CUDAToolkit_ROOT and then CUDA_PATH for equivalence with
      # FindCUDAToolkit. In FindCUDAToolkit CUDAToolkit_ROOT is searched
      # automatically due to being in a find_package(). First we search
      # candidate non-default paths to give them priority.
      find_program(
        _CUDA_NVCC_EXECUTABLE
        NAMES nvcc nvcc.exe
        PATHS ${CUDAToolkit_ROOT} ENV CUDAToolkit_ROOT ENV CUDA_PATH
        PATH_SUFFIXES bin
        NO_DEFAULT_PATH)
      ...
      # If the user specified CUDAToolkit_ROOT but nvcc could not be found, this
      # is an error.
      if(NOT _CUDA_NVCC_EXECUTABLE AND (DEFINED CUDAToolkit_ROOT
                                        OR DEFINED ENV{CUDAToolkit_ROOT}))
        set(fail_base "Could not find nvcc executable in path specified by")

        if(DEFINED CUDAToolkit_ROOT)
          message(
            FATAL_ERROR "${fail_base} CUDAToolkit_ROOT=${CUDAToolkit_ROOT}")
      ...
      # CUDAToolkit_ROOT cmake/env variable not specified, try platform
      # defaults.
      #
      ...
      #
      # We will also search the default symlink location /usr/local/cuda first
      # since if CUDAToolkit_ROOT is not specified, it is assumed that the
      # symlinked directory is the desired location.
      if(NOT _CUDA_NVCC_EXECUTABLE)
        ...
        # Sort numerically in descending order, so we try the newest versions
        # first.
        list(
          SORT versions
          COMPARE NATURAL
    ...
    execute_process(
      COMMAND ${_CUDA_NVCC_EXECUTABLE} "-v" "__cmake_determine_cuda"
      OUTPUT_VARIABLE _CUDA_NVCC_OUT
      ERROR_VARIABLE _CUDA_NVCC_OUT)
    if(_CUDA_NVCC_OUT MATCHES "\\#\\$ TOP=([^\r\n]*)")
      get_filename_component(CMAKE_CUDA_COMPILER_TOOLKIT_ROOT
                             "${CMAKE_MATCH_1}" ABSOLUTE)
    else()
      get_filename_component(CMAKE_CUDA_COMPILER_TOOLKIT_ROOT
                             "${_CUDA_NVCC_EXECUTABLE}" DIRECTORY)
      get_filename_component(CMAKE_CUDA_COMPILER_TOOLKIT_ROOT
                             "${CMAKE_CUDA_COMPILER_TOOLKIT_ROOT}" DIRECTORY)
    endif()

    if(_CUDA_NVCC_OUT MATCHES "\\#\\$ NVVMIR_LIBRARY_DIR=([^\r\n]*)")
      get_filename_component(_CUDA_NVVMIR_LIBRARY_DIR "${CMAKE_MATCH_1}"
                             ABSOLUTE)

      # We require the path to end in `/nvvm/libdevice'
      if(_CUDA_NVVMIR_LIBRARY_DIR MATCHES "nvvm/libdevice$")
        get_filename_component(_CUDA_NVVMIR_LIBRARY_DIR
                               "${_CUDA_NVVMIR_LIBRARY_DIR}/../.." ABSOLUTE)
        set(CMAKE_CUDA_COMPILER_LIBRARY_ROOT_FROM_NVVMIR_LIBRARY_DIR
            "${_CUDA_NVVMIR_LIBRARY_DIR}")
      endif()
    ...
    set(CMAKE_CUDA_DEVICE_LINKER
        "${CMAKE_CUDA_COMPILER_TOOLKIT_ROOT}/bin/nvlink${CMAKE_EXECUTABLE_SUFFIX}"
    )
    set(CMAKE_CUDA_FATBINARY
        "${CMAKE_CUDA_COMPILER_TOOLKIT_ROOT}/bin/fatbinary${CMAKE_EXECUTABLE_SUFFIX}"
    )

    # In a non-scattered installation the following are equivalent to
    # CMAKE_CUDA_COMPILER_TOOLKIT_ROOT. We first check for a non-scattered
    # installation to prefer it over a scattered installation.
    ...
    elseif(CMAKE_SYSROOT_LINK
           AND EXISTS "${CMAKE_SYSROOT_LINK}/usr/lib/cuda/nvvm/libdevice")
    ...
    # CMAKE_CUDA_COMPILER_TOOLKIT_LIBRARY_ROOT contains the linking stubs
    # necessary for device linking and other low-level library files.
    if(CMAKE_SYSROOT_LINK
       AND EXISTS
           "${CMAKE_SYSROOT_LINK}/usr/lib/nvidia-cuda-toolkit/bin/crt/link.stub"
    )
      set(CMAKE_CUDA_COMPILER_TOOLKIT_LIBRARY_ROOT
          "${CMAKE_SYSROOT_LINK}/usr/lib/nvidia-cuda-toolkit")
    elseif(EXISTS
           "${CMAKE_SYSROOT}/usr/lib/nvidia-cuda-toolkit/bin/crt/link.stub")
      set(CMAKE_CUDA_COMPILER_TOOLKIT_LIBRARY_ROOT
          "${CMAKE_SYSROOT}/usr/lib/nvidia-cuda-toolkit")
    else()
      set(CMAKE_CUDA_COMPILER_TOOLKIT_LIBRARY_ROOT
          "${CMAKE_CUDA_COMPILER_TOOLKIT_ROOT}")
    endif()
  ...
  # For regular nvcc we the toolkit version is the same as the compiler version
  # and we can parse it from the vendor test output. For Clang we need to invoke
  # nvcc to get version output.
  ...
  if(CMAKE_CUDA_COMPILER_ID STREQUAL "NVIDIA")
    ...
    # work. If we don't, users will get confusing errors later about failure to
    # detect a default value for CMAKE_CUDA_ARCHITECTURES
    set(CMAKE_CUDA_COMPILER_ID_REQUIRE_SUCCESS ON)
  elseif(CMAKE_CUDA_COMPILER_ID STREQUAL "Clang")
    set(clang_test_flags "--cuda-path=\"${CMAKE_CUDA_COMPILER_LIBRARY_ROOT}\"")
    if(CMAKE_CROSSCOMPILING)
      # Need to pass the host target and include directories if we're
      # crosscompiling.
      string(
        APPEND clang_test_flags
        " --sysroot=\"${CMAKE_SYSROOT}\" --target=${CMAKE_CUDA_COMPILER_TARGET}"
      )
    endif()
  endif()

  # Rest of the code treats an empty value as equivalent to "use the defaults".
  # Error out early to prevent confusing errors as a result of this. Note that
  # this also catches invalid non-numerical values such as "a".
  ...
  if(CMAKE_CUDA_COMPILER_ID STREQUAL "Clang")
    ...
    # Try in reverse order of deprecation with the most recent at front (i.e.
    # the most likely to work for new setups).
    foreach(arch "52" "30" "20")
      list(APPEND CMAKE_CUDA_COMPILER_ID_TEST_FLAGS_FIRST
           "${clang_test_flags} --cuda-gpu-arch=sm_${arch}")
    endforeach()
  elseif(CMAKE_CUDA_COMPILER_ID STREQUAL "NVIDIA")
    list(APPEND CMAKE_CUDA_COMPILER_ID_TEST_FLAGS_FIRST "${nvcc_test_flags}")
  endif()

  # We perform compiler identification for a second time to extract implicit
  # linking info and host compiler for NVCC. We need to unset the compiler ID
  # otherwise CMAKE_DETERMINE_COMPILER_ID() doesn't work.
...
if(${CMAKE_GENERATOR} MATCHES "Visual Studio")
  ...
  # We do not currently detect CMAKE_CUDA_HOST_IMPLICIT_LINK_LIBRARIES but we do
  # need to detect CMAKE_CUDA_RUNTIME_LIBRARY_DEFAULT from the compiler by
  # looking at which cudart library exists in the implicit link libraries passed
  # to the host linker.
  if(CMAKE_CUDA_COMPILER_PRODUCED_OUTPUT MATCHES
     "link\\.exe [^\n]*cudart_static\\.lib")
    set(CMAKE_CUDA_RUNTIME_LIBRARY_DEFAULT "STATIC")
  elseif(CMAKE_CUDA_COMPILER_PRODUCED_OUTPUT MATCHES
         "link\\.exe [^\n]*cudart\\.lib")
...
elseif(CMAKE_CUDA_COMPILER_ID STREQUAL "Clang")
  string(REGEX MATCHALL "-target-cpu sm_([0-9]+)" target_cpus
  ...
  if(CMAKE_CROSSCOMPILING)
    ...
    if(EXISTS
       "${CMAKE_CUDA_COMPILER_TOOLKIT_ROOT}/targets/${_CUDA_TARGET_NAME}")
  ...
  elseif(EXISTS "${_CUDA_TARGET_DIR}/lib")
    ...
    message(
      FATAL_ERROR
        "Unable to find _CUDA_LIBRARY_DIR based on _CUDA_TARGET_DIR=${_CUDA_TARGET_DIR}"
    )
  endif()

  # _CUDA_TARGET_DIR always points to the directory containing the include
  # directory. On a scattered installation /usr, on a non-scattered something
  # like /usr/local/cuda or /usr/local/cuda-10.2/targets/aarch64-linux.
  if(EXISTS "${_CUDA_TARGET_DIR}/include/cuda_runtime.h")
    set(_CUDA_INCLUDE_DIR "${_CUDA_TARGET_DIR}/include")
  else()
    message(
      FATAL_ERROR
        "Unable to find cuda_runtime.h in \"${_CUDA_TARGET_DIR}/include\" for _CUDA_INCLUDE_DIR."
    )
  endif()

  # Clang does not add any CUDA SDK libraries or directories when invoking the
  # host linker. Add the CUDA toolkit library directory ourselves so that
  # linking works. The CUDA runtime libraries are handled elsewhere by
...
elseif(CMAKE_CUDA_COMPILER_ID STREQUAL "NVIDIA")
  ...
  if(_nvcc_output_orig MATCHES "#\\\$ +PATH= *([^\n]*)\n")
    ...
    set(_nvcc_path "")
    string(REPLACE "\n" "\n    " _nvcc_output_log "\n${_nvcc_output_orig}")
    string(APPEND _nvcc_log
           "  no 'PATH=' string found in nvcc output:${_nvcc_output_log}\n")
  endif()
  if(_nvcc_output_orig MATCHES "#\\\$ +LIBRARIES= *([^\n]*)\n")
    set(_nvcc_libraries "${CMAKE_MATCH_1}")
    string(APPEND _nvcc_log
           "  found 'LIBRARIES=' string: [${_nvcc_libraries}]\n")
  else()
    set(_nvcc_libraries "")
    string(REPLACE "\n" "\n    " _nvcc_output_log "\n${_nvcc_output_orig}")
    string(
      APPEND _nvcc_log
      "  no 'LIBRARIES=' string found in nvcc output:${_nvcc_output_log}\n")
  endif()
    ...
    foreach(line IN LISTS _nvcc_output)
      ...
      elseif(_nvcc_libraries)
        if("${_nvcc_output_line}" MATCHES "(@\"?((tmp/)?a\\.exe\\.res)\"?)")
          ...
          set(_nvcc_link_res
              "${CMAKE_PLATFORM_INFO_DIR}/CompilerIdCUDA/${_nvcc_link_res_file}"
          )
        ...
        if(NOT _nvcc_libraries_pos EQUAL -1)
          ...
          string(APPEND _nvcc_log
                 "    extracted link line: [${_nvcc_link_line}]\n")
        endif()
      endif()
    endforeach()
  endif()
  ...
  if(_nvcc_link_line)
    if("x${CMAKE_CUDA_SIMULATE_ID}" STREQUAL "xMSVC")
      ...
      if(IS_ABSOLUTE "${_nvcc_host_link_launcher}")
        string(
          APPEND
          _nvcc_log
          "  extracted link launcher absolute path: [${_nvcc_host_link_launcher}]\n"
        )
        set(CMAKE_CUDA_HOST_LINK_LAUNCHER "${_nvcc_host_link_launcher}")
      else()
        string(
          APPEND _nvcc_log
          "  extracted link launcher name: [${_nvcc_host_link_launcher}]\n")
        find_program(
          _nvcc_find_host_link_launcher
        ...
        if(_nvcc_find_host_link_launcher)
          string(
            APPEND
            _nvcc_log
            "  found link launcher absolute path: [${_nvcc_find_host_link_launcher}]\n"
          )
          set(CMAKE_CUDA_HOST_LINK_LAUNCHER "${_nvcc_find_host_link_launcher}")
        else()
          string(APPEND _nvcc_log
                 "  could not find link launcher absolute path\n")
          set(CMAKE_CUDA_HOST_LINK_LAUNCHER "${_nvcc_host_link_launcher}")
        endif()
    ...
    # Detect CMAKE_CUDA_RUNTIME_LIBRARY_DEFAULT from the compiler by looking at
    # which cudart library exists in the implicit link libraries passed to the
    # host linker. This is required when a project sets the cuda runtime library
    # as part of the initial flags.
    if(";${CMAKE_CUDA_HOST_IMPLICIT_LINK_LIBRARIES};" MATCHES
       [[;cudart_static(\.lib)?;]])
      set(CMAKE_CUDA_RUNTIME_LIBRARY_DEFAULT "STATIC")
    elseif(";${CMAKE_CUDA_HOST_IMPLICIT_LINK_LIBRARIES};" MATCHES
           [[;cudart(\.lib)?;]])
      set(CMAKE_CUDA_RUNTIME_LIBRARY_DEFAULT "SHARED")
    else()
      set(CMAKE_CUDA_RUNTIME_LIBRARY_DEFAULT "NONE")
...
# libraries that the CUDA compiler implicitly passes to the host linker. CMake
# invokes the host linker directly and so needs to pass these libraries. We
# filter out those that should not be passed unconditionally both here and from
# CMAKE_CUDA_IMPLICIT_LINK_LIBRARIES in CMakeTestCUDACompiler.
set(CMAKE_CUDA_IMPLICIT_LINK_LIBRARIES_EXCLUDE
    ...
    pthread
    dl)
list(REMOVE_ITEM CMAKE_CUDA_HOST_IMPLICIT_LINK_LIBRARIES
     ${CMAKE_CUDA_IMPLICIT_LINK_LIBRARIES_EXCLUDE})
...
if(CMAKE_CUDA_COMPILER_ID STREQUAL "NVIDIA")
  ...
  if(_nvcc_output_orig MATCHES "#\\\$ +INCLUDES= *([^\n]*)\n")
    ...
    string(REPLACE "\n" "\n    " _nvcc_output_log "\n${_nvcc_output_orig}")
    string(APPEND _nvcc_log
           "  no 'INCLUDES=' string found in nvcc output:${_nvcc_output_log}\n")
  endif()
  if(_nvcc_includes)
  ...
  string(REGEX MATCHALL "-arch compute_([0-9]+)" target_cpus
               "${CMAKE_CUDA_COMPILER_PRODUCED_OUTPUT}")
...
# If the user didn't set the architectures, then set them to a default. If the
# user did, then make sure those architectures worked.
if("${CMAKE_CUDA_ARCHITECTURES}" STREQUAL "")
  cmake_policy(GET CMP0104 _CUDA_CMP0104)

  if(NOT CMAKE_CUDA_COMPILER_ID STREQUAL "NVIDIA" OR _CUDA_CMP0104 STREQUAL
                                                     "NEW")
    set(CMAKE_CUDA_ARCHITECTURES
...
configure_file(${CMAKE_ROOT}/Modules/CMakeCUDACompiler.cmake.in
               ${CMAKE_PLATFORM_INFO_DIR}/CMakeCUDACompiler.cmake @ONLY)
