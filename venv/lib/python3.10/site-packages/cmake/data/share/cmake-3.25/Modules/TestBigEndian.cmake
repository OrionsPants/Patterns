...
function(TEST_BIG_ENDIAN VARIABLE)
  if(";${CMAKE_C_BYTE_ORDER};${CMAKE_CXX_BYTE_ORDER};${CMAKE_CUDA_BYTE_ORDER};${CMAKE_OBJC_BYTE_ORDER};${CMAKE_OBJCXX_BYTE_ORDER};"
     MATCHES
...
macro(__TEST_BIG_ENDIAN_LEGACY_IMPL VARIABLE)
  if(NOT DEFINED HAVE_${VARIABLE})
    ...
    elseif(CMAKE_CXX_COMPILER_LOADED)
      ...
      message(
        FATAL_ERROR "TEST_BIG_ENDIAN needs either C or CXX language enabled")
    endif()
    ...
    file(READ "${CMAKE_ROOT}/Modules/TestEndianess.c.in"
         TEST_ENDIANESS_FILE_CONTENT)
    string(CONFIGURE "${TEST_ENDIANESS_FILE_CONTENT}"
                     TEST_ENDIANESS_FILE_CONTENT @ONLY)

    try_compile(
      HAVE_${VARIABLE} SOURCE_FROM_VAR
      "${_test_file}" TEST_ENDIANESS_FILE_CONTENT
    ...
    if(HAVE_${VARIABLE})
      ...
      file(
        STRINGS "${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/TestEndianess.bin"
      ...
      # on mac, if there are universal binaries built both will be true return
      # the result depending on the machine on which cmake runs
      if(CMAKE_TEST_ENDIANESS_STRINGS_BE AND CMAKE_TEST_ENDIANESS_STRINGS_LE)
        ...
        message(
          STATUS
            "TEST_BIG_ENDIAN found different results, consider setting CMAKE_OSX_ARCHITECTURES or CMAKE_TRY_COMPILE_OSX_ARCHITECTURES to one or no architecture !"
        )
      endif()
      ...
      if(CMAKE_TEST_ENDIANESS_STRINGS_LE)
        set(${VARIABLE}
            0
            CACHE INTERNAL "Result of TEST_BIG_ENDIAN" FORCE)
