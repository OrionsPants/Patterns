...
# ----------------------------------------------------------------------------
# History: This module is derived from the module originally found in the VTK
# source tree.
#
# ----------------------------------------------------------------------------
# Note: PostgreSQL_ADDITIONAL_VERSIONS is a variable that can be used to set the
# version number of the implementation of PostgreSQL. In Windows the default
# installation of PostgreSQL uses that as part of the path. E.g C:\Program
# Files\PostgreSQL\8.4. Currently, the following version numbers are known to
# this module: "14" "13" "12" "11" "10" "9.6" "9.5" "9.4" "9.3" "9.2" "9.1"
# "9.0" "8.4" "8.3" "8.2" "8.1" "8.0"
#
# To use this variable just do something like this:
# set(PostgreSQL_ADDITIONAL_VERSIONS "9.2" "8.4.4") before calling
# find_package(PostgreSQL) in your CMakeLists.txt file. This will mean that the
# versions you set here will be found first in the order specified before the
# default ones are searched.
#
# ----------------------------------------------------------------------------
# You may need to manually set: PostgreSQL_INCLUDE_DIR  - the path to where the
# PostgreSQL include files are. PostgreSQL_LIBRARY_DIR  - The path to where the
# PostgreSQL library files are. If FindPostgreSQL.cmake cannot find the include
# files or the library files.
#
# ----------------------------------------------------------------------------
# The following variables are set if PostgreSQL is found: PostgreSQL_FOUND - Set
# to true when PostgreSQL is found. PostgreSQL_INCLUDE_DIRS  - Include
# directories for PostgreSQL PostgreSQL_LIBRARY_DIRS  - Link directories for
...
#
# ----------------------------------------------------------------------------
# If you have installed PostgreSQL in a non-standard location. (Please note that
# in the following comments, it is assumed that <Your Path> points to the root
# directory of the include directory of PostgreSQL.) Then you have three
# options. 1) After CMake runs, set PostgreSQL_INCLUDE_DIR to <Your
# Path>/include and PostgreSQL_LIBRARY_DIR to wherever the library pq (or libpq
# in windows) is 2) Use CMAKE_INCLUDE_PATH to set a path to <Your
# Path>/PostgreSQL<-version>. This will allow find_path() to locate
# PostgreSQL_INCLUDE_DIR by utilizing the PATH_SUFFIXES option. e.g. In your
# CMakeLists.txt file set(CMAKE_INCLUDE_PATH ${CMAKE_INCLUDE_PATH} "<Your
# Path>/include") 3) Set an environment variable called ${PostgreSQL_ROOT} that
# points to the root of where you have installed PostgreSQL, e.g. <Your Path>.
#
# ----------------------------------------------------------------------------
...
set(PostgreSQL_LIBRARY_PATH_DESCRIPTION
...
foreach(suffix ${PostgreSQL_KNOWN_VERSIONS})
  ...
  if(UNIX)
    list(APPEND PostgreSQL_LIBRARY_ADDITIONAL_SEARCH_SUFFIXES
         "postgresql${suffix}" "postgresql@${suffix}" "pgsql-${suffix}/lib")
    list(APPEND PostgreSQL_INCLUDE_ADDITIONAL_SEARCH_SUFFIXES
         "postgresql${suffix}" "postgresql@${suffix}" "postgresql/${suffix}"
         "pgsql-${suffix}/include")
    list(APPEND PostgreSQL_TYPE_ADDITIONAL_SEARCH_SUFFIXES
         "postgresql${suffix}/server" "postgresql@${suffix}/server"
         "postgresql/${suffix}/server" "pgsql-${suffix}/include/server")
  endif()
endforeach()
...
# For compatibility with versions prior to this multi-config search, honor any
# PostgreSQL_LIBRARY that is already specified and skip the search.
if(PostgreSQL_LIBRARY)
  ...
  elseif(PostgreSQL_LIBRARY_DEBUG)
    get_filename_component(PostgreSQL_LIBRARY_DIR "${PostgreSQL_LIBRARY_DEBUG}"
                           PATH)
  else()
...
if(PostgreSQL_INCLUDE_DIR)
  ...
  # This is a temporary workaround.  A better solution would be to compile a
  # dummy c file and extract the value of the symbol.
  ...
  foreach(_PG_CONFIG_HEADER ${_PG_CONFIG_HEADERS})
    if(EXISTS "${_PG_CONFIG_HEADER}")
      ...
      if(pgsql_version_str)
        string(REGEX
               REPLACE "^#define[\t ]+PG_VERSION_NUM[\t ]+([0-9]*).*" "\\1"
                       _PostgreSQL_VERSION_NUM "${pgsql_version_str}")
        break()
      endif()
    endif()
  endforeach()
  if(_PostgreSQL_VERSION_NUM)
    ...
    if(_PostgreSQL_VERSION_NUM LESS 100000)
      ...
      math(EXPR _PostgreSQL_minor_version
...
function(__postgresql_import_library _target _var _config)
  ...
  if(EXISTS "${_lib}")
    ...
    set_target_properties(
      ${_target} PROPERTIES IMPORTED_LOCATION${_config_suffix} "${_lib}")
  endif()
endfunction()
...
if(PostgreSQL_FOUND)
  ...
  if(NOT TARGET PostgreSQL::PostgreSQL)
    ...
    set_target_properties(
      PostgreSQL::PostgreSQL PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                        "${PostgreSQL_INCLUDE_DIRS}")
