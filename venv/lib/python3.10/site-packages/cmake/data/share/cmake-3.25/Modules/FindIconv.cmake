...
# iconv can only be provided in libc on a POSIX system. If any cache variable is
# already set, we'll skip this test.
if(NOT DEFINED Iconv_IS_BUILT_IN)
  if(UNIX
     AND NOT DEFINED Iconv_INCLUDE_DIR
     AND NOT DEFINED Iconv_LIBRARY)
    cmake_push_check_state(RESET)
    # We always suppress the message here: Otherwise on supported systems not
    # having iconv in their C library (e.g. those using libiconv) would always
    # display a confusing "Looking for iconv - not found" message
    set(CMAKE_FIND_QUIETLY TRUE)
    # The following code will not work, but it's sufficient to see if it
    # compiles. Note: libiconv will define the iconv functions as macros, so
    # CheckSymbolExists will not yield correct results.
...
if(Iconv_IS_BUILT_IN)
  ...
  find_library(
    Iconv_LIBRARY
    NAMES iconv libiconv NAMES_PER_DIR
...
if(Iconv_INCLUDE_DIR AND EXISTS "${Iconv_INCLUDE_DIR}/iconv.h")
  file(STRINGS ${Iconv_INCLUDE_DIR}/iconv.h Iconv_VERSION_DEFINE
       REGEX "_LIBICONV_VERSION (.*)")
  ...
  if(Iconv_VERSION_DEFINE MATCHES "(0x[A-Fa-f0-9]+)")
    ...
    math(EXPR Iconv_VERSION_MINOR
         "${Iconv_VERSION_NUMBER} - (${Iconv_VERSION_MAJOR} << 8)"
         OUTPUT_FORMAT HEXADECIMAL)

    math(EXPR Iconv_VERSION_MAJOR "${Iconv_VERSION_MAJOR}"
         OUTPUT_FORMAT DECIMAL)
    math(EXPR Iconv_VERSION_MINOR "${Iconv_VERSION_MINOR}"
         OUTPUT_FORMAT DECIMAL)
