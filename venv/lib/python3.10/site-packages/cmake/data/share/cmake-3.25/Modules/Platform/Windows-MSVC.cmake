...
# (where link was hardcoded) and where CMAKE_LINKER isn't in the cache and still
# cmake didn't fail in CMakeFindBinUtils.cmake (because it isn't rerun) hardcode
# CMAKE_LINKER here to link, so it behaves as it did before, Alex
...
if(NOT MSVC_VERSION)
  ...
  if("${_compiler_version}" MATCHES "^([0-9]+)\\.([0-9]+)")
    ...
    message(
      FATAL_ERROR
        "MSVC compiler version not detected properly: ${_compiler_version}")
  endif()
...
if(WINCE)
  ...
  set(_PLATFORM_DEFINES_C
      " /D${_MSVC_C_ARCHITECTURE_FAMILY} /D_${_MSVC_C_ARCHITECTURE_FAMILY_UPPER}_"
  )
  set(_PLATFORM_DEFINES_CXX
      " /D${_MSVC_CXX_ARCHITECTURE_FAMILY} /D_${_MSVC_CXX_ARCHITECTURE_FAMILY_UPPER}_"
  )
  ...
  foreach(lang C CXX)
    if(_MSVC_${lang}_ARCHITECTURE_FAMILY STREQUAL "ARM")
      string(APPEND _PLATFORM_DEFINES_${lang}
...
elseif(WINDOWS_PHONE OR WINDOWS_STORE)
  ...
  elseif(WINDOWS_PHONE)
    set(CMAKE_C_STANDARD_LIBRARIES_INIT
        "WindowsPhoneCore.lib RuntimeObject.lib PhoneAppModelHost.lib")
  elseif(
  ...
  set(_PLATFORM_DEFINES "/DWIN32")
  if((_MSVC_C_ARCHITECTURE_FAMILY STREQUAL "ARM64EC")
     OR (_MSVC_CXX_ARCHITECTURE_FAMILY STREQUAL "ARM64EC"))
    set(_PLATFORM_DEFINES
        "${_PLATFORM_DEFINES} /D_AMD64_ /DAMD64 /D_ARM64EC_ /DARM64EC")
  endif()
  if(_MSVC_C_ARCHITECTURE_FAMILY STREQUAL "ARM" OR _MSVC_CXX_ARCHITECTURE_FAMILY
                                                   STREQUAL "ARM")
    set(CMAKE_C_STANDARD_LIBRARIES_INIT "kernel32.lib user32.lib")
...
foreach(t EXE SHARED MODULE)
  ...
  if(CMAKE_COMPILER_SUPPORTS_PDBTYPE)
    string(APPEND CMAKE_${t}_LINKER_FLAGS_DEBUG_INIT
           " /debug /pdbtype:sept ${MSVC_INCREMENTAL_YES_FLAG}")
    string(APPEND CMAKE_${t}_LINKER_FLAGS_RELWITHDEBINFO_INIT
           " /debug /pdbtype:sept ${MSVC_INCREMENTAL_YES_FLAG}")
  else()
    string(APPEND CMAKE_${t}_LINKER_FLAGS_DEBUG_INIT
           " /debug ${MSVC_INCREMENTAL_YES_FLAG}")
    string(APPEND CMAKE_${t}_LINKER_FLAGS_RELWITHDEBINFO_INIT
           " /debug ${MSVC_INCREMENTAL_YES_FLAG}")
  endif()
...
macro(__windows_compiler_msvc lang)
  ...
  if(CMAKE_${lang}_COMPILER_ID STREQUAL "Clang")
    ...
    # interprets paths starting with /U as macro undefines, so we need to put a
    # -- before the input file path to force it to be treated as a path.
    string(REPLACE "-c <SOURCE>" "-c -- <SOURCE>" CMAKE_${lang}_COMPILE_OBJECT
                   "${CMAKE_${lang}_COMPILE_OBJECT}")
    string(REPLACE "-c <SOURCE>" "-c -- <SOURCE>"
  ...
  set(CMAKE_${lang}_COMPILE_OPTIONS_USE_PCH /Yu<PCH_HEADER> /Fp<PCH_FILE>
                                            /FI<PCH_HEADER>)
  set(CMAKE_${lang}_COMPILE_OPTIONS_CREATE_PCH /Yc<PCH_HEADER> /Fp<PCH_FILE>
                                               /FI<PCH_HEADER>)
  ...
  if("x${CMAKE_${lang}_COMPILER_ID}" STREQUAL "xMSVC")
    ...
    string(REPLACE "<LINK_FLAGS> " "/LTCG <LINK_FLAGS> "
                   CMAKE_${lang}_CREATE_STATIC_LIBRARY_IPO
                   "${CMAKE_${lang}_CREATE_STATIC_LIBRARY}")
  elseif("x${CMAKE_${lang}_COMPILER_ID}" STREQUAL "xClang"
         OR "x${CMAKE_${lang}_COMPILER_ID}" STREQUAL "xFlang")
    set(_CMAKE_${lang}_IPO_SUPPORTED_BY_CMAKE YES)
    set(_CMAKE_${lang}_IPO_MAY_BE_SUPPORTED_BY_COMPILER YES)
  ...
  if("x${lang}" STREQUAL "xC" OR "x${lang}" STREQUAL "xCXX")
    ...
    if(CMAKE_VS_PLATFORM_TOOLSET MATCHES "v[0-9]+_clang_.*")
      ...
      # this does not allow one to compile many projects that include MS's own
      # headers. CMake itself is affected project too.
      string(
        APPEND
      ...
      string(APPEND CMAKE_${lang}_FLAGS_DEBUG_INIT
             "${_MDd} -gline-tables-only -fno-inline -O0 ${_RTC1}")
      string(APPEND CMAKE_${lang}_FLAGS_RELEASE_INIT "${_MD} -O2 -DNDEBUG")
      string(APPEND CMAKE_${lang}_FLAGS_RELWITHDEBINFO_INIT
             "${_MD} -gline-tables-only -O2 -fno-inline -DNDEBUG")
      string(APPEND CMAKE_${lang}_FLAGS_MINSIZEREL_INIT "${_MD} -DNDEBUG"
      )# TODO: Add '-Os' once VS generator maps it properly for Clang
    else()
      string(
        APPEND
        CMAKE_${lang}_FLAGS_INIT
    ...
    set(CMAKE_${lang}_COMPILE_OPTIONS_MSVC_RUNTIME_LIBRARY_MultiThreadedDebugDLL
        -MDd)
    set(CMAKE_${lang}_COMPILE_OPTIONS_MSVC_DEBUG_INFORMATION_FORMAT_Embedded
        -Z7)
    set(CMAKE_${lang}_COMPILE_OPTIONS_MSVC_DEBUG_INFORMATION_FORMAT_ProgramDatabase
        -Zi)
    set(CMAKE_${lang}_COMPILE_OPTIONS_MSVC_DEBUG_INFORMATION_FORMAT_EditAndContinue
        -ZI)
  endif()
  set(CMAKE_${lang}_LINKER_SUPPORTS_PDB ON)

  __windows_compiler_msvc_enable_rc(
    "${_PLATFORM_DEFINES} ${_PLATFORM_DEFINES_${lang}}")
...
macro(__windows_compiler_msvc_enable_rc flags)
  ...
  enable_language(RC)
  if(NOT DEFINED CMAKE_NINJA_CMCLDEPS_RC AND CMAKE_HOST_SYSTEM_NAME STREQUAL
                                             "Windows")
