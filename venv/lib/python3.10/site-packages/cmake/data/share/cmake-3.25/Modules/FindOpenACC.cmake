...
macro(_OPENACC_PREPARE_SOURCE LANG CONTENT_ID NAME_PREFIX FULLNAME_VAR
      CONTENT_VAR)
  if("${LANG}" STREQUAL "C")
...
function(_OPENACC_GET_FLAGS_CANDIDATE LANG FLAG_VAR)
  ...
  if(DEFINED ACC_FLAG_${CMAKE_${LANG}_COMPILER_ID})
    set("${FLAG_VAR}"
        "${ACC_FLAG_${CMAKE_${LANG}_COMPILER_ID}}"
        PARENT_SCOPE)
  else()
...
function(_OPENACC_GET_ACCEL_TARGET_FLAG LANG TARGET FLAG_VAR)
  ...
  if(DEFINED ACC_TARGET_FLAG_${CMAKE_${LANG}_COMPILER_ID})
    set("${FLAG_VAR}"
        "${ACC_TARGET_FLAG_${CMAKE_${LANG}_COMPILER_ID}}=${TARGET}"
        PARENT_SCOPE)
  endif()
endfunction()
...
function(_OPENACC_GET_VERBOSE_FLAG LANG FLAG_VAR)
  ...
  if(DEFINED ACC_VERBOSE_FLAG_${CMAKE_${LANG}_COMPILER_ID})
    set("${FLAG_VAR}"
        "${ACC_VERBOSE_FLAG_${CMAKE_${LANG}_COMPILER_ID}}"
        PARENT_SCOPE)
  endif()
endfunction()
...
function(_OPENACC_GET_FLAGS LANG FLAG_VAR)
  ...
  foreach(FLAG IN LISTS FLAG_CANDIDATES)
    try_compile(
      OpenACC_FLAG_TEST_RESULT SOURCE_FROM_VAR
      "${_OPENACC_TEST_SRC_NAME}" _OPENACC_TEST_SRC_CONTENT
    ...
    if(OpenACC_FLAG_TEST_RESULT)
      ...
      if(DEFINED OpenACC_ACCEL_TARGET)
        _openacc_get_accel_target_flag("${LANG}" "${OpenACC_ACCEL_TARGET}"
                                       TARGET_FLAG)
...
function(_OPENACC_GET_SPEC_DATE LANG SPEC_DATE)
  ...
  set(BIN_FILE
      "${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/FindOpenACC/accver_${LANG}.bin"
  )
  try_compile(
    OpenACC_SPECTEST_${LANG} SOURCE_FROM_VAR
    "${_OPENACC_TEST_SRC_NAME}" _OPENACC_TEST_SRC_CONTENT
...
macro(_OPENACC_SET_VERSION_BY_SPEC_DATE LANG)
  set(OpenACC_SPEC_DATE_MAP
      ...
      "201308=2.0" "201306=2.0" "201111=1.0")

  string(REGEX MATCHALL "${OpenACC_${LANG}_SPEC_DATE}=([0-9]+)\\.([0-9]+)"
               _version_match "${OpenACC_SPEC_DATE_MAP}")
  if(NOT _version_match STREQUAL "")
    set(OpenACC_${LANG}_VERSION_MAJOR ${CMAKE_MATCH_1})
    set(OpenACC_${LANG}_VERSION_MINOR ${CMAKE_MATCH_2})
    set(OpenACC_${LANG}_VERSION
        "${OpenACC_${LANG}_VERSION_MAJOR}.${OpenACC_${LANG}_VERSION_MINOR}")
  else()
...
foreach(LANG IN ITEMS C CXX Fortran)
  if(CMAKE_${LANG}_COMPILER_LOADED)
    ...
    if(NOT DEFINED OpenACC_${LANG}_OPTIONS)
      separate_arguments(OpenACC_${LANG}_OPTIONS NATIVE_COMMAND
                         "${OpenACC_${LANG}_FLAGS}")
    endif()
...
foreach(LANG IN ITEMS C CXX Fortran)
  ...
  if(OpenACC_${LANG}_LIBRARIES)
    set_property(
      TARGET OpenACC::OpenACC_${LANG} PROPERTY INTERFACE_LINK_LIBRARIES
                                               "${OpenACC_${LANG}_LIBRARIES}")
  endif()
  if(OpenACC_${LANG}_FLAGS)
    set_property(
      TARGET OpenACC::OpenACC_${LANG}
      PROPERTY INTERFACE_COMPILE_OPTIONS
               "$<$<COMPILE_LANGUAGE:${LANG}>:${OpenACC_${LANG}_OPTIONS}>")
    set_property(
      TARGET OpenACC::OpenACC_${LANG}
