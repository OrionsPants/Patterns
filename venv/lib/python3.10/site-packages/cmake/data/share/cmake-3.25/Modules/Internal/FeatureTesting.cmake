macro(_record_compiler_features lang compile_flags feature_list)
  include(
    "${CMAKE_ROOT}/Modules/Compiler/${CMAKE_${lang}_COMPILER_ID}-${lang}-FeatureTests.cmake"
    OPTIONAL)
  ...
  if(CMAKE_${lang}_LINK_WITH_STANDARD_COMPILE_OPTION)
    # This toolchain requires use of the language standard flag when linking in
    # order to use the matching standard library.
  ...
  try_compile(
    CMAKE_${lang}_FEATURE_TEST SOURCE_FROM_VAR
    "feature_tests.${lang_lc}" _content
  ...
  if(_result EQUAL 0)
    ...
    if(EXISTS "${CMAKE_BINARY_DIR}/CMakeFiles/feature_tests.bin")
      file(STRINGS "${CMAKE_BINARY_DIR}/CMakeFiles/feature_tests.bin" features
           REGEX "${lang}_FEATURE:.*")
...
macro(_record_compiler_features_c std)
  ...
  get_property(lang_level_has_features GLOBAL
               PROPERTY CMAKE_C${std}_KNOWN_FEATURES)
...
macro(_record_compiler_features_cxx std)
  ...
  get_property(lang_level_has_features GLOBAL
               PROPERTY CMAKE_CXX${std}_KNOWN_FEATURES)
  if(lang_level_has_features)
...
macro(_record_compiler_features_cuda std)
  ...
  get_property(lang_level_has_features GLOBAL
               PROPERTY CMAKE_CUDA${std}_KNOWN_FEATURES)
  if(lang_level_has_features)
    _record_compiler_features(
      CUDA "${CMAKE_CUDA${std}_STANDARD_COMPILE_OPTION}"
      CMAKE_CUDA${std}_COMPILE_FEATURES)
  endif()
  unset(lang_level_has_features)
endmacro()
...
macro(_record_compiler_features_hip std)
  ...
  get_property(lang_level_has_features GLOBAL
               PROPERTY CMAKE_HIP${std}_KNOWN_FEATURES)
  if(lang_level_has_features)
    _record_compiler_features(HIP "${CMAKE_HIP${std}_STANDARD_COMPILE_OPTION}"
                              CMAKE_HIP${std}_COMPILE_FEATURES)
  endif()
  unset(lang_level_has_features)
endmacro()
...
macro(_has_compiler_features lang level compile_flags feature_list)
  ...
  get_property(known_features GLOBAL
               PROPERTY CMAKE_${lang}${level}_KNOWN_FEATURES)
...
macro(_has_compiler_features_cxx std)
  ...
  _has_compiler_features(
    CXX ${std} "${CMAKE_CXX${std}_STANDARD_COMPILE_OPTION}"
    CMAKE_CXX${std}_COMPILE_FEATURES)
endmacro()
macro(_has_compiler_features_cuda std)
  list(APPEND CMAKE_CUDA${std}_COMPILE_FEATURES cuda_std_${std})
  _has_compiler_features(
    CUDA ${std} "${CMAKE_CUDA${std}_STANDARD_COMPILE_OPTION}"
    CMAKE_CUDA${std}_COMPILE_FEATURES)
endmacro()
macro(_has_compiler_features_hip std)
  list(APPEND CMAKE_HIP${std}_COMPILE_FEATURES hip_std_${std})
  _has_compiler_features(
    HIP ${std} "${CMAKE_HIP${std}_STANDARD_COMPILE_OPTION}"
    CMAKE_HIP${std}_COMPILE_FEATURES)
endmacro()
