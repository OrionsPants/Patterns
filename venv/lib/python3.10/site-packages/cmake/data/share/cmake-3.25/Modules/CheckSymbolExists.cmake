...
macro(CHECK_SYMBOL_EXISTS SYMBOL FILES VARIABLE)
  if(CMAKE_C_COMPILER_LOADED)
    ...
    __check_symbol_exists_impl(CheckSymbolExists.c "${SYMBOL}" "${FILES}"
                               "${VARIABLE}")
    __check_symbol_exists_restore_flags(C)
  elseif(CMAKE_CXX_COMPILER_LOADED)
    __check_symbol_exists_filter_flags(CXX)
    __check_symbol_exists_impl(CheckSymbolExists.cxx "${SYMBOL}" "${FILES}"
                               "${VARIABLE}")
    __check_symbol_exists_restore_flags(CXX)
  else()
    message(
      FATAL_ERROR "CHECK_SYMBOL_EXISTS needs either C or CXX language enabled")
  endif()
endmacro()

macro(__CHECK_SYMBOL_EXISTS_FILTER_FLAGS LANG)
  set(__CMAKE_${LANG}_FLAGS_SAVED "${CMAKE_${LANG}_FLAGS}")
  string(REGEX REPLACE "(^| )-Werror([= ][^ ]*)?( |$)" " " CMAKE_${LANG}_FLAGS
                       "${CMAKE_${LANG}_FLAGS}")
  string(REGEX REPLACE "(^| )-pedantic-errors( |$)" " " CMAKE_${LANG}_FLAGS
                       "${CMAKE_${LANG}_FLAGS}")
endmacro()
...
macro(__CHECK_SYMBOL_EXISTS_IMPL SOURCEFILE SYMBOL FILES VARIABLE)
  if(NOT DEFINED "${VARIABLE}" OR "x${${VARIABLE}}" STREQUAL "x${VARIABLE}")
    ...
    if(CMAKE_REQUIRED_LINK_OPTIONS)
      set(CHECK_SYMBOL_EXISTS_LINK_OPTIONS LINK_OPTIONS
                                           ${CMAKE_REQUIRED_LINK_OPTIONS})
    else()
      set(CHECK_SYMBOL_EXISTS_LINK_OPTIONS)
    endif()
    if(CMAKE_REQUIRED_LIBRARIES)
      set(CHECK_SYMBOL_EXISTS_LIBS LINK_LIBRARIES ${CMAKE_REQUIRED_LIBRARIES})
    else()
    ...
    foreach(FILE ${FILES})
      string(APPEND _CSE_SOURCE "#include <${FILE}>\n")
    endforeach()
    string(
      APPEND
      _CSE_SOURCE
...
int main(int argc, char** argv)
{
  ...
  ${_CSE_CHECK_NON_MACRO}")
    ...
    try_compile(
      ...
      "${SOURCEFILE}" _CSE_SOURCE
      COMPILE_DEFINITIONS
        ${CMAKE_REQUIRED_DEFINITIONS} ${CHECK_SYMBOL_EXISTS_LINK_OPTIONS}
