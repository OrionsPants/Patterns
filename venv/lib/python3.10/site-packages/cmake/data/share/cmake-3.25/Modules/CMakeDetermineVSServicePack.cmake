...
if(NOT CMAKE_MINIMUM_REQUIRED_VERSION VERSION_LESS 2.8.8)
  message(DEPRECATION "This module is deprecated and should not be used.  "
                      "Use the CMAKE_<LANG>_COMPILER_VERSION variable instead.")
endif()

# [INTERNAL] Please do not call this function directly
...
# ##############################################################################
# [INTERNAL] Please do not call this function directly
function(_DetermineVSServicePack_FastCheckVersionWithCompiler _SUCCESS_VAR
         _VERSION_VAR)
  if(EXISTS ${CMAKE_CXX_COMPILER})
    ...
    if(_output MATCHES
       "Compiler Version (([0-9]+)\\.([0-9]+)\\.([0-9]+)(\\.([0-9]+))?)")
...
# ##############################################################################
# [INTERNAL] Please do not call this function directly
function(_DetermineVSServicePack_CheckVersionWithTryCompile _SUCCESS_VAR
         _VERSION_VAR)
  file(WRITE "${CMAKE_BINARY_DIR}/return0.cc" "int main() { return 0; }\n")

  try_compile(
    _CompileResult SOURCES
...
# ##############################################################################
# [INTERNAL] Please do not call this function directly
function(_DetermineVSServicePack_CheckVersionWithTryRun _SUCCESS_VAR
         _VERSION_VAR)
  file(
    WRITE "${CMAKE_BINARY_DIR}/return0.cc"
  ...
  try_run(_RunResult _CompileResult SOURCES "${CMAKE_BINARY_DIR}/return0.cc"
          RUN_OUTPUT_VARIABLE _runoutput)

  file(REMOVE "${CMAKE_BINARY_DIR}/return0.cc")

  string(REGEX MATCH "[0-9]+.[0-9]+.[0-9]+.[0-9]+" _cl_version "${_runoutput}")
...
#
# A function to call to determine the Visual Studio service pack in use.  See
# documentation above.
function(DetermineVSServicePack _pack)
  if(NOT DETERMINED_VS_SERVICE_PACK OR NOT ${_pack})
    ...
    _determinevsservicepack_fastcheckversionwithcompiler(
      DETERMINED_VS_SERVICE_PACK _cl_version)
    if(NOT DETERMINED_VS_SERVICE_PACK)
      _determinevsservicepack_checkversionwithtrycompile(
        DETERMINED_VS_SERVICE_PACK _cl_version)
      if(NOT DETERMINED_VS_SERVICE_PACK)
        _determinevsservicepack_checkversionwithtryrun(
          DETERMINED_VS_SERVICE_PACK _cl_version)
      endif()
    endif()
    ...
    if(DETERMINED_VS_SERVICE_PACK)
      ...
      if(_cl_version)
        ...
        if(_sp)
          set(${_pack}
              ${_sp}
              CACHE INTERNAL "The Visual Studio Release with Service Pack")
        endif()
      endif()
    endif()
  endif()
