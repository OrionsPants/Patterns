...
function(CMAKE_EXPAND_IMPORTED_TARGETS _RESULT)
  ...
  cmake_parse_arguments(CEIT "${options}" "${oneValueArgs}" "${multiValueArgs}"
                        ${ARGN})

  if(CEIT_UNPARSED_ARGUMENTS)
    message(
      FATAL_ERROR
        "Unknown keywords given to CMAKE_EXPAND_IMPORTED_TARGETS(): \"${CEIT_UNPARSED_ARGUMENTS}\""
    )
  endif()

  if(NOT CEIT_CONFIGURATION)
    # Would be better to test GENERATOR_IS_MULTI_CONFIG global property, but the
    # documented behavior specifically says we check CMAKE_CONFIGURATION_TYPES
    # and fall back to CMAKE_BUILD_TYPE if no config types are defined.
  ...
  while(_CHECK_FOR_IMPORTED_TARGETS)
    ...
    foreach(_CURRENT_LIB ${_CCSR_REQ_LIBS})
      ...
      if(_importedConfigs)
        # message(STATUS "Detected imported target ${_CURRENT_LIB}") Ok, so this
        # is an imported target. First we get the imported configurations. Then
        # we get the location of the actual library on disk of the first
        # configuration. then we'll get its link interface libraries property,
        # iterate through it and replace all imported targets we find there with
        # there actual location.

        # guard against infinite loop: abort after 100 iterations ( 100 is
        # arbitrary chosen)
        if("${_CCSR_LOOP_COUNTER}" LESS 100)
          set(_CHECK_FOR_IMPORTED_TARGETS TRUE)
          # else () message(STATUS "********* aborting loop, counter :
          # ${_CCSR_LOOP_COUNTER}")
        endif()

        # if one of the imported configurations equals
        # ${CMAKE_TRY_COMPILE_CONFIGURATION}, use it, otherwise simply use the
        # first one:
        ...
        get_target_property(
          _linkInterfaceLibs "${_CURRENT_LIB}"
  ...
  # Finally we iterate once more over all libraries. This loop only removes all
  # remaining imported target names (there shouldn't be any left anyway).
  set(_CCSR_NEW_REQ_LIBS)
  foreach(_CURRENT_LIB ${_CCSR_REQ_LIBS})
    if(TARGET "${_CURRENT_LIB}")
      get_target_property(_importedConfigs "${_CURRENT_LIB}"
    ...
    if(NOT _importedConfigs)
      list(APPEND _CCSR_NEW_REQ_LIBS "${_CURRENT_LIB}")
      # message(STATUS "final: appending ${_CURRENT_LIB}") else ()
