...
# Detect the Fortran/C interface on the first run or when the configuration
# changes.
if(NOT EXISTS ${FortranCInterface_BINARY_DIR}/Output.cmake
   OR NOT EXISTS ${FortranCInterface_BINARY_DIR}/Input.cmake
   OR NOT ${FortranCInterface_BINARY_DIR}/Output.cmake IS_NEWER_THAN
      ${FortranCInterface_BINARY_DIR}/Input.cmake
   OR NOT ${FortranCInterface_SOURCE_DIR}/Output.cmake IS_NEWER_THAN
      ${FortranCInterface_SOURCE_DIR}/Output.cmake.in
   OR NOT ${FortranCInterface_BINARY_DIR}/Output.cmake IS_NEWER_THAN
      ${FortranCInterface_SOURCE_DIR}/CMakeLists.txt
   OR NOT ${FortranCInterface_BINARY_DIR}/Output.cmake IS_NEWER_THAN
...
# Perform detection with only one architecture so that the info strings are not
# repeated.
...
try_compile(
  FortranCInterface_COMPILED PROJECT
  FortranCInterface
  TARGET
  FortranCInterface
  SOURCE_DIR
...
if(FortranCInterface_EXE)
  file(
    STRINGS "${FortranCInterface_EXE}" _info_strings
    LIMIT_COUNT 8
...
set(_module_regex
    "^(_*)([A-Za-z$]*)(mymodule|MYMODULE)([A-Za-z_$]*)(mysub|MYSUB)([_$]*)$")
set(_module__regex
    "^(_*)([A-Za-z$]*)(my_module|MY_MODULE)([A-Za-z_$]*)(my_sub|MY_SUB)([_$]*)$"
)
...
foreach(symbol ${FortranCInterface_SYMBOLS})
  foreach(form "" "_")
    ...
    string(REGEX REPLACE "${_global_${form}regex}" "\\1;\\2;\\3" pieces
                         "${symbol}")
    ...
    string(REGEX REPLACE "${_module_${form}regex}" "\\1\\2;\\3;\\4;\\5;\\6"
                         pieces "${symbol}")
    list(LENGTH pieces len)
    if(len EQUAL 5)
...
foreach(scope GLOBAL MODULE)
  if(FortranCInterface_${scope}_SYMBOL AND FortranCInterface_${scope}__SYMBOL)
