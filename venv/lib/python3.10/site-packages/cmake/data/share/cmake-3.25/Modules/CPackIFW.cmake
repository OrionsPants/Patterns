...
# TODO: All of the internal implementation CMake modules for other CPack
# generators have been moved into the Internal/CPack directory. This one has
# not, because it contains user-facing macros which would be lost if it were
# moved. At some point, this module should be split into user-facing macros
# (which would live in this module) and internal implementation details (which
# would live in Internal/CPack/CPackIFW.cmake).
...
foreach(_CPACK_IFW_PATH_VAR "CPACK_IFW_ROOT" "QTIFWDIR" "QTDIR")
  if(DEFINED ${_CPACK_IFW_PATH_VAR} AND NOT "${${_CPACK_IFW_PATH_VAR}}"
                                        STREQUAL "")
...
endforeach()
if(WIN32)
  list(APPEND _CPACK_IFW_PATHS "$ENV{HOMEDRIVE}/Qt" "C:/Qt")
else()
  list(APPEND _CPACK_IFW_PATHS "$ENV{HOME}/Qt" "/opt/Qt")
endif()
...
foreach(_CPACK_IFW_PREFIX ${_CPACK_IFW_PREFIXES})
  foreach(_CPACK_IFW_VERSION ${_CPACK_IFW_VERSIONS})
    list(APPEND _CPACK_IFW_SUFFIXES
         "${_CPACK_IFW_PREFIX}${_CPACK_IFW_VERSION}/bin")
  endforeach()
endforeach()
...
if(NOT CPackIFW_CMake_INCLUDED)
  ...
  set(CPACK_IFW_FRAMEWORK_VERSION_TIMEOUT
      1
      CACHE
        STRING
        "The timeout to return QtIFW framework version string from \"installerbase\" executable"
  )
  mark_as_advanced(CPACK_IFW_FRAMEWORK_VERSION_TIMEOUT)
  if(CPACK_IFW_INSTALLERBASE_EXECUTABLE AND NOT
    ...
    foreach(_ifw_version_argument --version --framework-version)
      if(NOT CPACK_IFW_FRAMEWORK_VERSION)
        execute_process(
          COMMAND "${CPACK_IFW_INSTALLERBASE_EXECUTABLE}"
          ...
          OUTPUT_STRIP_TRAILING_WHITESPACE ENCODING UTF8)
        if(NOT CPACK_IFW_FRAMEWORK_VERSION_RESULT
           AND CPACK_IFW_FRAMEWORK_VERSION_OUTPUT)
          ...
          if(CPACK_IFW_FRAMEWORK_VERSION)
            if("${_ifw_version_argument}" STREQUAL "--framework-version")
              set(CPACK_IFW_FRAMEWORK_VERSION_SOURCE
                  "INSTALLERBASE_FRAMEWORK_VERSION")
  ...
  if(CPACK_IFW_VERBOSE)
    if(CPACK_IFW_FRAMEWORK_VERSION AND CPACK_IFW_FRAMEWORK_VERSION_FORCED)
      message(
        STATUS "Found QtIFW ${CPACK_IFW_FRAMEWORK_VERSION} (forced) version")
  ...
  if(CPACK_IFW_INSTALLERBASE_EXECUTABLE AND NOT CPACK_IFW_FRAMEWORK_VERSION)
    message(
      WARNING
        "Could not detect QtIFW tools version. Set used version to variable \"CPACK_IFW_FRAMEWORK_VERSION_FORCED\" manually."
    )
  endif()
  ...
  macro(_cpack_ifw_resolve_script _variable)
    ...
    if(DEFINED ${_ifw_script_macro})
      ...
      if(NOT EXISTS ${_ifw_script_file})
        message(
          WARNING
            "CPack IFW: script file \"${_ifw_script_file}\" does not exist")
    ...
    if(${_variable})
      ...
      foreach(_ifw_licenses_arg ${${_variable}})
        if(_ifw_license_file)
          get_filename_component(_ifw_licenses_arg "${_ifw_licenses_arg}"
                                 ABSOLUTE)
          set(_ifw_license_file FALSE)
        else()
    ...
    if(${_variable})
      ...
      foreach(_ifw_file_arg ${${_variable}})
        ...
        if(EXISTS ${_ifw_file_arg})
          ...
          message(
            WARNING
              "CPack IFW: page file \"${_ifw_file_arg}\" does not exist. Skipping"
          )
        endif()
  ...
  macro(cpack_ifw_configure_component compname)
    ...
    cmake_parse_arguments(
      CPACK_IFW_COMPONENT_${_CPACK_IFWCOMP_UNAME} "${_IFW_OPT}" "${_IFW_ARGS}"
      "${_IFW_MULTI_ARGS}" ${ARGN})

    _cpack_ifw_resolve_script(
      CPACK_IFW_COMPONENT_${_CPACK_IFWCOMP_UNAME}_SCRIPT)
    _cpack_ifw_resolve_lisenses(
      CPACK_IFW_COMPONENT_${_CPACK_IFWCOMP_UNAME}_LICENSES)
    _cpack_ifw_resolve_file_list(
      CPACK_IFW_COMPONENT_${_CPACK_IFWCOMP_UNAME}_USER_INTERFACES)
    _cpack_ifw_resolve_file_list(
      CPACK_IFW_COMPONENT_${_CPACK_IFWCOMP_UNAME}_TRANSLATIONS)
  ...
  macro(cpack_ifw_configure_component_group grpname)
    ...
    set(_IFW_MULTI_ARGS
        ...
        REPLACES)
    cmake_parse_arguments(
      CPACK_IFW_COMPONENT_GROUP_${_CPACK_IFWGRP_UNAME} "${_IFW_OPT}"
      "${_IFW_ARGS}" "${_IFW_MULTI_ARGS}" ${ARGN})

    _cpack_ifw_resolve_script(
      CPACK_IFW_COMPONENT_GROUP_${_CPACK_IFWGRP_UNAME}_SCRIPT)
    _cpack_ifw_resolve_lisenses(
      CPACK_IFW_COMPONENT_GROUP_${_CPACK_IFWGRP_UNAME}_LICENSES)
    _cpack_ifw_resolve_file_list(
      CPACK_IFW_COMPONENT_GROUP_${_CPACK_IFWGRP_UNAME}_USER_INTERFACES)
    _cpack_ifw_resolve_file_list(
      CPACK_IFW_COMPONENT_GROUP_${_CPACK_IFWGRP_UNAME}_TRANSLATIONS)
  ...
  macro(cpack_ifw_add_repository reponame)
    ...
    set(_IFW_MULTI_ARGS)
    cmake_parse_arguments(
      CPACK_IFW_REPOSITORY_${_CPACK_IFWREPO_UNAME} "${_IFW_OPT}" "${_IFW_ARGS}"
      "${_IFW_MULTI_ARGS}" ${ARGN})
    ...
    string(APPEND _CPACK_IFWREPO_STR
           "list(APPEND CPACK_IFW_REPOSITORIES_ALL ${reponame})\n")
