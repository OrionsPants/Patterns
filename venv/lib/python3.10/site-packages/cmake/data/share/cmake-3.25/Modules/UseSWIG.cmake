...
set(SWIG_MANAGE_SUPPORT_FILES_SCRIPT
    "${CMAKE_CURRENT_LIST_DIR}/UseSWIG/ManageSupportFiles.cmake")
...
function(__SWIG_COMPUTE_TIMESTAMP name language infile workingdir __timestamp)
  ...
  set(${__timestamp}
      "${workingdir}/${filename}${language}.stamp"
      PARENT_SCOPE)
  # get_filename_component(filename "${infile}" ABSOLUTE) string(UUID uuid
  # NAMESPACE 9735D882-D2F8-4E1D-88C9-A0A4F1F6ECA4 NAME
  # ${name}-${language}-${filename} TYPE SHA1) set(${__timestamp}
  # "${workingdir}/${uuid}.stamp" PARENT_SCOPE)
endfunction()
...
macro(SWIG_MODULE_INITIALIZE name language)
  ...
  if(SWIG_MODULE_${name}_NOPROXY
     AND NOT ("-noproxy" IN_LIST CMAKE_SWIG_FLAGS
              OR "-noproxy" IN_LIST SWIG_MODULE_${name}_EXTRA_FLAGS))
...
function(SWIG_GET_EXTRA_OUTPUT_FILES language outfiles generatedpath infile)
  set(files)
  get_source_file_property(module_basename "${infile}" SWIG_MODULE_NAME)
  if(NOT module_basename)

    # try to get module name from "%module foo" syntax
    if(EXISTS "${infile}")
      file(STRINGS "${infile}" module_basename
           REGEX "[ ]*%module[ ]*[a-zA-Z0-9_]+.*")
    endif()
    if(module_basename)
      string(REGEX REPLACE "[ ]*%module[ ]*([a-zA-Z0-9_]+).*" "\\1"
      ...
      if(module_basename)
        string(REGEX REPLACE "[ ]*%module[ ]*\\(.*\\)[ ]*([a-zA-Z0-9_]+).*"
                             "\\1" module_basename "${module_basename}")
...
function(SWIG_ADD_SOURCE_TO_MODULE name outfiles infile)
  ...
  if(CMAKE_GENERATOR MATCHES
     "Make|Ninja|Xcode|Visual Studio (1[1-9]|[2-9][0-9])")
    get_property(
      use_swig_dependencies_set
      SOURCE "${infile}"
      PROPERTY USE_SWIG_DEPENDENCIES
      SET)
  ...
  list(
    APPEND
    swig_source_file_flags
    "$<TARGET_GENEX_EVAL:${target_name},$<TARGET_PROPERTY:${target_name},SWIG_COMPILE_OPTIONS>>"
  )
  ...
  if(NOT SWIG_MODULE_${name}_NOPROXY)
    ...
    swig_get_extra_output_files(
      ${SWIG_MODULE_${name}_LANGUAGE} swig_extra_generated_files "${outdir}"
      "${swig_source_file_fullname}")
  endif()
  set(swig_generated_file_fullname "${outfiledir}/${swig_source_file_name_we}")
  # add the language into the name of the file (i.e. TCL_wrap) this allows for
  # the same .i file to be wrapped into different languages
  ...
  if(cmake_include_directories)
    set(swig_include_dirs
        "$<$<BOOL:${cmake_include_directories}>:-I$<JOIN:${cmake_include_directories},$<SEMICOLON>-I>>"
    )
  endif()
  ...
  if(SWIG_MODULE_${name}_LANGUAGE STREQUAL "CSHARP")
    if(NOT ("-dllimport" IN_LIST swig_source_file_flags
            OR "-dllimport" IN_LIST SWIG_MODULE_${name}_EXTRA_FLAGS))
      # This makes sure that the name used in the generated DllImport matches
      # the library name created by CMake
      list(
        APPEND
        SWIG_MODULE_${name}_EXTRA_FLAGS
        "-dllimport"
        "$<TARGET_FILE_PREFIX:${target_name}>$<TARGET_FILE_BASE_NAME:${target_name}>"
      )
    endif()
  endif()
  if(SWIG_MODULE_${name}_LANGUAGE STREQUAL "PYTHON"
     AND NOT SWIG_MODULE_${name}_NOPROXY)
    if(SWIG_USE_INTERFACE
       AND NOT ("-interface" IN_LIST swig_source_file_flags
                OR "-interface" IN_LIST SWIG_MODULE_${name}_EXTRA_FLAGS))
      # This makes sure that the name used in the proxy code matches the library
      # name created by CMake
      list(
        APPEND
        SWIG_MODULE_${name}_EXTRA_FLAGS
        "-interface"
  ...
  # dependencies
  set(swig_dependencies DEPENDS ${SWIG_MODULE_${name}_EXTRA_DEPS}
                        $<TARGET_PROPERTY:${target_name},SWIG_DEPENDS>)
  ...
  if(UseSWIG_MODULE_VERSION VERSION_GREATER 1)
    # as part of custom command, start by removing old generated files to ensure
    # obsolete files do not stay
    set(swig_file_outdir "${workingdir}/${swig_source_file_name_we}.files")
    set(swig_cleanup_command
        COMMAND "${CMAKE_COMMAND}"
        "-DSUPPORT_FILES_WORKING_DIRECTORY=${swig_file_outdir}"
        "-DSUPPORT_FILES_OUTPUT_DIRECTORY=${outdir}" -DACTION=CLEAN -P
        "${SWIG_MANAGE_SUPPORT_FILES_SCRIPT}")
    set(swig_copy_command
        COMMAND "${CMAKE_COMMAND}"
        "-DSUPPORT_FILES_WORKING_DIRECTORY=${swig_file_outdir}"
        "-DSUPPORT_FILES_OUTPUT_DIRECTORY=${outdir}" -DACTION=COPY -P
        "${SWIG_MANAGE_SUPPORT_FILES_SCRIPT}")
  else()
  ...
  if(NOT use_swig_dependencies AND CMAKE_GENERATOR MATCHES "Make")
    ...
    # removed. We need an extra step with timestamp and custom target, see
    # #16830 As this is needed only for Makefile generator do it conditionally
    __swig_compute_timestamp(
      ${name} ${SWIG_MODULE_${name}_LANGUAGE} "${infile}" "${workingdir}"
      swig_generated_timestamp)
    set(swig_custom_output "${swig_generated_timestamp}")
    set(swig_custom_products BYPRODUCTS "${swig_generated_file_fullname}"
                             ${swig_extra_generated_files})
    set(swig_timestamp_command COMMAND ${CMAKE_COMMAND} -E touch
                               "${swig_generated_timestamp}")
    list(APPEND swig_dependencies IMPLICIT_DEPENDS CXX
         "${swig_source_file_fullname}")
  else()
    set(swig_generated_timestamp)
    set(swig_custom_output "${swig_generated_file_fullname}"
                           ${swig_extra_generated_files})
  ...
  add_custom_command(
    OUTPUT ${swig_custom_output} ${swig_custom_products} ${swig_cleanup_command}
    # Let's create the ${outdir} at execution time, in case dir contains
    # $(OutDir)
    COMMAND "${CMAKE_COMMAND}" -E make_directory "${workingdir}" "${outdir}"
            "${outfiledir}" ${swig_timestamp_command}
    COMMAND
      "${CMAKE_COMMAND}" -E env "SWIG_LIB=${SWIG_DIR}" "${SWIG_EXECUTABLE}"
      "-${SWIG_MODULE_${name}_SWIG_LANGUAGE_FLAG}" "${swig_source_file_flags}"
      -outdir "${swig_file_outdir}" ${swig_special_flags} ${swig_extra_flags}
      ${swig_depends_flags} "${swig_include_dirs}" -o
      "${swig_generated_file_fullname}" "${swig_source_file_fullname}"
      ${swig_copy_command}
    MAIN_DEPENDENCY "${swig_source_file_fullname}"
    ${swig_dependencies}
    COMMENT
      "Swig compile ${infile} for ${SWIG_MODULE_${name}_SWIG_LANGUAGE_FLAG}"
  ...
  if(SWIG_MODULE_${name}_SWIG_LANGUAGE_FLAG MATCHES "php")
    set_property(
      SOURCE "${swig_generated_file_fullname}"
      APPEND
      PROPERTY INCLUDE_DIRECTORIES "${outdir}")
  endif()
...
macro(SWIG_ADD_MODULE name language)
  message(
    DEPRECATION "SWIG_ADD_MODULE is deprecated. Use SWIG_ADD_LIBRARY instead.")
  swig_add_library(
    ${name}
    LANGUAGE ${language}
    TYPE MODULE
...
function(SWIG_ADD_LIBRARY name)
  ...
  set(oneValueArgs LANGUAGE TYPE OUTPUT_DIR OUTFILE_DIR)
  set(multiValueArgs SOURCES)
  cmake_parse_arguments(_SAM "${options}" "${oneValueArgs}" "${multiValueArgs}"
                        ${ARGN})

  if(_SAM_UNPARSED_ARGUMENTS)
    message(
      FATAL_ERROR
        "SWIG_ADD_LIBRARY: ${_SAM_UNPARSED_ARGUMENTS}: unexpected arguments")
  endif()
  ...
  if(target_name_policy STREQUAL "NEW")
    ...
    elseif(NOT UseSWIG_TARGET_NAME_PREFERENCE MATCHES "^(LEGACY|STANDARD)$")
      message(
        FATAL_ERROR
          "UseSWIG_TARGET_NAME_PREFERENCE: ${UseSWIG_TARGET_NAME_PREFERENCE}: invalid value. 'LEGACY' or 'STANDARD' is expected."
      )
    endif()
  endif()
  ...
  elseif(NOT UseSWIG_MODULE_VERSION MATCHES "^(1|2)$")
    message(
      FATAL_ERROR
        "UseSWIG_MODULE_VERSION: ${UseSWIG_MODULE_VERSION}: invalid value. 1 or 2 is expected."
    )
  endif()
  ...
  if(UseSWIG_TARGET_NAME_PREFERENCE STREQUAL "LEGACY"
     AND SWIG_MODULE_${name}_LANGUAGE STREQUAL "PYTHON"
     AND NOT SWIG_MODULE_${name}_NOPROXY)
    # swig will produce a module.py containing an 'import _modulename'
    # statement, which implies having a corresponding _modulename.so (*NIX),
    # _modulename.pyd (Win32), unless the -noproxy flag is used
  ...
  if(TARGET ${target_name})
    # a target with same name is already defined. call NOW add_library command
    # to raise the most useful error message
    add_library(${target_name})
    return()
  endif()

  set(workingdir
      "${CMAKE_CURRENT_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/${target_name}.dir")
  # set special variable to pass extra information to command
  # SWIG_ADD_SOURCE_TO_MODULE which cannot be changed due to legacy
  # compatibility
  ...
  # set again, locally, predefined variables to ensure compatibility with
  # command SWIG_ADD_SOURCE_TO_MODULE
  ...
  if(CMAKE_GENERATOR MATCHES
     "Make|Ninja|Xcode|Visual Studio (1[1-9]|[2-9][0-9])")
    # For Makefiles, Ninja, Xcode and Visual Studio generators, use SWIG
    # generated dependencies if requested
  ...
  string(REGEX REPLACE "([$^.*+?|()-])" "\\\\\\1" swig_source_ext_regex
                       "${SWIG_SOURCE_FILE_EXTENSIONS}")
  ...
  set_property(
    DIRECTORY
    APPEND
    PROPERTY ADDITIONAL_CLEAN_FILES ${swig_generated_sources}
             ${swig_generated_timestamps})
  if(UseSWIG_MODULE_VERSION VERSION_GREATER 1)
    set_property(
      DIRECTORY
      APPEND
      PROPERTY ADDITIONAL_CLEAN_FILES ${swig_generated_outdirs})
  endif()

  add_library(${target_name} ${_SAM_TYPE} ${swig_generated_sources}
                             ${swig_other_sources})
  if(swig_generated_timestamps)
    # see IMPLICIT_DEPENDS above
    add_custom_target(${name}_swig_compilation
                      DEPENDS ${swig_generated_timestamps})
  ...
  elseif(swig_lowercase_language STREQUAL "java")
    # In java you want: System.loadLibrary("LIBRARY"); then JNI will look for a
    # library whose name is platform dependent, namely MacOS  :
    # libLIBRARY.jnilib Windows: LIBRARY.dll Linux  : libLIBRARY.so
    if(APPLE)
  ...
  elseif(swig_lowercase_language STREQUAL "python")
    ...
    # Python extension modules on Windows must have the extension ".pyd" instead
    # of ".dll" as of Python 2.5.  Older python versions do support this suffix.
    # http://docs.python.org/whatsnew/ports.html#SECTION0001510000000000000000
    # <quote> Windows: .dll is no longer supported as a filename extension for
  ...
  elseif(swig_lowercase_language STREQUAL "ruby")
    # In ruby you want: require 'LIBRARY' then ruby will look for a library
    # whose name is platform dependent, namely MacOS  : LIBRARY.bundle Windows:
    # LIBRARY.dll Linux  : LIBRARY.so
    set_target_properties(${target_name} PROPERTIES PREFIX "")
    if(APPLE)
  ...
  # target property SWIG_SUPPORT_FILES_DIRECTORY specify output directories of
  # support files
  set_property(TARGET ${target_name} PROPERTY SWIG_SUPPORT_FILES_DIRECTORY
                                              ${swig_generated_outdirs})
  ...
  if(NOT SWIG_MODULE_${name}_NOPROXY)
    ...
    foreach(swig_it IN
            LISTS SWIG_${swig_uppercase_language}_EXTRA_FILE_EXTENSIONS)
    ...
    set_property(TARGET ${target_name} PROPERTY SWIG_SUPPORT_FILES
                                                ${swig_all_support_files})
  endif()

  # to ensure legacy behavior, export some variables
  set(SWIG_MODULE_${name}_LANGUAGE
      "${SWIG_MODULE_${name}_LANGUAGE}"
      PARENT_SCOPE)
  set(SWIG_MODULE_${name}_SWIG_LANGUAGE_FLAG
      "${SWIG_MODULE_${name}_SWIG_LANGUAGE_FLAG}"
      PARENT_SCOPE)
  set(SWIG_MODULE_${name}_REAL_NAME
      "${target_name}"
      PARENT_SCOPE)
  set(SWIG_MODULE_${name}_NOPROXY
      "${SWIG_MODULE_${name}_NOPROXY}"
      PARENT_SCOPE)
  set(SWIG_MODULE_${name}_EXTRA_FLAGS
      "${SWIG_MODULE_${name}_EXTRA_FLAGS}"
      PARENT_SCOPE)
  # the last one is a bit crazy but it is documented, so... NOTA: works as
  # expected if only ONE input file is specified
  set(swig_generated_file_fullname
      "${swig_generated_file_fullname}"
      PARENT_SCOPE)
endfunction()
