...
# Backward compatibility Include CPackComponent macros if it has not already
# been included before.
include(CPackComponent)

# Macro for setting values if a user did not overwrite them Mangles
# CMake-special characters. Only kept for backwards compatibility.
...
# file find any variable that starts with CPACK and create a variable
# _CPACK_OTHER_VARIABLES_ that contains SET commands for each cpack variable.
# _CPACK_OTHER_VARIABLES_ is then used as an @ replacement in configure_file for
# the CPackConfig.
...
if(CMAKE_PROJECT_HOMEPAGE_URL)
  _cpack_set_default(CPACK_PACKAGE_HOMEPAGE_URL "${CMAKE_PROJECT_HOMEPAGE_URL}")
endif()
...
# Root dir: default value should be the string literal "$PROGRAMFILES" for
# backwards compatibility. Projects may set this value to anything. When
# creating 64 bit binaries we set the default value to "$PROGRAMFILES64"
...
macro(cpack_check_file_exists file description)
  if(NOT EXISTS "${file}")
    message(
      SEND_ERROR "CPack ${description} file: \"${file}\" could not be found.")
  endif()
endmacro()

cpack_check_file_exists("${CPACK_PACKAGE_DESCRIPTION_FILE}"
                        "package description")
...
# Provide options to choose generators we might check here if the required tools
# for the generators exist and set the defaults according to the results.
  ...
  if(UNIX)
    if(CYGWIN)
      ...
      if(APPLE)
        ...
        option(CPACK_BINARY_DRAGNDROP
               "Enable to build OSX Drag And Drop package" OFF)
        option(CPACK_BINARY_PRODUCTBUILD
               "Enable to build productbuild packages" OFF)
        mark_as_advanced(CPACK_BINARY_BUNDLE CPACK_BINARY_DRAGNDROP
    ...
    mark_as_advanced(CPACK_BINARY_7Z CPACK_BINARY_NSIS CPACK_BINARY_NUGET
                     CPACK_BINARY_WIX CPACK_BINARY_ZIP)
  endif()
...
if(NOT CPACK_SOURCE_GENERATOR)
  if(UNIX)
    if(CYGWIN)
      ...
      mark_as_advanced(CPACK_SOURCE_RPM CPACK_SOURCE_TBZ2 CPACK_SOURCE_TGZ
                       CPACK_SOURCE_TXZ CPACK_SOURCE_TZ CPACK_SOURCE_ZIP)
    endif()
  else()
    ...
    mark_as_advanced(CPACK_SOURCE_7Z CPACK_SOURCE_ZIP)
  endif()
...
# if the user has set CPACK_NSIS_DISPLAY explicitly, then use that as the
# default value of CPACK_NSIS_PACKAGE_NAME  instead of
# CPACK_PACKAGE_INSTALL_DIRECTORY
...
if(CPACK_NSIS_DISPLAY_NAME_SET)
  ...
  _cpack_set_default(CPACK_NSIS_PACKAGE_NAME
                     "${CPACK_PACKAGE_INSTALL_DIRECTORY}")
endif()
...
if(NOT DEFINED CPACK_DMG_SLA_USE_RESOURCE_FILE_LICENSE
   ...
   AND NOT CPACK_RESOURCE_FILE_LICENSE STREQUAL
       "${CMAKE_ROOT}/Templates/CPack.GenericLicense.txt")
  cmake_policy(GET CMP0133 _CPack_CMP0133)
  if(NOT "x${_CPack_CMP0133}x" STREQUAL "xNEWx")
    if(NOT "x${_CPack_CMP0133}x" STREQUAL "xOLDx"
       AND CMAKE_POLICY_WARNING_CMP0133)
...
if(DEFINED CPACK_COMPONENTS_ALL)
  if(CPACK_MONOLITHIC_INSTALL)
    ...
    # The user has provided the set of components to be installed as part of a
    # component-based installation; trust her.
    set(CPACK_COMPONENTS_ALL_SET_BY_USER TRUE)
  endif()
else()
  # If the user has not specifically requested a monolithic installer but has
  # specified components in various "install" commands, tell CPack about those
  # components.
  if(NOT CPACK_MONOLITHIC_INSTALL)
    get_cmake_property(CPACK_COMPONENTS_ALL COMPONENTS)
    list(LENGTH CPACK_COMPONENTS_ALL CPACK_COMPONENTS_LEN)
    if(CPACK_COMPONENTS_LEN EQUAL 1)
      # Only one component: this is not a component-based installation (at
      # least, it isn't a component-based installation, but may become one later
      # if the user uses the cpack_add_* commands).
      set(CPACK_COMPONENTS_ALL)
    endif()
...
# CMake always generates a component named "Unspecified", which is used to
# install everything that doesn't have an explicitly-provided component. Since
# these files should always be installed, we'll make them hidden and required.
