...
# This function handles pushing all of the test files needed to the device. It
# places the data files in the object store and makes links to them from the
# appropriate directories.
#
# This function accepts the following named parameters: DIRS          : one or
# more directories needed for testing. FILES         : one or more files needed
# for testing. LIBS          : one or more libraries needed for testing.
# DIRS_DEST     : specify where the directories should be installed. FILES_DEST
# : specify where the files should be installed. LIBS_DEST     : specify where
# the libraries should be installed. DEV_OBJ_STORE : specify where the actual
# data files should be placed. DEV_TEST_DIR  : specify the root file for the
# module test directory. The DEV_OBJ_STORE and DEV_TEST_DIR variables are
# required.
...
function(android_push_test_files_to_device)
  ...
  function(execute_adb_command)
    ...
    if(res_var)
      ...
      message(
        FATAL_ERROR
          "Error occurred during adb command: adb ${com}\nError: ${err_var}.")
    endif()
  endfunction()
  ...
  macro(set_absolute_path relative_path absolute_path)
    ...
    if(NOT EXISTS ${${absolute_path}})
      if(EXISTS ${relative_path})
        ...
        message(
          FATAL_ERROR "Cannot find file for specified path: ${relative_path}")
      endif()
    endif()
  endmacro()

  # This function pushes the data into the device object store and creates a
  # link to that data file in a specified location.
  #
  # This function requires the following un-named parameters: data_path        :
  # absolute path to data to load into dev obj store. dev_object_store :
  # absolute path to the device object store directory. link_origin      :
  # absolute path to the origin of the link to the dev obj store data file.
  function(push_and_link data_path dev_object_store link_origin)
    ...
    # TODO: Verify that the object store file is indeed hashed correctly. Could
    # use md5.
  ...
  foreach(TYPE ${multiValueArgs})
    if(_ptd_${TYPE})
      ...
      foreach(relative_path ${_ptd_${TYPE}})
        # The absolute path can be through the source directory or the build
        # directory. If the file/dir exists in the build directory that version
        # is chosen.
        ...
        if(${TYPE} STREQUAL FILES)
          ...
          # dest was determined earlier, relative_path is a dir, file is path
          # from relative path to a data
