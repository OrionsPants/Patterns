...
# Function to compile a source file to identify the compiler ABI. This is used
# internally by CMake and should not be included by user code.
  ...
  if(NOT DEFINED CMAKE_${lang}_ABI_COMPILED)
    ...
    if(lang STREQUAL "CUDA")
      if(CMAKE_CUDA_ARCHITECTURES STREQUAL "native")
        # We are about to detect the native architectures, so we do not yet know
        # them.  Use all architectures during detection.
        set(CMAKE_CUDA_ARCHITECTURES "all")
      endif()
      set(CMAKE_CUDA_RUNTIME_LIBRARY "Static")
    endif()
    if(NOT "x${CMAKE_${lang}_COMPILER_ID}" STREQUAL "xMSVC")
      # Avoid adding our own platform standard libraries for compilers from
      # which we might detect implicit link libraries.
    ...
    # Save the current LC_ALL, LC_MESSAGES, and LANG environment variables and
    # set them to "C" that way GCC's "search starts here" text is in English and
    # we can grok it.
    ...
    try_compile(
      CMAKE_${lang}_ABI_COMPILED SOURCES
      ${src}
      CMAKE_FLAGS
        ${CMAKE_FLAGS}
      ...
      COPY_FILE "${BIN}" COPY_FILE_ERROR _copy_error __CMAKE_INTERNAL ABI)
    ...
    set(CMAKE_${lang}_ABI_COMPILED
        ${CMAKE_${lang}_ABI_COMPILED}
        PARENT_SCOPE)
    ...
    if(CMAKE_${lang}_ABI_COMPILED)
      ...
      elseif(CMAKE_${lang}_SIZEOF_DATA_PTR_DEFAULT)
        set(CMAKE_${lang}_SIZEOF_DATA_PTR
            "${CMAKE_${lang}_SIZEOF_DATA_PTR_DEFAULT}"
            PARENT_SCOPE)
      endif()

      if(ABI_BYTE_ORDER)
        set(CMAKE_${lang}_BYTE_ORDER
            "${ABI_BYTE_ORDER}"
            PARENT_SCOPE)
      endif()

      if(ABI_NAME)
        set(CMAKE_${lang}_COMPILER_ABI
            "${ABI_NAME}"
            PARENT_SCOPE)
      endif()
      ...
      if(CMAKE_${lang}_VERBOSE_FLAG)
        ...
        if("${rv}" STREQUAL "done")
          # Entries that we have been told to explicitly pass as standard
          # include directories will not be implicitly added by the compiler.
          if(CMAKE_${lang}_STANDARD_INCLUDE_DIRECTORIES)
            list(REMOVE_ITEM implicit_incdirs
                 ${CMAKE_${lang}_STANDARD_INCLUDE_DIRECTORIES})
          endif()

          # We parsed implicit include directories, so override the default
          # initializer.
          set(_CMAKE_${lang}_IMPLICIT_INCLUDE_DIRECTORIES_INIT
              "${implicit_incdirs}")
        endif()
      endif()
      set(CMAKE_${lang}_IMPLICIT_INCLUDE_DIRECTORIES
          "${_CMAKE_${lang}_IMPLICIT_INCLUDE_DIRECTORIES_INIT}"
          PARENT_SCOPE)
      ...
      # for VS IDE Intel Fortran we have to figure out the implicit link path
      # for the fortran run time using a try-compile
      if("${lang}" MATCHES "Fortran" AND "${CMAKE_GENERATOR}" MATCHES
                                         "Visual Studio")
        message(CHECK_START
                "Determine Intel Fortran Compiler Implicit Link Path")
        # Build a sample project which reports symbols.
        try_compile(
          IFORT_LIB_PATH_COMPILED PROJECT
          ...
          IntelFortranImplicit
          CMAKE_FLAGS "-DCMAKE_Fortran_FLAGS:STRING=${CMAKE_Fortran_FLAGS}"
      ...
      # Implicit link libraries cannot be used explicitly for multiple OS X
      # architectures, so we skip it.
      ...
      cmake_parse_library_architecture(${lang} "${implicit_dirs}"
                                       "${implicit_objs}" architecture_flag)
      if(architecture_flag)
        set(CMAKE_${lang}_LIBRARY_ARCHITECTURE
            "${architecture_flag}"
            PARENT_SCOPE)
      endif()
