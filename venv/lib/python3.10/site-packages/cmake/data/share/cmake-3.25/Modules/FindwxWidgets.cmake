...
#
# FIXME: check this and provide a correct sample usage... Remember to connect
# back to the upper text. Sample usage with monolithic wx build:
#
# find_package(wxWidgets COMPONENTS mono) ...

# NOTES
#
# This module has been tested on the WIN32 platform with wxWidgets 2.6.2, 2.6.3,
# and 2.5.3. However, it has been designed to easily extend support to all
# possible builds, e.g., static/shared, debug/release, unicode, universal,
# multilib/monolithic, etc..
#
...
# AUTHOR
#
# Miguel A. Figueroa-Villanueva (miguelf at ieee dot org). Jan Woetzel (jw at
# mip.informatik.uni-kiel.de).
#
# Based on previous works of: Jan Woetzel (FindwxWindows.cmake), Jorgen Bodde
# and Jerry Fath (FindwxWin.cmake).

# TODO/ideas
#
# (1) Option/Setting to use all available wx libs In contrast to expert
# developer who lists the minimal set of required libs in wxWidgets_USE_LIBS
# there is the newbie user: - who just wants to link against WX with more
# 'magic' - doesn't know the internal structure of WX or how it was built, in
# particular if it is monolithic or not - want to link against all available WX
# libs Basically, the intent here is to mimic what wx-config would do by default
# (i.e., `wx-config --libs`).
#
# Possible solution: Add a reserved keyword "std" that initializes to what
# wx-config would default to. If the user has not set the wxWidgets_USE_LIBS,
# default to "std" instead of "base core" as it is now. To implement "std" will
# basically boil down to a FOR_EACH lib-FOUND, but maybe checking whether a
# minimal set was found.

# FIXME: This and all the DBG_MSG calls should be removed after the module
# stabilizes.
#
# Helper macro to control the debugging output globally. There are two versions
# for controlling how verbose your output should be.
macro(DBG_MSG _MSG)
  # message(STATUS
...
# wxWidgets_USE_LIBS.
#
# If wxWidgets_USE_LIBS is set: - if using <components>, then override
# wxWidgets_USE_LIBS - else set wxWidgets_FIND_COMPONENTS to wxWidgets_USE_LIBS
...
#
# Get dir of this file which may reside in: - CMAKE_MAKE_ROOT/Modules on CMake
# installation - CMAKE_MODULE_PATH if user prefers his own specialized version
set(wxWidgets_USE_FILE "")
get_filename_component(wxWidgets_CURRENT_LIST_DIR ${CMAKE_CURRENT_LIST_FILE}
                       PATH)
# Prefer an existing customized version, but the user might override the
# FindwxWidgets module and not the UsewxWidgets one.
...
macro(wx_extract_version)
  ...
  if(NOT _wx_filename)
    message(
      FATAL_ERROR
        "wxWidgets wx/version.h file not found in ${wxWidgets_INCLUDE_DIRS}.")
  endif()

  file(READ "${_wx_filename}" _wx_version_h)
  unset(_wx_filename CACHE)

  string(REGEX REPLACE "^(.*\n)?#define +wxMAJOR_VERSION +([0-9]+).*" "\\2"
                       wxWidgets_VERSION_MAJOR "${_wx_version_h}")
  string(REGEX REPLACE "^(.*\n)?#define +wxMINOR_VERSION +([0-9]+).*" "\\2"
                       wxWidgets_VERSION_MINOR "${_wx_version_h}")
  string(REGEX REPLACE "^(.*\n)?#define +wxRELEASE_NUMBER +([0-9]+).*" "\\2"
                       wxWidgets_VERSION_PATCH "${_wx_version_h}")
...
if(wxWidgets_FIND_STYLE STREQUAL "win32")
  ...
  # -------------------------------------------------------------------
  #
  # Get filename components for a configuration. For example, if _CONFIGURATION
  # = mswunivud, then _PF="msw", _UNV=univ, _UCD=u _DBG=d if _CONFIGURATION =
  # mswu,      then _PF="msw", _UNV="",   _UCD=u _DBG=""
  #
  ...
  macro(WX_FIND_LIBS _PF _UNV _UCD _DBG _VER)
    ...
    # FIXME: What if both regex libs are available. regex should be found
    # outside the loop and only wx${LIB}${_UCD}${_DBG}. Find wxWidgets common
    # libraries.
    foreach(LIB ${wxWidgets_COMMON_LIBRARIES} scintilla)
      find_library(
        WX_${LIB}${_DBG}
        NAMES wx${LIB}${_UCD}${_DBG} # for regex
              wx${LIB}${_DBG}
        PATHS ${WX_LIB_DIR}
        NO_DEFAULT_PATH)
  ...
  #
  # Set the wxWidgets_LIBRARIES variable. Also, Sets output variable
  # wxWidgets_FOUND to FALSE if it fails.
  #
    ...
    if(WX_USE_REL_AND_DBG)
      foreach(LIB ${${_LIBS}})
        ...
        if(WX_${LIB} AND WX_${LIB}d)
          ...
          list(APPEND wxWidgets_LIBRARIES debug ${WX_${LIB}d} optimized
               ${WX_${LIB}})
          set(wxWidgets_${LIB}_FOUND TRUE)
        elseif(NOT wxWidgets_FIND_REQUIRED_${LIB})
          dbg_msg_v(
            "- ignored optional missing WX_${LIB}=${WX_${LIB}} or WX_${LIB}d=${WX_${LIB}d}"
          )
        else()
          dbg_msg_v(
            "- not found due to missing WX_${LIB}=${WX_${LIB}} or WX_${LIB}d=${WX_${LIB}d}"
          )
          set(wxWidgets_FOUND FALSE)
        endif()
      endforeach()
      ...
      foreach(LIB ${${_LIBS}})
        ...
        elseif(NOT wxWidgets_FIND_REQUIRED_${LIB})
          dbg_msg_v(
            "- ignored optional missing WX_${LIB}${_DBG}=${WX_${LIB}${_DBG}}")
        else()
          dbg_msg_v(
  ...
  if(WX_ROOT_DIR)
    # Select one default tree inside the already determined wx tree. Prefer
    # static/shared order usually consistent with build settings.
    set(_WX_TOOL "")
    set(_WX_TOOLVER "")
    set(_WX_ARCH "")
    if(MINGW)
    ...
    if(BUILD_SHARED_LIBS)
      find_path(
        ...
        PATHS
          ${WX_ROOT_DIR}/lib/${_WX_TOOL}${_WX_TOOLVER}_xp${_WX_ARCH}_dll # prefer
                                                                         # shared
          ${WX_ROOT_DIR}/lib/${_WX_TOOL}${_WX_TOOLVER}${_WX_ARCH}_dll # prefer
                                                                      # shared
          ${WX_ROOT_DIR}/lib/${_WX_TOOL}${_WX_TOOLVERx}_xp${_WX_ARCH}_dll # prefer
                                                                          # shared
          ${WX_ROOT_DIR}/lib/${_WX_TOOL}${_WX_TOOLVERx}${_WX_ARCH}_dll # prefer
                                                                       # shared
      ...
      find_path(
        ...
        NAMES qtu/wx/setup.h
              ...
              mswunivud/wx/setup.h
        PATHS
          ${WX_ROOT_DIR}/lib/${_WX_TOOL}${_WX_TOOLVER}_xp${_WX_ARCH}_lib # prefer
                                                                         # static
          ${WX_ROOT_DIR}/lib/${_WX_TOOL}${_WX_TOOLVER}${_WX_ARCH}_lib # prefer
                                                                      # static
          ${WX_ROOT_DIR}/lib/${_WX_TOOL}${_WX_TOOLVERx}_xp${_WX_ARCH}_lib # prefer
                                                                          # static
          ${WX_ROOT_DIR}/lib/${_WX_TOOL}${_WX_TOOLVERx}${_WX_ARCH}_lib # prefer
                                                                       # static
    ...
    if(NOT WX_LIB_DIR STREQUAL wxWidgets_LIB_DIR)
      set(WX_LIB_DIR
          ${wxWidgets_LIB_DIR}
          CACHE INTERNAL "wxWidgets_LIB_DIR")
    ...
    if(WX_LIB_DIR)
      ...
      if(WX_CONFIGURATION)
        ...
        # If the selected configuration wasn't found force the default one.
        # Otherwise, use it but still force a refresh for updating the doc
        # string with the current list of available configurations.
        if(NOT WX_${wxWidgets_CONFIGURATION}_FOUND)
          set(wxWidgets_CONFIGURATION
              ${WX_CONFIGURATION}
              CACHE STRING
                    "Set wxWidgets configuration (${WX_CONFIGURATION_LIST})"
                    FORCE)
        else()
          set(wxWidgets_CONFIGURATION
        ...
        if(WX_${wxWidgets_CONFIGURATION}d_FOUND)
          ...
          if(wxWidgets_USE_REL_AND_DBG)
            set(wxWidgets_USE_REL_AND_DBG
                FALSE
                CACHE BOOL "No ${wxWidgets_CONFIGURATION}d found." FORCE)
          endif()
        ...
        if(EXISTS ${WX_LIB_DIR}/${wxWidgets_CONFIGURATION}/wx/setup.h)
          ...
          dbg_msg(
            "wxWidgets_FOUND FALSE because ${WX_LIB_DIR}/${wxWidgets_CONFIGURATION}/wx/setup.h does not exists."
          )
          set(wxWidgets_FOUND FALSE)
        endif()
        ...
        if(EXISTS ${WX_ROOT_DIR}/include/wx/wx.h)
          ...
          dbg_msg(
            "wxWidgets_FOUND FALSE because WX_ROOT_DIR=${WX_ROOT_DIR} has no ${WX_ROOT_DIR}/include/wx/wx.h"
          )
          set(wxWidgets_FOUND FALSE)
        endif()
...
if(wxWidgets_FIND_STYLE STREQUAL "unix")
  ...
  macro(WX_CONFIG_SELECT_GET_DEFAULT)
    execute_process(
      COMMAND sh "${wxWidgets_CONFIG_EXECUTABLE}" ${wxWidgets_CONFIG_OPTIONS}
              --selected-config
  ...
  #
  # Query a boolean configuration option to determine if the system has both
  # builds available. If so, provide the selection option to the user.
  #
  macro(WX_CONFIG_SELECT_QUERY_BOOL _OPT_NAME _OPT_HELP)
    ...
    if(_wx_result_yes EQUAL 0 AND _wx_result_no EQUAL 0)
      option(wxWidgets_USE_${_UPPER_OPT_NAME} ${_OPT_HELP}
             ${wxWidgets_DEFAULT_${_UPPER_OPT_NAME}})
    else()
      ...
      if(DEFINED wxWidgets_USE_${_UPPER_OPT_NAME})
        if(_wx_result_yes EQUAL 0)
          set(wxWidgets_USE_${_UPPER_OPT_NAME}
              ON
              CACHE BOOL ${_OPT_HELP} FORCE)
        else()
          set(wxWidgets_USE_${_UPPER_OPT_NAME}
              OFF
              CACHE BOOL ${_OPT_HELP} FORCE)
  ...
  #
  # Set wxWidgets_SELECT_OPTIONS to wx-config options for selecting among
  # multiple builds.
  #
  ...
  #
  # Look for wx-config -- this can be set in the environment, or try versioned
  # and toolchain-versioned variants of the -config executable as well.
  set(wx_config_names "wx-config")
  foreach(version ${wx_versions})
    list(APPEND wx_config_names "wx-config-${version}"
         "wxgtk3u-${version}-config" "wxgtk2u-${version}-config")
  endforeach()
  find_program(
    wxWidgets_CONFIG_EXECUTABLE
    NAMES $ENV{WX_CONFIG} ${wx_config_names}
  ...
  if(wxWidgets_CONFIG_EXECUTABLE)
    ...
    if(RET EQUAL 0)
      ...
      # parse definitions and include dirs from cxxflags drop the -D and -I
      # prefixes
      set(wxWidgets_CXX_FLAGS)
    ...
    # run the wx-config program to get the libs - NOTE: wx-config doesn't verify
    # that the libs requested exist it just produces the names. Maybe a
    # TRY_COMPILE would be useful here...
    unset(_cmp_req)
    unset(_cmp_opt)
    ...
    if(RET EQUAL 0)
      ...
      string(REPLACE "-framework;" "-framework " wxWidgets_LIBRARIES
                     "${wxWidgets_LIBRARIES}")
      string(REPLACE "-weak_framework;" "-weak_framework " wxWidgets_LIBRARIES
                     "${wxWidgets_LIBRARIES}")
      string(REPLACE "-arch;" "-arch " wxWidgets_LIBRARIES
                     "${wxWidgets_LIBRARIES}")
      string(REPLACE "-isysroot;" "-isysroot " wxWidgets_LIBRARIES
                     "${wxWidgets_LIBRARIES}")

      # extract linkdirs (-L) for rpath (i.e., LINK_DIRECTORIES)
      string(REGEX MATCHALL "-L[^;]+" wxWidgets_LIBRARY_DIRS
                   "${wxWidgets_LIBRARIES}")
      string(REGEX REPLACE "-L([^;]+)" "\\1" wxWidgets_LIBRARY_DIRS
      ...
      set(wxWidgets_FOUND FALSE)
      dbg_msg(
        "${wxWidgets_CONFIG_EXECUTABLE} --libs ${_cmp_req} ${_cmp_opt} FAILED with RET=${RET}"
      )
    endif()
    unset(_cmp_req)
    unset(_cmp_opt)
  ...
  # When using wx-config in MSYS, the include paths are UNIX style paths which
  # may or may not work correctly depending on you MSYS/MinGW configuration.
  # CMake expects native paths internally.
...
if(_wx_lib_missing)
  ...
  dbg_msg_v(
    "wxWidgets not found due to following missing libraries: ${_wx_lib_missing}"
  )
  set(wxWidgets_FOUND FALSE)
...
# FIXME: set wxWidgets_<comp>_FOUND for wx-config branch and use
# HANDLE_COMPONENTS on Unix too
if(wxWidgets_FIND_STYLE STREQUAL "win32")
...
find_package_handle_standard_args(
  ...
  VERSION_VAR wxWidgets_VERSION_STRING ${wxWidgets_HANDLE_COMPONENTS})
unset(wxWidgets_HANDLE_COMPONENTS)

# =====================================================================
# Macros for use in wxWidgets apps. - This module will not fail to find
# wxWidgets based on the code below. Hence, it's required to check for validity
# of:
#
...
#
# Example usage: function(WXWIDGETS_ADD_RESOURCES outfiles)
...
# FindPackageStandardArgs.cmake. At the time of this writing FindQt4.cmake has a
# QT4_EXTRACT_OPTIONS, which I basically copied here a bit more generalized. So,
# there are already two find modules using this approach.
#
function(WX_SPLIT_ARGUMENTS_ON _keyword _leftvar _rightvar)
  # FIXME: Document that the input variables will be cleared. list(APPEND
  # ${_leftvar}  "") list(APPEND ${_rightvar} "")
  set(${_leftvar} "")
  set(${_rightvar} "")
...
#
# WX_GET_DEPENDENCIES_FROM_XML( <depends> <match_pattern> <clean_pattern>
# <xml_contents> <depends_path> )
#
# FIXME: Add documentation here...
#
function(WX_GET_DEPENDENCIES_FROM_XML _depends _match_patt _clean_patt
         _xml_contents _depends_path)

  string(REGEX MATCHALL ${_match_patt} dep_file_list "${${_xml_contents}}")
...
#
# WXWIDGETS_ADD_RESOURCES(<sources> <xrc_files> OPTIONS <options> [NO_CPP_CODE])
#
# Adds a custom command for resource file compilation of the <xrc_files> and
# appends the output files to <sources>.
#
# Example usages: WXWIDGETS_ADD_RESOURCES(sources xrc/main_frame.xrc)
...
function(WXWIDGETS_ADD_RESOURCES _outfiles)
  ...
  foreach(rc_file ${rc_file_list})
    ...
    # All files have absolute paths or paths relative to the location of the rc
    # file.
    file(READ "${rc_file_abs}" rc_file_contents)

    # get bitmap/bitmap2 files
    wx_get_dependencies_from_xml(rc_depends "<bitmap[^<]+" "^<bitmap[^>]*>"
                                 rc_file_contents depends_path)

    # get url files
    wx_get_dependencies_from_xml(rc_depends "<url[^<]+" "^<url[^>]*>"
                                 rc_file_contents depends_path)

    # get wxIcon files
    wx_get_dependencies_from_xml(rc_depends "<object[^>]*class=\"wxIcon\"[^<]+"
                                 "^<object[^>]*>" rc_file_contents depends_path)
  endforeach()

  #
  # Parse options.
  #
  # If NO_CPP_CODE option specified, then produce .xrs file rather than a .cpp
  # file (i.e., don't add the default --cpp-code option).
  ...
  # Get output name for use in ADD_CUSTOM_COMMAND. - short option scanning
  ...
  if(NOT short_index EQUAL -1 OR NOT long_index EQUAL -1)
    ...
    set_source_files_properties("${outfile_header}" PROPERTIES GENERATED TRUE)
  endif()
