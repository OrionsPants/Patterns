...
# CrayPrgEnv: loaded when compiling through the Cray compiler wrapper. The
# compiler wrapper can run on a front-end node or a compute node.

cmake_policy(PUSH)
cmake_policy(SET CMP0057 NEW) # if IN_LIST

# One-time setup of the craype environment.  First, check the wrapper config.
# The wrapper's selection of a compiler (gcc, clang, intel, etc.) and default
# include/library paths is selected using the "module" command. The
# CRAYPE_LINK_TYPE environment variable partly controls if static or dynamic
# binaries are generated (see __cmake_craype_linktype below). Running cmake and
# then changing module and/or linktype configuration may cause build problems
# (since the data in the cmake cache may no longer be correct after the change).
# We can look for this and warn the user about it.  Second, use the "module"
# provided PKG_CONFIG_PATH-like environment variable to add additional prefixes
# to the system prefix path.
function(__cmake_craype_setupenv)
  if(NOT DEFINED __cmake_craype_setupenv_done) # only done once per run
    set(__cmake_craype_setupenv_done
        1
        PARENT_SCOPE)
    unset(__cmake_check)
    set(CMAKE_CRAYPE_LINKTYPE
        "$ENV{CRAYPE_LINK_TYPE}"
        CACHE STRING "saved value of CRAYPE_LINK_TYPE environment variable")
    set(CMAKE_CRAYPE_LOADEDMODULES
        "$ENV{LOADEDMODULES}"
        CACHE STRING "saved value of LOADEDMODULES environment variable")
    ...
    foreach(pkgcfgvar PKG_CONFIG_PATH PKG_CONFIG_PATH_DEFAULT
      ...
      foreach(path ${pkgcfg})
        ...
        if(NOT "${path}" STREQUAL "" AND NOT "${path}" IN_LIST
...
# The wrapper disables dynamic linking by default.  Dynamic linking is enabled
# either by setting $ENV{CRAYPE_LINK_TYPE} to "dynamic" or by specifying
# "-dynamic" to the wrapper when linking.  Specifying "-static" to the wrapper
# when linking takes priority over $ENV{CRAYPE_LINK_TYPE}. Furthermore, if you
# specify multiple "-dynamic" and "-static" flags to the wrapper when linking,
# the last one will win.  In this case, the wrapper will also print a warning
# like: Warning: -dynamic was already seen on command line, overriding with
# -static.
#
...
# variable.
#
# Note: As of CrayPE v19.06 (which translates to the craype/2.6.0 module) the
# default has changed and is now dynamic by default.  This is handled
# accordingly
function(__cmake_craype_linktype lang rv)
  # start with ENV, but allow flags to override
  if(("$ENV{CRAYPE_VERSION}" STREQUAL "") OR ("$ENV{CRAYPE_VERSION}"
                                              VERSION_LESS "2.6"))
...
macro(__CrayPrgEnv_setup lang)
  ...
  elseif(DEFINED ENV{ASYNCPE_VERSION})
    message(
      STATUS "Cray XT Programming Environment $ENV{ASYNCPE_VERSION} ${lang}")
  else()
