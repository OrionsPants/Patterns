...
# helper function that parses implicit include dirs from a single line for
# compilers that report them that way.  on success we return the list of dirs in
# id_var and set state_var to the 'done' state.
function(cmake_parse_implicit_include_line line lang id_var log_var state_var)
  ...
  if(CMAKE_${lang}_COMPILER_ID STREQUAL "Cray"
     AND line MATCHES "^/"
     AND line MATCHES "/ccfe |/ftnfe "
     AND line MATCHES " -isystem| -I")
    string(REGEX MATCHALL " (-I ?|-isystem )(\"[^\"]+\"|[^ \"]+)" incs
                 "${line}")
    foreach(inc IN LISTS incs)
      string(REGEX REPLACE " (-I ?|-isystem )(\"[^\"]+\"|[^ \"]+)" "\\2" idir
                           "${inc}")
      list(APPEND rv "${idir}")
    endforeach()
    if(rv)
  ...
  if(CMAKE_${lang}_COMPILER_ID STREQUAL "PGI")
    ...
    if((lang STREQUAL "C" OR lang STREQUAL "Fortran")
       AND line MATCHES "^/"
       AND line MATCHES "/pgc |/pgf901 |/pgftnc "
       AND line MATCHES " -cmdline ")
    ...
    elseif(
      lang STREQUAL "CXX"
      AND line MATCHES "^/"
      AND line MATCHES "/pggpp1 "
      AND line MATCHES " -I")
      # oddly, -Mnostdinc does not get rid of system -I's, at least in PGI
      # 18.10.1 ...
  ...
  # SunPro compiler
  if(CMAKE_${lang}_COMPILER_ID STREQUAL "SunPro"
     AND (line MATCHES "-D__SUNPRO_C" OR line MATCHES "-D__SUNPRO_F"))
    string(REGEX MATCHALL " (-I ?)([^ ]*)" incs "${line}")
    foreach(inc IN LISTS incs)
      string(REGEX REPLACE " (-I ?)([^ ]*)" "\\2" idir "${inc}")
  ...
  # XL compiler
  if((CMAKE_${lang}_COMPILER_ID STREQUAL "XL" OR CMAKE_${lang}_COMPILER_ID
                                                 STREQUAL "XLClang")
     AND line MATCHES "^/"
     AND ((lang STREQUAL "Fortran"
           AND line MATCHES "/xl[fF]entry "
           AND line MATCHES "OSVAR\\([^ ]+\\)")
          OR ((lang STREQUAL "C" OR lang STREQUAL "CXX")
              AND line MATCHES "/xl[cC]2?entry "
              AND line MATCHES " -qosvar=")
         ))
  ...
  # Fujitsu compiler
  if(CMAKE_${lang}_COMPILER_ID STREQUAL "Fujitsu" AND line MATCHES "/ccpcom")
    string(REGEX MATCHALL
                 " (-I *|--sys_include=|--preinclude +)(\"[^\"]+\"|[^ \"]+)"
                 incs "${line}")
    foreach(inc IN LISTS incs)
      string(REGEX
             REPLACE " (-I *|--sys_include=|--preinclude +)(\"[^\"]+\"|[^ \"]+)"
                     "\\2" idir "${inc}")
      list(APPEND rv "${idir}")
    endforeach()
    if(rv)
...
# top-level function to parse implicit include directory information from
# verbose compiler output. sets state_var in parent to 'done' on success.
function(cmake_parse_implicit_include_info text lang dir_var log_var state_var)
  ...
  foreach(d IN LISTS implicit_dirs_tmp)
    ...
    elseif("${d}" MATCHES [[^\.\.[\/]\.\.[\/](.*)$]])
      ...
      get_filename_component(dir "${CMAKE_BINARY_DIR}/${CMAKE_MATCH_1}"
                             ABSOLUTE)
      list(APPEND implicit_dirs "${dir}")
