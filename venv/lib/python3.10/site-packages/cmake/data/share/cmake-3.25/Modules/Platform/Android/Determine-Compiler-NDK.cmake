...
if(CMAKE_ANDROID_NDK_TOOLCHAIN_UNIFIED)
  if(CMAKE_ANDROID_NDK_TOOLCHAIN_VERSION
     AND NOT CMAKE_ANDROID_NDK_TOOLCHAIN_VERSION STREQUAL "clang")
    message(
      FATAL_ERROR
  ...
  set(_ANDROID_TOOL_C_COMPILER
      "${CMAKE_ANDROID_NDK_TOOLCHAIN_UNIFIED}/bin/clang${_ANDROID_HOST_EXT}")
  ...
  set(_ANDROID_TOOL_C_TOOLCHAIN_PREFIX
      "${CMAKE_ANDROID_NDK_TOOLCHAIN_UNIFIED}/bin/${CMAKE_ANDROID_ARCH_TRIPLE}-"
  )
...
#
# There is also a "toolchains" directory with the prebuilt toolchains
# themselves:
#
# * <triple-or-arch>-<gcc-version>/prebuilt/<host>/bin/<triple>-gcc(.exe)? The
#   gcc compiler to be invoked.
#
# * llvm*/prebuilt/<host>/bin/clang The clang compiler to be invoked with flags:
#   -target <triple> -gcc-toolchain
#   <ndk>/toolchains/<triple-or-arch>-<gcc-version>
...
elseif(CMAKE_ANDROID_NDK_TOOLCHAIN_VERSION)
  ...
  # If we can find any gcc toolchains then use one by default. Otherwise we look
  # for clang toolchains (e.g. NDK r18+).
...
foreach(config_mk IN LISTS _ANDROID_CONFIG_MKS)
  ...
  file(STRINGS "${config_mk}" _ANDROID_TOOL_ABIS
       REGEX "^TOOLCHAIN_ABIS :=.* ${CMAKE_ANDROID_ARCH_ABI}( |$)")
  ...
  if("${config_mk}" MATCHES [[/([^/]+-((clang)?([0-9]\.[0-9]|)))/config.mk$]])
    ...
    elseif(_ANDROID_CUR_VERS STREQUAL "" OR _ANDROID_CUR_VERS VERSION_GREATER
                                            _ANDROID_TOOL_VERS)
      ...
      string(REPLACE "/config.mk" "/setup.mk" _ANDROID_TOOL_SETUP_MK
                     "${config_mk}")
    endif()
...
if(NOT _ANDROID_TOOL_NAME)
  if(_ANDROID_CONFIG_MKS)
    string(REPLACE ";" "\n  " _ANDROID_TOOLS_MSG
                   "after considering:;${_ANDROID_CONFIG_MKS}")
  else()
...
# Parse the toolchain setup.mk file to extract information we need. Their
# content is not standardized across toolchains or NDK versions, so we match
# known cases.  Note that the parsing is stateful across lines because we need
# to substitute for some Make variable references.
if(CMAKE_ANDROID_NDK_TOOLCHAIN_DEBUG)
  message(STATUS "loading: ${_ANDROID_TOOL_SETUP_MK}")
endif()
file(STRINGS "${_ANDROID_TOOL_SETUP_MK}" _ANDROID_TOOL_SETUP
     REGEX "^(LLVM|TOOLCHAIN)_[A-Z_]+ +:= +.*$")
...
foreach(line IN LISTS _ANDROID_TOOL_SETUP)
  ...
  elseif(_ANDROID_TOOL_CLANG_NAME)
    ...
    elseif(line MATCHES [[^TOOLCHAIN_NAME +:= +(.*\$\(TOOLCHAIN_VERSION\)) *$]])
      # We just matched the gcc toolchain name with a version number
      # placeholder, so substitute it. The gcc toolchain version number will
      # have already been extracted from a TOOLCHAIN_VERSION line.
      string(REPLACE "$(TOOLCHAIN_VERSION)" "${_ANDROID_TOOL_VERS}"
                     _ANDROID_TOOL_NAME "${CMAKE_MATCH_1}")
    elseif(line MATCHES [[^TOOLCHAIN_NAME +:= +([^$/ ]+) *$]])
      # We just matched the gcc toolchain name without version number.  Save it
      # for later.
      set(_ANDROID_TOOL_NAME_ONLY "${CMAKE_MATCH_1}")
    elseif(line MATCHES
           [[^TOOLCHAIN_PREFIX +:= +.*/bin/(\$\(TOOLCHAIN_NAME\)-) *$]])
      # We just matched the toolchain prefix with a name placeholder, so
      # substitute it. The gcc toolchain name will have already been extracted
      # without version number from a TOOLCHAIN_NAME line.
      string(REPLACE "$(TOOLCHAIN_NAME)" "${_ANDROID_TOOL_NAME_ONLY}"
                     _ANDROID_TOOL_PREFIX "${CMAKE_MATCH_1}")
    elseif(line MATCHES [[^LLVM_VERSION +:= +([0-9.]+)$]])
      # We just matched the llvm prebuilt binary toolchain version number.  Save
      # it for later.
      set(_ANDROID_TOOL_LLVM_VERS "${CMAKE_MATCH_1}")
    elseif(line MATCHES [[^LLVM_NAME +:= +(llvm-\$\(LLVM_VERSION\)) *$]])
      # We just matched the llvm prebuilt binary toolchain directory name with a
      # version number placeholder, so substitute it. The llvm prebuilt binary
      # toolchain version number will have already been extracted from a
      # LLVM_VERSION line.
      string(REPLACE "$(LLVM_VERSION)" "${_ANDROID_TOOL_LLVM_VERS}"
                     _ANDROID_TOOL_LLVM_NAME "${CMAKE_MATCH_1}")
    elseif(
      line
      MATCHES
      [[^LLVM_TOOLCHAIN_PREBUILT_ROOT +:= +\$\(call get-toolchain-root.*,([^$ ]+)\) *$]]
    )
    ...
    elseif(
      line
      MATCHES
      [[^TOOLCHAIN_ROOT +:= +\$\(call get-toolchain-root.*,(\$\(TOOLCHAIN_NAME\)-[0-9.]+)\) *$]]
    )
      # We just matched a placeholder for the name followed by a version number.
      # The gcc toolchain name will have already been extracted without version
      # number from a TOOLCHAIN_NAME line. Substitute for the placeholder to get
      # the full gcc toolchain name.
      string(REPLACE "$(TOOLCHAIN_NAME)" "${_ANDROID_TOOL_NAME_ONLY}"
                     _ANDROID_TOOL_NAME "${CMAKE_MATCH_1}")
    elseif(line MATCHES
           [[^TOOLCHAIN_ROOT +:= +\$\(call get-toolchain-root.*,([^$ ]+)\) *$]])
...
if(NOT _ANDROID_TOOL_PREFIX AND "${_ANDROID_TOOL_NAME}" MATCHES
                                "^(.*-)[0-9.]+$")
  set(_ANDROID_TOOL_PREFIX "${CMAKE_MATCH_1}")
endif()
...
string(REGEX REPLACE "-$" "" _ANDROID_TOOL_C_TOOLCHAIN_MACHINE
                     "${_ANDROID_TOOL_PREFIX}")

set(_ANDROID_TOOL_C_TOOLCHAIN_VERSION "${_ANDROID_TOOL_VERS}")
set(_ANDROID_TOOL_C_TOOLCHAIN_PREFIX
    "${CMAKE_ANDROID_NDK}/toolchains/${_ANDROID_TOOL_NAME}/prebuilt/${CMAKE_ANDROID_NDK_TOOLCHAIN_HOST_TAG}/bin/${_ANDROID_TOOL_PREFIX}"
)
set(_ANDROID_TOOL_C_TOOLCHAIN_SUFFIX "${_ANDROID_HOST_EXT}")
...
if(_ANDROID_TOOL_CLANG_NAME)
  message(
    STATUS
      "Android: Selected Clang toolchain '${_ANDROID_TOOL_CLANG_NAME}' with GCC toolchain '${_ANDROID_TOOL_NAME}'"
  )
  set(_ANDROID_TOOL_C_COMPILER
      "${CMAKE_ANDROID_NDK}/toolchains/${_ANDROID_TOOL_LLVM_NAME}/prebuilt/${CMAKE_ANDROID_NDK_TOOLCHAIN_HOST_TAG}/bin/clang${_ANDROID_HOST_EXT}"
  )
  set(_ANDROID_TOOL_C_COMPILER_EXTERNAL_TOOLCHAIN
  ...
  set(_ANDROID_TOOL_C_COMPILER
      "${_ANDROID_TOOL_C_TOOLCHAIN_PREFIX}gcc${_ANDROID_TOOL_C_TOOLCHAIN_SUFFIX}"
  )
  set(_ANDROID_TOOL_C_COMPILER_EXTERNAL_TOOLCHAIN "")
  set(_ANDROID_TOOL_CXX_COMPILER
