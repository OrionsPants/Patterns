...
if(NOT NAME)
  message(
    FATAL_ERROR
      "Name of the package to be searched not specified. Set the CMake variable NAME, e.g. -DNAME=JPEG ."
  )
endif()
...
# require the current version. If we don't do this, Platforms/CYGWIN.cmake
# complains because it doesn't know whether it should set WIN32 or not:
cmake_minimum_required(
  VERSION ${CMAKE_MAJOR_VERSION}.${CMAKE_MINOR_VERSION}.${CMAKE_PATCH_VERSION})

macro(ENABLE_LANGUAGE)
  # disable the enable_language() command, otherwise --find-package breaks on
  # Windows. On Windows, enable_language(RC) is called in the platform files
  # unconditionally. But in --find-package mode, we don't want (and can't)
  # enable any language.
endmacro()
...
# Also load the system specific file, which sets up e.g. the search paths. This
# makes the FIND_XXX() calls work much better
include(CMakeSystemSpecificInformation)

if(UNIX)

  # try to guess whether we have a 64bit system, if it has not been set from the
  # outside
  if(NOT CMAKE_SIZEOF_VOID_P)
    ...
    if(EXISTS ${CMAKE_SYSROOT}/usr/lib64)
      ...
      if(FILE_EXECUTABLE)
        ...
        execute_process(
          COMMAND "${FILE_ABSPATH}" "${FILE_ABSPATH}"
          OUTPUT_VARIABLE fileOutput
          ERROR_QUIET)
...
function(set_compile_flags_var _packageName)
  ...
  # Check the following variables: FOO_INCLUDE_DIRS Foo_INCLUDE_DIRS
  # FOO_INCLUDES Foo_INCLUDES FOO_INCLUDE_DIR Foo_INCLUDE_DIR
  set(includes)
  ...
  # Check the following variables: FOO_DEFINITIONS Foo_DEFINITIONS
  set(definitions)
...
function(set_link_flags_var _packageName)
  string(TOUPPER "${_packageName}" PACKAGE_NAME)
  # Check the following variables: FOO_LIBRARIES Foo_LIBRARIES FOO_LIBS Foo_LIBS
  set(libs)
