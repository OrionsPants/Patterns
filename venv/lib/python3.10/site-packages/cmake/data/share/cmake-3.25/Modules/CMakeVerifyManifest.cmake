...
# directory.  Then globs *.exe and *.dll.  Each .exe and .dll is scanned for
# embedded manifests and the versions of CRT are compared to those found in the
# .manifest files from the first glob.

# crt_version: function to extract the CRT version from a file this can be
# passed a .exe, .dll, or a .manifest file it will put the list of versions
# found into the variable specified by list_var
function(crt_version file list_var)
  file(
    STRINGS "${file}" strings
    REGEX "Microsoft.VC...CRT"
    NEWLINE_CONSUME)
  foreach(s ${strings})
    set(has_match 1)
    string(
      REGEX
      REPLACE
        ".*<assembly.*\"Microsoft.VC...CRT\".*version=\"([^\"]*)\".*</assembly>.*$"
        "\\1"
        version
        "${s}")
...
# check_version:
#
# test a file against the shipped manifest versions for a directory
function(check_version file manifest_versions)
  ...
  foreach(ver ${file_versions})
    ...
    if("${found_version}" EQUAL -1)
      message(
        "ERROR: ${file} uses ${ver} not found in shipped manifests:[${manifest_versions}]."
      )
      set(fatal_error
          TRUE
          PARENT_SCOPE)
  ...
  if(${len} GREATER 1)
    message(
      "WARNING: found more than one version of MICROSOFT.VC80.CRT referenced in ${file}: [${file_versions}]"
    )
  endif()
endfunction()

# collect up the versions of CRT that are shipped in .manifest files
...
# now find all .exe and .dll files and call check_version on each of them
...
# report a fatal error if there were any so that cmake will return a non zero
# value
if(fatal_error)
  message(
    FATAL_ERROR
      "This distribution embeds dll "
