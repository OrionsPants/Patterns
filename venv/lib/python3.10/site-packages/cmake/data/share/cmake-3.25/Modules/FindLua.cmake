...
function(_lua_get_versions)
  ...
  elseif(Lua_FIND_VERSION)
    # once there is a different major version supported this should become a
    # loop
    if(NOT Lua_FIND_VERSION_MAJOR GREATER 5)
      if(Lua_FIND_VERSION_COUNT EQUAL 1)
        ...
        endforeach()
        # New version -> Search for it (heuristic only! Defines in include might
        # have changed)
  ...
  if(LUA_Debug)
    message(
      STATUS "Considering following Lua versions: ${_lua_append_versions}")
  endif()
...
function(_lua_set_version_vars)
  ...
  foreach(ver IN LISTS _lua_append_versions)
    ...
    list(APPEND _lua_include_subdirs_raw lua${CMAKE_MATCH_1}${CMAKE_MATCH_2}
...
function(_lua_get_header_version)
  ...
  # At least 5.[012] have different ways to express the version so all of them
  # need to be tested. Lua 5.2 defines LUA_VERSION and LUA_RELEASE as joined by
  # the C preprocessor, so avoid those.
  file(
    STRINGS "${_hdr_file}" lua_version_strings
    REGEX
      "^#define[ \t]+LUA_(RELEASE[ \t]+\"Lua [0-9]|VERSION([ \t]+\"Lua [0-9]|_[MR])).*"
  )
  ...
  if(LUA_VERSION_MAJOR MATCHES "^[0-9]+$")
    ...
    string(REGEX REPLACE "^([0-9]+)\\.[0-9.]*$" "\\1" LUA_VERSION_MAJOR
                         "${LUA_VERSION_STRING}")
    string(REGEX REPLACE "^[0-9]+\\.([0-9]+)[0-9.]*$" "\\1" LUA_VERSION_MINOR
                         "${LUA_VERSION_STRING}")
    string(REGEX REPLACE "^[0-9]+\\.[0-9]+\\.([0-9]).*" "\\1" LUA_VERSION_PATCH
                         "${LUA_VERSION_STRING}")
  endif()
  foreach(ver IN LISTS _lua_append_versions)
    if(ver STREQUAL "${LUA_VERSION_MAJOR}.${LUA_VERSION_MINOR}")
      set(LUA_VERSION_MAJOR
          ${LUA_VERSION_MAJOR}
          PARENT_SCOPE)
      set(LUA_VERSION_MINOR
          ${LUA_VERSION_MINOR}
          PARENT_SCOPE)
      set(LUA_VERSION_PATCH
          ${LUA_VERSION_PATCH}
          PARENT_SCOPE)
      set(LUA_VERSION_STRING
          ${LUA_VERSION_STRING}
...
function(_lua_find_header)
  ...
  while(TRUE)
    # Find the next header to test. Check each possible subdir in order This
    # prefers e.g. higher versions as they are earlier in the list It is also
    # consistent with previous versions of FindLua
    ...
    if(LUA_VERSION_STRING)
      if(LUA_Debug)
        message(
          STATUS
            "Found suitable version ${LUA_VERSION_STRING} in ${LUA_INCLUDE_DIR}/lua.h"
        )
      endif()
      return()
    endif()
...
find_library(
  LUA_LIBRARY
  NAMES ${_lua_library_names} lua NAMES_PER_DIR
  HINTS ENV LUA_DIR
  PATH_SUFFIXES lib)
unset(_lua_library_names)
