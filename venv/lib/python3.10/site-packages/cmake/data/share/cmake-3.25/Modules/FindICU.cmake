...
set(icu_data Makefile.inc pkgdata.inc)

# The ICU checks are contained in a function due to the large number of
...
function(_ICU_FIND)
  ...
  foreach(program ${icu_programs})
    ...
    find_program(
      ...
      DOC "ICU ${program} executable" NO_PACKAGE_ROOT_PATH)
  ...
  foreach(component ${ICU_FIND_COMPONENTS})
    ...
    # Special case deliberate library naming mismatches between Unix and Windows
    # builds
    ...
    find_library(
      "${component_cache_release}"
      NAMES ${component_libnames}
      HINTS ${icu_roots}
      PATH_SUFFIXES ${icu_library_suffixes}
      DOC "ICU ${component} library (release)" NO_PACKAGE_ROOT_PATH)
    find_library(
      "${component_cache_debug}"
      NAMES ${component_debug_libnames}
      HINTS ${icu_roots}
      PATH_SUFFIXES ${icu_library_suffixes}
      DOC "ICU ${component} library (debug)" NO_PACKAGE_ROOT_PATH)
    ...
    if(${component_cache})
      ...
      if(ICU_FIND_REQUIRED_${component})
        list(APPEND ICU_LIBS_FOUND
             "${component} (required): ${${component_cache}}")
      else()
        list(APPEND ICU_LIBS_FOUND
             "${component} (optional): ${${component_cache}}")
      endif()
    else()
      if(ICU_FIND_REQUIRED_${component})
    ...
    set("${component_found}"
        "${${component_found}}"
        PARENT_SCOPE)
    set("${component_found_compat}"
        "${${component_found_compat}}"
        PARENT_SCOPE)
  endforeach()
  set(_ICU_REQUIRED_LIBS_FOUND
      "${ICU_REQUIRED_LIBS_FOUND}"
      PARENT_SCOPE)
  set(ICU_LIBRARY
      "${ICU_LIBRARY}"
      PARENT_SCOPE)
  ...
  list(APPEND icu_data_suffixes "${_lib64}/icu/${ICU_VERSION}"
       "lib/icu/${ICU_VERSION}" "${_lib64}/icu" "lib/icu")
...
find_package_handle_standard_args(
  ICU
  FOUND_VAR ICU_FOUND
  REQUIRED_VARS ICU_INCLUDE_DIR ICU_LIBRARY _ICU_REQUIRED_LIBS_FOUND
      ...
      if(NOT TARGET ${_ICU_imported_target})
        ...
        if(ICU_INCLUDE_DIR)
          set_target_properties(
            ${_ICU_imported_target} PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                               "${ICU_INCLUDE_DIR}")
        endif()
        if(EXISTS "${${_ICU_component_cache}}")
          set_target_properties(
            ${_ICU_imported_target}
        ...
        if(CMAKE_DL_LIBS AND _ICU_component STREQUAL "uc")
          set_target_properties(
            ${_ICU_imported_target} PROPERTIES INTERFACE_LINK_LIBRARIES
                                               "${CMAKE_DL_LIBS}")
        endif()
      endif()
    endif()
...
if(ICU_DEBUG)
  ...
  foreach(component IN LISTS ICU_FIND_COMPONENTS)
    ...
    message(
      STATUS
        "${component} library found: ${component_found}=${${component_found}}")
    message(
      STATUS
        "${component} library found (compat name): ${component_found_compat}=${${component_found_compat}}"
    )
