...
# prefix for the tools (e.g. arm-elf-gcc etc.) If the cmake variable
# _CMAKE_TOOLCHAIN_LOCATION is set, the compiler is searched only there. The
# other tools are at first searched there, then also in the default locations.
#
# Sets the following variables: CMAKE_AR CMAKE_RANLIB CMAKE_LINKER CMAKE_MT
# CMAKE_STRIP CMAKE_INSTALL_NAME_TOOL
...
function(__resolve_tool_path CMAKE_TOOL SEARCH_PATH DOCSTRING)
  ...
  if(${CMAKE_TOOL})
    # We only get here if CMAKE_TOOL was specified using -D or a pre-made
    # CMakeCache.txt (e.g. via ctest) or set in CMAKE_TOOLCHAIN_FILE.
...
__resolve_tool_path(CMAKE_LINKER "${_CMAKE_TOOLCHAIN_LOCATION}"
                    "Default Linker")
__resolve_tool_path(CMAKE_MT "${_CMAKE_TOOLCHAIN_LOCATION}"
                    "Default Manifest Tool")

set(_CMAKE_TOOL_VARS "")

# if it's the MS C/CXX compiler, search for link
if(("x${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_SIMULATE_ID}" STREQUAL "xMSVC"
    AND ("x${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_COMPILER_FRONTEND_VARIANT}"
         STREQUAL "xMSVC"
   ...
   OR (CMAKE_HOST_WIN32
       AND "x${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_COMPILER_ID}" STREQUAL "xPGI"
      )
   OR (CMAKE_HOST_WIN32
       AND "x${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_COMPILER_ID}" STREQUAL
           "xNVIDIA")
   OR (CMAKE_HOST_WIN32 AND "x${_CMAKE_PROCESSING_LANGUAGE}" STREQUAL "xISPC")
   OR (CMAKE_GENERATOR MATCHES "Visual Studio" AND NOT CMAKE_VS_PLATFORM_NAME
  ...
  if("x${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_COMPILER_ID}" STREQUAL "xClang")
    ...
    # llvm-mt does not support all flags we need in vs_link_exe list(PREPEND
    # _CMAKE_MT_NAMES "llvm-mt")
    list(PREPEND _CMAKE_LINKER_NAMES "lld-link")
    list(APPEND _CMAKE_TOOL_VARS NM)
  elseif("x${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_COMPILER_ID}" STREQUAL
         "xIntel")
...
elseif("x${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_COMPILER_ID}" MATCHES
       "^x(Open)?Watcom$")
...
elseif("x${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_COMPILER_ID}" MATCHES "^xIAR$")
  # Small helper declaring an IAR tool (e.g. linker) to avoid repeating the same
  # idiom every time
  ...
  __resolve_iar_hints("${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_COMPILER}"
                      _CMAKE_TOOLCHAIN_LOCATION)
  set(_CMAKE_IAR_ITOOLS
      "ARM"
      "RX"
      "RH850"
      "RL78"
  ...
  if("${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_COMPILER_ARCHITECTURE_ID}" IN_LIST
     _CMAKE_IAR_ITOOLS)
    string(
      TOLOWER "${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_COMPILER_ARCHITECTURE_ID}"
              _CMAKE_IAR_LOWER_ARCHITECTURE_ID)
  ...
  elseif("${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_COMPILER_ARCHITECTURE_ID}"
    ...
    message(
      FATAL_ERROR
        "Failed to find linker and librarian for ${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_COMPILER_ID} on ${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_COMPILER_ARCHITECTURE_ID}."
    )
  endif()
  ...
  if("${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_COMPILER_ID}" STREQUAL Clang)
    ...
    if(APPLE)
      # llvm-ar does not generate a symbol table that the Apple ld64 linker
      # accepts.
    ...
    if("${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_COMPILER_VERSION}"
      ...
      # llvm-strip versions prior to 11 require additional flags we do not yet
      # add.
      list(PREPEND _CMAKE_STRIP_NAMES "llvm-strip")
    endif()
    list(PREPEND _CMAKE_NM_NAMES "llvm-nm")
    if("${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_COMPILER_VERSION}"
       VERSION_GREATER_EQUAL 9)
...
foreach(_CMAKE_TOOL IN LISTS _CMAKE_TOOL_VARS)
  ...
  foreach(_CMAKE_TOOL_NAME IN LISTS _CMAKE_${_CMAKE_TOOL}_NAMES)
    list(
      APPEND
      _CMAKE_${_CMAKE_TOOL}_FIND_NAMES
  ...
  find_program(
    CMAKE_${_CMAKE_TOOL}
    NAMES ${_CMAKE_${_CMAKE_TOOL}_FIND_NAMES}
...
if(NOT CMAKE_RANLIB)
  set(CMAKE_RANLIB
      :
      CACHE INTERNAL "noop for ranlib")
endif()

if(CMAKE_PLATFORM_HAS_INSTALLNAME)
  find_program(
    CMAKE_INSTALL_NAME_TOOL
    NAMES ${_CMAKE_TOOLCHAIN_PREFIX}install_name_tool
    HINTS ${_CMAKE_TOOLCHAIN_LOCATION}
    NO_CMAKE_PATH NO_CMAKE_ENVIRONMENT_PATH)
...
if("x${CMAKE_${_CMAKE_PROCESSING_LANGUAGE}_COMPILER_ID}" MATCHES "^xIAR$")
  ...
  set(CMAKE_IAR_ARCHIVE
      "${CMAKE_AR}"
      CACHE FILEPATH "The IAR archiver")
  set(CMAKE_IAR_LINKER
      "${CMAKE_LINKER}"
      CACHE FILEPATH "The IAR ILINK linker")
