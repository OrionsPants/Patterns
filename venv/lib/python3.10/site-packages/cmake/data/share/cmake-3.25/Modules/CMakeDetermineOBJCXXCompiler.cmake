...
# determine the compiler to use for Objective-C++ programs NOTE, a generator may
# set CMAKE_OBJCXX_COMPILER before loading this file to force a compiler. use
# environment variable OBJCXX first if defined by user, next use the cmake
# variable CMAKE_GENERATOR_OBJCXX which can be defined by a generator as a
# default compiler If the internal cmake variable _CMAKE_TOOLCHAIN_PREFIX is
# set, this is used as prefix for the tools (e.g. arm-elf-g++, arm-elf-ar etc.)
#
# Sets the following variables: CMAKE_OBJCXX_COMPILER
# CMAKE_COMPILER_IS_GNUOBJCXX CMAKE_COMPILER_IS_CLANGOBJCXX CMAKE_AR
# CMAKE_RANLIB
#
...
if("${CMAKE_GENERATOR}" MATCHES "Xcode")
  ...
  if(NOT CMAKE_OBJCXX_COMPILER)
    ...
    foreach(var OBJCXX CXX)
      if($ENV{${var}} MATCHES ".+")
        get_filename_component(CMAKE_OBJCXX_COMPILER_INIT $ENV{${var}} PROGRAM
                               PROGRAM_ARGS CMAKE_OBJCXX_FLAGS_ENV_INIT)
        if(CMAKE_OBJCXX_FLAGS_ENV_INIT)
          set(CMAKE_OBJCXX_COMPILER_ARG1
              "${CMAKE_OBJCXX_FLAGS_ENV_INIT}"
              CACHE STRING "Arguments to Objective-C++ compiler")
        endif()
        if(NOT EXISTS ${CMAKE_OBJCXX_COMPILER_INIT})
          message(
            FATAL_ERROR
              "Could not find compiler set in environment variable ${var}:\n  $ENV{${var}}"
          )
        endif()
        break()
      endif()
    ...
    if(NOT CMAKE_OBJCXX_COMPILER_INIT)
      set(CMAKE_OBJCXX_COMPILER_LIST ${_CMAKE_TOOLCHAIN_PREFIX}c++
                                     ${_CMAKE_TOOLCHAIN_PREFIX}g++ clang++)
    endif()

    _cmake_find_compiler(OBJCXX)

  else()
    # we only get here if CMAKE_OBJCXX_COMPILER was specified using -D or a
    # pre-made CMakeCache.txt (e.g. via ctest) or set in CMAKE_TOOLCHAIN_FILE if
    # CMAKE_OBJCXX_COMPILER is a list, use the first item as
    ...
    # if a compiler was specified by the user but without path, now try to find
    # it with the full path if it is found, force it into the cache, if not,
    # don't overwrite the setting (which was given by the user) with "NOTFOUND"
    # if the C compiler already had a path, reuse it for searching the CXX
    # compiler
    get_filename_component(_CMAKE_USER_OBJCXX_COMPILER_PATH
                           "${CMAKE_OBJCXX_COMPILER}" PATH)
    if(NOT _CMAKE_USER_OBJCXX_COMPILER_PATH)
      find_program(CMAKE_OBJCXX_COMPILER_WITH_PATH
                   NAMES ${CMAKE_OBJCXX_COMPILER})
      if(CMAKE_OBJCXX_COMPILER_WITH_PATH)
        set(CMAKE_OBJCXX_COMPILER
            ${CMAKE_OBJCXX_COMPILER_WITH_PATH}
            CACHE STRING "Objective-C++ compiler" FORCE)
      endif()
  ...
  # line to see if it helps produce a valid identification file.
...
if(NOT CMAKE_OBJCXX_COMPILER_ID_RUN)
  ...
  # /path/to/cc ...CompilerIdOBJCXX/... to extract the compiler front-end for
  # the language.
...
if(NOT _CMAKE_TOOLCHAIN_LOCATION)
  get_filename_component(_CMAKE_TOOLCHAIN_LOCATION "${CMAKE_OBJCXX_COMPILER}"
                         PATH)
endif()

# if we have a g++ cross compiler, they have usually some prefix, like e.g.
# powerpc-linux-g++, arm-elf-g++ or i586-mingw32msvc-g++ , optionally with a
# 3-component version number at the end (e.g. arm-eabi-gcc-4.5.2). The other
# tools of the toolchain usually have the same prefix NAME_WE cannot be used
# since then this test will fail for names like
...
if(NOT _CMAKE_TOOLCHAIN_PREFIX)
  ...
  if("${CMAKE_OBJCXX_COMPILER_ID}" MATCHES "GNU|Clang|QCC")
    ...
    elseif(COMPILER_BASENAME MATCHES "QCC(\\.exe)?$")
      if(CMAKE_OBJCXX_COMPILER_TARGET MATCHES
         "gcc_nto([a-z0-9]+_[0-9]+|[^_le]+)(le)")
...
configure_file(${CMAKE_ROOT}/Modules/CMakeOBJCXXCompiler.cmake.in
               ${CMAKE_PLATFORM_INFO_DIR}/CMakeOBJCXXCompiler.cmake @ONLY)
